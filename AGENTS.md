...
- **Files present at minimum:** `.printing-press.json` (full manifest with `api_name`, `cli_name`, `spec_format`, `spec_checksum`, `spec_source`, `printing_press_version`, plus MCP fields if applicable), `cmd/<slug>-pp-cli/main.go`, `internal/cli/root.go` and the per-resource files, `internal/client/client.go`, `SKILL.md`, `README.md`, `AGENTS.md`, `LICENSE`, `NOTICE`, `Makefile`, `.goreleaser.yaml`, `.golangci.yml`, `go.mod`, `go.sum`, and a populated `.manuscripts/<run-id>/research/` plus `.manuscripts/<run-id>/proofs/` with `phase5-acceptance.json` (`status: pass`) and/or `publish-live-gate.json`. A stamped `proofs/<stamp>-dogfood-results.json` is extra generation output, not a substitute for the phase5 marker, and not required at CLI root. A reprint that drops the manuscripts is not a publishable reprint.
...
library/<category>/<cli-slug>/      — one published CLI per directory
  ...
  .manuscripts/<run-id>/proofs/phase5-acceptance.json — live dogfood (`status: pass`)
  .manuscripts/<run-id>/proofs/publish-live-gate.json — live-gate verdict (`verdict: PASS`)
