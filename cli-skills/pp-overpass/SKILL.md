...
<!-- GENERATED FILE — DO NOT EDIT.
     This file is a verbatim mirror of library/travel/overpass/SKILL.md,
     regenerated post-merge by tools/generate-skills/. Hand-edits here are
     silently overwritten on the next regen. Edit the library/ source instead.
     See the repository agent guide, section "Generated artifacts: registry.json, cli-skills/". -->

# Overpass — Printing Press CLI

## Prerequisites: Install the CLI

This skill drives the `overpass-pp-cli` binary. **You must verify the CLI is installed before invoking any command from this skill.** If it is missing, install it first:

1. Install via the Printing Press installer. It defaults binaries to `$HOME/.local/bin` on macOS/Linux and `%LOCALAPPDATA%\Programs\PrintingPress\bin` on Windows:
   ```bash
   npx -y @mvanhorn/printing-press-library install overpass --cli-only
   ```
2. Verify: `overpass-pp-cli --version`
...
Use this when the task is finding physical things in the world worth going to look at — structures, viewpoints, industrial or architectural subjects — near a place or along a route. It also exposes raw Overpass QL for queries the curated taxonomy does not cover.
...
- `overpass-pp-cli geocoding` — Keyless, but Nominatim requires a descriptive User-Agent and permits roughly one request per second.

**overpass** — Manage overpass

- `overpass-pp-cli overpass query` — The escape hatch. Accepts Overpass QL directly for queries the curated subject taxonomy does not cover.
- `overpass-pp-cli overpass status` — Returns plain text, not JSON. Useful for seeing whether a mirror has free slots before sending a large query.
...
```bash
overpass-pp-cli which "<capability in your own words>"
```
...
Run `overpass-pp-cli doctor` to verify setup.
...
- **Filterable** — `--select` keeps a subset of fields. Dotted paths descend into nested structures; arrays traverse element-wise. Critical for keeping context small on verbose APIs:
  ...
  ```bash
  overpass-pp-cli geocoding --q "San Pedro, CA" --agent --select id,name,status
  ```
- **Previewable** — `--dry-run` shows the request without sending
- **Offline-friendly** — `search --data-source local` and `export` read the local SQLite store, which an explicit `sync --resources ...` populates. Bare `sync` is a no-op here: Overpass has no enumerable collections, and `near`/`route`/`geojson` do not persist their results.
...
- Use `--home <dir>` for one invocation, or set `OVERPASS_HOME=<dir>` to relocate all four path kinds under one root.
- Use per-kind env vars only when a specific kind must diverge: `OVERPASS_CONFIG_DIR`, `OVERPASS_DATA_DIR`, `OVERPASS_STATE_DIR`, `OVERPASS_CACHE_DIR`.
- Resolution order is per-kind env var, `--home`, `OVERPASS_HOME`, XDG (`XDG_CONFIG_HOME`, `XDG_DATA_HOME`, `XDG_STATE_HOME`, `XDG_CACHE_HOME`), then platform defaults.
- `config` contains settings like `config.toml` and profiles. `data` contains `credentials.toml`, `data.db`, cookies, and auth sidecars. `state` contains persisted queries, jobs, and `teach.log`. `cache` contains regenerable HTTP/cache files.
- Stored secrets live in `credentials.toml` under the data dir. Existing legacy `config.toml` secrets are read for compatibility and leave `config.toml` on the first auth write.
- Run `overpass-pp-cli doctor --fail-on warn` to surface path and credential-location warnings. `agent-context` exposes a schema v4 `paths` block for agents that need the resolved dirs.
...
Fleet precedence: an inherited per-kind env var overrides an explicit `--home` for that kind. Use `OVERPASS_HOME` or per-kind vars as durable fleet levers, and use `--home` only for a single invocation. Relocation is not reversible by unsetting env vars; move files manually before clearing `OVERPASS_HOME`, or `doctor` will not find credentials left under the former root.
...
```bash
overpass-pp-cli recall "<user's question>" --agent
```
...
```json
{
  ...
  "candidates": [
    { "id": 12, "class": "flag_alias | playbook_candidate",
      ...
      "rationale": "...",
      "next_action": ["<trial command>", "overpass-pp-cli learnings confirm 12"] }
  ],
  "playbook": {
    "query_family": "...",
    "playbook": {
...
if Playbook present:
    ...
    -> the Playbook's expected_tool_calls is a budget; if you find yourself running
       materially more, record the divergence via `overpass-pp-cli playbook amend`
       at end-of-session.
...
- `lookup_refresh_available` (top-level): an entity in the query has no lookup row yet, but synced data could provide one. Run `overpass-pp-cli sync` to refresh entity lookups.
...
```bash
overpass-pp-cli teach --query "<user's question>" --resource-type <type> --resource <id1> --resource <id2>
...
overpass-pp-cli teach-playbook \
  --query "<user's question>" \
  --playbook-file ~/playbooks/<shape>.json \
...
```bash
overpass-pp-cli playbook amend \
...
`overpass-pp-cli learnings stats` reports recall hit rate, teach-to-reuse, playbook resolution rate, and candidate confirm/reject counts from the local `learn_events` table. Rates are null until they have a denominator; everything stays on this machine. Use it to check whether the loop is earning its keep for this CLI.

### Disabling learning

- `--no-learn` on a single command short-circuits both `recall` and the `teach` write path. Use for deterministic agent flows or tests that must not be affected by accumulated learnings.
- `OVERPASS_NO_LEARN=true` in the environment globally disables the pipeline.
...
```
overpass-pp-cli feedback "the --since flag is inclusive but docs say exclusive"
overpass-pp-cli feedback --stdin < notes.txt
overpass-pp-cli feedback list --json --limit 10
```

Entries are stored locally as `feedback.jsonl` under the resolved data dir. They are never POSTed unless `OVERPASS_FEEDBACK_ENDPOINT` is set AND either `--send` is passed or `OVERPASS_FEEDBACK_AUTO_SEND=true`. Default behavior is local-only.
...
```
overpass-pp-cli profile save briefing --json
overpass-pp-cli --profile briefing geocoding --q "San Pedro, CA"
overpass-pp-cli profile list --json
overpass-pp-cli profile show briefing
overpass-pp-cli profile delete briefing --yes
```
...
1. **Empty, `help`, or `--help`** → show `overpass-pp-cli --help` output
...
```bash
claude mcp add overpass-pp-mcp -- overpass-pp-mcp
```
...
1. Check if installed: `which overpass-pp-cli`
   If not found, offer to install (see Prerequisites at the top of this skill).
2. Match the user query to the best command from the Unique Capabilities and Command Reference above.
3. Execute with the `--agent` flag:
   ```bash
   overpass-pp-cli <command> [subcommand] [args] --agent
   ```
4. If ambiguous, drill into subcommand help: `overpass-pp-cli <command> --help`.
