...
<!-- GENERATED FILE — DO NOT EDIT.
     This file is a verbatim mirror of library/other/anac-pl/SKILL.md,
...
- **There is no page number.** The service paginates by token and silently ignores a page parameter, always returning the first page. Use `cerca --pages N` (or `affidamenti --pages N`) to walk N pages of `--size` results and get them merged, deduplicated by `idAvviso`; `avvisi search` returns a single page, so there you raise `--size`.
