...
// TestMigrate_AddsColumnsOnUpgrade_PostingApi verifies that opening a
...
func TestMigrate_AddsColumnsOnUpgrade_PostingApi(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "posting_api" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	rows, err := s.DB().Query(`PRAGMA table_info("posting_api")`)
	if err != nil {
	...
	for _, want := range []string{
		...
		"location",
		"published_at",
		"should_display_compensation_on_job_postings",
		"team",
		"title",
		"workplace_type",
	} {
		if !hasColumn[want] {
			t.Fatalf("%s column missing from posting_api after migrate", want)
		}
	}
}
