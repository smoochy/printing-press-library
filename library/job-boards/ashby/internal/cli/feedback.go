...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("ASHBY_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("ASHBY_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "ashby-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  ashby-pp-cli feedback \"the location filter was surprising\"",
		Long: `Feedback is captured locally first in the CLI data directory's feedback.jsonl.
When ` + "`ASHBY_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`ASHBY_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  ashby-pp-cli feedback list
  ashby-pp-cli feedback list --limit 5
  ashby-pp-cli feedback list --json`,
