...
type rootFlags struct {
	...
	// invocation. Mirrors the ASHBY_NO_LEARN env var.
	noLearn                 bool
	selectFields            string
	configPath              string
	homePath                string
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "ashby-pp-cli",
		Short: "Manage ashby resources via the ashby API",
		Long: `Manage ashby resources via the ashby API.
...
Run 'ashby-pp-cli doctor' to verify auth and connectivity.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("ashby-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.selectFields, "select", "", "Comma-separated fields to include in output (e.g. --select address,applyUrl,compensation)")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'ashby-pp-cli profile list')")
	...
	rootCmd.AddCommand(newPostingApiPromotedCmd(flags))
