...
type Config struct {
	...
	RedmineApiKey    string          `toml:"api_key"`
...
func Load(configPath string) (*Config, error) {
	...
	// Env var overrides
	if v := strings.ToLower(strings.TrimSpace(os.Getenv("REDMINE_SKIP_TLS_VERIFY"))); v == "1" || v == "true" || v == "yes" || v == "on" {
		cfg.SetSkipTLSVerify(true)
	}
	if v := os.Getenv("REDMINE_API_KEY"); v != "" {
		cfg.RedmineApiKey = v
		cfg.markEnvOverride("RedmineApiKey")
		cfg.AuthSource = "env:REDMINE_API_KEY"
		cfg.CredentialSource = "env:REDMINE_API_KEY"
	}
	...
	if v := os.Getenv("REDMINE_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	...
	if v := strings.TrimSpace(os.Getenv("REDMINE_PROJECT_ID")); v != "" {
		cfg.TemplateVars["project_id"] = v
	} else if verifyMode {
		cfg.TemplateVars["project_id"] = "project_id_placeholder"
	}
	return cfg, nil
}
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "redmine-pp-cli", "config.toml"), nil
}
...
func (c *Config) StoreScopeCredential() string {
	...
	if c.RedmineApiKey != "" {
		parts = append(parts, "api_key="+c.RedmineApiKey)
	}
	if len(parts) == 0 {
		return ""
	}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	if c.RedmineApiKey == "" {
		c.RedmineApiKey = creds.RedmineApiKey
	}
}
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.RedmineApiKey = token
	delete(c.envOverrides, "RedmineApiKey")
	c.updateFileConfigField("RedmineApiKey")
	if err := c.saveCredentialsFirst(); err != nil {
		return err
	}
	return c.save()
...
func (c *Config) configForSave() Config {
	...
	if c.fileConfig != nil {
		...
		if c.envOverrides["RedmineApiKey"] {
			out.RedmineApiKey = c.fileConfig.RedmineApiKey
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "RedmineApiKey":
		c.fileConfig.RedmineApiKey = c.RedmineApiKey
	}
}
