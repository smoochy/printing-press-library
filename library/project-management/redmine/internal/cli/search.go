...
In local mode: searches locally synced data only.`,
  ...
  redmine-pp-cli search "error timeout"

  # Force local search only
  redmine-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  redmine-pp-cli search "status" --type attachments --data-source local
  # JSON output for piping
  redmine-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /search.json
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get(cmd.Context(), "/search.json", map[string]string{
					"q": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'redmine-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
