...
type rootFlags struct {
	...
	// invocation. Mirrors the REDMINE_NO_LEARN env var.
	noLearn bool
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "redmine-pp-cli",
		Short: `Redmine CLI — Every Redmine feature, plus a local mirror that finally answers the cross-project questions the Roadmap page never…`,
		Long: `Redmine CLI — Every Redmine feature, plus a local mirror that finally answers the cross-project questions the Roadmap page never could.
...
Health check: run 'redmine-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("redmine-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.selectFields, "select", "", "Comma-separated fields to include in output (e.g. --select attachment)")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'redmine-pp-cli profile list')")
	...
	rootCmd.AddCommand(newEnumerationsCmd(flags))
	rootCmd.AddCommand(newGroupsCmd(flags))
	rootCmd.AddCommand(newGroupsJsonCmd(flags))
	rootCmd.AddCommand(newIssueCategoriesCmd(flags))
	rootCmd.AddCommand(newIssuesCmd(flags))
	rootCmd.AddCommand(newIssuesJsonCmd(flags))
	...
	rootCmd.AddCommand(newProjectsJsonCmd(flags))
	rootCmd.AddCommand(newRelationsCmd(flags))
	rootCmd.AddCommand(newTimeEntriesCmd(flags))
	rootCmd.AddCommand(newTimeEntriesJsonCmd(flags))
	rootCmd.AddCommand(newUsersCmd(flags))
	rootCmd.AddCommand(newUsersJsonCmd(flags))
	rootCmd.AddCommand(newVersionsCmd(flags))
	...
	rootCmd.AddCommand(newCustomFieldsJsonPromotedCmd(flags))
	rootCmd.AddCommand(newIssueStatusesJsonPromotedCmd(flags))
	rootCmd.AddCommand(newJournalsPromotedCmd(flags))
	rootCmd.AddCommand(newNewsJsonPromotedCmd(flags))
	rootCmd.AddCommand(newQueriesJsonPromotedCmd(flags))
	rootCmd.AddCommand(newRolesPromotedCmd(flags))
	rootCmd.AddCommand(newRolesJsonPromotedCmd(flags))
	rootCmd.AddCommand(newSearchJsonPromotedCmd(flags))
	rootCmd.AddCommand(newTrackersJsonPromotedCmd(flags))
	...
	addNovelCommandIfAbsent(rootCmd, newNovelRoadmapCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelWorkloadCmd(flags))
