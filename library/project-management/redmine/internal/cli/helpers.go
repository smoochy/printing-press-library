...
func readRawRequestBody(filePath string, stdin bool) ([]byte, error) {
	if stdin && filePath != "" && filePath != "-" {
		return nil, fmt.Errorf("--file and --stdin are mutually exclusive")
	}
	if stdin || filePath == "-" {
		data, err := io.ReadAll(os.Stdin)
		if err != nil {
			return nil, fmt.Errorf("reading raw request body from stdin: %w", err)
		}
		return data, nil
	}
	if filePath == "" {
	...
	if err != nil {
		return nil, fmt.Errorf("reading raw request body from %q: %w", filePath, err)
	}
	return data, nil
}
...
func classifyAPIErrorOnly(err error) error {
	...
	case strings.Contains(msg, "HTTP 400") && cliutil.LooksLikeAuthError(msg):
		return authErr(fmt.Errorf("%w\nhint: the API rejected the request — this usually means auth is missing or invalid."+
			...
			"\n      Run 'redmine-pp-cli doctor' to check auth status."+
			"\n      Response: "+cliutil.SanitizeErrorBody(msg), err))
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: check your API key."+
			" Set your API key with: export REDMINE_API_KEY=\"your-token-here\""+
			"\n      See API docs: https://www.redmine.org/projects/redmine/wiki/Rest_api"+
			"\n      Run 'redmine-pp-cli doctor' to check auth status.", err))
...
func nonJSONPayloadError(data json.RawMessage) error {
	...
	if len(trimmed) > 0 && trimmed[0] == '<' {
		return authErr(fmt.Errorf("not authenticated or session expired; API returned HTML instead of JSON. " + "Set your API key with: export REDMINE_API_KEY=\"your-token-here\""))
	}
	if len(trimmed) == 0 {
