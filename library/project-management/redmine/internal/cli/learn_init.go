...
func initLearn(ctx context.Context, db *sql.DB) error {
	...
	seeds := map[string][]lookups.SeedConfig{
		"issue_priority": {
			{Canonical: "Low", Values: []string{"low priority"}},
			{Canonical: "Normal", Values: []string{"default", "medium"}},
			{Canonical: "High", Values: []string{"high priority"}},
			{Canonical: "Urgent", Values: []string{"urgent priority"}},
			{Canonical: "Immediate", Values: []string{"critical", "asap", "immediate priority"}},
		},
		"issue_status": {
			{Canonical: "New", Values: []string{"open", "unstarted"}},
			{Canonical: "In Progress", Values: []string{"inprogress", "in-progress", "wip", "working"}},
			{Canonical: "Resolved", Values: []string{"fixed"}},
			{Canonical: "Feedback", Values: []string{"needs feedback", "waiting"}},
			{Canonical: "Closed", Values: []string{"done", "complete", "completed"}},
			{Canonical: "Rejected", Values: []string{"wontfix", "invalid", "declined"}},
		},
		"tracker": {
			{Canonical: "Bug", Values: []string{"bugs", "defect", "defects"}},
			{Canonical: "Feature", Values: []string{"features", "feat", "enhancement"}},
			{Canonical: "Support", Values: []string{"support request", "question"}},
		},
	}
...
func runLearnInitOnce(ctx context.Context) {
	learnInitOnce.Do(func() {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "warning: redmine-pp-cli: learn init: open store: %v\n", err)
			return
		}
		defer s.Close()
		if err := initLearn(ctx, s.DB()); err != nil {
			fmt.Fprintf(os.Stderr, "warning: redmine-pp-cli: learn init: %v\n", err)
		}
	})
}
