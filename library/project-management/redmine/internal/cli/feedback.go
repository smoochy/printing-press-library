...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("REDMINE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("REDMINE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "redmine-pp-cli/feedback")
...
When ` + "`REDMINE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`REDMINE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
maintainer sees it.`,
  ...
  redmine-pp-cli feedback "the --project flag should accept identifiers, not just numeric ids"
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  redmine-pp-cli feedback list
  redmine-pp-cli feedback list --limit 5
  redmine-pp-cli feedback list --json`,
