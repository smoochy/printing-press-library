...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  redmine-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  redmine-pp-cli workflow archive --full

  # Archive without a wall-clock timeout
  redmine-pp-cli workflow archive --timeout 0`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"custom-fields-json", "enumerations", "enumerations-issue-priorities-json", "enumerations-time-entry-activities-json", "groups-json", "issue-statuses-json", "issues-json", "news-json", "projects-json", "queries-json", "roles-json", "time-entries-json", "trackers-json", "users-json"}
			if cliutil.IsDogfoodEnv() {
				if len(resources) > 3 {
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  redmine-pp-cli workflow status

  # Show status as JSON
  redmine-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("redmine-pp-cli")
			}
