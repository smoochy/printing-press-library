...
large datasets as it has no memory pressure.`,
  ...
  redmine-pp-cli export <resource> --format jsonl --output data.jsonl

  # Export with limit
  redmine-pp-cli export <resource> --format jsonl --limit 1000

  # Pipe to another tool
  redmine-pp-cli export <resource> --format jsonl | jq '.id'`,
		Args: cobra.RangeArgs(1, 2),
		RunE: func(cmd *cobra.Command, args []string) (err error) {
			validResources := map[string]bool{
				"custom-fields-json":  true,
				"enumerations":        true,
				"groups-json":         true,
				"issue-statuses-json": true,
				"issues-json":         true,
				"news-json":           true,
				"projects-json":       true,
				"queries-json":        true,
