...
// doctorProbePath selects an identity endpoint that accepts the configured
// credential type. Board credentials are not agent credentials, so probing
// /api/agents/me with a board API key or board session produces a false 401.
func doctorProbePath(cfg *config.Config) string {
	if cfg == nil {
		return "/api/health"
	}
	switch cfg.ResolvedAuthMode() {
	case "board-session", "board-api-key":
		return "/api/cli-auth/me"
	case "agent-bearer", "custom-header":
...
func doctorAuthConfigured(cfg *config.Config) bool {
	if cfg == nil {
		return false
	}
	switch cfg.ResolvedAuthMode() {
	case "board-session":
		return strings.TrimSpace(cfg.CookieHeader()) != ""
	case "board-api-key", "agent-bearer", "custom-header":
  ...
  paperclip-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				...
				// Match newClient so doctor reports and probes the effective auth
				// configuration, including command-line overrides.
				if flags.authMode != "" {
					cfg.AuthMode = flags.authMode
				}
				if flags.sessionCookie != "" {
					cfg.SessionCookie = flags.sessionCookie
				}
				if flags.boardAPIKey != "" {
					cfg.BoardAPIKey = flags.boardAPIKey
				}
				if flags.agentToken != "" {
					cfg.AgentToken = flags.agentToken
				}
				report["config"] = "ok"
			...
			// Check auth
			if cfg != nil {
				report["auth"] = cfg.ResolvedAuthMode()
			} else {
				report["auth"] = "unknown"
			}
				...
				if clientErr != nil {
					...
					case errors.As(reachErr, &reachAPIErr):
						...
						if vendor := looksLikeDoctorInterstitial([]byte(reachAPIErr.Body)); vendor != "" {
							...
							report["api"] = fmt.Sprintf("reachable (HTTP %d at %s)", status, probePath)
						}
					default:
