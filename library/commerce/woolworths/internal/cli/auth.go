...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:         "auth",
		Short:       "Manage authentication for Woolworths",
...
func requiredAuthCookies() []string {
	raw := []string{"w-rctx", "wow-auth-token", "prodwow-auth-token"}
	out := raw[:0]
...
Use --chrome to read cookies from Chrome for woolworths.com.au.
...
profile by name when the installed backend supports it.`,
		Example: `  woolworths-pp-cli auth login --chrome
  woolworths-pp-cli auth login --browser
  woolworths-pp-cli auth login --cookies-file storage-state.json
  woolworths-pp-cli auth login --chrome --profile "Work"`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !browserFlag && cookiesFile == "" {
				...
				fmt.Fprintf(cmd.OutOrStdout(), "  woolworths-pp-cli auth login --chrome\n")
				fmt.Fprintf(cmd.OutOrStdout(), "  woolworths-pp-cli auth login --browser\n")
				fmt.Fprintf(cmd.OutOrStdout(), "  woolworths-pp-cli auth login --cookies-file storage-state.json\n")
				return nil
			}
			if browserFlag && cookiesFile != "" {
			...
			requiredCookies := []string{"w-rctx", "wow-auth-token", "prodwow-auth-token"}
			...
			cfg.CredentialDomain = "woolworths.com.au"
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  woolworths-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			if !credentialConfigured {
				...
				fmt.Fprintf(w, "  woolworths-pp-cli auth login --chrome\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}

			fmt.Fprintln(w, green("Authenticated"))
			fmt.Fprintf(w, "  Source: %s\n", cfg.AuthSource)
			fmt.Fprintf(w, "  Domain: woolworths.com.au\n")
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  woolworths-pp-cli auth set-token eyJhbGciOi...",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  woolworths-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
