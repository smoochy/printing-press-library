...
func newSavedlistsCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:         "savedlists",
		Short:       "Saved shopping lists (requires an imported Chrome session)",
		Annotations: map[string]string{"mcp:read-only": "true", "pp:parent-group": "true", "pp:api-resource": "true", "pp:typed-exit-codes": "0,2"},
		RunE:        parentNoSubcommandRunE(flags),
	}

	cmd.AddCommand(newSavedlistsGetCmd(flags))
	cmd.AddCommand(newSavedlistsListCmd(flags))
	return cmd
}
