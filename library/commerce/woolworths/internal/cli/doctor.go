...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  woolworths-pp-cli doctor
  woolworths-pp-cli doctor --json
  woolworths-pp-cli doctor --fail-on warn
  woolworths-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if credentialRefused {
				...
				if !credentialConfigured {
					...
					report["auth_hint"] = "woolworths-pp-cli auth login --chrome"
				} else {
					authConfigured = true
					...
					if cfg != nil && cfg.CredentialConfigured() {
						...
						report["auth_domain"] = "woolworths.com.au"
					}
				}
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						authHeaders["Accept-Language"] = "en-AU,en;q=0.9"
						authHeaders["Referer"] = "https://www.woolworths.com.au/shop"
						authHeaders["Origin"] = "https://www.woolworths.com.au"
						authHeaders["Sec-Fetch-Site"] = "same-origin"
						authHeaders["Sec-Fetch-Mode"] = "cors"
						verifyPath := "/api/v3/ui/savedlists"
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'woolworths-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'woolworths-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		...
		report["hint"] = "No sync recorded; run 'woolworths-pp-cli sync' to hydrate API-backed resources. Rows written by other paths are not tracked in sync_state."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'woolworths-pp-cli sync' to refresh."
	}
	return report
}
