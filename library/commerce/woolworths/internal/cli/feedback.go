...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("WOOLWORTHS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("WOOLWORTHS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "woolworths-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		// pp:hand-edit  The generated parent carried no Example, which fails
		// dogfood's help check ("missing Examples section"). Generator gap.
...
When ` + "`WOOLWORTHS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`WOOLWORTHS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  woolworths-pp-cli feedback list
  woolworths-pp-cli feedback list --limit 5
  woolworths-pp-cli feedback list --json`,
