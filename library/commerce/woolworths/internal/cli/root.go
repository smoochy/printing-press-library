...
type rootFlags struct {
	...
	// invocation. Mirrors the WOOLWORTHS_NO_LEARN env var.
	noLearn bool
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "woolworths-pp-cli",
		Short: `Woolworths CLI — Every Woolworths catalogue, specials, store and trolley surface, plus a local price history that can tell a genuine…`,
		Long: `Woolworths CLI — Every Woolworths catalogue, specials, store and trolley surface, plus a local price history that can tell a genuine half-price from an inflated was-price.
...
Health check: run 'woolworths-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("woolworths-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'woolworths-pp-cli profile list')")
	...
	rootCmd.AddCommand(newSavedlistsCmd(flags))
	rootCmd.AddCommand(newSettingsCmd(flags))
	rootCmd.AddCommand(newTrolleyCmd(flags))
	...
	rootCmd.AddCommand(newPastshopsPromotedCmd(flags))
	...
	addNovelCommandIfAbsent(rootCmd, newNovelCycleCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelMultibuyCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelRealSpecialCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelSpecialsDiffCmd(flags))
