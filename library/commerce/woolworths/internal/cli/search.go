...
In local mode: searches locally synced data only.`,
  ...
  woolworths-pp-cli search "error timeout"

  # Force local search only
  woolworths-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  woolworths-pp-cli search "status" --type categories --data-source local
  # JSON output for piping
  woolworths-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /apis/ui/v2/Search/count
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get(cmd.Context(), "/apis/ui/v2/Search/count", map[string]string{
					"searchTerm": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'woolworths-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
