...
func TestMCPPathResolutionMatchesCLIResolverWithPlatformDefaults(t *testing.T) {
	...
	if want := filepath.Join(home, ".config", "woolworths-pp-cli", "config.json"); cfg.Path != want {
	...
	if want := filepath.Join(home, ".local", "share", "woolworths-pp-cli", "data.db"); gotDB != want {
...
func TestMCPRegisterToolsPreservesTypedSpecialTools(t *testing.T) {
	s := server.NewMCPServer("woolworths", "test")
	RegisterTools(s)
