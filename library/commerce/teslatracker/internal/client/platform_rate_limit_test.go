...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/commerce/teslatracker/internal/config"
	"github.com/mvanhorn/printing-press-library/library/commerce/teslatracker/internal/platform"
)
...
func platformRateLimitTestClient(t *testing.T, server *httptest.Server) (*Client, *platform.Session) {
	...
	session, err := platform.PrepareProfileSourceFromConfig(&platform.Profile{
		...
		Sources: map[string]platform.SourceProfile{
			"teslatracker": {CredentialRef: "fixture://Test/Synthetic/token", ExpectedAccountID: "acct-a", ExpectedOrgName: "Tenant A", ExpectedStoreID: "store-a", ExpectedStoreDomain: "tenant-a.example.test", ExpectedPropertyID: "123", ExpectedBaseURL: "https://tenant-a.example.test"},
		},
	}, "teslatracker-pp-cli", "teslatracker")
	if err != nil {
		t.Fatal(err)
	}
