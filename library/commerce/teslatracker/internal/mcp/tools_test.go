...
func TestMCPPathResolutionMatchesCLIResolverWithHomeEnv(t *testing.T) {
	...
	t.Setenv("TESLATRACKER_HOME", root)
...
func TestMCPPathResolutionMatchesCLIResolverWithPlatformDefaults(t *testing.T) {
	...
	if want := filepath.Join(home, ".config", "teslatracker-pp-cli", "config.json"); cfg.Path != want {
	...
	if want := filepath.Join(home, ".local", "share", "teslatracker-pp-cli", "data.db"); gotDB != want {
...
func TestMCPRegisterToolsPreservesTypedSpecialTools(t *testing.T) {
	s := server.NewMCPServer("teslatracker", "test")
	RegisterTools(s)
