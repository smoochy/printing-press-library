...
func RegisterIntents(s *server.MCPServer) {
	s.AddTool(
		mcplib.NewTool("what_did_i_miss_since_yesterday",
			mcplib.WithDescription("New, price-changed and departed since each saved search's own cursor."),
			mcplib.WithBoolean("all", mcplib.Description("Override the recipe's --all value.")),
		),
		handleWhatDidIMissSinceYesterday,
	)
}

// handleWhatDidIMissSinceYesterday runs the what_did_i_miss_since_yesterday recipe intent: New, price-changed and departed since each saved search's own cursor.
func handleWhatDidIMissSinceYesterday(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
