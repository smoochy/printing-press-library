...
var readCommandResources = map[string][]string{
	"teslatracker-pp-cli inventory":        {"inventory"},
	"teslatracker-pp-cli inventory list":   {"inventory"},
	"teslatracker-pp-cli inventory get":    {"inventory"},
	"teslatracker-pp-cli inventory search": {"inventory"},
}
...
func cachePolicy() cliutil.Policy {
	...
	envOptOut := "TESLATRACKER_NO_AUTO_REFRESH"
...
func autoRefreshIfStale(ctx context.Context, flags *rootFlags, resources []string) (meta cliutil.FreshnessMeta) {
	...
	if err := runAutoRefresh(refreshCtx, flags, db, resources); err != nil {
		fmt.Fprintf(os.Stderr, "warning: using stale teslatracker-pp-cli cache (refresh failed: %v)\n", err)
