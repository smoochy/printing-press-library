...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("TESLATRACKER_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("TESLATRACKER_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "teslatracker-pp-cli/feedback")
...
When ` + "`TESLATRACKER_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`TESLATRACKER_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
maintainer sees it.`,
		Example: `  teslatracker-pp-cli feedback "radius silently dropped a car with bad coordinates"
  teslatracker-pp-cli feedback --stdin < notes.txt
  teslatracker-pp-cli feedback "comps cohort was too small to be useful" --send`,
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  teslatracker-pp-cli feedback list
  teslatracker-pp-cli feedback list --limit 5
  teslatracker-pp-cli feedback list --json`,
