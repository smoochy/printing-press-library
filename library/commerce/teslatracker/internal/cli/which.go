...
var whichIndex = []whichEntry{
	{Command: "warranty", Description: "See exactly how many months and miles of Tesla warranty are left on the day you would take delivery, and which limit runs out first.", Group: "Coverage and condition", WhyItMatters: "Reach for this when the question is coverage or risk on one specific car, not price."},
	{Command: "degradation", Description: "Compare a car's actual range against its rated range, and see where that gap sits among every similar car in your local store.", Group: "Coverage and condition", WhyItMatters: "Use for range condition. This is the source's published spread, not a measured battery test."},
	{Command: "comps", Description: "See where one car's landed cost sits in the distribution of comparable cars, with the sample size and the arithmetic shown.", Group: "Price intelligence", WhyItMatters: "The honest answer to 'is this price good' — placement and arithmetic, never a verdict."},
	{Command: "gone", Description: "See which cars left inventory, at what landed price, and after how many days listed.", Group: "Local state that compounds", WhyItMatters: "Calibrates a ceiling against what actually clears. A departure is not a sale."},
	{Command: "price-history", Description: "See a single car's full observed price path: every cut, its size, and how long it has held the current price.", Group: "Local state that compounds", WhyItMatters: "Use for one car's own history; use stale to find candidates across the corpus."},
	{Command: "premium", Description: "See what a configuration attribute actually costs across the market, computed within matched cohorts.", Group: "Price intelligence", WhyItMatters: "Answers 'can I afford HW4 at my ceiling'. Observational, not causal."},
	{Command: "stale", Description: "Rank the corpus by days listed and flag cars that are both long-listed and have never had a price cut.", Group: "Price intelligence", WhyItMatters: "Finds negotiation candidates. Deliberately does not judge whether the price is good."},
	{Command: "radius", Description: "See how the price floor and median change as you widen your search radius, with real transport fees included at every step.", Group: "Price intelligence", WhyItMatters: "Answers 'is shopping farther actually worth it' in dollars. Returns a curve, not cars."},
	{Command: "watch", Description: "Save a named search and see only what is new, price-changed, or departed since the last time you ran it.", Group: "Local state that compounds", WhyItMatters: "The right entry point for a recurring session instead of re-running search."},
	{Command: "coverage", Description: "See how complete and how fresh your local mirror is, field by field, including whether any pages were dropped during sync.", Group: "Agent-native plumbing", WhyItMatters: "Call before asserting any aggregate. Reports on the store, never on cars."},
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one`,
		Example: `  teslatracker-pp-cli which "stale tickets"
  teslatracker-pp-cli which "bottleneck"
  teslatracker-pp-cli which --limit 1 "send message"
  teslatracker-pp-cli which                                # list the full capability index`,
