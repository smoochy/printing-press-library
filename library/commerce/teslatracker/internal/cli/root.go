...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/commerce/teslatracker/internal/learn"
...
type rootFlags struct {
	...
	// invocation. Mirrors the TESLATRACKER_NO_LEARN env var.
	noLearn                 bool
	selectFields            string
	configPath              string
	homePath                string
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "teslatracker-pp-cli",
		Short: `Every used Tesla TeslaTracker lists, in a local store that answers questions the site cannot — price paths, cohort placement, warranty left, and what already left the lot.`,
		Long: `Every used Tesla TeslaTracker lists, in a local store that answers questions the site cannot — price paths, cohort placement, warranty left, and what already left the lot.
...
Health check: run 'teslatracker-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("teslatracker-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'teslatracker-pp-cli profile list')")
	...
	rootCmd.AddCommand(newDegradationCmd(flags))
	rootCmd.AddCommand(newGoneCmd(flags))
	rootCmd.AddCommand(newPremiumCmd(flags))
	rootCmd.AddCommand(newPriceHistoryCmd(flags))
	rootCmd.AddCommand(newRadiusCmd(flags))
	rootCmd.AddCommand(newStaleCmd(flags))
	rootCmd.AddCommand(newHydrateCmd(flags))
	rootCmd.AddCommand(newWarrantyCmd(flags))
	...
	applyTeslatrackerFixtures(rootCmd)
	return rootCmd
}
