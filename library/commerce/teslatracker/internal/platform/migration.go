...
func AdoptVerifiedLegacyDatabase(ctx context.Context, session *Session, state *LegacyMigrationState) error {
	...
	// SQLite's VACUUM INTO takes a string literal, not a bind parameter, so the
	// destination path must be concatenated. targetPath is derived internally from
	// the resolved profile directory (never raw user SQL) and single quotes are
	// doubled above, which is the escape SQLite defines for string literals.
	// #nosec G202 -- VACUUM INTO cannot be parameterized; path is internally derived and quote-escaped
