...
In local mode: searches locally synced data only.`,
  ...
  quickcommerce-pp-cli search "error timeout"

  # Force local search only
  quickcommerce-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  quickcommerce-pp-cli search "status" --type account --data-source local
  # JSON output for piping
  quickcommerce-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /v1/search
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get(cmd.Context(), "/v1/search", map[string]string{
					"q": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'quickcommerce-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
