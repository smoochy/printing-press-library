...
var whichIndex = []whichEntry{
	{Command: "history prices", Description: "Query local observations to see price, stock, rating, and availability movement over time.", Group: "Local state that compounds", WhyItMatters: "Choose this when an agent needs to explain what changed instead of fetching only the current value."},
	{Command: "history diff", Description: "Show the field-level changes between the latest saved observations.", Group: "Local state that compounds", WhyItMatters: "Choose this after ingestion when an agent needs a precise price, stock, or ETA delta."},
	{Command: "delivery fastest", Description: "Rank currently available delivery options while preserving closed or unparseable platforms.", Group: "Decision-ready comparisons", WhyItMatters: "Choose this when an agent must recommend a viable fastest platform rather than list raw ETA rows."},
	{Command: "requests plan", Description: "Calculate fan-out credit cost and affordability before making paid platform requests.", Group: "Agent-native safety", WhyItMatters: "Choose this before an agent launches a multi-platform search or ETA fan-out."},
	{Command: "mirror ingest", Description: "Persist real QuickCommerce command responses and metadata into the local SQLite mirror.", Group: "Local state that compounds", WhyItMatters: "Choose this when an agent wants future history, offline search, or a reproducible observation record."},
	{Command: "mirror stale", Description: "Find saved product and ETA observations that are older than a chosen trust window.", Group: "Agent-native safety", WhyItMatters: "Choose this before making decisions from local data whose location-sensitive freshness matters."},
	{Command: "mirror coverage", Description: "Report observed, missing, and stale platform coverage for a location.", Group: "Decision-ready comparisons", WhyItMatters: "Choose this when an agent needs to know whether a location has usable evidence across platforms."},
	{Command: "prices value", Description: "Compare price per unit from explicit pack quantities without guessing missing units.", Group: "Decision-ready comparisons", WhyItMatters: "Choose this when the cheapest sticker price may not be the cheapest comparable quantity."},
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one`,
		Example: `  quickcommerce-pp-cli which "stale tickets"
  quickcommerce-pp-cli which "bottleneck"
  quickcommerce-pp-cli which --limit 1 "send message"
  quickcommerce-pp-cli which                                # list the full capability index`,
