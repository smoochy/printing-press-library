...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:         "auth",
		Short:       "Manage authentication for QuickCommerce API",
...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  quickcommerce-pp-cli auth setup\n  quickcommerce-pp-cli auth setup --launch",
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			fmt.Fprintln(w, "Get a key at: https://quickcommerceapi.com/auth/signup")
			fmt.Fprintln(w, "  Copy the API key from the QuickCommerce dashboard.")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Then set:")
			fmt.Fprintln(w, "  export QUICKCOMMERCE_API_KEY=\"your-token-here\"")
			fmt.Fprintln(w, "  quickcommerce-pp-cli auth set-token <token>")
			if !launch {
				return nil
			}
			launchURL := "https://quickcommerceapi.com/auth/signup"
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  quickcommerce-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !authed {
				...
				fmt.Fprintln(w, "  export QUICKCOMMERCE_API_KEY=\"your-token-here\"")
				fmt.Fprintf(w, "  quickcommerce-pp-cli auth set-token <token>\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  quickcommerce-pp-cli auth set-token YOUR_TOKEN_HERE",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  quickcommerce-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			envStillSet := ""
			if envStillSet == "" && os.Getenv("QUICKCOMMERCE_API_KEY") != "" {
				envStillSet = "QUICKCOMMERCE_API_KEY"
			}
