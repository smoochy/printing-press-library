...
func initLearn(ctx context.Context, db *sql.DB) error {
	...
	seeds := map[string][]lookups.SeedConfig{
		"platform": {
			{Canonical: "BlinkIt", Values: []string{"blinkit", "blink it"}},
			{Canonical: "Zepto", Values: []string{"zepto"}},
			{Canonical: "Swiggy", Values: []string{"swiggy instamart", "instamart"}},
			{Canonical: "BigBasket", Values: []string{"bigbasket", "big basket"}},
			{Canonical: "DMart", Values: []string{"dmart", "d mart", "dmart ready"}},
			{Canonical: "JioMart", Values: []string{"jiomart", "jio mart"}},
			{Canonical: "Minutes", Values: []string{"flipkart minutes", "minutes"}},
			{Canonical: "Amazon", Values: []string{"amazon india"}},
			{Canonical: "Nykaa", Values: []string{"nykaa"}},
			{Canonical: "Myntra", Values: []string{"myntra"}},
			{Canonical: "Flipkart", Values: []string{"flipkart marketplace"}},
		},
	}
...
func runLearnInitOnce(ctx context.Context) {
	learnInitOnce.Do(func() {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "warning: quickcommerce-pp-cli: learn init: open store: %v\n", err)
			return
		}
		defer s.Close()
		if err := initLearn(ctx, s.DB()); err != nil {
			fmt.Fprintf(os.Stderr, "warning: quickcommerce-pp-cli: learn init: %v\n", err)
		}
	})
}
