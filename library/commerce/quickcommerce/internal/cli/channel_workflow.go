...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  quickcommerce-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  quickcommerce-pp-cli workflow archive --full

  # Archive without a wall-clock timeout
  quickcommerce-pp-cli workflow archive --timeout 0`,
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  quickcommerce-pp-cli workflow status

  # Show status as JSON
  quickcommerce-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("quickcommerce-pp-cli")
			}
