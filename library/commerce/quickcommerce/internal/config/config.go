...
type Config struct {
	...
	QuickcommerceApiKey string          `toml:"api_key"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://api.quickcommerceapi.com",
	}
	...
	// Env var overrides
	if v := os.Getenv("QUICKCOMMERCE_API_KEY"); v != "" {
		cfg.QuickcommerceApiKey = v
		cfg.markEnvOverride("QuickcommerceApiKey")
		cfg.AuthSource = "env:QUICKCOMMERCE_API_KEY"
		cfg.CredentialSource = "env:QUICKCOMMERCE_API_KEY"
	}
	...
	if v := os.Getenv("QUICKCOMMERCE_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("QUICKCOMMERCE_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "quickcommerce-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	token := c.QuickcommerceApiKey
	if token == "" {
		return ""
	}
	if c.QuickcommerceApiKey == "" {
		return ""
	}
	return token
}
...
func (c *Config) StoreScopeCredential() string {
	...
	if c.QuickcommerceApiKey != "" {
		parts = append(parts, "api_key="+c.QuickcommerceApiKey)
	}
	if len(parts) == 0 {
		return ""
	}
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		QuickcommerceApiKey: c.QuickcommerceApiKey,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	if c.QuickcommerceApiKey == "" {
		c.QuickcommerceApiKey = creds.QuickcommerceApiKey
	}
}
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.QuickcommerceApiKey = token
	delete(c.envOverrides, "QuickcommerceApiKey")
	c.updateFileConfigField("QuickcommerceApiKey")
	if err := c.saveCredentialsFirst(); err != nil {
		return err
	}
	return c.save()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["QuickcommerceApiKey"] {
			out.QuickcommerceApiKey = c.fileConfig.QuickcommerceApiKey
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "QuickcommerceApiKey":
		c.fileConfig.QuickcommerceApiKey = c.QuickcommerceApiKey
	}
}
