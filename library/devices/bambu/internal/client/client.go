...
const (
	...
	maxBinaryResponseBytes int64 = 128 << 20
)

// ResponseTooLargeError reports a response rejected before the generated
// client can materialize an unbounded body in memory.
...
func (e *ResponseTooLargeError) Error() string {
	if e.ContentLength >= 0 {
		return fmt.Sprintf("response body is %d bytes; limit is %d bytes", e.ContentLength, e.Limit)
	}
	return fmt.Sprintf("response body exceeds %d bytes", e.Limit)
}

func responseBodyLimit(binary bool) int64 {
	if binary {
		return maxBinaryResponseBytes
	}
	return maxJSONResponseBytes
}

func readBoundedResponseBody(resp *http.Response, limit int64) ([]byte, error) {
	if resp.ContentLength > limit {
		return nil, &ResponseTooLargeError{Limit: limit, ContentLength: resp.ContentLength}
	}
	...
	httpClient.CheckRedirect = func(req *http.Request, via []*http.Request) error {
		...
		if req.URL.Host == via[0].URL.Host {
			if h, err := c.authHeader(req.Context()); err == nil && h != "" {
				req.Header.Set("X-Bambu-Access-Code", h)
			}
		} else {
			...
			req.Header.Del("X-Bambu-Access-Code")
		}
		return nil
	}
	return c
...
func (c *Client) doInternal(ctx context.Context, method, path string, params map[string]string, body any, headerOverrides map[string]string, readOnlyIntent bool) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if authHeader != "" {
			req.Header.Set("X-Bambu-Access-Code", authHeader)
		}
		if c.Config != nil {
		...
		respBody, err := readBoundedResponseBody(resp, responseBodyLimit(binaryResponse))
		_ = resp.Body.Close()
		if err != nil {
...
func (c *Client) dryRun(method, targetURL, path string, params map[string]string, body []byte, headerOverrides map[string]string, authHeader string) (json.RawMessage, int, error) {
	...
	if authHeader != "" {
		fmt.Fprintf(os.Stderr, "  %s: %s\n", "X-Bambu-Access-Code", maskToken(authHeader))
	}
...
func authPlaceholderCredentialError(cfg *config.Config) error {
	return authPlaceholderCredentialErrorWithSetup(cfg, "export BAMBU_SERIAL=<your-token> or bambu-pp-cli auth set-token <token>")
}
...
func (c *Client) maskCredentialText(text string, extraCredentials ...string) string {
	...
	if c != nil && c.Config != nil {
		...
		addCredential(c.Config.BambuAccessCode)
	}
