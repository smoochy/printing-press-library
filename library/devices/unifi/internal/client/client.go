...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/devices/unifi/internal/config"
	"github.com/mvanhorn/printing-press-library/library/devices/unifi/internal/platform"
	"io"
	"math"
	"net"
	"net/http"
...
// newHTTPClient builds the shared HTTP transport. insecureSkipVerify comes
// from Config.InsecureSkipVerify() (see internal/config/config.go) — self-
// hosted UniFi OS gateways serve a self-signed certificate by default, so
// this is threaded through from New() below rather than left at Go's default
// (verify everything). PATCH: see
// .printing-press-patches/tls-skip-verify-for-local-gateway.md.
func newHTTPClient(timeout time.Duration, jar http.CookieJar, insecureSkipVerify bool) *http.Client {
	...
	if insecureSkipVerify {
		...
		tr.TLSClientConfig.InsecureSkipVerify = true //nolint:gosec // opt-in, scoped to private/loopback/link-local hosts or explicit UNIFI_INSECURE_SKIP_VERIFY override
	}
...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	httpClient := newHTTPClient(timeout, nil, cfg.InsecureSkipVerify())
	c := &Client{
...
func (c *Client) doInternal(ctx context.Context, method, path string, params map[string]string, body any, headerOverrides map[string]string, readOnlyIntent bool, mutationIntent bool) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if req.Header.Get("User-Agent") == "" {
			req.Header.Set("User-Agent", "unifi-pp-cli/10.5.67")
		}
...
func authPlaceholderCredentialError(cfg *config.Config) error {
	return authPlaceholderCredentialErrorWithSetup(cfg, "export UNIFI_API_KEY=<your-token> or unifi-pp-cli auth set-token <token>")
}
