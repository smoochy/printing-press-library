...
type Config struct {
	...
	BmwCardataAccessToken string            `toml:"cardata_access_token"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("BMW_CARDATA_CONFIG")
	}
	if path == "" {
	...
	// Env var overrides
	if v := os.Getenv("BMW_CARDATA_ACCESS_TOKEN"); v != "" {
		cfg.BmwCardataAccessToken = v
		cfg.markEnvOverride("BmwCardataAccessToken")
		cfg.AuthSource = "env:BMW_CARDATA_ACCESS_TOKEN"
	}
	...
	if cfg.AuthSource == "" && cfg.BmwCardataAccessToken != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("BMW_CARDATA_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.BmwCardataAccessToken != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:BMW_CARDATA_ACCESS_TOKEN"
		}
		return "Bearer " + c.BmwCardataAccessToken
	}
	if c.AccessToken != "" {
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientID")
	c.updateFileConfigField("ClientSecret")
	c.BmwCardataAccessToken = ""
	delete(c.envOverrides, "BmwCardataAccessToken")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["BmwCardataAccessToken"] {
			out.BmwCardataAccessToken = c.fileConfig.BmwCardataAccessToken
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "BmwCardataAccessToken":
		c.fileConfig.BmwCardataAccessToken = c.BmwCardataAccessToken
	}
}
