...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "bmw-cardata-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("BMW_CARDATA_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("BMW_CARDATA_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "bmw-cardata-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/bmw-cardata-pp-cli/feedback.jsonl.
When ` + "`BMW_CARDATA_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`BMW_CARDATA_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  bmw-cardata-pp-cli feedback list
  bmw-cardata-pp-cli feedback list --limit 5
  bmw-cardata-pp-cli feedback list --json`,
