# BMW CarData Printed CLI Agent Guide

This directory is a generated `bmw-cardata-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
bmw-cardata-pp-cli doctor --json
bmw-cardata-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
bmw-cardata-pp-cli which "<capability>" --json
bmw-cardata-pp-cli <command> --help
```
...
```bash
bmw-cardata-pp-cli <command> --agent
```
...
```bash
bmw-cardata-pp-cli <command> --help
bmw-cardata-pp-cli <command> --dry-run --agent
```
