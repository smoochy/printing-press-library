# Googlehealth Printed CLI Agent Guide

This directory is a generated `googlehealth-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
googlehealth-pp-cli doctor --json
googlehealth-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
googlehealth-pp-cli which "<capability>" --json
googlehealth-pp-cli <command> --help
```
...
```bash
googlehealth-pp-cli <command> --agent
```
...
```bash
googlehealth-pp-cli <command> --help
googlehealth-pp-cli <command> --dry-run --agent
```
