...
type Config struct {
	...
	GooglehealthOauth2c string          `toml:"oauth2c"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("GOOGLEHEALTH_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "googlehealth-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("GOOGLEHEALTH_OAUTH2C"); v != "" {
		cfg.GooglehealthOauth2c = v
		cfg.markEnvOverride("GooglehealthOauth2c")
		cfg.AuthSource = "env:GOOGLEHEALTH_OAUTH2C"
	}
	...
	if cfg.AuthSource == "" && cfg.GooglehealthOauth2c != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("GOOGLEHEALTH_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	if v := os.Getenv("GOOGLEHEALTH_AUTHORIZATION_URL"); v != "" {
		cfg.AuthorizationURL = v
	}
	if v := os.Getenv("GOOGLEHEALTH_TOKEN_URL"); v != "" {
		cfg.TokenURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.GooglehealthOauth2c != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:GOOGLEHEALTH_OAUTH2C"
		}
		return "Bearer " + c.GooglehealthOauth2c
	}
	if c.AccessToken != "" {
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientID")
	c.updateFileConfigField("ClientSecret")
	c.GooglehealthOauth2c = ""
	delete(c.envOverrides, "GooglehealthOauth2c")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["GooglehealthOauth2c"] {
			out.GooglehealthOauth2c = c.fileConfig.GooglehealthOauth2c
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "GooglehealthOauth2c":
		c.fileConfig.GooglehealthOauth2c = c.GooglehealthOauth2c
	}
}
