...
Resource scoping:
  ...
  googlehealth-pp-cli sync

  # Sync specific resources only
  googlehealth-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  googlehealth-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  googlehealth-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  googlehealth-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  googlehealth-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(resources) == 0 {
				if humanFriendly {
					fmt.Fprintln(os.Stderr, "googlehealth-pp-cli sync: no default sync resources in spec; use --resources for explicit sync targets or populate the store via single-fetch commands.")
				} else {
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/googlehealth-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "data_types":
		return db.UpsertDataTypes(data)
	case "identity":
		return db.UpsertIdentity(data)
	case "irn_profile":
		return db.UpsertIrnProfile(data)
	case "paired_devices":
		return db.UpsertPairedDevices(data)
	case "profile":
...
// googleHealthDataTypes is the full set of Google Health data-point
// collections (kebab-case data type IDs from the DataPoint union). Each
// is syncable as users/me/dataTypes/<type>/dataPoints. Hand-wired because
// the API's parent-keyed paths (users/{usersId}/dataTypes/{dataTypesId})
// cannot be auto-derived by flat-list sync; "me" is Google Health's alias
// for the authenticated user.
func googleHealthDataTypes() []string {
	return []string{
		"active-energy-burned", "active-minutes", "active-zone-minutes",
		"activity-level", "altitude", "basal-energy-burned", "blood-glucose",
		"body-fat", "core-body-temperature", "daily-heart-rate-variability",
		"daily-heart-rate-zones", "daily-oxygen-saturation",
		"daily-respiratory-rate", "daily-resting-heart-rate",
		"daily-sleep-temperature-derivations", "daily-vo2-max", "distance",
		"electrocardiogram", "exercise", "floors", "food",
		"food-measurement-unit", "heart-rate", "heart-rate-variability",
		"height", "hydration-log", "irregular-rhythm-notification",
		"nutrition-log", "oxygen-saturation", "respiratory-rate-sleep-summary",
		"run-vo2-max", "sedentary-period", "sleep", "steps",
		"swim-lengths-data", "time-in-heart-rate-zone", "vo2-max", "weight",
	}
}

func defaultSyncResources() []string {
	// Curated common metrics so a bare `sync` doesn't fan out to all 38
	// data types. Use `sync --resources <type>` for any other known type.
	return []string{
		"steps", "distance", "floors", "weight", "body-fat",
		"heart-rate", "daily-resting-heart-rate", "heart-rate-variability",
		"daily-heart-rate-variability", "active-zone-minutes", "active-minutes",
		"active-energy-burned", "sleep", "daily-oxygen-saturation", "daily-vo2-max",
	}
}
...
func syncResourcePath(resource string) (string, error) {
	for _, dt := range googleHealthDataTypes() {
		if dt == resource {
			return "/v4/users/me/dataTypes/" + resource + "/dataPoints", nil
		}
	}
