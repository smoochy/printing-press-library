...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "googlehealth-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("GOOGLEHEALTH_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("GOOGLEHEALTH_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "googlehealth-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/googlehealth-pp-cli/feedback.jsonl.
When ` + "`GOOGLEHEALTH_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`GOOGLEHEALTH_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  googlehealth-pp-cli feedback list
  googlehealth-pp-cli feedback list --limit 5
  googlehealth-pp-cli feedback list --json`,
