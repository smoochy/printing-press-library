...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  googlehealth-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  googlehealth-pp-cli workflow archive --full`,
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  googlehealth-pp-cli workflow status

  # Show status as JSON
  googlehealth-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("googlehealth-pp-cli")
			}
