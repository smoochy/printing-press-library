...
func TestUpsertBatch_TypedFailureDoesNotStrandSubscribersGeneric(t *testing.T) {
	...
	if typed != 0 {
		t.Fatalf("subscribers count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "projects_id")
	}
}
...
func TestUpsertBatch_KeysSubscribersByChildAndParent(t *testing.T) {
	...
	parentQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s" WHERE "projects_id" = ?`, "subscribers")
...
// TestUpsertBatch_PopulatesDataTypesTable verifies that UpsertBatch
// dispatches paginated items into both the generic resources table AND the
// typed data_types table. Regression for issue #268: before the fix, paginated
// syncs only filled the generic resources table, so domain commands that
// query the typed table saw zero rows.
func TestUpsertBatch_PopulatesDataTypesTable(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if _, _, err := s.UpsertBatch("data_types", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "data_types").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	if generic != len(items) {
		t.Fatalf("resources count = %d, want %d", generic, len(items))
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "data_types")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count data_types: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("data_types count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}

// TestUpsertBatch_TypedFailureDoesNotStrandDataTypesGeneric exercises
// the savepoint isolation around the typed-table dispatch. The fixture omits
// the NOT NULL parent FK column so the typed insert fails; the savepoint
// rolls back only the typed projection. The generic resources row inserted
// just before must survive. Regression for issue #1392, where a single
// outer transaction caused typed-table failures to cascade and silently
// discard every successfully fetched API row.
func TestUpsertBatch_TypedFailureDoesNotStrandDataTypesGeneric(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	stored, extractFailures, err := s.UpsertBatch("data_types", items)
	if err != nil {
		t.Fatalf("UpsertBatch: %v (typed-table failure must not propagate)", err)
	}
	if stored != len(items) {
	...
	if typed != 0 {
		t.Fatalf("data_types count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "users_id")
	}
}

// TestUpsertBatch_KeysDataTypesByChildAndParent verifies dependent
// sub-collection rows with the same child id but different parent ids do not
// overwrite one another in either the generic resources table or the typed
// table. Regression for issue #2471.
func TestUpsertBatch_KeysDataTypesByChildAndParent(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if typed != len(items) {
		t.Fatalf("data_types count = %d, want %d (typed rows collapsed on child id)", typed, len(items))
	}

	var parentMatches int
	parentQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s" WHERE "users_id" = ?`, "data_types")
	if err := db.QueryRow(parentQuery, "parent_A").Scan(&parentMatches); err != nil {
...
func TestUpsertBatch_TypedFailureDoesNotStrandIdentityGeneric(t *testing.T) {
	...
	if typed != 0 {
		t.Fatalf("identity count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "users_id")
	}
}
...
func TestUpsertBatch_KeysIdentityByChildAndParent(t *testing.T) {
	...
	var parentMatches int
	parentQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s" WHERE "users_id" = ?`, "identity")
	if err := db.QueryRow(parentQuery, "parent_A").Scan(&parentMatches); err != nil {
		t.Fatalf("count by users_id: %v", err)
	}
	if parentMatches != 2 {
...
// TestUpsertBatch_PopulatesIrnProfileTable verifies that UpsertBatch
// dispatches paginated items into both the generic resources table AND the
// typed irn_profile table. Regression for issue #268: before the fix, paginated
// syncs only filled the generic resources table, so domain commands that
// query the typed table saw zero rows.
func TestUpsertBatch_PopulatesIrnProfileTable(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if _, _, err := s.UpsertBatch("irn_profile", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "irn_profile").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	if generic != len(items) {
		t.Fatalf("resources count = %d, want %d", generic, len(items))
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "irn_profile")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count irn_profile: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("irn_profile count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}

// TestUpsertBatch_TypedFailureDoesNotStrandIrnProfileGeneric exercises
// the savepoint isolation around the typed-table dispatch. The fixture omits
// the NOT NULL parent FK column so the typed insert fails; the savepoint
// rolls back only the typed projection. The generic resources row inserted
// just before must survive. Regression for issue #1392, where a single
// outer transaction caused typed-table failures to cascade and silently
// discard every successfully fetched API row.
func TestUpsertBatch_TypedFailureDoesNotStrandIrnProfileGeneric(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	stored, extractFailures, err := s.UpsertBatch("irn_profile", items)
	if err != nil {
		t.Fatalf("UpsertBatch: %v (typed-table failure must not propagate)", err)
	}
	if stored != len(items) {
	...
	if typed != 0 {
		t.Fatalf("irn_profile count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "users_id")
	}
}

// TestUpsertBatch_KeysIrnProfileByChildAndParent verifies dependent
// sub-collection rows with the same child id but different parent ids do not
// overwrite one another in either the generic resources table or the typed
// table. Regression for issue #2471.
func TestUpsertBatch_KeysIrnProfileByChildAndParent(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if typed != len(items) {
		t.Fatalf("irn_profile count = %d, want %d (typed rows collapsed on child id)", typed, len(items))
	}

	var parentMatches int
	parentQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s" WHERE "users_id" = ?`, "irn_profile")
	if err := db.QueryRow(parentQuery, "parent_A").Scan(&parentMatches); err != nil {
		t.Fatalf("count by users_id: %v", err)
	}
	if parentMatches != 2 {
...
// TestUpsertBatch_PopulatesPairedDevicesTable verifies that UpsertBatch
// dispatches paginated items into both the generic resources table AND the
// typed paired_devices table. Regression for issue #268: before the fix, paginated
// syncs only filled the generic resources table, so domain commands that
// query the typed table saw zero rows.
func TestUpsertBatch_PopulatesPairedDevicesTable(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if _, _, err := s.UpsertBatch("paired_devices", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "paired_devices").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	if generic != len(items) {
		t.Fatalf("resources count = %d, want %d", generic, len(items))
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "paired_devices")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count paired_devices: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("paired_devices count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}

// TestUpsertBatch_TypedFailureDoesNotStrandPairedDevicesGeneric exercises
// the savepoint isolation around the typed-table dispatch. The fixture omits
// the NOT NULL parent FK column so the typed insert fails; the savepoint
// rolls back only the typed projection. The generic resources row inserted
// just before must survive. Regression for issue #1392, where a single
// outer transaction caused typed-table failures to cascade and silently
// discard every successfully fetched API row.
func TestUpsertBatch_TypedFailureDoesNotStrandPairedDevicesGeneric(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	stored, extractFailures, err := s.UpsertBatch("paired_devices", items)
	if err != nil {
		t.Fatalf("UpsertBatch: %v (typed-table failure must not propagate)", err)
	}
	if stored != len(items) {
	...
	if typed != 0 {
		t.Fatalf("paired_devices count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "users_id")
	}
}

// TestUpsertBatch_KeysPairedDevicesByChildAndParent verifies dependent
// sub-collection rows with the same child id but different parent ids do not
// overwrite one another in either the generic resources table or the typed
// table. Regression for issue #2471.
func TestUpsertBatch_KeysPairedDevicesByChildAndParent(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if typed != len(items) {
		t.Fatalf("paired_devices count = %d, want %d (typed rows collapsed on child id)", typed, len(items))
	}

	var parentMatches int
	parentQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s" WHERE "users_id" = ?`, "paired_devices")
	if err := db.QueryRow(parentQuery, "parent_A").Scan(&parentMatches); err != nil {
		t.Fatalf("count by users_id: %v", err)
	}
	if parentMatches != 2 {
...
func TestUpsertBatch_TypedFailureDoesNotStrandProfileGeneric(t *testing.T) {
	...
	if typed != 0 {
		t.Fatalf("profile count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "users_id")
	}
}

// TestUpsertBatch_KeysProfileByChildAndParent verifies dependent
// sub-collection rows with the same child id but different parent ids do not
// overwrite one another in either the generic resources table or the typed
// table. Regression for issue #2471.
func TestUpsertBatch_KeysProfileByChildAndParent(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if typed != len(items) {
		t.Fatalf("profile count = %d, want %d (typed rows collapsed on child id)", typed, len(items))
	}

	var parentMatches int
	parentQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s" WHERE "users_id" = ?`, "profile")
	if err := db.QueryRow(parentQuery, "parent_A").Scan(&parentMatches); err != nil {
		t.Fatalf("count by users_id: %v", err)
	}
	if parentMatches != 2 {
...
func TestUpsertBatch_TypedFailureDoesNotStrandSettingsGeneric(t *testing.T) {
	...
	if typed != 0 {
		t.Fatalf("settings count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "users_id")
	}
}
...
func TestUpsertBatch_KeysSettingsByChildAndParent(t *testing.T) {
	...
	var parentMatches int
	parentQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s" WHERE "users_id" = ?`, "settings")
	if err := db.QueryRow(parentQuery, "parent_A").Scan(&parentMatches); err != nil {
		t.Fatalf("count by users_id: %v", err)
	}
	if parentMatches != 2 {
