...
const (
	journalSessionEnvVar   = "SYNOLOGY_LEARN_SESSION"
	journalNoLearnEnvVar   = "SYNOLOGY_NO_LEARN"
	journalNoCaptureEnvVar = "SYNOLOGY_LEARN_NO_CAPTURE"
)
...
// `passwd` and `otp` are hand-added: DSM's login flags are `--passwd` and
// `--otp-code`, neither of which matches `password`.
var journalRedactedFlagPattern = regexp.MustCompile(`(?i)token|key|secret|password|passwd|otp|cookie|auth`)
