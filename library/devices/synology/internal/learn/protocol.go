...
const RecallFirstProtocol = `0. Never paste a user-controlled string into a shell command line, and never embed it in the command text at all, not in quotes and not in a heredoc, whose delimiter the question can itself contain. Write the question verbatim to a file with your file-writing tool, read that file into a variable (QUERY=$(cat /path/to/question.txt)), then pass "$QUERY" as the argument. Command substitution on a file only ever yields data: the shell never parses the file's bytes as syntax. The same applies to any other user-controlled argument, such as an --add-note correction or a learnings forget query.
