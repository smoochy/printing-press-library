...
func resetCredentialEnv(t *testing.T) (home, configPath string) {
	...
	return home, filepath.Join(home, ".config", "synology-pp-cli", "config.json")
}
...
func TestCorruptCredentialsFallsBackToLegacyConfig(t *testing.T) {
	...
	credentialsPath := filepath.Join(home, ".local", "share", "synology-pp-cli", "credentials.toml")
...
func TestAuthWriteScrubsLegacyConfigWhenRelocated(t *testing.T) {
	...
	activeConfigPath := filepath.Join(newConfigDir, "synology-pp-cli", "config.json")
	...
	credsPath := filepath.Join(newDataDir, "synology-pp-cli", "credentials.toml")
