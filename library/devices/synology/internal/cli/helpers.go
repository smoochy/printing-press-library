...
func classifyAPIError(err error, flags *rootFlags) error {
	...
	msg := err.Error()
	switch {
	// Hand-added: DSM answers HTTP 200 for its own failures, so these codes
	// never reach the HTTP-status cases below.
	case strings.Contains(msg, "DSM error 119"), strings.Contains(msg, "DSM error 106"), strings.Contains(msg, "DSM error 107"):
		return authErr(fmt.Errorf("%w\nhint: the DSM session is no longer valid."+
			"\n      Run 'synology-pp-cli session login --account <user> --passwd <password>' again."+
			"\n      Set SYNOLOGY_ACCOUNT and SYNOLOGY_PASSWORD to let an expired session renew itself.", err))
	case strings.Contains(msg, "DSM error 105"), strings.Contains(msg, "DSM error 402"), strings.Contains(msg, "DSM error 403"):
		return authErr(fmt.Errorf("%w\nhint: the account is authenticated but lacks the privilege this call needs."+
			"\n      Most SYNO.Core.* and SYNO.Storage.* calls require an administrator account.", err))
	case strings.Contains(msg, "DSM error 102"), strings.Contains(msg, "DSM error 103"), strings.Contains(msg, "DSM error 104"):
		return notFoundErr(fmt.Errorf("%w\nhint: this DSM does not offer that API, method, or version."+
			"\n      Run 'synology-pp-cli session apis' to list what this NAS actually exposes.", err))
	...
	case strings.Contains(msg, "HTTP 400") && cliutil.LooksLikeAuthError(msg):
		return authErr(fmt.Errorf("%w\nhint: the API rejected the request — this usually means auth is missing or invalid."+
			...
			"\n      Run 'synology-pp-cli doctor' to check auth status."+
			"\n      Response: "+cliutil.SanitizeErrorBody(msg), err))
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: check your API credentials."+
			" Run 'synology-pp-cli auth setup' for credential setup steps."+
			"\n      Run 'synology-pp-cli doctor' to check auth status.", err))
...
func applyResponsePath(data json.RawMessage, responsePath string, isList bool) json.RawMessage {
	if pathData, ok := responsePayloadAtPath(data, responsePath); ok {
		return pathData
	}
	if isList && strings.TrimSpace(responsePath) == "" {
		if listData, ok := listPayloadArraySibling(data); ok {
			return listData
		}
	}
	return data
...
// listPayloadArraySibling pulls the row array out of a DSM list payload.
//
// After dsmUnwrapSuccess strips the transport envelope, a DSM list answer is
// still one level nested and names its rows after the resource itself:
// {"offset":0,"total":14,"users":[...]}. Without this the table renderer never
// fires (it needs a JSON array), --select only sees offset/total/users, and the
// local cache stores the wrapper instead of the rows.
//
// The key name is per-endpoint, so this reuses the framework's existing
// single-array-sibling heuristic rather than hardcoding one name per command:
// exactly one array of objects alongside nothing but known pagination metadata.
// It runs only for list commands with no explicit responsePath, so a
// single-object GET that merely carries an array field is never collapsed.
func listPayloadArraySibling(data json.RawMessage) (json.RawMessage, bool) {
	var root map[string]json.RawMessage
	if err := json.Unmarshal(data, &root); err != nil {
		return nil, false
	}
	items, ok := extractWriteThroughSingleArraySibling(root, decodeWriteThroughArray)
	if !ok {
		return nil, false
	}
...
func nonJSONPayloadError(data json.RawMessage) error {
	...
	if len(trimmed) > 0 && trimmed[0] == '<' {
		return authErr(fmt.Errorf("not authenticated or session expired; API returned HTML instead of JSON. " + "Run 'synology-pp-cli auth setup' for credential setup steps."))
	}
	if len(trimmed) == 0 {
