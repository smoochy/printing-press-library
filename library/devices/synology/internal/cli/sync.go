...
Resource scoping:
  ...
  synology-pp-cli sync

  # Sync specific resources only
  synology-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  synology-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  synology-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  synology-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  synology-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dbPath == "" {
				dbPath = defaultDBPath("synology-pp-cli")
			}
...
func determinePaginationDefaults(resource string) paginationDefaults {
	...
	case "files-entry-cgi?api=syno-file-station-search&method=list&version=2":
		return paginationDefaults{
			cursorParam: "offset",
			cursorType:  "offset",
			limitParam:  "limit",
...
func knownSyncResourceNames() []string {
	names := []string{
		"files",
		"files-entry-cgi?api=syno-file-station-search&method=list&version=2",
		"files-entry-cgi?api=syno-file-station-search&method=start&version=2",
		"files-entry-cgi?api=syno-file-station-search&method=stop&version=2",
		"folder",
		"group",
		"log",
		"package",
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{ // #nosec G101 -- endpoint paths, not credentials.
		...
		"files-entry-cgi?api=syno-file-station-search&method=list&version=2":  "/webapi/entry.cgi?api=SYNO.FileStation.Search&method=list&version=2",
		"files-entry-cgi?api=syno-file-station-search&method=start&version=2": "/webapi/entry.cgi?api=SYNO.FileStation.Search&method=start&version=2",
		"files-entry-cgi?api=syno-file-station-search&method=stop&version=2":  "/webapi/entry.cgi?api=SYNO.FileStation.Search&method=stop&version=2",
