...
// synology-pp-cli CLI: `teach` (fire-and-forget, silent on success,
...
const noLearnEnvVar = "SYNOLOGY_NO_LEARN"
...
func learnDBPath(explicit string) string {
	...
	return defaultDBPath("synology-pp-cli")
}
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "list",
		Short: "List recorded learnings, filtered by query, source, resource, action, or confidence",
		Example: `  synology-pp-cli learnings list --agent
  synology-pp-cli learnings list --query "<substring>"
  synology-pp-cli learnings list --warnings --agent`,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
...
a whole family.`,
		Example: `  synology-pp-cli teach-pattern \
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  synology-pp-cli teach-lookup --kind country --canonical "United States" --value USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
			}
