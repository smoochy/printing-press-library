...
var whichIndex = []whichEntry{
	{Command: "session apis", Description: "List every API namespace this particular NAS exposes, with its version range and CGI path.", Group: "Knowing what this NAS actually offers", WhyItMatters: "Run this first when a call fails with DSM error 102 - it tells you whether the namespace exists on this NAS at all or whether the package is simply not installed."},
	{Command: "storage smart", Description: "Read a drive's raw SMART attribute table; 'storage smart-schedule' reads the scheduled SMART test configuration.", Group: "Knowing what this NAS actually offers", WhyItMatters: "Reach for this when asked whether a disk is failing - it is the only machine-readable path to per-attribute drive health on a Synology NAS."},
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one`,
		Example: `  synology-pp-cli which "stale tickets"
  synology-pp-cli which "bottleneck"
  synology-pp-cli which --limit 1 "send message"
  synology-pp-cli which                                # list the full capability index`,
