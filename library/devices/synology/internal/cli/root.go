...
type rootFlags struct {
	...
	// invocation. Mirrors the SYNOLOGY_NO_LEARN env var.
	noLearn bool
	// insecureTLS is hand-added: opt out of certificate verification for a
	// NAS that still serves DSM's own self-signed certificate.
	insecureTLS   bool
	selectFields  string
	configPath    string
	homePath      string
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "synology-pp-cli",
		Short: "Every read surface of a Synology NAS - system, storage, SMART, shares, users, logs and File Station - as one agent-friendly CLI with a local mirror.",
		Long: `Every read surface of a Synology NAS - system, storage, SMART, shares, users, logs and File Station - as one agent-friendly CLI with a local mirror.
...
Run 'synology-pp-cli doctor' to verify auth and connectivity.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("synology-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().BoolVar(&flags.insecureTLS, "insecure", false, "Skip TLS certificate verification, for a NAS still using DSM's self-signed certificate on port 5001 (also SYNOLOGY_INSECURE_TLS=1)")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'synology-pp-cli profile list')")
	...
	rootCmd.AddCommand(newLogPromotedCmd(flags))
	rootCmd.AddCommand(newNfsPromotedCmd(flags))
	rootCmd.AddCommand(newPackagePromotedCmd(flags))
...
func (f *rootFlags) newClient() (*client.Client, error) {
	...
	// Hand-added: --insecure only ever turns verification off, so a config
	// file or env var that already opted in stays in force.
	if f.insecureTLS {
