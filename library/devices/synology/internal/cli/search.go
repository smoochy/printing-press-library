...
In local mode: searches locally synced data only.`,
  ...
  synology-pp-cli search "error timeout"

  # Force local search only
  synology-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  synology-pp-cli search "status" --type files --data-source local
  # JSON output for piping
  synology-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /webapi/entry.cgi?api=SYNO.FileStation.Search&method=list&version=2
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get(cmd.Context(), "/webapi/entry.cgi?api=SYNO.FileStation.Search&method=list&version=2", map[string]string{
					"q": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'synology-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
