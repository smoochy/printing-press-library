...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SYNOLOGY_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SYNOLOGY_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "synology-pp-cli/feedback")
...
When ` + "`SYNOLOGY_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SYNOLOGY_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  synology-pp-cli feedback list
  synology-pp-cli feedback list --limit 5
  synology-pp-cli feedback list --json`,
