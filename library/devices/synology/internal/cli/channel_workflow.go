...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  synology-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  synology-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"folder", "group", "log", "package", "user"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  synology-pp-cli workflow status

  # Show status as JSON
  synology-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("synology-pp-cli")
			}
