...
// insecureTLS is hand-added: DSM ships a self-signed certificate on port 5001,
// so a LAN user with no custom certificate cannot verify the chain. It stays an
// explicit opt-in (--insecure / SYNOLOGY_INSECURE_TLS); never infer it from the
// port, or a NAS behind a real certificate silently loses verification too.
func newHTTPClient(timeout time.Duration, jar http.CookieJar, insecureTLS bool) *http.Client {
	...
	if insecureTLS {
		if tr.TLSClientConfig == nil {
			tr.TLSClientConfig = &tls.Config{} //nolint:gosec // opt-in only, see comment above
		}
		tr.TLSClientConfig.InsecureSkipVerify = true //nolint:gosec // opt-in only, see comment above
	}
...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	sess := newSessionManager(timeout, cfg.Path, cfg.BaseURL, cfg.InsecureTLS)
	httpClient := newHTTPClient(timeout, sess.CookieJar(), cfg.InsecureTLS)
...
func (c *Client) GetWithHeaders(ctx context.Context, path string, params map[string]string, headers map[string]string) (json.RawMessage, error) {
	...
	cacheEnabled := c.responseCacheEnabled(path, binaryResponse)
...
func (c *Client) GetWithHeadersNoCache(ctx context.Context, path string, params map[string]string, headers map[string]string) (json.RawMessage, error) {
	binaryResponse := c.wantsBinaryResponse(headers)
	result, _, err := c.do(ctx, "GET", path, params, nil, headers)
	if err == nil && c.responseCacheEnabled(path, binaryResponse) {
		c.writeCache(path, params, result)
	}
	return result, err
}
...
func (c *Client) doInternal(ctx context.Context, method, path string, params map[string]string, body any, headerOverrides map[string]string, readOnlyIntent bool) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if authHeader != "" {
			q := req.URL.Query()
			q.Set("_sid", authHeader)
			req.URL.RawQuery = q.Encode()
			// Hand-added: DSM 7.3.2+ rejects state-changing calls that omit
			// this header. It goes on every request rather than on non-GET
			// verbs alone, because DSM's own `method=` parameter decides
			// whether a call mutates - plenty of GET-shaped calls are writes.
			// DSM ignores the header on read-only calls.
			if c.Session != nil {
				if synoToken := c.Session.SynoToken(); synoToken != "" {
		...
		if resp.StatusCode < 400 {
			...
			// this here rather than in the command keeps it true for every
			// caller (CLI, MCP server, sync) and survives regeneration of
			// the command files.
			if c.Session != nil {
				switch {
				case isDSMAuthCall(path, "login"):
					if res, perr := parseDSMLogin(respBody); perr == nil {
						c.Session.SetSession(res)
					}
...
func (c *Client) dryRun(method, targetURL, path string, params map[string]string, body []byte, headerOverrides map[string]string, authHeader string) (json.RawMessage, int, error) {
	...
	if params != nil {
		...
		for _, k := range keys {
			...
			// Hand-added: maskCredentialText only redacts values the client
			// already knows are credentials. A bare password value matches
			// nothing, so redact the DSM login parameters by name.
			value := c.maskCredentialText(params[k], authHeader)
			if isDSMSecretParam(k) {
				value = "***"
			}
...
func authPlaceholderCredentialError(cfg *config.Config) error {
	return authPlaceholderCredentialErrorWithSetup(cfg, "synology-pp-cli auth set-token <token>")
}
...
func (c *Client) maskCredentialText(text string, extraCredentials ...string) string {
	...
	// Hand-added: the value-based masks above only cover credentials the client
	// knows about. DSM puts the account password in the login request's query
	// string, and Go's *url.Error quotes that URL verbatim, so redact those
	// parameters by name as well.
	return scrubDSMSecrets(masked)
}
