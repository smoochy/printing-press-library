# Synology Printed CLI Agent Guide

This directory is a generated `synology-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
synology-pp-cli doctor --json
synology-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
synology-pp-cli which "<capability>" --json
synology-pp-cli <command> --help
```
...
```bash
synology-pp-cli <command> --help
synology-pp-cli <command> --dry-run --agent
```
...
Disable the loop with `--no-learn` per-invocation or `SYNOLOGY_NO_LEARN=true` for the whole session - useful for deterministic agent flows that don't want a learning row to silently change subsequent query results.
