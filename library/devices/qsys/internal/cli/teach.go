...
// qsys-pp-cli CLI: `teach` (fire-and-forget, silent on success,
...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/devices/qsys/internal/learn/entities"
	"github.com/mvanhorn/printing-press-library/library/devices/qsys/internal/learn/lookups"
	"github.com/mvanhorn/printing-press-library/library/devices/qsys/internal/learn/patterns"
...
const noLearnEnvVar = "QSYS_NO_LEARN"
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  qsys-pp-cli teach --query "<question>" --resource-type <type> --resource <id> --resource <id> &`,
...
when learnings exist.`,
		Example: `  qsys-pp-cli recall "<question>" --agent
  qsys-pp-cli recall "<question>" --agent --min-confidence 2`,
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  qsys-pp-cli learnings forget "<question>" --resource <id>
  qsys-pp-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
a whole family.`,
		Example: `  qsys-pp-cli teach-pattern --query-template "items in {entity}" --resource-template "GROUP-{entity:category}" --resource-type "items" --entity-kind "category" --strategy substitute`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  qsys-pp-cli teach-lookup --kind country --canonical "United States" --value USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
