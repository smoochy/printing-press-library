...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("QSYS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("QSYS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "qsys-pp-cli/feedback")
...
When ` + "`QSYS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`QSYS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  qsys-pp-cli feedback list
  qsys-pp-cli feedback list --limit 5
  qsys-pp-cli feedback list --json`,
