...
import (
	...
	"errors"
	"fmt"
	"github.com/mvanhorn/printing-press-library/library/devices/eero/internal/cliutil"
	"github.com/mvanhorn/printing-press-library/library/devices/eero/internal/config"
	"github.com/mvanhorn/printing-press-library/library/devices/eero/internal/platform"
	"io"
	"math"
	"net"
	"net/http"
...
func (c *Client) doInternal(ctx context.Context, method, path string, params map[string]string, body any, headerOverrides map[string]string, readOnlyIntent bool, mutationIntent bool) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		req.Header.Set("User-Agent", "eero-pp-cli/0.1 (unofficial; community API client)")
		...
		if req.Header.Get("User-Agent") == "" {
			...
			if ua := os.Getenv("EERO_USER_AGENT"); ua != "" {
...
func authPlaceholderCredentialError(cfg *config.Config) error {
	return authPlaceholderCredentialErrorWithSetup(cfg, "export EERO_SESSION_TOKEN=<your-token> or eero-pp-cli auth set-token <token>")
}
...
func (c *Client) maskCredentialText(text string, extraCredentials ...string) string {
	...
	if c != nil && c.Config != nil {
		...
		addCredential(c.Config.EeroSessionToken)
	}
