...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  agentmail-pp-cli doctor
  agentmail-pp-cli doctor --json
  agentmail-pp-cli doctor --fail-on warn
  agentmail-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if credentialRefused {
				...
				if cfg != nil {
					...
					if !configured {
						...
						report["auth_hint"] = "Set it with: agentmail-pp-cli auth set-token <token> or export AGENTMAIL_API_KEY=\"your-token-here\""
					} else {
						authConfigured = true
			...
			if os.Getenv("AGENTMAIL_API_KEY") != "" {
				authEnvSet = append(authEnvSet, "AGENTMAIL_API_KEY")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "AGENTMAIL_API_KEY")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						authHeaders["User-Agent"] = "agentmail-pp-cli"
						verifyPath := "/v0/auth/me"
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'agentmail-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'agentmail-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		...
		report["hint"] = "No sync recorded; run 'agentmail-pp-cli sync' to hydrate API-backed resources. Rows written by other paths are not tracked in sync_state."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'agentmail-pp-cli sync' to refresh."
	}
	return report
}
