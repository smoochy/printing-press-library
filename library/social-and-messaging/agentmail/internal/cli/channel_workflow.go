...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  agentmail-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  agentmail-pp-cli workflow archive --full

  # Archive without a wall-clock timeout
  agentmail-pp-cli workflow archive --timeout 0`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"api-keys", "api-keys-public-keys", "domains", "drafts", "inboxes", "metrics", "pods", "threads", "webhooks"}
			if cliutil.IsDogfoodEnv() {
				if len(resources) > 3 {
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  agentmail-pp-cli workflow status

  # Show status as JSON
  agentmail-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("agentmail-pp-cli")
			}
