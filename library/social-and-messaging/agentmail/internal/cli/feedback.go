...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("AGENTMAIL_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("AGENTMAIL_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "agentmail-pp-cli/feedback")
...
When ` + "`AGENTMAIL_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`AGENTMAIL_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
maintainer sees it.`,
		Example: "  agentmail-pp-cli feedback \"The triage queue needs a wider date filter\" --json",
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  agentmail-pp-cli feedback list
  agentmail-pp-cli feedback list --limit 5
  agentmail-pp-cli feedback list --json`,
