...
type rootFlags struct {
	...
	// invocation. Mirrors the AGENTMAIL_NO_LEARN env var.
	noLearn bool
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "agentmail-pp-cli",
		Short: `Agentmail CLI — AgentMail operations with local memory, safe sends, and fleet-wide insight.`,
		Long: `Agentmail CLI — AgentMail operations with local memory, safe sends, and fleet-wide insight.
...
Health check: run 'agentmail-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("agentmail-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'agentmail-pp-cli profile list')")
	...
	rootCmd.AddCommand(newReferenceAuthPromotedCmd(flags))
	...
	addNovelCommandIfAbsent(rootCmd, newNovelDeliveryCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelFleetCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelScheduleCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelSendCmd(flags))
