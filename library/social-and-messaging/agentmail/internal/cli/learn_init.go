...
func initLearn(ctx context.Context, db *sql.DB) error {
	...
	seeds := map[string][]lookups.SeedConfig{
		"domain": {
			{Canonical: "domain", Values: []string{"sending domain", "custom domain"}},
		},
		"draft": {
			{Canonical: "draft", Values: []string{"unsent email", "scheduled email"}},
		},
		"inbox": {
			{Canonical: "inbox", Values: []string{"mailbox", "email inbox"}},
		},
		"message": {
			{Canonical: "message", Values: []string{"email", "mail"}},
		},
		"thread": {
			{Canonical: "thread", Values: []string{"conversation", "email thread"}},
		},
	}
...
func runLearnInitOnce(ctx context.Context) {
	learnInitOnce.Do(func() {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "warning: agentmail-pp-cli: learn init: open store: %v\n", err)
			return
		}
		defer s.Close()
		if err := initLearn(ctx, s.DB()); err != nil {
			fmt.Fprintf(os.Stderr, "warning: agentmail-pp-cli: learn init: %v\n", err)
		}
	})
}
