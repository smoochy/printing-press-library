...
type Config struct {
	...
	AgentmailApiKey  string          `toml:"api_key"`
}
...
func Load(configPath string) (*Config, error) {
	...
	if v := os.Getenv("AGENTMAIL_API_KEY"); v != "" {
		cfg.AgentmailApiKey = v
		cfg.markEnvOverride("AgentmailApiKey")
		cfg.AuthSource = "env:AGENTMAIL_API_KEY"
		cfg.CredentialSource = "env:AGENTMAIL_API_KEY"
	}
	...
	if v := os.Getenv("AGENTMAIL_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("AGENTMAIL_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "agentmail-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.AgentmailApiKey != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:AGENTMAIL_API_KEY"
		}
		return "Bearer " + c.AgentmailApiKey
	}
	if c.AccessToken != "" {
...
func (c *Config) StoreScopeCredential() string {
	...
	if c.AgentmailApiKey != "" {
		parts = append(parts, "api_key="+c.AgentmailApiKey)
	}
	if len(parts) == 0 {
		return ""
	}
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		AgentmailApiKey: c.AgentmailApiKey,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	if c.AgentmailApiKey == "" {
		c.AgentmailApiKey = creds.AgentmailApiKey
	}
}
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.AgentmailApiKey = ""
	delete(c.envOverrides, "AgentmailApiKey")
	c.updateFileConfigField("AgentmailApiKey")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["AgentmailApiKey"] {
			out.AgentmailApiKey = c.fileConfig.AgentmailApiKey
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "AgentmailApiKey":
		c.fileConfig.AgentmailApiKey = c.AgentmailApiKey
	}
}
