...
func (c *Client) doInternal(ctx context.Context, method, path string, params map[string]string, body any, headerOverrides map[string]string, readOnlyIntent bool) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if req.Header.Get("User-Agent") == "" {
			req.Header.Set("User-Agent", "3cx-xapi-pp-cli/1.0.0")
		}
...
func (c *Client) dryRun(method, targetURL, path string, params map[string]string, body []byte, headerOverrides map[string]string, authHeader string) (json.RawMessage, int, error) {
	...
	if body != nil {
		...
		decoder.UseNumber()
		if decoder.Decode(&pretty) == nil {
			enc := json.NewEncoder(os.Stderr)
			enc.SetIndent("  ", "  ")
			fmt.Fprintf(os.Stderr, "  Body:\n")
			enc.Encode(redactDryRunValue(pretty))
		}
	}
	if authHeader != "" {
...
func redactDryRunValue(value any) any {
	...
	case map[string]any:
		...
		for key, child := range typed {
			...
			if strings.Contains(normalized, "password") || strings.Contains(normalized, "secret") || strings.Contains(normalized, "token") || strings.Contains(normalized, "apikey") || strings.Contains(normalized, "credential") || strings.Contains(normalized, "privatekey") {
				redacted[key] = "***"
				continue
			}
			redacted[key] = redactDryRunValue(child)
		}
		return redacted
	case []any:
		redacted := make([]any, len(typed))
		for i, child := range typed {
			redacted[i] = redactDryRunValue(child)
		}
		return redacted
	default:
		return value
...
func authPlaceholderCredentialError(cfg *config.Config) error {
	return authPlaceholderCredentialErrorWithSetup(cfg, "3cx-xapi-pp-cli auth login or 3cx-xapi-pp-cli auth set-token <token>")
}
...
func resolveClientCredentials(cfg *config.Config) (string, string) {
	...
	if id == "" {
		id = os.Getenv("TCX_CLIENT_ID")
	}
	if secret == "" {
		secret = os.Getenv("TCX_CLIENT_SECRET")
	}
	return id, secret
}

func resolveClientCredentialsScope() string {
	if scope := os.Getenv("API_3CX_XAPI_OAUTH_SCOPE"); scope != "" {
		return scope
	}
	return ""
}
...
func resolveClientCredentialsUserAgent() string {
	if ua := strings.TrimSpace(os.Getenv("API_3CX_XAPI_USER_AGENT")); ua != "" {
		return ua
	}
	return "3cx-xapi-pp-cli/1.0.0"
}

func (c *Client) mintClientCredentials(ctx context.Context, clientID, clientSecret string) error {
	tokenURL := resolveOAuthTokenURL(c.Config)
	if tokenURL == "" {
		return nil
	}
	form := url.Values{
