...
type Config struct {
	...
	UnipileApiKey    string          `toml:"api_key"`
...
func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://api49.unipile.com:17995",
	}
	...
	// Env var overrides
	if v := os.Getenv("UNIPILE_API_KEY"); v != "" {
		cfg.UnipileApiKey = v
		cfg.markEnvOverride("UnipileApiKey")
		cfg.AuthSource = "env:UNIPILE_API_KEY"
		cfg.CredentialSource = "env:UNIPILE_API_KEY"
	}
	...
	if v := os.Getenv("UNIPILE_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("UNIPILE_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "unipile-pp-cli", "config.toml"), nil
}
...
func (c *Config) StoreScopeCredential() string {
	...
	if c.UnipileApiKey != "" {
		parts = append(parts, "api_key="+c.UnipileApiKey)
	}
	if len(parts) == 0 {
		return ""
	}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	if c.UnipileApiKey == "" {
		c.UnipileApiKey = creds.UnipileApiKey
	}
}
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.UnipileApiKey = token
	delete(c.envOverrides, "UnipileApiKey")
	c.updateFileConfigField("UnipileApiKey")
	if err := c.saveCredentialsFirst(); err != nil {
		return err
	}
	return c.save()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["UnipileApiKey"] {
			out.UnipileApiKey = c.fileConfig.UnipileApiKey
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "UnipileApiKey":
		c.fileConfig.UnipileApiKey = c.UnipileApiKey
	}
}
