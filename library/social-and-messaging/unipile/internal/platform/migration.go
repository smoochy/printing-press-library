...
func AdoptVerifiedLegacyDatabase(ctx context.Context, session *Session, state *LegacyMigrationState) error {
	...
	// VACUUM INTO takes an expression, so the destination binds as a real
	// parameter. That keeps the path out of the SQL text entirely instead of
	// relying on manual quote escaping.
