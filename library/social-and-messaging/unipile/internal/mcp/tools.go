...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/social-and-messaging/unipile/internal/client"
	...
	"github.com/mvanhorn/printing-press-library/library/social-and-messaging/unipile/internal/mcp/cobratree"
		...
		if err != nil {
			...
			case strings.Contains(msg, "HTTP 400") && cliutil.LooksLikeAuthError(msg):
				return mcpToolError("authentication error: " + cliutil.SanitizeErrorBody(msg) +
					...
					"\n      Set your API key with: export UNIPILE_API_KEY=\"your-token-here\"" +
					"\n      Run 'unipile-pp-cli doctor' to check auth status."), nil
...
func openMCPReadOnlyStore(path string) (*store.Store, *mcplib.CallToolResult) {
	...
	if err != nil {
		return nil, mcplib.NewToolResultError(fmt.Sprintf("opening local data store %s: %v. Run unipile-pp-cli sync to refresh the store, or use live endpoint MCP tools for unsynced data.", path, err))
	}
	return db, nil
}

func mcpMissingStoreMessage(path string) string {
	return fmt.Sprintf("No local data store found at %s. Run unipile-pp-cli sync before using MCP search/sql, or use live endpoint MCP tools for unsynced data.", path)
}
...
func mcpEmptyStoreNextStep() string {
	return "Run unipile-pp-cli sync to populate the local SQLite store before using MCP search/sql."
}
...
func handleContext(_ context.Context, _ mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
	...
	ctx := map[string]any{
		"api":         "unipile",
		"description": "Unipile Communication is an **HTTP API**.",
		"archetype":   "communication",
		"tool_count":  94,
		"paths":       paths,
		// tool_surface tells agents which surface a capability lives on.
		"tool_surface": "MCP exposes typed endpoint tools plus a runtime mirror of user-facing CLI commands. Endpoint tools keep typed schemas; command-mirror tools shell out to the companion unipile-pp-cli binary.",
		"auth": map[string]any{
			"type": "api_key",
		...
		"resources": []map[string]any{
			{
				...
				"endpoints":   []string{"connect", "delete", "get", "list", "reconnect", "resend-checkpoint", "solve-checkpoint", "update"},
				"syncable":    true,
				"searchable":  true,
				"writable":    true,
			},
			{
				"name":        "accounts.restart",
			...
			},
			{
				...
				"endpoints":   []string{"resync-account"},
			},
			{
				"name":        "calendars",
			...
			{
				"name":        "chat-attendees",
				"description": "Manage chat attendees",
				"endpoints":   []string{"get", "list"},
				"syncable":    true,
				"searchable":  true,
			},
			{
				"name":        "chat-attendees.chats",
				"description": "Manage chats",
				"endpoints":   []string{"list"},
			},
			{
				"name":        "chat-attendees.messages",
				"description": "Manage messages",
				"endpoints":   []string{"list"},
			},
			{
				"name":        "chat-attendees.picture",
				"description": "Manage picture",
				"endpoints":   []string{"chat-attendees"},
			},
			{
				"name":        "chat-attendees.sync",
				"description": "Manage sync",
				"endpoints":   []string{"chat-attendees"},
			},
			{
				"name":        "chats",
				"description": "Manage chats",
				"endpoints":   []string{"delete", "get", "list", "start", "update"},
				"syncable":    true,
				"searchable":  true,
				"writable":    true,
			},
			{
				"name":        "chats.attendees",
			...
			},
			{
				"name":        "chats.messages",
				"description": "Manage messages",
				"endpoints":   []string{"list", "send"},
				"writable":    true,
			},
			{
				"name":        "chats.sync",
				"description": "Manage sync",
				"endpoints":   []string{"chats"},
			},
			{
				"name":        "drafts",
			...
			},
			{
				...
				"endpoints":   []string{"delete", "get", "list", "list-contacts", "send", "update"},
				"syncable":    true,
				"searchable":  true,
				"writable":    true,
			},
			...
			},
			{
				...
				"endpoints":   []string{"link"},
				"writable":    true,
			},
			{
				"name":        "linkedin",
				"description": "Manage linkedin",
				"endpoints":   []string{"action-user", "close-jobs", "company", "create-jobs", "endorse-profile", "get-applicants", "get-jobs", "get-projects", "inmail-balance", "list-applicants", "list-contracts", "list-jobs", "list-projects", "parameters-search", "publish-jobs", "raw", "resume-applicants", "search", "select-contracts", "solve-checkpoint-jobs", "update-jobs"},
				"syncable":    true,
				"searchable":  true,
				"writable":    true,
			},
			...
			},
			{
				...
				"description": "Manage reaction",
				"endpoints":   []string{"react-messages"},
				"writable":    true,
			},
			{
				"name":        "posts",
				"description": "Posts features",
				"endpoints":   []string{"create", "get", "react"},
				"searchable":  true,
				"writable":    true,
			},
			{
			...
			},
			{
				...
				"endpoints":   []string{"cancel-sent", "edit-me", "followers", "following", "get", "invite", "list-received", "list-sent", "me", "relations", "respond-received"},
				"syncable":    true,
				"searchable":  true,
				"writable":    true,
			},
		...
		"command_mirror_capabilities": []map[string]string{
			{"name": "Rate-budget ledger", "command": "budget", "description": "See how many LinkedIn invitations you have left today and this week, counted from your synced invitation history.", "rationale": "Unipile enforces no limits of its own; LinkedIn enforces them silently and answers with 422/429/500 or an account restriction. Only the local invitation history can say how much headroom is left, and it includes invitations sent from the LinkedIn UI as well as from this CLI.", "via": "mcp-command-mirror"},
			{"name": "Offline cross-provider search", "command": "search", "description": "Full-text search every message, email, attendee, and relation across all connected providers at once, offline.", "rationale": "The Unipile API has no search route that spans providers; this requires the whole mirror in one local FTS index.", "via": "mcp-command-mirror"},
			{"name": "Contact 360", "command": "contact", "description": "Everything the local mirror knows about one person: connection state, invitation history, and every conversation with per-direction message counts.", "rationale": "Joins attendees, relations, invitation history, chats, emails, and posts, which today means six API calls and a hand-written join.", "via": "mcp-command-mirror"},
			{"name": "Unified unread inbox", "command": "inbox", "description": "One table of everything unread across LinkedIn, WhatsApp, Telegram, Instagram, Messenger, and email.", "rationale": "The API reports unread per provider with no cross-provider ordering; unified triage only exists once the mirror is local.", "via": "mcp-command-mirror"},
			{"name": "Silence detection", "command": "silent", "description": "Find conversations where you sent the last message and got no reply for N days.", "rationale": "Requires per-chat last-message direction plus timestamp math over the local mirror; no endpoint returns it.", "via": "mcp-command-mirror"},
			{"name": "Accepted but not messaged", "command": "accepted", "description": "New LinkedIn connections since your last sync that you have not messaged yet.", "rationale": "Unipile's own docs call this a core need and warn that polling relations on a fixed schedule looks like automation; local relation-set diffing answers it without a poll loop.", "via": "mcp-command-mirror"},
			{"name": "Invitation funnel", "command": "funnel", "description": "Sent, accepted, and replied counts with conversion rates over a time window.", "rationale": "Joins the local action ledger against relations and chats, three sources the API never returns together.", "via": "mcp-command-mirror"},
			{"name": "Cross-provider digest", "command": "digest", "description": "What changed across every connected provider since the last sync.", "rationale": "Requires sync-cursor bookkeeping per resource per account that the API does not keep for you.", "via": "mcp-command-mirror"},
			{"name": "Resolved thread view", "command": "thread", "description": "Read one conversation end to end with attendee IDs resolved to real names.", "rationale": "Attendee IDs are opaque; resolution requires the local attendee mirror joined to the message list.", "via": "mcp-command-mirror"},
			{"name": "Post engagement crossmatch", "command": "engagement", "description": "Who reacted to or commented on your posts, flagged by whether they are already a connection.", "rationale": "Cross-references post reactions and comments against the local relations and invitation history.", "via": "mcp-command-mirror"},
		},
		"playbook": []map[string]string{
			{"topic": "Rate-budget ledger", "insight": ""},
			{"topic": "Offline cross-provider search", "insight": ""},
			{"topic": "Contact 360", "insight": ""},
			{"topic": "Unified unread inbox", "insight": ""},
			{"topic": "Silence detection", "insight": ""},
			{"topic": "Accepted but not messaged", "insight": ""},
			{"topic": "Invitation funnel", "insight": ""},
			{"topic": "Cross-provider digest", "insight": ""},
			{"topic": "Resolved thread view", "insight": ""},
			{"topic": "Post engagement crossmatch", "insight": ""},
