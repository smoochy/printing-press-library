...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("UNIPILE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("UNIPILE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "unipile-pp-cli/feedback")
...
When ` + "`UNIPILE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`UNIPILE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
maintainer sees it.`,
		Example: `  unipile-pp-cli feedback "budget should also count InMail credits"
  unipile-pp-cli feedback --stdin < notes.txt
  unipile-pp-cli feedback list --limit 5`,
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  unipile-pp-cli feedback list
  unipile-pp-cli feedback list --limit 5
  unipile-pp-cli feedback list --json`,
