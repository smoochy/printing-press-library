...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  unipile-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  unipile-pp-cli workflow archive --full

  # Archive without a wall-clock timeout
  unipile-pp-cli workflow archive --timeout 0`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"accounts", "calendars", "chat-attendees", "chats", "emails", "emails-contacts", "folders", "linkedin", "linkedin-jobs", "linkedin-projects", "messages", "users", "users-invite-received", "users-invite-sent", "users-relations", "webhooks"}
			if cliutil.IsDogfoodEnv() {
				if len(resources) > 3 {
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  unipile-pp-cli workflow status

  # Show status as JSON
  unipile-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("unipile-pp-cli")
			}
