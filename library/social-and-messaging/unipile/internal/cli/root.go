...
type rootFlags struct {
	...
	// invocation. Mirrors the UNIPILE_NO_LEARN env var.
	noLearn bool
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "unipile-pp-cli",
		Short: `Unipile CLI — Every Unipile endpoint as a typed command, plus a local mirror that makes cross-provider search possible and a rate…`,
		Long: `Unipile CLI — Every Unipile endpoint as a typed command, plus a local mirror that makes cross-provider search possible and an invitation ledger that shows how much LinkedIn headroom you have left.
...
Health check: run 'unipile-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("unipile-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.selectFields, "select", "", "Comma-separated fields to include in output (e.g. --select connection_params,created_at,current_signature)")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'unipile-pp-cli profile list')")
	...
	rootCmd.AddCommand(newChatAttendeesCmd(flags))
	...
	rootCmd.AddCommand(newDraftsPromotedCmd(flags))
	rootCmd.AddCommand(newHostedPromotedCmd(flags))
	...
	addNovelCommandIfAbsent(rootCmd, newNovelAcceptedCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelBudgetCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelContactCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelDigestCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelEngagementCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelFunnelCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelInboxCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelSilentCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelThreadCmd(flags))
