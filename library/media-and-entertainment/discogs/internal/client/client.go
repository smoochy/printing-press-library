...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/media-and-entertainment/discogs/internal/config"
	"io"
	"math"
	"net/http"
	"net/url"
	...
	httpClient.CheckRedirect = func(req *http.Request, via []*http.Request) error {
		...
		if req.URL.Host == via[0].URL.Host {
			if h, err := c.authHeader(req.Context()); err == nil && h != "" {
				req.Header.Set("Authorization", h) // #nosec G119 -- redirect policy deliberately strips Authorization on cross-host hops; this re-adds only for same-host redirects
			}
		} else {
...
func (c *Client) doInternal(ctx context.Context, method, path string, params map[string]string, body any, headerOverrides map[string]string, readOnlyIntent bool) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		req.Header.Set("User-Agent", "discogs-pp-cli/1.0 +https://github.com/mvanhorn/printing-press-library")
		if c.Config != nil {
		...
		if req.Header.Get("User-Agent") == "" {
			req.Header.Set("User-Agent", "discogs-pp-cli/")
		}
...
func (c *Client) dryRun(method, targetURL, path string, params map[string]string, body []byte, headerOverrides map[string]string, authHeader string) (json.RawMessage, int, error) {
	...
	if body != nil {
		...
		if json.Unmarshal(body, &pretty) == nil {
			...
			enc.Encode(pretty) // #nosec G104 -- best-effort diagnostic write; error intentionally ignored
		}
	}
	if authHeader != "" {
...
func authPlaceholderCredentialError(cfg *config.Config) error {
	return authPlaceholderCredentialErrorWithSetup(cfg, "export DISCOGS_TOKEN=<your-token> or discogs-pp-cli auth set-token <token>")
}
...
func (c *Client) maskCredentialText(text string, extraCredentials ...string) string {
	...
	if c != nil && c.Config != nil {
		...
		addCredential(c.Config.DiscogsToken)
	}
