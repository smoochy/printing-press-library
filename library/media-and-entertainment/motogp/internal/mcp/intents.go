...
func RegisterIntents(s *server.MCPServer) {
	s.AddTool(
		mcplib.NewTool("catch_up_on_the_season",
			mcplib.WithDescription("Finished rounds so far this season; add --winners for race winners."),
			mcplib.WithString("id", mcplib.Required(), mcplib.Description("Override the recipe's positional id value.")),
		),
		handleCatchUpOnTheSeason,
	)
}

// handleCatchUpOnTheSeason runs the catch_up_on_the_season recipe intent: Finished rounds so far this season; add --winners for race winners.
func handleCatchUpOnTheSeason(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
