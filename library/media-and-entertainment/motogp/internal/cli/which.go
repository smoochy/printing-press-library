...
var whichIndex = []whichEntry{
	{Command: "results", Description: "Query races by year, class name, event name, and session type instead of chained UUIDs.", Group: "Human-friendly access", WhyItMatters: "Lets an agent answer 'who won the Dutch MotoGP race in 2024' in one call instead of four UUID lookups."},
	{Command: "title-race", Description: "See how championship points evolved round-by-round across a season.", Group: "Local analyses that compound", WhyItMatters: "Answers 'when did the title get decided' without pulling every race by hand."},
	{Command: "h2h", Description: "Compare two riders' career and season stats side by side.", Group: "Local analyses that compound", WhyItMatters: "One call to settle a rider-versus-rider debate with real numbers."},
	{Command: "circuit-history", Description: "List winners at a given circuit across every synced season.", Group: "Local analyses that compound", WhyItMatters: "Surfaces track specialists and streaks an agent can't get from one API call."},
	{Command: "career", Description: "Merge a rider's profile with career and season-by-season stats into one timeline.", Group: "Human-friendly access", WhyItMatters: "Full career context in one command instead of stitching multiple endpoints."},
	{Command: "since", Description: "Show finished events and winners for a season window.", Group: "Human-friendly access", WhyItMatters: "Fast catch-up on results without paging the whole calendar."},
	{Command: "calendar", Description: "Export a season calendar to an ICS file from the local store.", Group: "Local analyses that compound", WhyItMatters: "Drop the race calendar straight into any calendar app, offline."},
	{Command: "live", Description: "Fetch the current live-timing feed as an agent-friendly JSON snapshot.", Group: "Human-friendly access", WhyItMatters: "Lets an agent poll race state during a session and get a defined empty result otherwise."},
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one`,
		Example: `  motogp-pp-cli which "stale tickets"
  motogp-pp-cli which "bottleneck"
  motogp-pp-cli which --limit 1 "send message"
  motogp-pp-cli which                                # list the full capability index`,
