...
type rootFlags struct {
	...
	// invocation. Mirrors the MOTOGP_NO_LEARN env var.
	noLearn       bool
	selectFields  string
	configPath    string
	homePath      string
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "motogp-pp-cli",
		Short: `Motogp CLI — The first CLI and MCP server for MotoGP — human-friendly commands, offline SQLite history, and analyses no single…`,
		Long: `Motogp CLI — The first CLI and MCP server for MotoGP — human-friendly commands, offline SQLite history, and analyses no single endpoint provides.
...
Health check: run 'motogp-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("motogp-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'motogp-pp-cli profile list')")
	...
	rootCmd.AddCommand(newBroadcastEventsCmd(flags))
	rootCmd.AddCommand(newRidersCmd(flags))
	...
	rootCmd.AddCommand(newNovelCareerCmd(flags))
	rootCmd.AddCommand(newNovelCircuitHistoryCmd(flags))
	rootCmd.AddCommand(newNovelH2hCmd(flags))
	rootCmd.AddCommand(newNovelLiveCmd(flags))
	rootCmd.AddCommand(newNovelResultsCmd(flags))
	rootCmd.AddCommand(newNovelSinceCmd(flags))
	rootCmd.AddCommand(newNovelTitleRaceCmd(flags))
	rootCmd.AddCommand(newAPICmd(flags))
	rootCmd.AddCommand(newBroadcastCategoriesPromotedCmd(flags))
	rootCmd.AddCommand(newCategoriesPromotedCmd(flags))
	rootCmd.AddCommand(newClassificationPromotedCmd(flags))
	rootCmd.AddCommand(newEventsPromotedCmd(flags))
	rootCmd.AddCommand(newGridPromotedCmd(flags))
	rootCmd.AddCommand(newSeasonsPromotedCmd(flags))
