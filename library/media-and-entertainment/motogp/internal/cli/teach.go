...
// motogp-pp-cli CLI: `teach` (fire-and-forget, silent on success,
...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/media-and-entertainment/motogp/internal/learn/entities"
	"github.com/mvanhorn/printing-press-library/library/media-and-entertainment/motogp/internal/learn/lookups"
	"github.com/mvanhorn/printing-press-library/library/media-and-entertainment/motogp/internal/learn/patterns"
...
const noLearnEnvVar = "MOTOGP_NO_LEARN"
...
func learnDBPath(explicit string) string {
	...
	return defaultDBPath("motogp-pp-cli")
}
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  motogp-pp-cli teach --query "<question>" --resource-type <type> \
...
when learnings exist.`,
		Example: `  motogp-pp-cli recall "<question>" --agent
  motogp-pp-cli recall "<question>" --agent --min-confidence 2`,
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "list",
		Short: "List recorded learnings from the teach log, filterable by query text, source, resource, action, and minimum confidence.",
		Example: `  motogp-pp-cli learnings list --agent
  motogp-pp-cli learnings list --query "<substring>"
  motogp-pp-cli learnings list --warnings --agent`,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  motogp-pp-cli learnings forget "<question>" --resource <id>
  motogp-pp-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
a whole family.`,
		Example: `  motogp-pp-cli teach-pattern \
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  motogp-pp-cli teach-lookup --kind country --canonical "United States" --value USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
			}
