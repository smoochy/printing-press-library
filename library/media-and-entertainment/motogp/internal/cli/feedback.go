...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("MOTOGP_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("MOTOGP_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "motogp-pp-cli/feedback")
...
When ` + "`MOTOGP_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`MOTOGP_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  motogp-pp-cli feedback list
  motogp-pp-cli feedback list --limit 5
  motogp-pp-cli feedback list --json`,
