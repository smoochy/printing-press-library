...
Resource scoping:
  ...
  motogp-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  motogp-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  motogp-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  motogp-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  motogp-pp-cli sync --latest-only`,
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "broadcast-categories":
		return db.UpsertBroadcastCategories(data)
	case "broadcast-events":
		return db.UpsertBroadcastEvents(data)
	case "categories":
		return db.UpsertCategories(data)
	case "events":
		return db.UpsertEvents(data)
	case "riders":
		return db.UpsertRiders(data)
	case "sessions":
...
var reconcileTypedTables = map[string]string{
	"broadcast-categories": "broadcast_categories",
