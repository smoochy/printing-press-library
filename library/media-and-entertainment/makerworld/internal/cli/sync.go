...
Resource scoping:
  ...
  makerworld-pp-cli sync

  # Sync specific resources only
  makerworld-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  makerworld-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  makerworld-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  makerworld-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  makerworld-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/makerworld-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "designers":
		return db.UpsertDesigners(data)
	case "designs":
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"categories":      "/search-service/homepage/nav",
		"designs":         "/search-service/select/design2",
		"designs-nav":     "/search-service/select/design/nav",
		"designs-youlike": "/search-service/recommand/youlike",
	}
