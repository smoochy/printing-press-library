...
type rootFlags struct {
	...
	// invocation. Mirrors the YOUTUBE_NO_LEARN env var.
	noLearn                 bool
	selectFields            string
	configPath              string
	homePath                string
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "youtube-pp-cli",
		Short: `Youtube CLI — A self-maintained competitor-monitoring machine for YouTube: the read surfaces that matter for market and competitor research plus a local databank of channel…`,
		Long: `Youtube CLI — A self-maintained competitor-monitoring machine for YouTube: the read surfaces that matter for market and competitor research plus a local databank of channel histories, snapshots, comments, and packaging assets - market data hours old, not weeks.
...
Highlights (not in the official API docs):
  ...
  • auth keys   Store multiple named YouTube API keys, switch between them instantly, and optionally fail over automatically via --rotate when one runs out of quota.`,
	...
	rootCmd.PersistentFlags().StringVar(&flags.selectFields, "select", "", "Comma-separated fields to include in output (e.g. --select contentDetails,etag,id)")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'youtube-pp-cli profile list')")
	...
	if parent, _, err := rootCmd.Find(strings.Fields("auth")); err == nil {
		addNovelCommandIfAbsent(parent, newNovelAuthKeysCmd(flags))
	}
	addNovelCommandIfAbsent(rootCmd, newNovelAuthKeysCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelBackfillCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelBreakoutsCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelCommentsMineCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelGrowthCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelMonitorCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelPackagingCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelVelocityCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelWatchCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelWorkspaceCmd(flags))
