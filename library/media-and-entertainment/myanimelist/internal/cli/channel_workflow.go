...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  myanimelist-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  myanimelist-pp-cli workflow archive --full

  # Archive without a wall-clock timeout
  myanimelist-pp-cli workflow archive --timeout 0`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"anime-api-video-around-json"}
			if cliutil.IsDogfoodEnv() {
				if len(resources) > 3 {
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  myanimelist-pp-cli workflow status

  # Show status as JSON
  myanimelist-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("myanimelist-pp-cli")
			}
