...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("MYANIMELIST_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("MYANIMELIST_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "myanimelist-pp-cli/feedback")
...
When ` + "`MYANIMELIST_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`MYANIMELIST_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
maintainer sees it.`,
		Example: `  myanimelist-pp-cli feedback "the --since flag is inclusive but docs say exclusive"
  myanimelist-pp-cli feedback --stdin < notes.txt
  myanimelist-pp-cli feedback list --limit 10`,
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  myanimelist-pp-cli feedback list
  myanimelist-pp-cli feedback list --limit 5
  myanimelist-pp-cli feedback list --json`,
