...
In local mode: searches locally synced data only.`,
  ...
  myanimelist-pp-cli search "error timeout"

  # Force local search only
  myanimelist-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  myanimelist-pp-cli search "status" --type anime --data-source local
  # JSON output for piping
  myanimelist-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /search/prefix.json
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get(cmd.Context(), "/search/prefix.json", map[string]string{
					"keyword": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'myanimelist-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "anime":
				results, err = db.SearchAnime(query, limit)
			case "feed":
				results, err = db.SearchFeed(query, limit)
			case "forum":
				results, err = db.SearchForum(query, limit)
			case "manga":
				results, err = db.SearchManga(query, limit)
			case "member":
				results, err = db.SearchMember(query, limit)
			case "news":
			...
			case "":
				...
				{
					partial, searchErr := db.SearchAnime(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search anime failed: %w", searchErr)
					}
				...
				{
					partial, searchErr := db.SearchFeed(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search feed failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchForum(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search forum failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchManga(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search manga failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchMember(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search member failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
