...
Resource scoping:
  ...
  myanimelist-pp-cli sync
  # Sync specific resources only
  myanimelist-pp-cli sync --resources anime,anime-api-video-around-json

  # Full resync (ignore previous checkpoint)
  myanimelist-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  myanimelist-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  myanimelist-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  myanimelist-pp-cli sync --latest-only`,
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "anime":
		...
		return db.UpsertAnime(typedData)
	case "character":
		typedData, err := typedSingleObjectData(data, obj, id, forceTypedID)
		if err != nil {
			return err
		}
		return db.UpsertCharacter(typedData)
	case "feed":
		typedData, err := typedSingleObjectData(data, obj, id, forceTypedID)
		if err != nil {
			return err
		}
		return db.UpsertFeed(typedData)
	case "forum":
		typedData, err := typedSingleObjectData(data, obj, id, forceTypedID)
		if err != nil {
			return err
		}
		return db.UpsertForum(typedData)
	case "genre":
		typedData, err := typedSingleObjectData(data, obj, id, forceTypedID)
		if err != nil {
			return err
		}
		return db.UpsertGenre(typedData)
	case "magazine":
		typedData, err := typedSingleObjectData(data, obj, id, forceTypedID)
		if err != nil {
			return err
		}
		return db.UpsertMagazine(typedData)
	case "manga":
		typedData, err := typedSingleObjectData(data, obj, id, forceTypedID)
		if err != nil {
			return err
		}
		return db.UpsertManga(typedData)
	case "member":
		typedData, err := typedSingleObjectData(data, obj, id, forceTypedID)
		if err != nil {
			return err
		}
		return db.UpsertMember(typedData)
	case "news":
		typedData, err := typedSingleObjectData(data, obj, id, forceTypedID)
		if err != nil {
			return err
		}
		return db.UpsertNews(typedData)
	case "person":
		typedData, err := typedSingleObjectData(data, obj, id, forceTypedID)
		if err != nil {
			return err
		}
		return db.UpsertPerson(typedData)
	case "ranking":
		typedData, err := typedSingleObjectData(data, obj, id, forceTypedID)
		if err != nil {
			return err
		}
		return db.UpsertRanking(typedData)
	case "season":
		typedData, err := typedSingleObjectData(data, obj, id, forceTypedID)
		if err != nil {
			return err
		}
		return db.UpsertSeason(typedData)
	case "sitemap":
		typedData, err := typedSingleObjectData(data, obj, id, forceTypedID)
		if err != nil {
			return err
		}
		return db.UpsertSitemap(typedData)
	case "studio":
		typedData, err := typedSingleObjectData(data, obj, id, forceTypedID)
		if err != nil {
			return err
		}
		return db.UpsertStudio(typedData)
	default:
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{ // #nosec G101 -- endpoint paths, not credentials.
		"anime":                       "/anime.php",
		"anime-api-video-around-json": "/anime/api/video/around.json",
		"feed":                        "/rss/featured.xml",
		"feed-news-xml":               "/rss/news.xml",
		"feed-rss-php":                "/rss.php",
		"forum":                       "/forum/",
		"instant":                     "/search/prefix.json",
		"manga":                       "/manga.php",
		"news":                        "/news",
		"ranking":                     "/topanime.php",
		"ranking-topmanga-php":        "/topmanga.php",
		"season":                      "/anime/season/archive",
		"season-later":                "/anime/season/later",
		"season-schedule":             "/anime/season/schedule",
...
func syncHTMLRequestHeaders(resource, parentResource string) map[string]string {
	if _, ok := syncHTMLExtractionOptions(resource, parentResource, "", "", "", nil); ok {
		return map[string]string{"X-Printing-Press-HTML-Response": "true"}
	}
	return nil
}
...
var htmlPageModeResources = map[string]bool{
	...
	"person":               true,
	"ranking":              true,
	"ranking-topmanga-php": true,
	"season":               true,
	"season-later":         true,
	"season-schedule":      true,
	"studio":               true,
}
