...
var whichIndex = []whichEntry{
	{Command: "anime divisive", Description: "See whether a title is universally loved or bitterly split, instead of reading one average score.", Group: "Reception analytics the site hides", WhyItMatters: "Use it before committing to a long series when the average score hides a love-it-or-hate-it split."},
	{Command: "drift", Description: "See how a title's score, member count, favorites, and rank have moved since you last looked.", Group: "Local state that compounds", WhyItMatters: "Reach for it when a question depends on change over time rather than the current number."},
	{Command: "week", Description: "Get a timezone-correct seven-day grid of just the shows you track, with same-hour collisions flagged.", Group: "Local state that compounds", WhyItMatters: "Use it to plan what to watch tonight without hand-converting Japan time for every show."},
	{Command: "anime drop-risk", Description: "Judge how likely a title is to be abandoned before you commit dozens of episodes to it.", Group: "Reception analytics the site hides", WhyItMatters: "Use it to triage a plan-to-watch backlog by follow-through rather than by average score."},
	{Command: "anime consistency", Description: "See whether a show improves, holds, or falls apart across its episodes using per-episode poll data.", Group: "Reception analytics the site hides", WhyItMatters: "Use it to decide whether to keep watching mid-season rather than finishing a dud."},
	{Command: "adaptation", Description: "Learn whether an anime covered its source manga and how much source material is left.", Group: "Franchise intelligence", WhyItMatters: "Use it when a user asks where to pick up the manga after the anime."},
	{Command: "franchise gap", Description: "Find the sequel, movie, or side story you skipped in every franchise you have already started.", Group: "Franchise intelligence", WhyItMatters: "Use it to catch missing entries before starting a new season of a franchise."},
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one. Machine output (--json/--csv/--plain/--quiet, or a pipe) still exits 2 and writes {"matches":[]} (or the equivalent empty table) to stdout.`,
		SilenceUsage: true,
		Example: `  myanimelist-pp-cli which "stale tickets"
  myanimelist-pp-cli which "bottleneck"
  myanimelist-pp-cli which --limit 1 "send message"
  myanimelist-pp-cli which                                # list the full capability index`,
