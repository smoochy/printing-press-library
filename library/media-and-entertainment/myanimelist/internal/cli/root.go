...
type rootFlags struct {
	...
	// invocation. Mirrors the MYANIMELIST_NO_LEARN env var.
	noLearn                 bool
	selectFields            string
	configPath              string
	homePath                string
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "myanimelist-pp-cli",
		Short: `Every public MyAnimeList surface in one binary, plus offline analytics the site itself cannot answer`,
		Long: `Every public MyAnimeList surface in one binary, plus offline analytics the site itself cannot answer
...
Health check: run 'myanimelist-pp-cli doctor' to verify connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("myanimelist-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.selectFields, "select", "", "Comma-separated fields to include in output (e.g. --select id,title,url)")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'myanimelist-pp-cli profile list')")
	...
	rootCmd.AddCommand(newAnimeCmd(flags))
	rootCmd.AddCommand(newForumCmd(flags))
	rootCmd.AddCommand(newGenreCmd(flags))
	rootCmd.AddCommand(newMangaCmd(flags))
	rootCmd.AddCommand(newMemberCmd(flags))
	rootCmd.AddCommand(newNewsCmd(flags))
	rootCmd.AddCommand(newRankingCmd(flags))
	...
	rootCmd.AddCommand(newCharacterPromotedCmd(flags))
	rootCmd.AddCommand(newInstantPromotedCmd(flags))
	rootCmd.AddCommand(newMagazinePromotedCmd(flags))
	rootCmd.AddCommand(newPersonPromotedCmd(flags))
	rootCmd.AddCommand(newStudioPromotedCmd(flags))
	...
	addNovelCommandIfAbsent(rootCmd, newNovelAdaptationCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelDriftCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelFranchiseCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelWeekCmd(flags))
