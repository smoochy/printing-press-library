...
func RegisterIntents(s *server.MCPServer) {
	s.AddTool(
		mcplib.NewTool("is_this_show_split_or_universally_loved",
			mcplib.WithDescription("Turns the 1-10 vote distribution into a polarization reading."),
			mcplib.WithString("id", mcplib.Required(), mcplib.Description("Override the recipe's positional id value.")),
			mcplib.WithOpenWorldHintAnnotation(true),
		),
		handleIsThisShowSplitOrUniversallyLoved,
	)
}

// handleIsThisShowSplitOrUniversallyLoved runs the is_this_show_split_or_universally_loved recipe intent: Turns the 1-10 vote distribution into a polarization reading.
func handleIsThisShowSplitOrUniversallyLoved(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
