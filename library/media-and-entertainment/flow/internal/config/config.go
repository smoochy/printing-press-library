...
type Config struct {
	...
	FlowSessionToken string          `toml:"session_token"`
}
...
func Load(configPath string) (*Config, error) {
	...
	// Env var overrides
	if v := strings.ToLower(strings.TrimSpace(os.Getenv("FLOW_SKIP_TLS_VERIFY"))); v == "1" || v == "true" || v == "yes" || v == "on" {
		cfg.SetSkipTLSVerify(true)
	}
	if v := os.Getenv("FLOW_SESSION_TOKEN"); v != "" {
		cfg.FlowSessionToken = v
		cfg.markEnvOverride("FlowSessionToken")
		cfg.AuthSource = "env:FLOW_SESSION_TOKEN"
		cfg.CredentialSource = "env:FLOW_SESSION_TOKEN"
	}
	...
	if v := os.Getenv("FLOW_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("FLOW_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "flow-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	// session_handshake auth: the harvested/env-supplied token lands in
	// FlowSessionToken (FLOW_SESSION_TOKEN, manually captured from the
	// browser -- Flow has no server-side token-mint endpoint this CLI can
	// call, see internal/client/client.go's doInternal) or the generic
	// AccessToken field (legacy config, stored credentials).
	// Neither is templated into AuthHeaderVal elsewhere, so construct the
	// header here per the spec's `auth.format: "Bearer {token}"`.
...
func (c *Config) StoreScopeCredential() string {
	...
	if c.FlowSessionToken != "" {
		parts = append(parts, "session_token="+c.FlowSessionToken)
	}
	if len(parts) == 0 {
		return ""
	}
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		FlowSessionToken: c.FlowSessionToken,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	if c.FlowSessionToken == "" {
		c.FlowSessionToken = creds.FlowSessionToken
	}
}
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.FlowSessionToken = ""
	delete(c.envOverrides, "FlowSessionToken")
	c.updateFileConfigField("FlowSessionToken")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
...
func (c *Config) configForSave() Config {
	...
	if c.fileConfig != nil {
		...
		if c.envOverrides["FlowSessionToken"] {
			out.FlowSessionToken = c.fileConfig.FlowSessionToken
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "FlowSessionToken":
		c.fileConfig.FlowSessionToken = c.FlowSessionToken
	}
}
