...
func RegisterIntents(s *server.MCPServer) {
	s.AddTool(
		mcplib.NewTool("check_a_batch_fits_your_credits_before_spending_anything",
			mcplib.WithDescription("Sums the queue's expected Veo-tier cost against your live balance and flags what to trim."),
			mcplib.WithString("domain", mcplib.Required(), mcplib.Description("Override the recipe's positional domain value.")),
		),
		handleCheckABatchFitsYourCreditsBeforeSpendingAnything,
	)
}

// handleCheckABatchFitsYourCreditsBeforeSpendingAnything runs the check_a_batch_fits_your_credits_before_spending_anything recipe intent: Sums the queue's expected Veo-tier cost against your live balance and flags what to trim.
func handleCheckABatchFitsYourCreditsBeforeSpendingAnything(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
