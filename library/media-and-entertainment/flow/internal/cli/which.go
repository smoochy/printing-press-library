...
var whichIndex = []whichEntry{
	{Command: "script draft-prompts", Description: "Turn a Scribe recap-script JSON into a ready-to-approve queue of per-segment Flow prompts, each matched to the right seed image automatically.", Group: "Audio-drama pipeline", WhyItMatters: "Reach for this whenever the user has a Scribe-produced recap script and wants shot-by-shot Flow prompts drafted without hand-typing each one."},
	{Command: "episode import", Description: "Pull a whole episode's assets from two separate Google Drive folders -- Scribe's session output and the images folder -- into a Flow project and draft the prompt queue, in one command.", Group: "Audio-drama pipeline", WhyItMatters: "Reach for this to start a new episode end-to-end from the two Drive folders instead of running drive import and script draft-prompts separately."},
	{Command: "mux", Description: "Lay your real audio-drama mp3 back over the rendered Flow clips, in the right order and at the right offsets, with one local command.", Group: "Audio-drama pipeline", WhyItMatters: "Reach for this as the last step of any audio-drama animation pass -- it replaces the manual video-editor re-assembly step entirely."},
	{Command: "queue estimate", Description: "See whether a prepared batch of generations fits your remaining Flow credits before you spend a single one.", Group: "Batch economics", WhyItMatters: "Reach for this before submitting any batch, especially when running multiple client projects against a shared credit pool."},
	{Command: "drive import", Description: "Pull seed images straight out of a local Google Drive folder into a Flow project, with character tags inferred automatically from filenames (requires --project for character names).", Group: "Drive ingestion", WhyItMatters: "Reach for this whenever new reference images land in a Drive folder and need staging into a Flow project -- eliminates the download-then-reupload dance entirely."},
	{Command: "video watch", Description: "Check on an entire submitted batch of generations with one command instead of clicking through each one.", Group: "Batch economics", WhyItMatters: "Reach for this after queuing several generations and walking away -- one glance at aggregate progress instead of N manual look-ups."},
	{Command: "scenes gaps", Description: "Find characters missing a reference image and see a media-status breakdown for a project, before you submit a batch.", Group: "Local sanity checks", WhyItMatters: "Reach for this as a pre-flight check before a big batch submission to catch missing assets early."},
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one`,
		Example: `  flow-pp-cli which "stale tickets"
  flow-pp-cli which "bottleneck"
  flow-pp-cli which --limit 1 "send message"
  flow-pp-cli which                                # list the full capability index`,
