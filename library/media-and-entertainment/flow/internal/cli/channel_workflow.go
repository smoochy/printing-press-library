...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  flow-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  flow-pp-cli workflow archive --full

  # Archive without a wall-clock timeout
  flow-pp-cli workflow archive --timeout 0`,
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  flow-pp-cli workflow status

  # Show status as JSON
  flow-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("flow-pp-cli")
			}
