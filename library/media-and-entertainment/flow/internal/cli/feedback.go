...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("FLOW_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("FLOW_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "github.com/mvanhorn/printing-press-library/library/media-and-entertainment/flow/feedback")
...
When ` + "`FLOW_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`FLOW_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
maintainer sees it.`,
		Example: `  flow-pp-cli feedback "credits --api-key needed a placeholder that looked like a real key or --dry-run silently ate it"
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  flow-pp-cli feedback list
  flow-pp-cli feedback list --limit 5
  flow-pp-cli feedback list --json`,
