...
func initLearn(ctx context.Context, db *sql.DB) error {
	...
	seeds := map[string][]lookups.SeedConfig{
		"emotional_tone": {
			{Canonical: "tense", Values: []string{"suspenseful"}},
			{Canonical: "heroic", Values: []string{"triumphant"}},
			{Canonical: "ominous", Values: []string{"foreboding"}},
		},
		"scene_type": {
			{Canonical: "combat", Values: []string{"fight", "battle"}},
			{Canonical: "exploration", Values: []string{"explore", "travel"}},
			{Canonical: "intrigue", Values: []string{"politics", "scheming"}},
			{Canonical: "social", Values: []string{"roleplay", "conversation"}},
			{Canonical: "mystery", Values: []string{"investigation"}},
			{Canonical: "dramatic", Values: []string{"drama"}},
			{Canonical: "climax", Values: []string{"finale"}},
		},
	}
...
func runLearnInitOnce(ctx context.Context) {
	learnInitOnce.Do(func() {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "warning: flow-pp-cli: learn init: open store: %v\n", err)
			return
		}
		defer s.Close()
		if err := initLearn(ctx, s.DB()); err != nil {
			fmt.Fprintf(os.Stderr, "warning: flow-pp-cli: learn init: %v\n", err)
		}
	})
}
