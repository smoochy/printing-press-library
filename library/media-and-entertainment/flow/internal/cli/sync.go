...
Resource scoping:
  ...
  flow-pp-cli sync --resources projects

  # Full resync (ignore previous checkpoint)
  flow-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  flow-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  flow-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  flow-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(resources) == 0 {
				if humanFriendly {
					fmt.Fprintln(os.Stderr, "flow-pp-cli sync: no default sync resources in spec; use --resources for explicit sync targets or populate the store via single-fetch commands.")
				} else {
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "flowWorkflows":
		return db.UpsertFlowWorkflows(data)
	case "projects":
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{ // #nosec G101 -- endpoint paths, not credentials.
		// Absolute URL: labs.google's tRPC BFF, not aisandbox-pa.googleapis.com.
...
func responsePathForResource(resource, path string) []string {
	...
	case "projects\x00https://labs.google/fx/api/trpc/project.searchUserProjects":
		return []string{"result.data.json.result.projects"}
	}
	return nil
}
