...
type rootFlags struct {
	...
	// invocation. Mirrors the FLOW_NO_LEARN env var.
	noLearn bool
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "flow-pp-cli",
		Short: `Flow CLI — Everything Flow's community CLIs and MCP servers do, plus the two things none of them do: real Google Drive ingestion…`,
		Long: `Flow CLI — Everything Flow's community CLIs and MCP servers do, plus the two things none of them do: real Google Drive ingestion and an honest audio-drama pipeline that closes the loop Flow itself cannot.
...
Health check: run 'flow-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("flow-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.selectFields, "select", "", "Comma-separated fields to include in output (e.g. --select remainingCredits,planTier)")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'flow-pp-cli profile list')")
	...
	rootCmd.AddCommand(newFlowWorkflowsPromotedCmd(flags))
	rootCmd.AddCommand(newProjectPromotedCmd(flags))
	rootCmd.AddCommand(newProjectsPromotedCmd(flags))
	rootCmd.AddCommand(newVideoPromotedCmd(flags))
	...
	addNovelCommandIfAbsent(rootCmd, newNovelDriveCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelEpisodeCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelMuxCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelQueueCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelScenesCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelScriptCmd(flags))
