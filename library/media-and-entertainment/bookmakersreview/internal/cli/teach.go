...
// bookmakersreview-pp-cli CLI: `teach` (fire-and-forget, silent on success,
...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/media-and-entertainment/bookmakersreview/internal/learn/patterns"
...
const noLearnEnvVar = "BOOKMAKERSREVIEW_NO_LEARN"
...
func learnDBPath(explicit string) string {
	...
	return defaultDBPath("bookmakersreview-pp-cli")
}
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  bookmakersreview-pp-cli teach --query "<question>" --resource-type <type> \
...
when learnings exist.`,
		Example: `  bookmakersreview-pp-cli recall "<question>" --agent
  bookmakersreview-pp-cli recall "<question>" --agent --min-confidence 2`,
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  bookmakersreview-pp-cli learnings list --agent
  bookmakersreview-pp-cli learnings list --query "<substring>"
  bookmakersreview-pp-cli learnings list --warnings --agent`,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  bookmakersreview-pp-cli learnings forget "<question>" --resource <id>
  bookmakersreview-pp-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
a whole family.`,
		Example: `  bookmakersreview-pp-cli teach-pattern \
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  bookmakersreview-pp-cli teach-lookup --kind country --canonical "United States" --value USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
			}
