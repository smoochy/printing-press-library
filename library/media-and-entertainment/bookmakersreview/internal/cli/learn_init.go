...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/media-and-entertainment/bookmakersreview/internal/learn/entities"
	"github.com/mvanhorn/printing-press-library/library/media-and-entertainment/bookmakersreview/internal/learn/lookups"
...
func initLearn(ctx context.Context, db *sql.DB) error {
	...
	seeds := map[string][]lookups.SeedConfig{
		"league": {
			{Canonical: "National Football League", Values: []string{"nfl"}},
			{Canonical: "National Basketball Association", Values: []string{"nba"}},
			{Canonical: "Major League Baseball", Values: []string{"mlb"}},
			{Canonical: "NHL", Values: []string{"nhl"}},
			{Canonical: "NCAA Football", Values: []string{"ncaaf", "college football"}},
			{Canonical: "WNBA", Values: []string{"wnba"}},
			{Canonical: "Ultimate Fighting Championship", Values: []string{"ufc", "mma"}},
			{Canonical: "Premier League", Values: []string{"epl", "english premier league"}},
			{Canonical: "Major League Soccer", Values: []string{"mls"}},
		},
		"sportsbook": {
			{Canonical: "Bovada", Values: []string{"bovada"}},
			{Canonical: "MyBookie", Values: []string{"mybookie"}},
			{Canonical: "BetOnline", Values: []string{"betonline", "bol"}},
			{Canonical: "Pinnacle", Values: []string{"pinnacle"}},
			{Canonical: "Bookmaker", Values: []string{"bookmaker"}},
			{Canonical: "Heritage Sports", Values: []string{"heritage"}},
			{Canonical: "5Dimes", Values: []string{"5dimes"}},
		},
	}
...
func runLearnInitOnce(ctx context.Context) {
	learnInitOnce.Do(func() {
		dbPath := defaultDBPath("bookmakersreview-pp-cli")
		s, err := store.OpenWithContext(ctx, dbPath)
		if err != nil {
			fmt.Fprintf(os.Stderr, "warning: bookmakersreview-pp-cli: learn init: open store: %v\n", err)
			return
		}
		defer s.Close()
		if err := initLearn(ctx, s.DB()); err != nil {
			fmt.Fprintf(os.Stderr, "warning: bookmakersreview-pp-cli: learn init: %v\n", err)
		}
	})
}
