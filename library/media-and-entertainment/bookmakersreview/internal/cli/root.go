...
type rootFlags struct {
	...
	// invocation. Mirrors the BOOKMAKERSREVIEW_NO_LEARN env var.
	noLearn       bool
	selectFields  string
	configPath    string
	homePath      string
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "bookmakersreview-pp-cli",
		Short: `BookmakersReview CLI — Every sportsbook's price, BMR's own sharp consensus, and full line history — for free, from the terminal, no API key…`,
		Long: `BookmakersReview CLI — Every sportsbook's price, BMR's own sharp consensus, and full line history — for free, from the terminal, no API key required.
...
Health check: run 'bookmakersreview-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("bookmakersreview-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'bookmakersreview-pp-cli profile list')")
	...
	rootCmd.AddCommand(newNovelArbCmd(flags))
	rootCmd.AddCommand(newNovelBetsCmd(flags))
	rootCmd.AddCommand(newNovelOddsCmd(flags))
	rootCmd.AddCommand(newNovelSteamCmd(flags))
