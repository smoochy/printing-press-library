...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("BOOKMAKERSREVIEW_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("BOOKMAKERSREVIEW_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "bookmakersreview-pp-cli/feedback")
...
When ` + "`BOOKMAKERSREVIEW_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`BOOKMAKERSREVIEW_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  bookmakersreview-pp-cli feedback list
  bookmakersreview-pp-cli feedback list --limit 5
  bookmakersreview-pp-cli feedback list --json`,
