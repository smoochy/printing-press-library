# Amc Theatres Printed CLI Agent Guide

This directory is a generated `amc-theatres-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
amc-theatres-pp-cli doctor --json
amc-theatres-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
amc-theatres-pp-cli which "<capability>" --json
amc-theatres-pp-cli <command> --help
```
...
```bash
amc-theatres-pp-cli <command> --agent
```
...
```bash
amc-theatres-pp-cli <command> --help
amc-theatres-pp-cli <command> --dry-run --agent
```
...
Before running these recipes, place the user's question in `AMC_THEATRES_USER_QUESTION` through the agent runtime's environment API (not by interpolating it into shell source). Then keep the quoted variable reference shown below so shell metacharacters remain inert.

1. On a new user question, call `amc-theatres-pp-cli recall "$AMC_THEATRES_USER_QUESTION" --agent` FIRST. If `found=true` and the top result has `entity_match == "exact"` and `confidence >= 2`, skip discovery and go straight to the live fetch for the returned resource IDs. If the store is cold (recall finds nothing and `learnings list` and `learnings candidates` are both empty), skip recall for the rest of the session.
...
3. After answering, always fire `amc-theatres-pp-cli teach --query "$AMC_THEATRES_USER_QUESTION" --resource <id> --resource-type <type> &` in the background - teaching is unconditional and is the anchor that triggers playbook synthesis. Teach the structural question with identifiers stripped (no names, emails, phone numbers, account ids); the CLI warns on obvious PII shapes but does not block.
4. Use `learnings list` to inspect taught rows, `learnings forget "$AMC_THEATRES_USER_QUESTION"` to undo a bad teach, `learnings candidates` for the full open candidate set, and `learnings stats` for the loop's local metrics. `teach-pattern` and `teach-lookup` install manual generalization rules when one teach should cover a whole family (e.g. one country alias unlocks every per-country query).
...
Disable the loop with `--no-learn` per-invocation or `AMC_THEATRES_NO_LEARN=true` for the whole session - useful for deterministic agent flows that don't want a learning row to silently change subsequent query results.
