...
func TestUpsertBatch_PopulatesMoviesTable(t *testing.T) {
	...
	items := []json.RawMessage{
		json.RawMessage(`{"id": "test-001", "theatres_id": "test-parent-001"}`),
		json.RawMessage(`{"id": "test-002", "theatres_id": "test-parent-001"}`),
		json.RawMessage(`{"id": "test-003", "theatres_id": "test-parent-001"}`),
	}
...
// TestUpsertBatch_TypedFailureDoesNotStrandMoviesGeneric exercises
...
func TestUpsertBatch_TypedFailureDoesNotStrandMoviesGeneric(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	// Items deliberately omit "theatres_id" so the typed NOT NULL
	// constraint fires.
	items := []json.RawMessage{
	...
	stored, extractFailures, err := s.UpsertBatch("movies", items)
	if err != nil {
	...
	if typed != 0 {
		t.Fatalf("movies count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "theatres_id")
	}
}

// TestUpsertBatch_KeysMoviesByChildAndParent verifies dependent
...
func TestUpsertBatch_KeysMoviesByChildAndParent(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	items := []json.RawMessage{
		json.RawMessage(`{"id": "shared_child", "theatres_id": "parent_A"}`),
		json.RawMessage(`{"id": "shared", "theatres_id": "child_parent_A"}`),
		json.RawMessage(`{"id": "shared_child", "theatres_id": "parent_B"}`),
		json.RawMessage(`{"id": "other_child", "theatres_id": "parent_A"}`),
	}
	stored, extractFailures, err := s.UpsertBatch("movies", items)
	if err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	...
	if typed != len(items) {
		t.Fatalf("movies count = %d, want %d (typed rows collapsed on child id)", typed, len(items))
	}

	var parentMatches int
	parentQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s" WHERE "theatres_id" = ?`, "movies")
	if err := db.QueryRow(parentQuery, "parent_A").Scan(&parentMatches); err != nil {
		t.Fatalf("count by theatres_id: %v", err)
	}
	if parentMatches != 2 {
		t.Fatalf("theatres_id=parent_A count = %d, want 2", parentMatches)
	}
}

// TestUpsertBatch_PopulatesTheatresShowtimesTable verifies that UpsertBatch
// dispatches paginated items into both the generic resources table AND the
// typed theatres_showtimes table. Regression for issue #268: before the fix, paginated
// syncs only filled the generic resources table, so domain commands that
// query the typed table saw zero rows.
func TestUpsertBatch_PopulatesTheatresShowtimesTable(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if _, _, err := s.UpsertBatch("theatres_showtimes", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "theatres_showtimes").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	if generic != len(items) {
		t.Fatalf("resources count = %d, want %d", generic, len(items))
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "theatres_showtimes")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count theatres_showtimes: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("theatres_showtimes count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}

// TestUpsertBatch_TypedFailureDoesNotStrandTheatresShowtimesGeneric exercises
// the savepoint isolation around the typed-table dispatch. The fixture omits
// the NOT NULL parent FK column so the typed insert fails; the savepoint
// rolls back only the typed projection. The generic resources row inserted
// just before must survive. Regression for issue #1392, where a single
// outer transaction caused typed-table failures to cascade and silently
// discard every successfully fetched API row.
func TestUpsertBatch_TypedFailureDoesNotStrandTheatresShowtimesGeneric(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	stored, extractFailures, err := s.UpsertBatch("theatres_showtimes", items)
	if err != nil {
		t.Fatalf("UpsertBatch: %v (typed-table failure must not propagate)", err)
	}
	if stored != len(items) {
	...
	if typed != 0 {
		t.Fatalf("theatres_showtimes count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "theatres_id")
	}
}

// TestUpsertBatch_KeysTheatresShowtimesByChildAndParent verifies dependent
// sub-collection rows with the same child id but different parent ids do not
// overwrite one another in either the generic resources table or the typed
// table. Regression for issue #2471.
func TestUpsertBatch_KeysTheatresShowtimesByChildAndParent(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if typed != len(items) {
		t.Fatalf("theatres_showtimes count = %d, want %d (typed rows collapsed on child id)", typed, len(items))
	}

	var parentMatches int
	parentQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s" WHERE "theatres_id" = ?`, "theatres_showtimes")
	if err := db.QueryRow(parentQuery, "parent_A").Scan(&parentMatches); err != nil {
		t.Fatalf("count by theatres_id: %v", err)
	}
	if parentMatches != 2 {
