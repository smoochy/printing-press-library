...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("AMC_THEATRES_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("AMC_THEATRES_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "amc-theatres-pp-cli/feedback")
...
When ` + "`AMC_THEATRES_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`AMC_THEATRES_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  amc-theatres-pp-cli feedback list
  amc-theatres-pp-cli feedback list --limit 5
  amc-theatres-pp-cli feedback list --json`,
