...
// amc-theatres-pp-cli CLI: `teach` (fire-and-forget, silent on success,
...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/media-and-entertainment/amc-theatres/internal/learn/entities"
	"github.com/mvanhorn/printing-press-library/library/media-and-entertainment/amc-theatres/internal/learn/lookups"
	"github.com/mvanhorn/printing-press-library/library/media-and-entertainment/amc-theatres/internal/learn/patterns"
...
const noLearnEnvVar = "AMC_THEATRES_NO_LEARN"
...
func learnDBPath(explicit string) string {
	...
	return defaultDBPath("amc-theatres-pp-cli")
}
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  amc-theatres-pp-cli teach --query "<question>" --resource-type <type> --resource <id> --resource <id> &`,
...
when learnings exist.`,
		Example: `  amc-theatres-pp-cli recall "<question>" --agent
  amc-theatres-pp-cli recall "<question>" --agent --min-confidence 2`,
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  amc-theatres-pp-cli learnings list --agent
  amc-theatres-pp-cli learnings list --query "<substring>"
  amc-theatres-pp-cli learnings list --warnings --agent`,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  amc-theatres-pp-cli learnings forget "<question>" --resource <id>
  amc-theatres-pp-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
a whole family.`,
		Example: `  amc-theatres-pp-cli teach-pattern --query-template "items in {entity}" --resource-template "GROUP-{entity:category}" --resource-type "items" --entity-kind "category" --strategy substitute`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
			}
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  amc-theatres-pp-cli teach-lookup --kind country --canonical "United States" --value USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
			}
