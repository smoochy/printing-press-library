...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  amc-theatres-pp-cli doctor
  amc-theatres-pp-cli doctor --json
  amc-theatres-pp-cli doctor --fail-on warn
  amc-theatres-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "Set your API key with: export AMC_THEATRES_VENDOR_KEY=\"your-token-here\""
				} else {
					authConfigured = true
			...
			if os.Getenv("AMC_THEATRES_VENDOR_KEY") != "" {
				authEnvSet = append(authEnvSet, "AMC_THEATRES_VENDOR_KEY")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "AMC_THEATRES_VENDOR_KEY")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "amc-theatres-pp-cli", suggestion)
						} else {
