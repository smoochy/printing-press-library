...
type Config struct {
	...
	AmcTheatresVendorKey string          `toml:"theatres_vendor_key"`
}
...
func Load(configPath string) (*Config, error) {
	...
	// Env var overrides
	if v := os.Getenv("AMC_THEATRES_VENDOR_KEY"); v != "" {
		cfg.AmcTheatresVendorKey = v
		cfg.markEnvOverride("AmcTheatresVendorKey")
		cfg.AuthSource = "env:AMC_THEATRES_VENDOR_KEY"
		cfg.CredentialSource = "env:AMC_THEATRES_VENDOR_KEY"
	}
	...
	if v := os.Getenv("AMC_THEATRES_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("AMC_THEATRES_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "amc-theatres-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	token := c.AmcTheatresVendorKey
	if token == "" {
		return ""
	}
	if c.AmcTheatresVendorKey == "" {
		return ""
	}
	return token
}
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		AmcTheatresVendorKey: c.AmcTheatresVendorKey,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	if c.AmcTheatresVendorKey == "" {
		c.AmcTheatresVendorKey = creds.AmcTheatresVendorKey
	}
}
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.AmcTheatresVendorKey = token
	delete(c.envOverrides, "AmcTheatresVendorKey")
	c.updateFileConfigField("AmcTheatresVendorKey")
	if err := c.saveCredentialsFirst(); err != nil {
		return err
	}
	return c.save()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["AmcTheatresVendorKey"] {
			out.AmcTheatresVendorKey = c.fileConfig.AmcTheatresVendorKey
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "AmcTheatresVendorKey":
		c.fileConfig.AmcTheatresVendorKey = c.AmcTheatresVendorKey
	}
}
