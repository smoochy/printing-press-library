...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("PLEXCTL_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("PLEXCTL_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "github.com/mvanhorn/printing-press-library/library/media-and-entertainment/plexctl/feedback")
...
When ` + "`PLEXCTL_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`PLEXCTL_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
maintainer sees it.`,
		Example: `  plexctl-pp-cli feedback "the --since flag is inclusive but docs say exclusive"
  plexctl-pp-cli feedback --stdin < notes.txt
  plexctl-pp-cli feedback list --limit 10`,
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  plexctl-pp-cli feedback list
  plexctl-pp-cli feedback list --limit 5
  plexctl-pp-cli feedback list --json`,
