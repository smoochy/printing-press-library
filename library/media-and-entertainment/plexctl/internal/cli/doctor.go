...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  plexctl-pp-cli doctor
  plexctl-pp-cli doctor --json
  plexctl-pp-cli doctor --fail-on warn
  plexctl-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if credentialRefused {
				...
				if cfg != nil {
					...
					if !configured {
						...
						report["auth_hint"] = "Set your API key with: export PLEXCTL_USER_TOKEN=\"your-token-here\""
						report["auth_key_url"] = "https://...`"
					} else {
						authConfigured = true
			...
			if os.Getenv("PLEXCTL_USER_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "PLEXCTL_USER_TOKEN")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "PLEXCTL_USER_TOKEN")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "plexctl-pp-cli", suggestion)
						} else {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'plexctl-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'plexctl-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny:
		...
		report["hint"] = "No sync recorded; run 'plexctl-pp-cli sync' to hydrate API-backed resources. Rows written by other paths are not tracked in sync_state."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'plexctl-pp-cli sync' to refresh."
	}
	return report
}
