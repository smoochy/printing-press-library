...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  plexctl-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  plexctl-pp-cli workflow archive --full

  # Archive without a wall-clock timeout
  plexctl-pp-cli workflow archive --timeout 0`,
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  plexctl-pp-cli workflow status

  # Show status as JSON
  plexctl-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("plexctl-pp-cli")
			}
