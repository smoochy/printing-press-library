...
func newPlaylistsGeneratorsCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:         "generators",
		Short:       "Get generators for playlists",
		Annotations: map[string]string{"mcp:read-only": "true", "pp:parent-group": "true", "pp:typed-exit-codes": "0,2"},
		RunE:        parentNoSubcommandRunE(flags),
	}

	cmd.AddCommand(newPlaylistsGeneratorsPlaylistGetCmd(flags))
	return cmd
}
