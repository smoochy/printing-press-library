...
In local mode: searches locally synced data only.`,
  ...
  plexctl-pp-cli search "error timeout"

  # Force local search only
  plexctl-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  plexctl-pp-cli search "status" --type activities --data-source local
  # JSON output for piping
  plexctl-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /hubs/search
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get(cmd.Context(), "/hubs/search", map[string]string{
					"query": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'plexctl-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
