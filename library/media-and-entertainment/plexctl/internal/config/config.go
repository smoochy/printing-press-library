...
type Config struct {
	...
	PlexctlUserToken string          `toml:"user_token"`
...
func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://1-2-3-4.{identifier}.plex.direct:{port}",
	}
	...
	// Env var overrides
	if v := os.Getenv("PLEXCTL_USER_TOKEN"); v != "" {
		cfg.PlexctlUserToken = v
		cfg.markEnvOverride("PlexctlUserToken")
		cfg.AuthSource = "env:PLEXCTL_USER_TOKEN"
		cfg.CredentialSource = "env:PLEXCTL_USER_TOKEN"
	}
	...
	if v := os.Getenv("PLEXCTL_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	...
	if v := strings.TrimSpace(os.Getenv("PLEXCTL_IDENTIFIER")); v != "" {
		cfg.TemplateVars["identifier"] = normalizeEndpointTemplateValue(v)
	} else {
		cfg.TemplateVars["identifier"] = "0123456789abcdef0123456789abcdef"
	}
	if v := strings.TrimSpace(os.Getenv("PLEXCTL_PORT")); v != "" {
		cfg.TemplateVars["port"] = normalizeEndpointTemplateValue(v)
	} else {
		cfg.TemplateVars["port"] = "32400"
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("PLEXCTL_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "plexctl-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	token := c.PlexctlUserToken
	if token == "" {
		return ""
	}
	if c.PlexctlUserToken == "" {
		return ""
	}
	return token
}
...
func (c *Config) StoreScopeCredential() string {
	...
	if c.PlexctlUserToken != "" {
		parts = append(parts, "user_token="+c.PlexctlUserToken)
	}
	if len(parts) == 0 {
		return ""
	}
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		PlexctlUserToken: c.PlexctlUserToken,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	if c.PlexctlUserToken == "" {
		c.PlexctlUserToken = creds.PlexctlUserToken
	}
}
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.PlexctlUserToken = token
	delete(c.envOverrides, "PlexctlUserToken")
	c.updateFileConfigField("PlexctlUserToken")
	if err := c.saveCredentialsFirst(); err != nil {
		return err
	}
	return c.save()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["PlexctlUserToken"] {
			out.PlexctlUserToken = c.fileConfig.PlexctlUserToken
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "PlexctlUserToken":
		c.fileConfig.PlexctlUserToken = c.PlexctlUserToken
	}
}
