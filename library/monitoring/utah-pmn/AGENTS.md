# Utah Public Notice Website Printed CLI Agent Guide

This directory is a generated `utah-pmn-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
utah-pmn-pp-cli doctor --json
utah-pmn-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
utah-pmn-pp-cli which "<capability>" --json
utah-pmn-pp-cli <command> --help
```
...
```bash
utah-pmn-pp-cli <command> --help
utah-pmn-pp-cli <command> --dry-run --agent
```
