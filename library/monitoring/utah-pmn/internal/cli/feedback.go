...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("UTAH_PMN_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("UTAH_PMN_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "utah-pmn-pp-cli/feedback")
...
When ` + "`UTAH_PMN_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`UTAH_PMN_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  utah-pmn-pp-cli feedback list
  utah-pmn-pp-cli feedback list --limit 5
  utah-pmn-pp-cli feedback list --json`,
