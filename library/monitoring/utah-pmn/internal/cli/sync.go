...
Resource scoping:
  ...
  utah-pmn-pp-cli sync

  # Sync specific resources only
  utah-pmn-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  utah-pmn-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  utah-pmn-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  utah-pmn-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  utah-pmn-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(resources) == 0 {
				if humanFriendly {
					fmt.Fprintln(os.Stderr, "utah-pmn-pp-cli sync: no default sync resources in spec; use --resources for explicit sync targets or populate the store via single-fetch commands.")
				} else {
