...
func (c *Client) doInternal(ctx context.Context, method, path string, params map[string]string, body any, headerOverrides map[string]string, readOnlyIntent bool) (json.RawMessage, int, error) {
	...
	// Judge.me's private API requires shop_domain alongside the X-Api-Token
	// header. Keep the token out of the URL, and inject only the non-secret
	// domain from its dedicated environment variable. Clone params so callers
	// do not observe authentication plumbing in their input map.
	if shopDomain := strings.TrimSpace(os.Getenv("JUDGEME_SHOP_DOMAIN")); shopDomain != "" {
		cloned := make(map[string]string, len(params)+1)
		for key, value := range params {
			cloned[key] = value
		}
		if cloned["shop_domain"] == "" {
			cloned["shop_domain"] = shopDomain
		}
		params = cloned
	}
	...
	canRetryRejectedRequest := readOnlyIntent || platform.CanRetryRequest(method, requestIdempotencyKey(c.Config, headerOverrides))
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if c.Config != nil {
			...
			if c.Config.JudgemeApiKey != "" {
				h := c.Config.JudgemeApiKey
				req.Header.Del("Authorization")
		...
		if req.Header.Get("User-Agent") == "" {
			req.Header.Set("User-Agent", "judgeme-pp-cli/")
		}
...
func (c *Client) dryRun(method, targetURL, path string, params map[string]string, body []byte, headerOverrides map[string]string, authHeader string) (json.RawMessage, int, error) {
	...
	if authHeader != "" && (c.Config == nil || c.Config.JudgemeApiKey == "") {
		fmt.Fprintf(os.Stderr, "  %s: %s\n", "Authorization", maskToken(authHeader))
	}
	if c.Config != nil && c.Config.JudgemeApiKey != "" {
		fmt.Fprintf(os.Stderr, "  %s: %s\n", "X-Api-Token", maskToken(c.Config.JudgemeApiKey))
	}
...
func authPlaceholderCredentialError(cfg *config.Config) error {
	return authPlaceholderCredentialErrorWithSetup(cfg, "export JUDGEME_API_KEY=<your-token> or judgeme-pp-cli auth set-token <token>")
}
...
func (c *Client) maskCredentialText(text string, extraCredentials ...string) string {
	...
	if c != nil && c.Config != nil {
		...
		addCredential(c.Config.JudgemeApiKey)
		addCredential(c.Config.JudgemeOauthApikey)
	}
