...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/marketing/placeit/internal/config"
)
...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "placeit-pp-cli", "http")
	...
	httpClient.CheckRedirect = func(req *http.Request, via []*http.Request) error {
		...
		// header (the same hazard the cookie-jar branch below avoids). On a
		// cross-host hop, drop the Cookie header entirely: no credential
		// (api-key cookie, a cookie-auth session token such as Placeit's
		// logged_session, or any jar-provided cookie) should follow a request
		// to a different host. Same-host redirects keep their cookies above.
...
func (c *Client) doInternal(ctx context.Context, method, path string, params map[string]string, body any, headerOverrides map[string]string, readOnlyIntent bool) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if authHeader != "" {
			req.AddCookie(&http.Cookie{Name: "api_key", Value: authHeader})
		}
		if c.Config != nil {
...
func authPlaceholderCredentialError(cfg *config.Config) error {
	return authPlaceholderCredentialErrorWithSetup(cfg, "placeit-pp-cli auth set-token <token>")
}
