...
type Config struct {
	...
	OpenaiAdsApiKey            string          `toml:"ads_api_key"`
	OpenaiAdsConversionsApiKey string          `toml:"ads_conversions_api_key"`
}
...
func Load(configPath string) (*Config, error) {
	...
	// Env var overrides
	if v := os.Getenv("OPENAI_ADS_API_KEY"); v != "" {
		cfg.OpenaiAdsApiKey = v
		cfg.markEnvOverride("OpenaiAdsApiKey")
		cfg.AuthSource = "env:OPENAI_ADS_API_KEY"
		cfg.CredentialSource = "env:OPENAI_ADS_API_KEY"
	}
	if v := os.Getenv("OPENAI_ADS_CONVERSIONS_API_KEY"); v != "" {
		cfg.OpenaiAdsConversionsApiKey = v
		cfg.markEnvOverride("OpenaiAdsConversionsApiKey")
		cfg.AuthSource = "env:OPENAI_ADS_CONVERSIONS_API_KEY"
		cfg.CredentialSource = "env:OPENAI_ADS_CONVERSIONS_API_KEY"
	}
	...
	if v := os.Getenv("OPENAI_ADS_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("OPENAI_ADS_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "openai-ads-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.OpenaiAdsApiKey != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:OPENAI_ADS_API_KEY"
		}
		return "Bearer " + c.OpenaiAdsApiKey
	}
	if c.OpenaiAdsConversionsApiKey != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:OPENAI_ADS_CONVERSIONS_API_KEY"
		}
		return "Bearer " + c.OpenaiAdsConversionsApiKey
	}
	if c.AccessToken != "" {
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		OpenaiAdsApiKey:            c.OpenaiAdsApiKey,
		OpenaiAdsConversionsApiKey: c.OpenaiAdsConversionsApiKey,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	if c.OpenaiAdsApiKey == "" {
		c.OpenaiAdsApiKey = creds.OpenaiAdsApiKey
	}
	if c.OpenaiAdsConversionsApiKey == "" {
		c.OpenaiAdsConversionsApiKey = creds.OpenaiAdsConversionsApiKey
	}
}
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.OpenaiAdsApiKey = ""
	delete(c.envOverrides, "OpenaiAdsApiKey")
	c.updateFileConfigField("OpenaiAdsApiKey")
	c.OpenaiAdsConversionsApiKey = ""
	delete(c.envOverrides, "OpenaiAdsConversionsApiKey")
	c.updateFileConfigField("OpenaiAdsConversionsApiKey")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["OpenaiAdsApiKey"] {
			out.OpenaiAdsApiKey = c.fileConfig.OpenaiAdsApiKey
		}
		if c.envOverrides["OpenaiAdsConversionsApiKey"] {
			out.OpenaiAdsConversionsApiKey = c.fileConfig.OpenaiAdsConversionsApiKey
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "OpenaiAdsApiKey":
		c.fileConfig.OpenaiAdsApiKey = c.OpenaiAdsApiKey
	case "OpenaiAdsConversionsApiKey":
		c.fileConfig.OpenaiAdsConversionsApiKey = c.OpenaiAdsConversionsApiKey
	}
}
