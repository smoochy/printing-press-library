...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Openai Ads",
...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  openai-ads-pp-cli auth setup\n  openai-ads-pp-cli auth setup --launch",
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			fmt.Fprintln(w, "No setup URL is configured for this CLI; check the API's docs.")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Then set:")
			fmt.Fprintln(w, "  export OPENAI_ADS_API_KEY=\"your-token-here\"")
			fmt.Fprintln(w, "  openai-ads-pp-cli auth set-token <token>")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Optional request credentials:")
			fmt.Fprintln(w, "  export OPENAI_ADS_CONVERSIONS_API_KEY=\"your-token-here\"")
			if !launch {
				return nil
			}
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  openai-ads-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !authed {
				...
				fmt.Fprintln(w, "  export OPENAI_ADS_API_KEY=\"your-token-here\" # Ads API key from Ads Manager > Settings. Scoped to one ad account. NOT the Conversions API key from Ads Manager > Conversions (that one returns 403 Unauthorized to read ads data).")
				fmt.Fprintf(w, "  openai-ads-pp-cli auth set-token <token>\n")
				fmt.Fprintln(w, "")
				fmt.Fprintln(w, "Optional request credentials:")
				fmt.Fprintln(w, "  export OPENAI_ADS_CONVERSIONS_API_KEY=\"your-token-here\" # Optional. Conversions API key from Ads Manager > Conversions, scoped to one conversion event in a data source. Only needed for server-side conversion event ingestion; it cannot read ads data.")
				return authErr(fmt.Errorf("no credentials configured"))
			}
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  openai-ads-pp-cli auth set-token YOUR_TOKEN_HERE",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  openai-ads-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			envStillSet := ""
			if envStillSet == "" && os.Getenv("OPENAI_ADS_API_KEY") != "" {
				envStillSet = "OPENAI_ADS_API_KEY"
			}
			if envStillSet == "" && os.Getenv("OPENAI_ADS_CONVERSIONS_API_KEY") != "" {
				envStillSet = "OPENAI_ADS_CONVERSIONS_API_KEY"
			}
