...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  openai-ads-pp-cli doctor
  openai-ads-pp-cli doctor --json
  openai-ads-pp-cli doctor --fail-on warn
  openai-ads-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if !configured {
					...
					report["auth_hint"] = "Set it with: openai-ads-pp-cli auth set-token <token> or export OPENAI_ADS_API_KEY=\"your-token-here\""
				} else {
					authConfigured = true
			...
			if os.Getenv("OPENAI_ADS_API_KEY") != "" {
				authEnvSet = append(authEnvSet, "OPENAI_ADS_API_KEY")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "OPENAI_ADS_API_KEY")
			}
			if strings.Contains("Optional. Conversions API key from Ads Manager > Conversions, scoped to one conversion event in a data source. Only needed for server-side conversion event ingestion; it cannot read ads data.", " OR ") {
				authEnvOptionalNames = append(authEnvOptionalNames, "OPENAI_ADS_CONVERSIONS_API_KEY")
				if os.Getenv("OPENAI_ADS_CONVERSIONS_API_KEY") != "" {
					authEnvSet = append(authEnvSet, "OPENAI_ADS_CONVERSIONS_API_KEY")
					authEnvOptionalSatisfied = true
				}
			} else if os.Getenv("OPENAI_ADS_CONVERSIONS_API_KEY") != "" {
				authEnvSet = append(authEnvSet, "OPENAI_ADS_CONVERSIONS_API_KEY")
			} else {
				authEnvInfo = append(authEnvInfo, "OPENAI_ADS_CONVERSIONS_API_KEY optional")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "openai-ads-pp-cli", suggestion)
						} else {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'openai-ads-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'openai-ads-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		...
		report["hint"] = "No sync recorded; run 'openai-ads-pp-cli sync' to hydrate API-backed resources. Rows written by other paths are not tracked in sync_state."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'openai-ads-pp-cli sync' to refresh."
	}
	return report
}
