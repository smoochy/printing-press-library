...
func newLearnConfig() *entities.Config {
	...
	if re, err := regexp.Compile(`^adacct_[a-f0-9]+$`); err != nil {
		fmt.Fprintf(os.Stderr, "warning: openai-ads-pp-cli: ticker pattern %q failed to compile: %v\n", `^adacct_[a-f0-9]+$`, err)
	} else {
		cfg.RegisterTickerPattern(re)
	}
	if re, err := regexp.Compile(`^cmpn_[a-f0-9]+$`); err != nil {
		fmt.Fprintf(os.Stderr, "warning: openai-ads-pp-cli: ticker pattern %q failed to compile: %v\n", `^cmpn_[a-f0-9]+$`, err)
	} else {
		cfg.RegisterTickerPattern(re)
	}
	if re, err := regexp.Compile(`^adgrp_[a-f0-9]+$`); err != nil {
		fmt.Fprintf(os.Stderr, "warning: openai-ads-pp-cli: ticker pattern %q failed to compile: %v\n", `^adgrp_[a-f0-9]+$`, err)
	} else {
		cfg.RegisterTickerPattern(re)
	}
	if re, err := regexp.Compile(`^ad_[a-f0-9]+$`); err != nil {
		fmt.Fprintf(os.Stderr, "warning: openai-ads-pp-cli: ticker pattern %q failed to compile: %v\n", `^ad_[a-f0-9]+$`, err)
	} else {
		cfg.RegisterTickerPattern(re)
	}
	if re, err := regexp.Compile(`^file_[a-f0-9]+$`); err != nil {
		fmt.Fprintf(os.Stderr, "warning: openai-ads-pp-cli: ticker pattern %q failed to compile: %v\n", `^file_[a-f0-9]+$`, err)
	} else {
		cfg.RegisterTickerPattern(re)
	}
...
func runLearnInitOnce(ctx context.Context) {
	learnInitOnce.Do(func() {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "warning: openai-ads-pp-cli: learn init: open store: %v\n", err)
			return
		}
		defer s.Close()
		if err := initLearn(ctx, s.DB()); err != nil {
			fmt.Fprintf(os.Stderr, "warning: openai-ads-pp-cli: learn init: %v\n", err)
		}
	})
}
