...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  openai-ads-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  openai-ads-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"ad-account", "campaigns", "conversions", "custom-audiences"}
			archiveMaxPages := 100
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  openai-ads-pp-cli workflow status

  # Show status as JSON
  openai-ads-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("openai-ads-pp-cli")
			}
