...
In local mode: searches locally synced data only.`,
  ...
  openai-ads-pp-cli search "error timeout"

  # Force local search only
  openai-ads-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  openai-ads-pp-cli search "status" --type ad-account --data-source local
  # JSON output for piping
  openai-ads-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /geo_lookup/search
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get(cmd.Context(), "/geo_lookup/search", map[string]string{
					"q": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'openai-ads-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "ad-groups":
				results, err = db.SearchAdGroups(query, limit)
			case "campaigns":
			...
			case "":
				...
				{
					partial, searchErr := db.SearchAdGroups(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search ad_groups failed: %w", searchErr)
					}
				...
				}
				{
					...
					if searchErr != nil {
						return fmt.Errorf("search custom_audiences failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
