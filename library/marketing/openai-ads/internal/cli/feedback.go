...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("OPENAI_ADS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("OPENAI_ADS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "openai-ads-pp-cli/feedback")
...
When ` + "`OPENAI_ADS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`OPENAI_ADS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  openai-ads-pp-cli feedback list
  openai-ads-pp-cli feedback list --limit 5
  openai-ads-pp-cli feedback list --json`,
