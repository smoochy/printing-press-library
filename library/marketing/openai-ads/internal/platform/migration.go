...
func AdoptVerifiedLegacyDatabase(ctx context.Context, session *Session, state *LegacyMigrationState) error {
	...
	// SQLite does not accept bind parameters in VACUUM INTO, so the destination
	// must be interpolated. targetPath is program-derived (never user SQL) and
	// single quotes are doubled above, which is the only escape SQLite string
	// literals require.
	quotedTarget := strings.ReplaceAll(targetPath, "'", "''")
	// #nosec G202 -- VACUUM INTO cannot be parameterized; path is program-derived and quote-escaped.
