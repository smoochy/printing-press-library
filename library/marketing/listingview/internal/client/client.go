...
func (c *Client) doInternal(ctx context.Context, method, path string, params map[string]string, body any, headerOverrides map[string]string, readOnlyIntent bool) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if authHeader != "" {
			// ListingView cookie auth: send the imported cookie jar as the
			// Cookie header and derive the X-CSRF-Token + shopid headers from it.
			// (Hand-fix: the generated default wrapped the whole cookie string in
			// a single cookie named "Cookie", which the backend rejects. Lives in
			// internal/client/listingview_csrf.go; re-apply if regenerated.)
			applyListingViewAuth(req, authHeader, c.Config)
		}
		req.Header.Set("X-Requested-With", "XMLHttpRequest")
		req.Header.Set("Origin", "https://app.listingview.io")
		req.Header.Set("Referer", "https://app.listingview.io/dashboard")
		if c.Config != nil {
...
func authPlaceholderCredentialError(cfg *config.Config) error {
	return authPlaceholderCredentialErrorWithSetup(cfg, "export LISTINGVIEW_COOKIES=<your-token> or listingview-pp-cli auth set-token <token>")
}
...
func (c *Client) maskCredentialText(text string, extraCredentials ...string) string {
	...
	if c != nil && c.Config != nil {
		...
		addCredential(c.Config.ListingviewCookies)
	}
