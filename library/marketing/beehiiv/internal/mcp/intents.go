...
func RegisterIntents(s *server.MCPServer) {
	s.AddTool(
		mcplib.NewTool("agent_ready_growth_snapshot",
			mcplib.WithDescription("Single read-only health summary computed from the local store."),
		...
		handleAgentReadyGrowthSnapshot,
	)
}

// handleAgentReadyGrowthSnapshot runs the agent_ready_growth_snapshot recipe intent: Single read-only health summary computed from the local store.
func handleAgentReadyGrowthSnapshot(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
