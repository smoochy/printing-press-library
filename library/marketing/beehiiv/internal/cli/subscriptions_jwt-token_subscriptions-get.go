...
func newSubscriptionsJwtTokenSubscriptionsGetCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Aliases:     []string{"get"},
		Short:       "Generate a JWT token that can be used to automatically log in subscribers via URL.",
		Example:     "  beehiiv-pp-cli subscriptions jwt-token subscriptions-get pub_00000000-0000-0000-0000-000000000000 sub_00000000-0000-0000-0000-000000000000",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// PATCH fresh-jwt-token-reads: subscriber JWTs are short-lived; never
			// serve them from the response cache. Bypass the cache read but keep
			// writing fresh responses so the mirror stays warm.
			data, err := c.GetWithHeadersNoCache(cmd.Context(), path, params, nil)
			if err != nil {
