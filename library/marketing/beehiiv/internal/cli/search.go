...
Run sync first to populate the local search index.`,
  ...
  beehiiv-pp-cli search "status"

  # Force local search explicitly
  beehiiv-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  beehiiv-pp-cli search "status" --type advertisement-opportunities --data-source local
  # JSON output for piping
  beehiiv-pp-cli search "status" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			case "podcasts":
				results, err = db.SearchPodcasts(query, limit)
			case "polls":
			...
			case "":
				...
				{
					partial, searchErr := db.SearchPodcasts(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search podcasts failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
