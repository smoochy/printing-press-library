...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
  ...
  beehiiv-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if credentialRefused {
				...
				if cfg != nil {
					...
					if !configured {
						...
						report["auth_hint"] = "Set it with: beehiiv-pp-cli auth set-token <token> or export BEEHIIV_API_KEY=\"your-token-here\""
					} else {
						authConfigured = true
			...
			if os.Getenv("BEEHIIV_API_KEY") != "" {
				authEnvSet = append(authEnvSet, "BEEHIIV_API_KEY")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "BEEHIIV_API_KEY")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "beehiiv-pp-cli", suggestion)
						} else {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	case !haveAny && len(resources) == 0:
		...
		report["hint"] = "No sync recorded; run 'beehiiv-pp-cli sync' to hydrate API-backed resources. Rows written by other paths are not tracked in sync_state."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
