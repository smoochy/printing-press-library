...
func newTiersIndexCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example:     "  beehiiv-pp-cli tiers index pub_00000000-0000-0000-0000-000000000000",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			data, prov, err := resolvePaginatedReadWithStrategy(cmd.Context(), c, flags, "live", "tiers", path, map[string]string{
			...
			if flags.asJSON || (!isTerminal(cmd.OutOrStdout()) && !flags.csv && !flags.quiet && !flags.plain) {
				...
				if flags.selectFields != "" {
					...
					filtered = compactFields(filtered, map[string]bool{"description": true, "id": true, "name": true, "prices": true, "stats": true, "status": true})
				}
			...
			return printOutputWithFlagsMeta(cmd.OutOrStdout(), formatData, flags, map[string]any{"source": "live"}, map[string]bool{"description": true, "id": true, "name": true, "prices": true, "stats": true, "status": true})
		},
	}
	cmd.Flags().StringVar(&flagExpand, "expand", "", "Optional list of expandable objects. `stats` - Returns statistics about the tier(s).")
