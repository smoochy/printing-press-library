...
func replacePathParam(path, name, value string) string {
	// PATCH escaped-path-params: beehiiv by_email paths need strict encoding.
	// url.PathEscape leaves '+' and '@' in path segments; beehiiv's router
	// misreads them, so encode both after the generic escape.
	escaped := cliutil.EscapePathParam(value)
	escaped = strings.ReplaceAll(escaped, "+", "%2B")
	escaped = strings.ReplaceAll(escaped, "@", "%40")
	return strings.ReplaceAll(path, "{"+name+"}", escaped)
}
...
func nonJSONPayloadError(data json.RawMessage) error {
	...
	if len(trimmed) > 0 && trimmed[0] == '<' {
		return authErr(fmt.Errorf("not authenticated or session expired; API returned HTML instead of JSON. " + "Set it with: beehiiv-pp-cli auth set-token <token> or export BEEHIIV_API_KEY=\"your-token-here\""))
	}
	if len(trimmed) == 0 {
