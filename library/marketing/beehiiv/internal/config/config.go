...
type Config struct {
	...
	BeehiivApiKey    string          `toml:"api_key"`
}
...
func Load(configPath string) (*Config, error) {
	...
	// Env var overrides
	if v := os.Getenv("BEEHIIV_API_KEY"); v != "" {
		cfg.BeehiivApiKey = v
		cfg.markEnvOverride("BeehiivApiKey")
		cfg.AuthSource = "env:BEEHIIV_API_KEY"
		cfg.CredentialSource = "env:BEEHIIV_API_KEY"
	}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("BEEHIIV_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "beehiiv-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.BeehiivApiKey != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:BEEHIIV_API_KEY"
		}
		return "Bearer " + c.BeehiivApiKey
	}
	if c.AccessToken != "" {
...
func (c *Config) StoreScopeCredential() string {
	...
	if c.BeehiivApiKey != "" {
		parts = append(parts, "api_key="+c.BeehiivApiKey)
	}
	if len(parts) == 0 {
		return ""
	}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	if c.BeehiivApiKey == "" {
		c.BeehiivApiKey = creds.BeehiivApiKey
	}
}
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.BeehiivApiKey = ""
	delete(c.envOverrides, "BeehiivApiKey")
	c.updateFileConfigField("BeehiivApiKey")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["BeehiivApiKey"] {
			out.BeehiivApiKey = c.fileConfig.BeehiivApiKey
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "BeehiivApiKey":
		c.fileConfig.BeehiivApiKey = c.BeehiivApiKey
	}
}
