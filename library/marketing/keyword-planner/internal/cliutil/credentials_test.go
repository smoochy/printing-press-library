...
func resetCredentialEnv(t *testing.T) (home, configPath string) {
	...
	for _, name := range []string{
		"KEYWORD_PLANNER_ACCESS_TOKEN",
	} {
		t.Setenv(name, "")
	}
	return home, filepath.Join(home, ".config", "keyword-planner-pp-cli", "config.json")
}
...
func TestExplicitConfigUsesSiblingCredentialsBeforeGlobal(t *testing.T) {
	...
	t.Setenv("KEYWORD_PLANNER_CONFIG", configPath)
...
func TestCorruptCredentialsDoesNotOverrideLegacyConfig(t *testing.T) {
	...
	credentialsPath := filepath.Join(home, ".local", "share", "keyword-planner-pp-cli", "credentials.toml")
	if err := os.MkdirAll(filepath.Dir(credentialsPath), 0o700); err != nil {
		t.Fatalf("mkdir credentials dir: %v", err)
	}
...
// The printed Planner replaces the generator's bearer environment login with
// its protected-file OAuth refresh flow. A malformed legacy file must not
// revive the retired bearer environment binding.
func TestCorruptCredentialsDoesNotReviveRetiredBearerEnv(t *testing.T) {
	home, _ := resetCredentialEnv(t)
	t.Setenv("KEYWORD_PLANNER_ACCESS_TOKEN", "env-secret")
	credentialsPath := filepath.Join(home, ".local", "share", "keyword-planner-pp-cli", "credentials.toml")
	if err := os.MkdirAll(filepath.Dir(credentialsPath), 0o700); err != nil {
		t.Fatalf("mkdir credentials dir: %v", err)
	}
	...
	stderr := captureCredentialStderr(t, func() {
		...
		if got := cfg.AuthHeader(); got != "" {
			t.Fatalf("AuthHeader() = %q, want no retired bearer credential", got)
		}
	})
...
func TestAuthWriteScrubsLegacyConfigWhenRelocated(t *testing.T) {
	...
	activeConfigPath := filepath.Join(newConfigDir, "keyword-planner-pp-cli", "config.json")
	...
	credsPath := filepath.Join(newDataDir, "keyword-planner-pp-cli", "credentials.toml")
