...
type Config struct {
	...
	KeywordPlannerAccessToken string          `json:"planner_access_token"`
}
...
func Load(configPath string) (*Config, error) {
	...
	if v := os.Getenv("KEYWORD_PLANNER_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("KEYWORD_PLANNER_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "keyword-planner-pp-cli", "config.json"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.KeywordPlannerAccessToken != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:KEYWORD_PLANNER_ACCESS_TOKEN"
		}
		return applyAuthFormat("Bearer {token}", map[string]string{
			"planner_access_token":         c.KeywordPlannerAccessToken,
			"KEYWORD_PLANNER_ACCESS_TOKEN": c.KeywordPlannerAccessToken,
			"token":                        c.KeywordPlannerAccessToken,
			"access_token":                 c.KeywordPlannerAccessToken,
		})
	}
	if c.AccessToken != "" {
...
func (c *Config) StoreScopeCredential() string {
	...
	if c.KeywordPlannerAccessToken != "" {
		parts = append(parts, "planner_access_token="+c.KeywordPlannerAccessToken)
	}
	if len(parts) == 0 {
		return ""
	}
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		KeywordPlannerAccessToken: c.KeywordPlannerAccessToken,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	if c.KeywordPlannerAccessToken == "" {
		c.KeywordPlannerAccessToken = creds.KeywordPlannerAccessToken
	}
}
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.KeywordPlannerAccessToken = ""
	delete(c.envOverrides, "KeywordPlannerAccessToken")
	c.updateFileConfigField("KeywordPlannerAccessToken")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["KeywordPlannerAccessToken"] {
			out.KeywordPlannerAccessToken = c.fileConfig.KeywordPlannerAccessToken
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "KeywordPlannerAccessToken":
		c.fileConfig.KeywordPlannerAccessToken = c.KeywordPlannerAccessToken
	}
}
