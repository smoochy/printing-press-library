...
func shellOutToCLI(cliPath func() (string, error), commandPath []string, blockedStructuredArgs map[string]bool, allowedStructuredArgs map[string]bool, positionals []positionalArg, readOnly bool, positionalWriteSinks map[int]bool) server.ToolHandlerFunc {
	...
	return func(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
		if lookupErr != nil {
			return boundedToolResultError(fmt.Sprintf("companion CLI binary not found: %v\nTried sibling lookup, KEYWORD_PLANNER_CLI_PATH env var, and PATH.", lookupErr)), nil
		}
...
func cliArgsFromMCP(args map[string]any, blocked map[string]bool) []string {
	...
	for _, k := range keys {
		...
		case []any:
			out = appendRepeatedMCPArrayArgs(out, k, tv)
		case []string:
			out = appendRepeatedMCPArrayArgs(out, k, tv)
		default:
			if v != nil {
...
// appendRepeatedMCPArrayArgs keeps each JSON array item as its own flag
// occurrence. This is required for pflag.StringArray: joining items with a
// comma changes two submitted keywords into one and makes a keyword that
// itself contains a comma impossible to represent. StringSlice still applies
// its own comma splitting when Cobra parses these repeated values.
func appendRepeatedMCPArrayArgs(out []string, name string, values any) []string {
	appendValue := func(value any) {
		if value == nil {
			return
		}
		out = append(out, "--"+name, fmt.Sprintf("%v", value))
	}
