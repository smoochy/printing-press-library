// Copyright 2026 Max Michel and contributors. Licensed under Apache-2.0. See LICENSE.
// Generated by CLI Printing Press (https://github.com/mvanhorn/cli-printing-press). DO NOT EDIT.

package cobratree

import (
	"os"
	"os/exec"
	"path/filepath"
	"runtime"
)

// SiblingCLIPath resolves the companion CLI via sibling-of-executable,
// KEYWORD_PLANNER_CLI_PATH env var, then PATH.
func SiblingCLIPath() (string, error) {
	if exe, err := os.Executable(); err == nil {
		for _, candidate := range siblingCLICandidates(runtime.GOOS, exe) {
			if _, err := os.Stat(candidate); err == nil {
				return candidate, nil
			}
		}
	}
	if v := os.Getenv("KEYWORD_PLANNER_CLI_PATH"); v != "" {
		return v, nil
	}
	return exec.LookPath(cliExecutableName(runtime.GOOS))
}

func siblingCLICandidates(goos, exePath string) []string {
	dir := filepath.Dir(exePath)
	if goos == "windows" {
		return []string{filepath.Join(dir, "keyword-planner.exe"), filepath.Join(dir, "keyword-planner"), filepath.Join(dir, "keyword-planner-pp-cli.exe"), filepath.Join(dir, "keyword-planner-pp-cli")}
	}
	return []string{filepath.Join(dir, "keyword-planner"), filepath.Join(dir, "keyword-planner-pp-cli")}
}

func cliExecutableName(goos string) string {
	name := "keyword-planner"
	if goos == "windows" {
		return name + ".exe"
	}
	return name
}
