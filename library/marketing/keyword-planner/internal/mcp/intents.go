...
func RegisterIntents(s *server.MCPServer) {
	s.AddTool(
		mcplib.NewTool("check_local_readiness",
			mcplib.WithDescription("Confirm local setup and portfolio schema without a network call."),
			mcplib.WithReadOnlyHintAnnotation(true),
			mcplib.WithDestructiveHintAnnotation(false),
			mcplib.WithOpenWorldHintAnnotation(false),
			mcplib.WithBoolean("dry_run", mcplib.Description("Preview the local check without opening the portfolio.")),
		),
		handleCheckLocalReadiness,
	)
}

// handleCheckLocalReadiness runs the check_local_readiness recipe intent: Confirm local setup and portfolio schema without a network call.
func handleCheckLocalReadiness(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
	...
	args = appendRecipeBoolFlag(args, "dry-run", input["dry_run"], false)
