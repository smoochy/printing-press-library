...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  keyword-planner-pp-cli doctor
  keyword-planner-pp-cli doctor --json
  keyword-planner-pp-cli doctor --fail-on warn
  keyword-planner-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if credentialRefused {
				...
				if cfg != nil {
					...
					if !configured {
						...
						report["auth_hint"] = "Set it with: echo \"$TOKEN\" | keyword-planner-pp-cli auth set-token or export KEYWORD_PLANNER_ACCESS_TOKEN=\"your-token-here\""
					} else {
						authConfigured = true
			...
			if os.Getenv("KEYWORD_PLANNER_ACCESS_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "KEYWORD_PLANNER_ACCESS_TOKEN")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "KEYWORD_PLANNER_ACCESS_TOKEN")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "keyword-planner-pp-cli", suggestion)
						} else {
