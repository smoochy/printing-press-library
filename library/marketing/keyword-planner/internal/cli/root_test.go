...
func TestDeclaredAPISurfaceReachable(t *testing.T) {
	...
	if len(missing) > 0 {
		t.Fatalf("curated API command paths missing from Cobra tree: %s", strings.Join(missing, ", "))
	}
	forbidden := []string{
		"customers",
	...
	var reachableForbidden []string
	for _, commandPath := range forbidden {
		if _, ok := actual[commandPath]; ok {
			reachableForbidden = append(reachableForbidden, commandPath)
		}
	}
	if len(reachableForbidden) > 0 {
		t.Fatalf("removed generic command paths remain reachable: %s", strings.Join(reachableForbidden, ", "))
	}
}
