...
func classifyAPIErrorOnly(err error) error {
	...
	case strings.Contains(msg, "HTTP 400") && cliutil.LooksLikeAuthError(msg):
		return authErr(fmt.Errorf("%w\nhint: the API rejected the request — this usually means auth is missing or invalid."+
			...
			"\n      Run 'keyword-planner-pp-cli doctor' to check auth status."+
			"\n      Response: "+cliutil.SanitizeErrorBody(msg), err))
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: check your token."+
			"\n      Set it with: echo \"$TOKEN\" | keyword-planner-pp-cli auth set-token or export KEYWORD_PLANNER_ACCESS_TOKEN=\"your-token-here\""+
			"\n      Run 'keyword-planner-pp-cli doctor' to check auth status.", err))
...
func wrapPlatformStructuredOutput(data json.RawMessage, flags *rootFlags, resultKey string, mergeOwnedEnvelope bool) (json.RawMessage, error) {
	...
	if err := decodeOutputJSON(metadataRaw, &platformMeta); err != nil {
		return nil, err
	}
	var envelope map[string]json.RawMessage
	if mergeOwnedEnvelope && decodeOutputJSON(data, &envelope) == nil && envelope["meta"] != nil && (envelope["data"] != nil || envelope["results"] != nil) {
		var meta map[string]any
		if err := decodeOutputJSON(envelope["meta"], &meta); err != nil {
			return nil, err
		}
...
func splitResultsMetaEnvelope(data json.RawMessage) (json.RawMessage, map[string]any, bool) {
	var obj map[string]json.RawMessage
	if err := decodeOutputJSON(data, &obj); err != nil {
	...
	var meta map[string]any
	if err := decodeOutputJSON(rawMeta, &meta); err != nil || meta == nil {
		return nil, nil, false
	}
...
func selectFieldKeys(data json.RawMessage) []string {
	...
	collect = func(value json.RawMessage) {
		var obj map[string]json.RawMessage
		if err := decodeOutputJSON(value, &obj); err == nil && obj != nil {
			for key := range obj {
				keys[key] = true
			}
			return
		...
		if err := decodeOutputJSON(value, &arr); err == nil {
...
func filterFieldsRecAtDepth(data json.RawMessage, paths [][]string, pathAnchored bool, envelopeDepth int) (json.RawMessage, selectMatchState) {
	var arr []json.RawMessage
	if err := decodeOutputJSON(data, &arr); err == nil {
...
func filterListEnvelopeFields(obj map[string]json.RawMessage, paths [][]string, envelopeDepth int) (map[string]json.RawMessage, bool, selectMatchState) {
	...
	for k, v := range obj {
		...
		if len(trimmed) > 0 {
			...
			case '[':
				var arr []json.RawMessage
				if decodeOutputJSON(v, &arr) == nil && arr != nil {
					foundArray = true
					var childState selectMatchState
			...
			case '{':
				...
				if decodeOutputJSON(v, &nestedObj) == nil && nestedObj != nil {
...
func isDryRunResponse(dryRun bool, data json.RawMessage) bool {
	...
	var envelope map[string]json.RawMessage
	if err := decodeOutputJSON(data, &envelope); err != nil || len(envelope) != 1 {
		return false
	}
	...
	var v bool
	return decodeOutputJSON(raw, &v) == nil && v
}
...
func compactFields(data json.RawMessage, documentedFields ...map[string]bool) json.RawMessage {
	...
	if err := decodeOutputJSON(data, &items); err == nil {
...
func compactObjectArrayValue(v any, documentedFields ...map[string]bool) (any, bool) {
	...
	var compacted any
	if err := decodeOutputJSON(compactedRaw, &compacted); err != nil {
		return nil, false
	}
...
func printQuiet(w io.Writer, data json.RawMessage) error {
	...
	var scalars []any
	if err := decodeOutputJSON(data, &scalars); err == nil {
...
func tabularObjectRows(data json.RawMessage) ([]map[string]any, bool) {
	...
	var items []map[string]any
	if err := decodeOutputJSON(projected, &items); err == nil && (len(items) > 0 || bytes.HasPrefix(bytes.TrimSpace(projected), []byte("["))) {
		return items, true
	}
	var obj map[string]any
	if err := decodeOutputJSON(projected, &obj); err != nil || len(obj) == 0 {
		return nil, false
	}
...
func printOutput(w io.Writer, data json.RawMessage, asJSON bool) error {
	...
	var items []map[string]any
	if err := decodeOutputJSON(data, &items); err == nil && len(items) > 0 {
