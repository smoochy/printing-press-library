...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("KEYWORD_PLANNER_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("KEYWORD_PLANNER_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "github.com/mvanhorn/printing-press-library/library/marketing/keyword-planner/feedback")
...
When ` + "`KEYWORD_PLANNER_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`KEYWORD_PLANNER_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
maintainer sees it.`,
		Example: `  keyword-planner-pp-cli feedback "the --since flag is inclusive but docs say exclusive"
  keyword-planner-pp-cli feedback --stdin < notes.txt
  keyword-planner-pp-cli feedback list --limit 10`,
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  keyword-planner-pp-cli feedback list
  keyword-planner-pp-cli feedback list --limit 5
  keyword-planner-pp-cli feedback list --json`,
