...
var whichIndex = []whichEntry{
	{Command: "portfolio trace", Description: "Trace one stored monthly value back to the exact request and raw receipt that produced it.", Group: "Evidence that compounds", WhyItMatters: "Reach for this when a number needs a receipt-backed explanation that a direct Planner call cannot provide."},
	{Command: "portfolio coverage", Description: "See requested, returned, missing, unavailable, raw-only, and incomplete coverage for a saved collection.", Group: "Evidence that compounds", WhyItMatters: "Use this before consuming a collection when the API response alone cannot explain what was absent or incomplete."},
	{Command: "portfolio variants", Description: "Follow submitted keywords through returned terms and close variants without double-counting demand.", Group: "Safe warehouse handoff", WhyItMatters: "Pick this when a Planner result must be interpreted with its returned-term and close-variant relationships intact."},
	{Command: "portfolio diff", Description: "Compare two preserved collections to see changed terms, monthly values, flags, and coverage.", Group: "Evidence that compounds", WhyItMatters: "Use this when the question is what changed between saved collections rather than what one live call returns."},
	{Command: "portfolio integrity", Description: "Verify that raw receipts, hashes, normalized rows, and coverage still reconcile.", Group: "Safe warehouse handoff", WhyItMatters: "Run this before a warehouse handoff when evidence integrity matters more than a fresh remote request."},
	{Command: "portfolio safe-stats", Description: "Compute descriptive monthly aggregates while excluding unavailable, ambiguous, anomalous, and incomplete rows.", Group: "Safe warehouse handoff", WhyItMatters: "Choose this for mechanical local summaries that must preserve warnings and avoid turning Planner signals into forecasts."},
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one. Machine output (--json/--csv/--plain/--quiet, or a pipe) still exits 2 and writes {"matches":[]} (or the equivalent empty table) to stdout.`,
		SilenceUsage: true,
		Example: `  keyword-planner-pp-cli which "stale tickets"
  keyword-planner-pp-cli which "bottleneck"
  keyword-planner-pp-cli which --limit 1 "send message"
  keyword-planner-pp-cli which                                # list the full capability index`,
