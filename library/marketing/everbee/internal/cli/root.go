...
type rootFlags struct {
	...
	// invocation. Mirrors the EVERBEE_NO_LEARN env var.
	noLearn bool
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "everbee-pp-cli",
		Short: `Etsy niche research whose scores come with their evidence attached — seeded search, honest confidence, and a local store that turns repeat research into trends.`,
		Long: `Etsy niche research whose scores come with their evidence attached — seeded search, honest confidence, and a local store that turns repeat research into trends.
...
See README.md or the bundled SKILL.md for recipes.`,
	...
	rootCmd.AddCommand(newKeywordResearchCmd(flags))
	...
	rootCmd.AddCommand(newNovelResearchCmd(flags))
	rootCmd.AddCommand(newNovelSelftestCmd(flags))
	rootCmd.AddCommand(newAPICmd(flags))
	rootCmd.AddCommand(newAccountPromotedCmd(flags))
	// Hand-wired: the seeded product search is hand-authored (see
	// products_search.go). generate --force re-emits this file, so if this
	// line goes missing after a regen, `products search` silently falls
	// through to the browse feed.
	productsCmd := newProductsPromotedCmd(flags)
	productsCmd.AddCommand(newProductsSearchCmd(flags))
