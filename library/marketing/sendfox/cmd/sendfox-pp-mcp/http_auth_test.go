...
func TestNewMCPHTTPServerBoundsRequestReads(t *testing.T) {
	t.Parallel()

	srv := newMCPHTTPServer("127.0.0.1:7777", "shared-secret", http.HandlerFunc(func(http.ResponseWriter, *http.Request) {}))
	if srv.ReadHeaderTimeout != mcpReadHeaderTimeout || srv.ReadHeaderTimeout <= 0 {
		t.Fatalf("ReadHeaderTimeout = %s, want %s", srv.ReadHeaderTimeout, mcpReadHeaderTimeout)
	}
	if srv.ReadTimeout != mcpReadTimeout || srv.ReadTimeout <= 0 {
		t.Fatalf("ReadTimeout = %s, want %s", srv.ReadTimeout, mcpReadTimeout)
	}
	if srv.IdleTimeout != mcpIdleTimeout || srv.IdleTimeout <= 0 {
