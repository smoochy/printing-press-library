import os,pathlib,json,hashlib,collections
r=pathlib.Path(os.environ['API_RUN_DIR']);w=pathlib.Path(os.environ['CLI_WORK_DIR']);lib=pathlib.Path(os.environ['PRESS_LIBRARY'])/'sendfox';proof=r/'proofs';archive=pathlib.Path(os.environ['PRESS_MANUSCRIPTS'])/'sendfox'/os.environ['RUN_ID']
ship=json.loads((proof/'shipcheck-final-local.json').read_text());assert ship['passed']
help=json.loads((proof/'help-matrix.json').read_text());mcp=json.loads((proof/'mcp-smoke.json').read_text());manifest=json.loads((lib/'.printing-press.json').read_text());assert manifest['run_id']==os.environ['RUN_ID']
score=manifest['scorecard']['steinberger'];verify=manifest['verify'];samples=json.loads((proof/'output-review-final-samples.json').read_text())['live_check'];assert samples['passed']==7 and samples['failed']==0
ops=json.loads((w/'internal/contract/operations.json').read_text());families=collections.Counter(o['id'].split('.')[0] for o in ops)
report=f"""# SendFox CLI local upgrade — verified, unpublished

Run: `{os.environ['RUN_ID']}`. Official OpenAPI v1.4.0; installed Printing Press v4.32.4 / skill v3.0.0. Recovered generation by normalizing ten multiline operation descriptions/summaries only in the derived generation spec. Immutable official source SHA-256 remains `e9c1a272f79238e2f94f857092a932002c64f21299304b930e33f5ca2e79b991`.

## Deliverable

- Local library: `{lib}`
- Working tree: `{w}`
- CLI: `{lib}/sendfox-pp-cli`
- MCP: `{lib}/sendfox-pp-mcp`; bundle: `{lib}/build/sendfox-pp-mcp-darwin-arm64.mcpb`
- Manuscripts: `{archive}`
- Proofs and exact phase ledger: `{proof}` and `{r}/pipeline/phase-receipts.jsonl`
- Prior 11-operation library preserved at `{r}/working/sendfox-library-before-promotion` (103 files, hashes recorded). Original failed emission also preserved.

## Coverage and product surface

**60/60 documented operations**, up from 11 (+49), with request/response schema metadata and runtime request validation. Families: {', '.join(f'{k}: {v}' for k,v in families.items())}.

**{help['commands']} command nodes, {help['leaves']} leaves, 60 distinct endpoint commands; {mcp['tool_count']} MCP tools.** MCP exposes thin `sendfox_search`, full-schema `sendfox_get`, guarded `sendfox_execute`, campaign-performance intent and seven explicit evidence intents. Sixty endpoint mirrors are hidden; other tools provide local data and agent workflows. Scorecard's “60 tools” wording counts contract endpoint metadata; actual stdio tools/list counts 41.
...
- Printing Press verify **{verify['passed']}/{verify['total']} ({verify['pass_rate']}%)**, data pipeline PASS.
...
- Scorecard **{score['total']}/100, {score['grade']}**. Live API, MCP description and token-efficiency dimensions are explicitly unscored; this is not a live quality score.
...
"""
path=pathlib.Path('/Users/cathrynlavery/printing-press/agent-runs/sendfox-cli-final-v2.md');path.write_text(report);(r/'REPORT.md').write_text(report)
state=json.loads((r/'state.json').read_text());state.update(phase='complete-local',status='locally_verified_unpublished',novel_features_built=7,live_verified=False,live_auth_authorized=False,publish_authorized=False,library_dir=str(lib),command_nodes=help['commands'],command_leaves=help['leaves'],mcp_tools=mcp['tool_count'],score=score['total'],shipcheck='PASS',live_gate='auth-aware-skip');state.setdefault('operation_coverage',{'api_version':'1.4.0','documented_operations':60,'generated_typed_operations':60,'missing':[],'extra':[]}).update(runtime_verified=True,verification_mode='loopback_mock',runtime_passed=60);(r/'state.json').write_text(json.dumps(state,indent=2)+'\n')
print(path)
