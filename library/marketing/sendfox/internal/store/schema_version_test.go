...
// TestMigrate_AddsColumnsOnUpgrade_AutomationEmails verifies that opening a
...
func TestMigrate_AddsColumnsOnUpgrade_AutomationEmails(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "automation_emails" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	rows, err := s.DB().Query(`SELECT name FROM pragma_table_xinfo(?)`, "automation_emails")
	if err != nil {
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from automation_emails after migrate", want)
		}
	}

	for _, wantIdx := range []string{
		"idx_automation_emails_automation_id",
		"idx_automation_emails_campaign_id",
		"idx_automation_emails_created_at",
		"idx_automation_emails_updated_at",
	} {
		var got string
		if err := s.DB().QueryRow(
			...
			t.Fatalf("%s index missing from automation_emails after migrate: %v", wantIdx, err)
		}
	}
}
...
func TestMigrate_AddsColumnsOnUpgrade_Campaigns(t *testing.T) {
	...
	// table_info hides generated columns; table_xinfo is the verify
	// surface for VIRTUAL backfills such as bare_id.
	rows, err := s.DB().Query(`SELECT name FROM pragma_table_xinfo(?)`, "campaigns")
	if err != nil {
		t.Fatalf("table_xinfo: %v", err)
	}
	defer rows.Close()
	...
	for _, wantIdx := range []string{
		"idx_campaigns_contact_id",
		"idx_campaigns_created_at",
		"idx_campaigns_updated_at",
	} {
		var got string
		if err := s.DB().QueryRow(
			`SELECT name FROM sqlite_master WHERE type='index' AND name=?`,
			...
			t.Fatalf("%s index missing from campaigns after migrate: %v", wantIdx, err)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_ContactFields verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_ContactFields(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "contact_fields" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// table_info hides generated columns; table_xinfo is the verify
	// surface for VIRTUAL backfills such as bare_id.
	rows, err := s.DB().Query(`SELECT name FROM pragma_table_xinfo(?)`, "contact_fields")
	if err != nil {
		t.Fatalf("table_xinfo: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from contact_fields after migrate", want)
		}
	}
	...
	for _, wantIdx := range []string{} {
		...
		if err := s.DB().QueryRow(
			...
			t.Fatalf("%s index missing from contact_fields after migrate: %v", wantIdx, err)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_ContactTags verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_ContactTags(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "contact_tags" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// table_info hides generated columns; table_xinfo is the verify
	// surface for VIRTUAL backfills such as bare_id.
	rows, err := s.DB().Query(`SELECT name FROM pragma_table_xinfo(?)`, "contact_tags")
	if err != nil {
		t.Fatalf("table_xinfo: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from contact_tags after migrate", want)
		}
	}

	for _, wantIdx := range []string{
		"idx_contact_tags_user_id",
		"idx_contact_tags_created_at",
		"idx_contact_tags_updated_at",
	} {
		var got string
		if err := s.DB().QueryRow(
			`SELECT name FROM sqlite_master WHERE type='index' AND name=?`,
			...
			t.Fatalf("%s index missing from contact_tags after migrate: %v", wantIdx, err)
		}
	}
}
...
func TestMigrate_AddsColumnsOnUpgrade_Contacts(t *testing.T) {
	...
	// table_info hides generated columns; table_xinfo is the verify
	// surface for VIRTUAL backfills such as bare_id.
	rows, err := s.DB().Query(`SELECT name FROM pragma_table_xinfo(?)`, "contacts")
	if err != nil {
		t.Fatalf("table_xinfo: %v", err)
	}
	defer rows.Close()
	...
	for _, wantIdx := range []string{
		"idx_contacts_target_id",
		"idx_contacts_user_id",
		"idx_contacts_created_at",
		"idx_contacts_updated_at",
	} {
		var got string
		if err := s.DB().QueryRow(
			`SELECT name FROM sqlite_master WHERE type='index' AND name=?`,
			...
			t.Fatalf("%s index missing from contacts after migrate: %v", wantIdx, err)
		}
	}
}
...
func TestMigrate_AddsColumnsOnUpgrade_Domains(t *testing.T) {
	...
	// table_info hides generated columns; table_xinfo is the verify
	// surface for VIRTUAL backfills such as bare_id.
	rows, err := s.DB().Query(`SELECT name FROM pragma_table_xinfo(?)`, "domains")
	if err != nil {
		t.Fatalf("table_xinfo: %v", err)
	}
	defer rows.Close()
	...
	for _, wantIdx := range []string{
		"idx_domains_sendgrid_whitelabel_domain_id",
		"idx_domains_created_at",
		"idx_domains_updated_at",
	} {
		var got string
		if err := s.DB().QueryRow(
			`SELECT name FROM sqlite_master WHERE type='index' AND name=?`,
			...
			t.Fatalf("%s index missing from domains after migrate: %v", wantIdx, err)
		}
	}
}
...
func TestMigrate_AddsColumnsOnUpgrade_Forms(t *testing.T) {
	...
	// table_info hides generated columns; table_xinfo is the verify
	// surface for VIRTUAL backfills such as bare_id.
	rows, err := s.DB().Query(`SELECT name FROM pragma_table_xinfo(?)`, "forms")
	if err != nil {
		t.Fatalf("table_xinfo: %v", err)
	}
	defer rows.Close()
	...
	for _, wantIdx := range []string{
		"idx_forms_user_id",
		"idx_forms_landing_page_id",
		"idx_forms_created_at",
		"idx_forms_updated_at",
	} {
		var got string
		if err := s.DB().QueryRow(
			`SELECT name FROM sqlite_master WHERE type='index' AND name=?`,
			...
			t.Fatalf("%s index missing from forms after migrate: %v", wantIdx, err)
		}
	}
}
...
func TestMigrate_AddsColumnsOnUpgrade_Lists(t *testing.T) {
	...
	// table_info hides generated columns; table_xinfo is the verify
	// surface for VIRTUAL backfills such as bare_id.
	rows, err := s.DB().Query(`SELECT name FROM pragma_table_xinfo(?)`, "lists")
	if err != nil {
		t.Fatalf("table_xinfo: %v", err)
	}
	defer rows.Close()
	...
	for _, wantIdx := range []string{
		"idx_lists_user_id",
		"idx_lists_created_at",
		"idx_lists_updated_at",
		"idx_lists_parent_id",
		"idx_lists_bare_id",
	} {
		var got string
		if err := s.DB().QueryRow(
			...
			t.Fatalf("%s index missing from lists after migrate: %v", wantIdx, err)
		}
	}
}
...
func TestMigrate_AddsColumnsOnUpgrade_Me(t *testing.T) {
	...
	// table_info hides generated columns; table_xinfo is the verify
	// surface for VIRTUAL backfills such as bare_id.
	rows, err := s.DB().Query(`SELECT name FROM pragma_table_xinfo(?)`, "me")
	if err != nil {
		t.Fatalf("table_xinfo: %v", err)
	}
	defer rows.Close()
	...
	for _, wantIdx := range []string{
		...
		if err := s.DB().QueryRow(
			...
			t.Fatalf("%s index missing from me after migrate: %v", wantIdx, err)
		}
	}
}
...
func TestMigrate_AddsColumnsOnUpgrade_Unsubscribe(t *testing.T) {
	...
	// table_info hides generated columns; table_xinfo is the verify
	// surface for VIRTUAL backfills such as bare_id.
	rows, err := s.DB().Query(`SELECT name FROM pragma_table_xinfo(?)`, "unsubscribe")
	if err != nil {
		t.Fatalf("table_xinfo: %v", err)
	}
	defer rows.Close()
	...
	for _, wantIdx := range []string{
		"idx_unsubscribe_created_at",
		"idx_unsubscribe_updated_at",
	} {
		var got string
		if err := s.DB().QueryRow(
			`SELECT name FROM sqlite_master WHERE type='index' AND name=?`,
			...
			t.Fatalf("%s index missing from unsubscribe after migrate: %v", wantIdx, err)
		}
	}
}
