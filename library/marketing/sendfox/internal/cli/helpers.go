...
func jwtExpirySource(cfg *config.Config) string {
	...
	if v := strings.TrimSpace(cfg.SendfoxApiToken); v != "" {
		return v
	}
	if v := strings.TrimSpace(cfg.SendfoxBearerAuth); v != "" {
		return v
	}
...
func classifyHTMLTransportError(err error) error {
	...
	if htmlLooksLikeAuthFailure(evidence) {
		return authErr(fmt.Errorf("not authenticated or session expired; API returned HTML instead of JSON. " + "Set credentials with: export SENDFOX_API_TOKEN=\"your-token-here\" SENDFOX_BEARER_AUTH=\"your-token-here\""))
	}
...
func setDefaultDBScopeCredential(credential string) {
	scopeHash := defaultDBScopeHash(credential)
...
func defaultDBPath(name string) string {
	...
	if err != nil {
		...
		return defaultDBPathInDir(".")
	}
	return defaultDBPathInDir(dir)
}

func defaultDBPathInDir(dir string) string {
	return defaultDBPathForScopeHash(dir, currentDefaultDBScopeHash())
}

// DefaultDBPathForCredential lets CLI and MCP callers share account isolation
// without relying on the CLI command tree's mutable process state.
func DefaultDBPathForCredential(dir, credential string) string {
	return defaultDBPathForScopeHash(dir, defaultDBScopeHash(credential))
}

func defaultDBPathForScopeHash(dir, scopeHash string) string {
