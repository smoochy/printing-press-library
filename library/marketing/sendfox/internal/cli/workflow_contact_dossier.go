...
func newNovelWorkflowContactDossierCmd(flags *rootFlags) *cobra.Command {
	o := &sendfoxEvidenceOptions{}
	cmd := &cobra.Command{Use: "contact-dossier", Short: "Join contact details, memberships, tags and available engagement into one support packet.", Example: "  sendfox-pp-cli workflow contact-dossier --input examples/snapshot.json --id 1 --agent"}
