...
Resource scoping:
  ...
  sendfox-pp-cli sync --resources lists,automations
...
func syncResource(ctx context.Context, c interface {
	...
	for {
		...
		if maxPages > 0 && pagesFetched >= maxPages {
			...
			truncatedByCap = truncatedByCap && (nextCursor != "" || !shortPageEndsPagination(pageSize.cursorType, fetchedThisPage, pageSize.limit))
			capResumed := false
			if truncatedByCap {
...
func extractPaginationFromEnvelope(envelope map[string]json.RawMessage, cursorParam, nextCursorPath string) (string, bool) {
	if cursorParam == "page" {
		var current int
		if json.Unmarshal(envelope["current_page"], &current) == nil && current > 0 {
			var last int
			if json.Unmarshal(envelope["last_page"], &last) == nil && last >= current {
				if current < last {
					return strconv.Itoa(current + 1), true
				}
				return "", false
			}
			var total, perPage int
			if json.Unmarshal(envelope["total"], &total) == nil && json.Unmarshal(envelope["per_page"], &perPage) == nil && total >= 0 && perPage > 0 {
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "automation_emails":
		return db.UpsertAutomationEmails(data)
	case "automations":
		return db.UpsertAutomations(data)
	case "campaigns":
		return db.UpsertCampaigns(data)
	case "contact_fields":
		return db.UpsertContactFields(data)
	case "contact_tags":
		return db.UpsertContactTags(data)
	case "contacts":
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{ // #nosec G101 -- endpoint paths, not credentials.
		...
		"contacts":              "/contacts",
		"contacts-unsubscribed": "/contacts/unsubscribed",
		"domains":               "/domains",
		"forms":                 "/forms",
		"lists":                 "/lists",
	}
...
func dependentResourceDefs() []dependentResourceDef {
	return []dependentResourceDef{
		{Name: "lists", ParentTable: "lists", ParentIDParam: "list_id", PathTemplate: "/lists/{list_id}/contacts", KeyField: "", ReconcileMode: "per_parent", GenericScopeJSONPath: "$.list", PathParams: []dependentPathParamDef{
...
var flatReconcileModes = map[string]string{
	...
	"contact_fields":        "flat_global",
	"contact_tags":          "flat_global",
	"contacts-unsubscribed": "flat_global",
	"domains":               "flat_global",
	"forms":                 "flat_global",
	"lists":                 "flat_global",
}
