...
func newNovelWorkflowMigrationReadinessCmd(flags *rootFlags) *cobra.Command {
	o := &sendfoxEvidenceOptions{}
	cmd := &cobra.Command{Use: "migration-readiness", Short: "Compare local platform snapshots with suppression, mapping, delta and cutover gates.", Example: "  sendfox-pp-cli workflow migration-readiness --input examples/snapshot.json --agent"}
