...
func newNovelWorkflowCampaignPreflightCmd(flags *rootFlags) *cobra.Command {
	o := &sendfoxEvidenceOptions{}
	cmd := &cobra.Command{Use: "campaign-preflight", Short: "Check draft content, targeting, suppression evidence and sender readiness before any send.", Example: "  sendfox-pp-cli workflow campaign-preflight --input examples/snapshot.json --agent"}
