...
func newNovelWorkflowExportBundleCmd(flags *rootFlags) *cobra.Command {
	o := &sendfoxEvidenceOptions{}
	cmd := &cobra.Command{Use: "export-bundle", Short: "Produce a read-only inventory bundle with completeness, provenance and hashes.", Example: "  sendfox-pp-cli workflow export-bundle --input examples/snapshot.json --agent"}
	name := "export-bundle"
