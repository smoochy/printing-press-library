...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
  ...
  sendfox-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if credentialRefused {
				...
				if cfg != nil {
					...
					if !configured {
						...
						report["auth_hint"] = "Set credentials with: export SENDFOX_API_TOKEN=\"your-token-here\" SENDFOX_BEARER_AUTH=\"your-token-here\""
						report["auth_key_url"] = "https://sendfox.com/account/oauth"
					} else {
						authConfigured = true
			...
			if os.Getenv("SENDFOX_API_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "SENDFOX_API_TOKEN")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "SENDFOX_API_TOKEN")
			}
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	case !haveAny:
		...
		report["hint"] = "No sync recorded; run 'sendfox-pp-cli sync' to hydrate API-backed resources. Rows written by other paths are not tracked in sync_state."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
