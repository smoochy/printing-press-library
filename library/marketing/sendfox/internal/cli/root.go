...
type rootFlags struct {
	...
	// invocation. Mirrors the SENDFOX_NO_LEARN env var.
	noLearn bool
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "sendfox-pp-cli",
		Short: `SendFox CLI — Manage audiences and campaigns, sync account data, and produce evidence-backed reports from the terminal or an agent.`,
		Long: `SendFox CLI — Manage audiences and campaigns, sync account data, and produce evidence-backed reports from the terminal or an agent.

Highlights (not in the official API docs):
  • workflow audience-health   Audit audience structure and evidence-graded engagement recency cohorts.
  • workflow campaign-preflight   Check draft content, targeting, suppression evidence and sender readiness before any send.
  • workflow campaign-review   Benchmark campaign counters and assemble an auditable resend plan.
  ...
  • workflow export-bundle   Produce a read-only inventory bundle with completeness, provenance and hashes.
  • workflow growth-report   Calculate timestamped audience growth and exact list deltas between snapshots.
  • workflow snapshot-save   Persist a private, dated and checksummed local snapshot history artifact.
...
Health check: run 'sendfox-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
	...
	rootCmd.PersistentFlags().StringVar(&flags.selectFields, "select", "", "Comma-separated fields to include in output (e.g. --select active,automation_items,automation_triggers)")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'sendfox-pp-cli profile list')")
	...
	rootCmd.PersistentFlags().Float64Var(&flags.rateLimit, "rate-limit", 0.9, "Max requests per second (0 to disable)")
	...
	rootCmd.PersistentPreRunE = func(cmd *cobra.Command, args []string) error {
		...
		if humanFriendly && !isTerminal(cmd.OutOrStdout()) {
			noColor = true
		}
	...
	rootCmd.AddCommand(newAutomationEmailsCmd(flags))
	rootCmd.AddCommand(newAutomationsCmd(flags))
	rootCmd.AddCommand(newCampaignsCmd(flags))
	rootCmd.AddCommand(newContactFieldsCmd(flags))
	rootCmd.AddCommand(newContactTagsCmd(flags))
	...
	if parent, _, err := rootCmd.Find(strings.Fields("workflow")); err == nil {
		addNovelCommandIfAbsent(parent, newNovelWorkflowAudienceHealthCmd(flags))
		addNovelCommandIfAbsent(parent, newNovelWorkflowAutomationPlanCmd(flags))
		addNovelCommandIfAbsent(parent, newNovelWorkflowCampaignPreflightCmd(flags))
		addNovelCommandIfAbsent(parent, newNovelWorkflowCampaignReviewCmd(flags))
		addNovelCommandIfAbsent(parent, newNovelWorkflowContactDossierCmd(flags))
		addNovelCommandIfAbsent(parent, newNovelWorkflowExportBundleCmd(flags))
		addNovelCommandIfAbsent(parent, newNovelWorkflowGrowthReportCmd(flags))
		addNovelCommandIfAbsent(parent, newNovelWorkflowMigrationReadinessCmd(flags))
		addNovelCommandIfAbsent(parent, newWorkflowSnapshotSaveCmd(flags))
	}
