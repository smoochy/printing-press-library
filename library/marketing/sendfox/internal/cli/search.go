...
Run sync first to populate the local search index.`,
  ...
  sendfox-pp-cli search "status"

  # Force local search explicitly
  sendfox-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  sendfox-pp-cli search "status" --type automation_emails --data-source local
  # JSON output for piping
  sendfox-pp-cli search "status" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			case "contact_fields":
				results, err = db.SearchContactFields(query, limit)
			case "forms":
			...
			case "":
				...
				{
					partial, searchErr := db.SearchContactFields(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search contact_fields failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
