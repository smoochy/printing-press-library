...
type Config struct {
	...
	SendfoxApiToken   string          `toml:"api_token"`
...
func Load(configPath string) (*Config, error) {
	...
	// Env var overrides
	if v := cliutil.EnvOverride("SENDFOX_API_TOKEN"); v != "" {
		cfg.SendfoxApiToken = v
		cfg.markEnvOverride("SendfoxApiToken")
		cfg.AuthSource = "env:SENDFOX_API_TOKEN"
		cfg.CredentialSource = "env:SENDFOX_API_TOKEN"
	}
	if v := cliutil.EnvOverride("SENDFOX_BEARER_AUTH"); v != "" {
		cfg.SendfoxBearerAuth = v
		cfg.markEnvOverride("SendfoxBearerAuth")
		cfg.AuthSource = "env:SENDFOX_BEARER_AUTH"
		cfg.CredentialSource = "env:SENDFOX_BEARER_AUTH"
	}
	...
	if v := cliutil.EnvOverride("SENDFOX_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("SENDFOX_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "sendfox-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.SendfoxApiToken != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:SENDFOX_API_TOKEN"
		}
		return "Bearer " + c.SendfoxApiToken
	}
...
func (c *Config) StoreScopeCredential() string {
	...
	if c.SendfoxApiToken != "" {
		parts = append(parts, "api_token="+c.SendfoxApiToken)
	}
	if c.SendfoxBearerAuth != "" {
		parts = append(parts, "bearer_auth="+c.SendfoxBearerAuth)
	}
	if len(parts) == 0 {
		return ""
	}
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		SendfoxApiToken:   c.SendfoxApiToken,
		SendfoxBearerAuth: c.SendfoxBearerAuth,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	if c.SendfoxApiToken == "" {
		c.SendfoxApiToken = creds.SendfoxApiToken
	}
	if c.SendfoxBearerAuth == "" {
		c.SendfoxBearerAuth = creds.SendfoxBearerAuth
	}
}
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.SendfoxApiToken = ""
	delete(c.envOverrides, "SendfoxApiToken")
	c.updateFileConfigField("SendfoxApiToken")
	c.SendfoxBearerAuth = ""
	delete(c.envOverrides, "SendfoxBearerAuth")
	c.updateFileConfigField("SendfoxBearerAuth")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["SendfoxApiToken"] {
			out.SendfoxApiToken = c.fileConfig.SendfoxApiToken
		}
		if c.envOverrides["SendfoxBearerAuth"] {
			out.SendfoxBearerAuth = c.fileConfig.SendfoxBearerAuth
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "SendfoxApiToken":
		c.fileConfig.SendfoxApiToken = c.SendfoxApiToken
	case "SendfoxBearerAuth":
		c.fileConfig.SendfoxBearerAuth = c.SendfoxBearerAuth
	}
}
