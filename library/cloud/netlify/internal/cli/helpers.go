...
import (
	...
	"errors"
	"fmt"
	"github.com/mvanhorn/printing-press-library/library/cloud/netlify/internal/client"
...
func classifyAPIError(err error, flags *rootFlags) error {
	...
	case strings.Contains(msg, "HTTP 400") && cliutil.LooksLikeAuthError(msg):
		return authErr(fmt.Errorf("%w\nhint: the API rejected the request — this usually means auth is missing or invalid."+
			"\n      Run 'netlify-pp-cli auth login' to re-authenticate."+
			"\n      See API docs: https://www.netlify.com/docs/api/"+
			"\n      Run 'netlify-pp-cli doctor' to check auth status."+
			"\n      Response: "+cliutil.SanitizeErrorBody(msg), err))
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: check your token."+
			"\n      Run 'netlify-pp-cli auth login' to re-authenticate."+
			"\n      See API docs: https://www.netlify.com/docs/api/"+
			"\n      Run 'netlify-pp-cli doctor' to check auth status.", err))
...
func nonJSONPayloadError(data json.RawMessage) error {
	...
	if len(trimmed) > 0 && trimmed[0] == '<' {
		return authErr(fmt.Errorf("not authenticated or session expired; API returned HTML instead of JSON. " + "Run 'netlify-pp-cli auth login' to re-authenticate."))
	}
	if len(trimmed) == 0 {
