...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  carol-bike-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  carol-bike-pp-cli workflow archive --full

  # Archive without a wall-clock timeout
  carol-bike-pp-cli workflow archive --timeout 0`,
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  carol-bike-pp-cli workflow status

  # Show status as JSON
  carol-bike-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("carol-bike-pp-cli")
			}
