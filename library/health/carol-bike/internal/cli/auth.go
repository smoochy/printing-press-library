...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:         "auth",
		Short:       "Manage authentication for CAROL Bike",
...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  carol-bike-pp-cli auth setup\n  carol-bike-pp-cli auth setup --launch",
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			fmt.Fprintln(w, "See API docs: https://i.carolbike.com")
			fmt.Fprintln(w, "  Use a bearer token from your own authenticated CAROL Bike rider dashboard session.")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Then set:")
			fmt.Fprintln(w, "  export CAROL_BIKE_TOKEN=\"your-token-here\"")
			fmt.Fprintf(w, "  printf '%%s' \"$CAROL_BIKE_TOKEN\" | carol-bike-pp-cli auth set-token\n")
			if !launch {
				return nil
			}
			launchURL := "https://i.carolbike.com"
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  carol-bike-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: `  printf '%s' "$CAROL_BIKE_TOKEN" | carol-bike-pp-cli auth set-token`,
		Args:    cobra.NoArgs,
		RunE: func(cmd *cobra.Command, args []string) error {
			const maxTokenBytes = 1 << 20
			rawToken, err := io.ReadAll(io.LimitReader(cmd.InOrStdin(), maxTokenBytes+1))
			if err != nil {
				return configErr(fmt.Errorf("reading token from stdin: %w", err))
			}
			if len(rawToken) > maxTokenBytes {
				return configErr(fmt.Errorf("token from stdin exceeds %d bytes", maxTokenBytes))
			}
			token := strings.TrimSpace(string(rawToken))
			if token == "" {
				return configErr(fmt.Errorf("token is required on stdin; refusing a command-line token to protect shell history and process listings"))
			}
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  carol-bike-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			envStillSet := ""
			if envStillSet == "" && os.Getenv("CAROL_BIKE_TOKEN") != "" {
				envStillSet = "CAROL_BIKE_TOKEN"
			}
