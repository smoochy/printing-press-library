...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  carol-bike-pp-cli doctor
  carol-bike-pp-cli doctor --json
  carol-bike-pp-cli doctor --fail-on warn
  carol-bike-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if credentialRefused {
				...
				if cfg != nil {
					...
					if !configured {
						...
						report["auth_hint"] = "Export CAROL_BIKE_TOKEN or pipe the token to carol-bike-pp-cli auth set-token; tokens are never accepted in argv."
						report["auth_docs_url"] = "https://i.carolbike.com"
						report["auth_instructions"] = "Use a bearer token from your own authenticated CAROL Bike rider dashboard session."
					} else {
						authConfigured = true
			...
			if os.Getenv("CAROL_BIKE_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "CAROL_BIKE_TOKEN")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "CAROL_BIKE_TOKEN")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "carol-bike-pp-cli", suggestion)
						} else {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	report := map[string]any{}
	dbPath := defaultDBPath("carol-bike-pp-cli")
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'carol-bike-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'carol-bike-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		...
		report["hint"] = "No sync recorded; run 'carol-bike-pp-cli sync' to hydrate API-backed resources. Rows written by other paths are not tracked in sync_state."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'carol-bike-pp-cli sync' to refresh."
	}
	return report
}
