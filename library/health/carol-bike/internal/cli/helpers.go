...
func classifyAPIErrorOnly(err error) error {
	...
	case strings.Contains(msg, "HTTP 400") && cliutil.LooksLikeAuthError(msg):
		return authErr(fmt.Errorf("%w\nhint: the API rejected the request — this usually means auth is missing or invalid."+
			...
			"\n      Run 'carol-bike-pp-cli doctor' to check auth status."+
	...
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: check your token."+
			...
			"\n      See API docs: https://i.carolbike.com"+
			"\n      Use a bearer token from your own authenticated CAROL Bike rider dashboard session."+
			"\n      Run 'carol-bike-pp-cli doctor' to check auth status.", err))
...
func paginatedGet(ctx context.Context, c interface {
	...
	// PATCH: The cursor param is exempt from the "0"/"false" strip for both
	// offset and page pagination. CAROL's page index is zero-based, so dropping
	// page=0 silently skips the newest 40 rides. Under id-cursor pagination 0
	// is not a real record id and remains stripped.
	clean := map[string]string{}
	for k, v := range params {
		if v == "" {
			continue
		}
		if (k == cursorParam && (paginationType == "offset" || paginationType == "page")) || (v != "0" && v != "false") {
			clean[k] = v
		}
	}
...
func nonJSONPayloadError(data json.RawMessage) error {
	...
	if len(trimmed) > 0 && trimmed[0] == '<' {
		return authErr(fmt.Errorf("not authenticated or session expired; API returned HTML instead of JSON. " + "Export CAROL_BIKE_TOKEN or pipe the token to carol-bike-pp-cli auth set-token; tokens are never accepted in argv."))
	}
	if len(trimmed) == 0 {
