...
Resource scoping:
  ...
  carol-bike-pp-cli sync
  # Sync specific resources only
  carol-bike-pp-cli sync --resources ride,stats

  # Full resync (ignore previous checkpoint)
  carol-bike-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  carol-bike-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  carol-bike-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  carol-bike-pp-cli sync --latest-only`,
...
func syncResourceAtPath(ctx context.Context, c interface {
	...
	// PATCH: CAROL's page paginator is zero-based. Seed page 0 explicitly so
	// full syncs do not begin at the API's default page 1 and omit 40 rides.
	if resource == "ride" && cursor == "" && pageSize.cursorType == "page" {
		cursor = "0"
	}
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{ // #nosec G101 -- endpoint paths, not credentials.
		"ride":   "/rider/{riderId}/ride/type/REHIT",
		"stats":  "/rider/{riderId}/stats/calendar/last",
		"trends": "/rider/{riderId}/trends",
	}
