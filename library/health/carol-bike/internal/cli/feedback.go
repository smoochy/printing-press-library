...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("CAROL_BIKE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("CAROL_BIKE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "carol-bike-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		// PATCH: Phase-5 live dogfood requires every executable command to expose
		// at least one copyable example in help output.
		Example: `  carol-bike-pp-cli feedback "Ride history should include page zero"
  printf '%s\n' "The sync output was clear" | carol-bike-pp-cli feedback --stdin`,
		Long: `Feedback is captured locally first in the CLI data directory's feedback.jsonl.
When ` + "`CAROL_BIKE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`CAROL_BIKE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  carol-bike-pp-cli feedback list
  carol-bike-pp-cli feedback list --limit 5
  carol-bike-pp-cli feedback list --json`,
