...
type rootFlags struct {
	...
	// invocation. Mirrors the CAROL_BIKE_NO_LEARN env var.
	noLearn                 bool
	selectFields            string
	configPath              string
	homePath                string
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "carol-bike-pp-cli",
		Short: `Carol Bike CLI — Read and locally mirror personal CAROL Bike workout data.`,
		Long: `Carol Bike CLI — Read and locally mirror personal CAROL Bike workout data.
...
Health check: run 'carol-bike-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("carol-bike-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.selectFields, "select", "", "Comma-separated fields to include in output (e.g. --select id,type,start)")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'carol-bike-pp-cli profile list')")
	...
	rootCmd.PersistentFlags().StringVar(&flags.templateVarRiderId, "rider-id", "", "Set {riderId} path template value (env: CAROL_BIKE_RIDER_ID)")
	...
	rootCmd.AddCommand(newRideCmd(flags))
...
func (f *rootFlags) newClient() (*client.Client, error) {
	...
	if f.templateVarRiderId != "" {
		cfg.TemplateVars["riderId"] = f.templateVarRiderId
	}
