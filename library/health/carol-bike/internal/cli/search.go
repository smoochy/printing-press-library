...
Run sync first to populate the local search index.`,
  ...
  carol-bike-pp-cli search "status"

  # Force local search explicitly
  carol-bike-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  carol-bike-pp-cli search "status" --type ride --data-source local
  # JSON output for piping
  carol-bike-pp-cli search "status" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'carol-bike-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
