	...
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !authed {
				...
				fmt.Fprintln(w, "  export GARMIN_ACCESS_TOKEN=\"your-token-here\" # An access token obtained elsewhere. Garmin issues no personal API key, so there is normally nothing to put here — run `auth login` instead. A token set here is used as-is, is never refreshed, and is not the account `auth status` reports for this home.")
				fmt.Fprintln(w, "  export GARMIN_TOKEN=\"your-token-here\" # Same as GARMIN_ACCESS_TOKEN.")
				return authErr(fmt.Errorf("no credentials configured"))
			}
