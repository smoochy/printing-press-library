...
Run sync first to populate the local search index.`,
  ...
  catapult-openfield-connect-pp-cli search "status"

  # Force local search explicitly
  catapult-openfield-connect-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  catapult-openfield-connect-pp-cli search "status" --type activities --data-source local
  # JSON output for piping
  catapult-openfield-connect-pp-cli search "status" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'catapult-openfield-connect-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
