...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  catapult-openfield-connect-pp-cli doctor
  catapult-openfield-connect-pp-cli doctor --json
  catapult-openfield-connect-pp-cli doctor --fail-on warn
  catapult-openfield-connect-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if !configured {
					...
					report["auth_hint"] = "Set it with: catapult-openfield-connect-pp-cli auth set-token <token> or export CATAPULT_TOKEN=\"your-token-here\""
				} else {
					authConfigured = true
			...
			if os.Getenv("CATAPULT_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "CATAPULT_TOKEN")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "CATAPULT_TOKEN")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "catapult-openfield-connect-pp-cli", suggestion)
						} else {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'catapult-openfield-connect-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'catapult-openfield-connect-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		...
		report["hint"] = "No sync recorded; run 'catapult-openfield-connect-pp-cli sync' to hydrate API-backed resources. Rows written by other paths are not tracked in sync_state."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'catapult-openfield-connect-pp-cli sync' to refresh."
	}
	return report
}
