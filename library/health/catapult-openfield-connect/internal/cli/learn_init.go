...
func initLearn(ctx context.Context, db *sql.DB) error {
	...
	seeds := map[string][]lookups.SeedConfig{
		"metric": {
			{Canonical: "total_player_load", Values: []string{"player load", "load", "total load", "workload"}},
			{Canonical: "max_vel", Values: []string{"max velocity", "top speed", "peak velocity", "max speed"}},
			{Canonical: "total_distance", Values: []string{"distance", "total meters", "meterage"}},
			{Canonical: "meterage_per_minute", Values: []string{"m/min", "meters per minute", "intensity"}},
			{Canonical: "total_duration", Values: []string{"duration", "session length"}},
		},
	}
...
func runLearnInitOnce(ctx context.Context) {
	learnInitOnce.Do(func() {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "warning: catapult-openfield-connect-pp-cli: learn init: open store: %v\n", err)
			return
		}
		defer s.Close()
		if err := initLearn(ctx, s.DB()); err != nil {
			fmt.Fprintf(os.Stderr, "warning: catapult-openfield-connect-pp-cli: learn init: %v\n", err)
		}
	})
}
