...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  catapult-openfield-connect-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  catapult-openfield-connect-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"activities", "annotations", "athletes", "parameters", "teams", "venues"}
			archiveMaxPages := 100
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  catapult-openfield-connect-pp-cli workflow status

  # Show status as JSON
  catapult-openfield-connect-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("catapult-openfield-connect-pp-cli")
			}
