...
Resource scoping:
  ...
  catapult-openfield-connect-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  catapult-openfield-connect-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  catapult-openfield-connect-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  catapult-openfield-connect-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  catapult-openfield-connect-pp-cli sync --latest-only`,
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "athletes":
		return db.UpsertAthletes(data)
	case "customer":
		return db.UpsertCustomer(data)
	case "parameters":
		return db.UpsertParameters(data)
	case "periods":
		return db.UpsertPeriods(data)
	case "sensor":
		return db.UpsertSensor(data)
	case "stats":
