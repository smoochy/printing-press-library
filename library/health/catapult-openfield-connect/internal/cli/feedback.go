...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("CATAPULT_OPENFIELD_CONNECT_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("CATAPULT_OPENFIELD_CONNECT_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "catapult-openfield-connect-pp-cli/feedback")
...
When ` + "`CATAPULT_OPENFIELD_CONNECT_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`CATAPULT_OPENFIELD_CONNECT_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
maintainer sees it.`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			entry := FeedbackEntry{
				Text:      text,
				CLI:       "catapult-openfield-connect-pp-cli",
				Version:   version,
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  catapult-openfield-connect-pp-cli feedback list
  catapult-openfield-connect-pp-cli feedback list --limit 5
  catapult-openfield-connect-pp-cli feedback list --json`,
