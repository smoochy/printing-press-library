...
type rootFlags struct {
	...
	// invocation. Mirrors the CATAPULT_OPENFIELD_CONNECT_NO_LEARN env var.
	noLearn                 bool
	selectFields            string
	configPath              string
	homePath                string
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "catapult-openfield-connect-pp-cli",
		Short: `Catapult Openfield Connect CLI — Every Catapult GPS metric — ACWR, velocity benchmarks, load reports — from one command, with a local SQLite mirror for…`,
		Long: `Catapult Openfield Connect CLI — Every Catapult GPS metric — ACWR, velocity benchmarks, load reports — from one command, with a local SQLite mirror for offline analysis.
...
Health check: run 'catapult-openfield-connect-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("catapult-openfield-connect-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'catapult-openfield-connect-pp-cli profile list')")
	...
	rootCmd.AddCommand(newPeriodsCmd(flags))
	...
	rootCmd.AddCommand(newParametersPromotedCmd(flags))
	...
	addNovelCommandIfAbsent(rootCmd, newNovelAcwrCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelBenchmarkCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelDiffCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelHeatmapCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelPbCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelReportCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelRtpCmd(flags))
