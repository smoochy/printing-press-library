...
type Config struct {
	...
	CatapultToken    string          `toml:"token"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://us.catapultsports.com/api/v6",
	}
	...
	// Env var overrides
	if v := os.Getenv("CATAPULT_TOKEN"); v != "" {
		cfg.CatapultToken = v
		cfg.markEnvOverride("CatapultToken")
		cfg.AuthSource = "env:CATAPULT_TOKEN"
		cfg.CredentialSource = "env:CATAPULT_TOKEN"
	}
	...
	if v := os.Getenv("CATAPULT_OPENFIELD_CONNECT_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("CATAPULT_OPENFIELD_CONNECT_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "catapult-openfield-connect-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.CatapultToken != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:CATAPULT_TOKEN"
		}
		return applyAuthFormat("Bearer {token}", map[string]string{
			"token":          c.CatapultToken,
			"CATAPULT_TOKEN": c.CatapultToken,
			"access_token":   c.CatapultToken,
		})
	}
	if c.AccessToken != "" {
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	if c.CatapultToken == "" {
		c.CatapultToken = creds.CatapultToken
	}
}
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.CatapultToken = ""
	delete(c.envOverrides, "CatapultToken")
	c.updateFileConfigField("CatapultToken")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["CatapultToken"] {
			out.CatapultToken = c.fileConfig.CatapultToken
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "CatapultToken":
		c.fileConfig.CatapultToken = c.CatapultToken
	}
}
