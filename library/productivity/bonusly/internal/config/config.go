...
func Load(configPath string) (*Config, error) {
	...
	// Env var overrides
	if v := os.Getenv("BONUSLY_API_TOKEN"); v != "" {
		cfg.BonuslyApiToken = v
		cfg.markEnvOverride("BonuslyApiToken")
		cfg.AuthSource = "env:BONUSLY_API_TOKEN"
		cfg.CredentialSource = "env:BONUSLY_API_TOKEN"
	}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("BONUSLY_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "bonusly-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.BonuslyApiToken != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:BONUSLY_API_TOKEN"
		}
		return applyAuthFormat("Bearer {token}", map[string]string{
			"api_token":         c.BonuslyApiToken,
			"BONUSLY_API_TOKEN": c.BonuslyApiToken,
			"token":             c.BonuslyApiToken,
			"access_token":      c.BonuslyApiToken,
		})
	}
	if c.AccessToken != "" {
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["BonuslyApiToken"] {
			out.BonuslyApiToken = c.fileConfig.BonuslyApiToken
		}
	}
