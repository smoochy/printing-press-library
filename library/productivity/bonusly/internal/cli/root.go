...
type rootFlags struct {
	...
	// invocation. Mirrors the BONUSLY_NO_LEARN env var.
	noLearn bool
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "bonusly-pp-cli",
		Short: `Bonusly CLI — The recognition analytics Bonusly reserves for admins -- rebuilt from data any employee can already read.`,
		Long: `Bonusly CLI — The recognition analytics Bonusly reserves for admins -- rebuilt from data any employee can already read.
...
Highlights (not in the official API docs):
  ...
  • recognition search-mine   Search your own given and received recognition offline, without scrolling the public company feed. Requires one live lookup to confirm your identity (no local fallback); not yet verified against a re…
...
See README.md or the bundled SKILL.md for recipes.`,
	...
	rootCmd.SetVersionTemplate("bonusly-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'bonusly-pp-cli profile list')")
	...
	// pp:hand-edit bonusly-remove-broken-commands — awards, groups, and
	// meetings were removed: no live-verified working endpoint could be
	// found for any of them (~20 live path probes each, all 404). See
	...
	rootCmd.AddCommand(newRecognitionCmd(flags))
	rootCmd.AddCommand(newRedemptionsCmd(flags))
	...
	// pp:hand-edit bonusly-remove-broken-commands — incentives claim used
	// /users/awards/{id}/claim, the same broken awards resource family.
	...
	rootCmd.AddCommand(newGiveCmd(flags))
