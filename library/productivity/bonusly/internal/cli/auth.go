...
func newAuthLoginCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "login",
		Short: "Authenticate with the Bonusly API (redirect/instructions)",
		Long: `Bonusly's API uses Personal Access Tokens (PATs) for authentication.
This command provides instructions on how to obtain and set your PAT.`,
		Example: "  bonusly-pp-cli auth login",
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			fmt.Fprintln(w, "Bonusly requires a Personal Access Token (PAT) for API access.")
			fmt.Fprintln(w, "To obtain a token:")
			fmt.Fprintln(w, "  1. Log in to bonus.ly in your browser.")
			fmt.Fprintln(w, "  2. Go to Settings -> Services (for regular users) or Company -> Integrations -> API & Tokens (for admins).")
			fmt.Fprintln(w, "  3. Generate or copy your Personal Access Token.")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Once you have your token, run the following command to save it:")
			fmt.Fprintln(w, "  bonusly-pp-cli auth set-token <your-token-here>")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Or configure it via the environment:")
			fmt.Fprintln(w, "  export BONUSLY_API_TOKEN=\"your-token-here\"")
			return nil
		},
	}
	cmd.Flags().BoolVar(&browserFlag, "chrome", false, "Read cookies from Chrome (deprecated, PAT is now required)")
	cmd.Flags().BoolVar(&browserFlag, "browser", false, "Alias for --chrome (deprecated)")
	cmd.Flags().StringVar(&profileFlag, "profile", "", "Chrome profile name (deprecated)")
	cmd.Flags().StringVar(&cookiesFile, "cookies-file", "", "Import cookies file (deprecated)")
	return cmd
}