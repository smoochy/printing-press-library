...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/productivity/concur/internal/learn/lookups"
...
Resource scoping:
  ...
  concur-pp-cli sync --resources attendee_types,attendees

  # Full resync (ignore previous checkpoint)
  concur-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  concur-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  concur-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  concur-pp-cli sync --latest-only`,
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "attendee_types":
		return db.UpsertAttendeeTypes(data)
	case "attendees":
		return db.UpsertAttendees(data)
	case "available_expenses":
		return db.UpsertAvailableExpenses(data)
	case "delegates":
		return db.UpsertDelegates(data)
	case "expense_types":
		return db.UpsertExpenseTypes(data)
	case "expenses":
		return db.UpsertExpenses(data)
	case "locations":
		return db.UpsertLocations(data)
	case "payment_types":
		return db.UpsertPaymentTypes(data)
	case "reports":
...
func syncResourcePath(resource string) (string, error) {
	...
	paths := map[string]string{ // #nosec G101 -- endpoint paths, not credentials.
		...
		// trips deliberately omitted: it now lives entirely behind a POST
		// GraphQL BFF (see trips_list.go), not a GET-able path this
		// generic path+params sync interface can drive. Confirmed 2026-08-17.
	}
