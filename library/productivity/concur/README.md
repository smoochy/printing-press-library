...
Because the browser never surfaces a usable expense ID, success is confirmed by diffing the report's expense list before and after the form's Save click, correlated against the amount and expense type this call actually submitted (not just "any single new ID") -- a shared report modified concurrently through manager/processor/proxy contexts could otherwise have another actor's unrelated new expense misattributed as this call's own. Transaction Date is filled via Concur's calendar picker (day buttons carry a fully descriptive accessible name; text entry into the field itself does not reliably replace an existing value) rather than typed directly, so `--date` honors any date -- past, present, or future -- not just today. Payment Type's only verified-live value remains its own default (Cash) -- rather than silently substitute Cash for a `--payment-type` this can't honor and report success anyway, that one is still rejected up front, before the browser even opens. Vendor Description and Business Purpose are filled if requested; if either field can't be found or filled, the fallback aborts before the irreversible Save click instead of saving without a value the caller explicitly asked for. Like the `reports create` fallback, a failure after Save actually fires is reported as a possible partial success, not an ordinary retryable error. Triggers for a `--stdin` body just as reliably as the flag-driven path -- the fields it needs are read from the already-constructed request body, not the command's flag variables (which are always empty for a `--stdin` call).

### Browser-automation fallback for expense update and report submission

Two sibling mutation endpoints hit their own live backend defects, fixed with the same pattern: `expenses update` (PATCH `.../expenses/{id}`, for `--purpose` and a `--amount` flag added specifically to support this) hits the identical 404 "No static resource" signature as expense creation's pre-fallback defect; `reports submit` (POST `.../reports/{id}/submit`) hits a different-shaped but same-root-cause defect -- every HTTP verb (POST, PUT, PATCH, DELETE) was tested against it live, and none reach the action server-side (POST/PUT/DELETE 405, PATCH 404).

`expenses update`'s fallback opens the expense's direct edit URL, fills whichever of `--purpose`/`--amount` was requested, and verifies the re-fetched expense's fields actually match what was requested before reporting success -- a save that silently didn't register (the click fired but nothing changed) is reported as a failure, not trusted just because the follow-up GET itself succeeded. Business Purpose is a required field on Concur's form, so `--purpose ""` (requesting a clear) is rejected up front rather than opened in the browser to fail there. `--comment`/`--custom-fields` are accepted by the direct API path but not yet verified live through this fallback, so they're rejected the same way `--date`/`--payment-type` were for expense creation, above.

`reports submit`'s fallback clicks the report page's "Submit Report" button, then a **second**, separate "Submit Report" button inside the confirmation dialog that opens -- both share an identical accessible name and role while the dialog is open, which the fallback disambiguates by excluding the ref it already clicked rather than risking a non-deterministic re-click of the wrong one. Submission is confirmed via the re-fetched report's `isSubmitted`/`approvalStatus` fields before reporting success, for the same reason expense update's fallback re-verifies its own save.

Both fallbacks share expense creation's partial-success reporting: once either fallback's own irreversible confirm/save click fires, any later failure is reported as a possible partial success (not an ordinary retryable error), so re-running the command blindly doesn't risk a duplicate submission or an overwritten concurrent edit.
...
- **`expenses create`** — Automatically and transparently retries expense creation via automated browser when the API returns its confirmed-live 404 defect on an otherwise fully-valid request. Success is confirmed by diffing the report's expense list rather than trusting a browser-surfaced ID, since the browser gives none. Any `--date`, not just today, is honored via Concur's calendar picker.
- **`expenses update`** — Same 404 defect as expense creation, on the sibling PATCH path. Fills `--purpose`/`--amount` and verifies the re-fetched expense's fields actually changed before reporting success.
- **`reports submit`** — A different-shaped (405, not 404) but same-root-cause defect: no HTTP verb reaches the submit action server-side. Clicks through Concur's confirmation dialog (disambiguating two identically-named "Submit Report" buttons) and confirms `isSubmitted` before reporting success.
...
under Authentication. That fallback honors any `--date` via Concur's calendar picker, but will
still reject rather than silently substitute a default for a non-Cash `--payment-type` it can't
reliably honor.
...
- **`expenses create` rejects `--currency` (non-USD), or the browser fallback rejects a non-Cash `--payment-type`** — Expected, not a bug: neither can be reliably honored on this endpoint/form as currently confirmed live (see the rows above and "Browser-automation fallback for expense creation" under Authentication). Earlier versions silently substituted a default (USD, Cash) and still reported success, which is a worse failure mode than a loud rejection -- creating an expense with data the caller didn't ask for. Omit the flag to accept the default explicitly, or correct the field manually in Concur after creation. (`--date` is NOT in this category -- any date is honored via Concur's calendar picker.)
- **`expenses update --purpose ""` is rejected** — Expected: Business Purpose is a required field on Concur's expense-edit form, so clearing it isn't honorable through this fallback. Set a non-empty `--purpose`, or clear it manually in Concur if your policy genuinely allows an empty value.
