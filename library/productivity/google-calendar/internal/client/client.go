...
func (c *Client) do(ctx context.Context, method, path string, params map[string]string, body any, headerOverrides map[string]string) (json.RawMessage, int, error) {
	// Structural event-safety barrier — see safety.go. Applies to every
	// mutating events-surface request from any code path, before transport.
	p2, h2, err := c.enforceEventSafety(ctx, method, path, params, body, headerOverrides)
	if err != nil {
		return nil, 0, err
	}
	return c.doInternal(ctx, method, path, p2, body, h2, false)
}
...
func authPlaceholderCredentialError(cfg *config.Config) error {
	return authPlaceholderCredentialErrorWithSetup(cfg, "google-calendar-pp-cli auth login")
}
...
func (c *Client) maskCredentialText(text string, extraCredentials ...string) string {
	...
	if c != nil && c.Config != nil {
		...
		addCredential(c.Config.CalendarOauth2c)
	}
