...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  zotero-research-library-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  zotero-research-library-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"collections", "collections-top", "items", "items-top", "items-trash", "searches"}
			archiveMaxPages := 100
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  zotero-research-library-pp-cli workflow status

  # Show status as JSON
  zotero-research-library-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("zotero-research-library-pp-cli")
			}
