...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("ZOTERO_RESEARCH_LIBRARY_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("ZOTERO_RESEARCH_LIBRARY_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "zotero-research-library-pp-cli/feedback")
...
When ` + "`ZOTERO_RESEARCH_LIBRARY_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`ZOTERO_RESEARCH_LIBRARY_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
maintainer sees it.`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			entry := FeedbackEntry{
				Text:      text,
				CLI:       "zotero-research-library-pp-cli",
				Version:   version,
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  zotero-research-library-pp-cli feedback list
  zotero-research-library-pp-cli feedback list --limit 5
  zotero-research-library-pp-cli feedback list --json`,
