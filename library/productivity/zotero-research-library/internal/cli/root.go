...
type rootFlags struct {
	...
	// invocation. Mirrors the ZOTERO_RESEARCH_LIBRARY_NO_LEARN env var.
	noLearn                 bool
	selectFields            string
	configPath              string
	homePath                string
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "zotero-research-library-pp-cli",
		Short: `Zotero Research Library CLI — Your entire Zotero research library, searchable offline — version-based incremental sync into SQLite with full-text…`,
		Long: `Zotero Research Library CLI — Your entire Zotero research library, searchable offline — version-based incremental sync into SQLite with full-text search no other Zotero tool has.
...
Health check: run 'zotero-research-library-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("zotero-research-library-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'zotero-research-library-pp-cli profile list')")
	...
	rootCmd.AddCommand(newDeletedPromotedCmd(flags))
	rootCmd.AddCommand(newFulltextPromotedCmd(flags))
	rootCmd.AddCommand(newKeysPromotedCmd(flags))
