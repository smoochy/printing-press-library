...
func initLearn(ctx context.Context, db *sql.DB) error {
	...
	seeds := map[string][]lookups.SeedConfig{
		"item_type": {
			{Canonical: "journalArticle", Values: []string{"article", "paper", "journal article"}},
			{Canonical: "book", Values: []string{"monograph"}},
			{Canonical: "bookSection", Values: []string{"chapter", "book chapter"}},
			{Canonical: "thesis", Values: []string{"dissertation", "phd thesis"}},
			{Canonical: "conferencePaper", Values: []string{"proceedings", "conference"}},
			{Canonical: "report", Values: []string{"white paper", "technical report"}},
		},
	}
...
func runLearnInitOnce(ctx context.Context) {
	learnInitOnce.Do(func() {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "warning: zotero-research-library-pp-cli: learn init: open store: %v\n", err)
			return
		}
		defer s.Close()
		if err := initLearn(ctx, s.DB()); err != nil {
			fmt.Fprintf(os.Stderr, "warning: zotero-research-library-pp-cli: learn init: %v\n", err)
		}
	})
}
