...
In local mode: searches locally synced data only.`,
  ...
  zotero-research-library-pp-cli search "error timeout"

  # Force local search only
  zotero-research-library-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  zotero-research-library-pp-cli search "status" --type collections --data-source local
  # JSON output for piping
  zotero-research-library-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if flags.dataSource != "local" {
				...
				data, getErr := c.Get(cmd.Context(), "/searches", map[string]string{
					"q": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'zotero-research-library-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			case "items":
				results, err = db.SearchItems(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchItems(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search items failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
