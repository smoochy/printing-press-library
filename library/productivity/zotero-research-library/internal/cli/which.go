...
var whichIndex = []whichEntry{
	{Command: "ground", Description: "Ranked full-text search over your synced library — title, abstract, creators, tags, and attachment full text — answered offline in under a second, with snippets and citation-ready metadata.", Group: "Research grounding that compounds", WhyItMatters: "Reach for this when asked what the user's own library says about a topic."},
	{Command: "items recent", Description: "What landed in the library recently, sorted by dateAdded from the local cache, with a --days cutoff and pipeable keys.", Group: "Research grounding that compounds", WhyItMatters: "Use for weekly triage of new additions."},
	{Command: "cite", Description: "Resolves a Better BibTeX citekey to its item from the local cache and exports citekey-faithful BibTeX; sync backfills citekeys via the desktop's local JSON-RPC when available.", Group: "Manuscript plumbing", WhyItMatters: "Use when a manuscript needs a citation by its Better BibTeX citekey."},
	{Command: "collections tree", Description: "Renders the full collection hierarchy with per-node item counts in one command — a shape the flat API cannot return in one call.", Group: "Research grounding that compounds", WhyItMatters: "Use for a hierarchical overview of how the library is organized."},
	{Command: "cache status", Description: "Sync cursors, row and FTS counts, and last-sync age from the local cache; 'cache reindex' rebuilds the FTS index from base tables with zero network.", Group: "Agent-native plumbing", WhyItMatters: "Use to check cache freshness offline before trusting search results."},
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one`,
		Example: `  zotero-research-library-pp-cli which "stale tickets"
  zotero-research-library-pp-cli which "bottleneck"
  zotero-research-library-pp-cli which --limit 1 "send message"
  zotero-research-library-pp-cli which                                # list the full capability index`,
