...
func RegisterIntents(s *server.MCPServer) {
	s.AddTool(
		mcplib.NewTool("full_text_deep_search",
			mcplib.WithDescription("Extends the search into attachment full text and resolves hits to their parent papers."),
			mcplib.WithBoolean("fulltext", mcplib.Description("Override the recipe's --fulltext value.")),
		),
		handleFullTextDeepSearch,
	)
}

// handleFullTextDeepSearch runs the full_text_deep_search recipe intent: Extends the search into attachment full text and resolves hits to their parent papers.
func handleFullTextDeepSearch(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
	...
	args = append(args, "nordic hamstring")
	args = appendRecipeBoolFlag(args, "fulltext", input["fulltext"], true)
