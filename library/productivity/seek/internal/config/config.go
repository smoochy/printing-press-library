...
type Config struct {
	...
	SeekCookie       string          `toml:"cookie"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://au.seek.com",
	}
	...
	if v := os.Getenv("SEEK_COOKIE"); v != "" {
		cfg.SeekCookie = v
		cfg.markEnvOverride("SeekCookie")
		cfg.AuthSource = "env:SEEK_COOKIE"
		cfg.CredentialSource = "env:SEEK_COOKIE"
	...
	if v := os.Getenv("SEEK_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("SEEK_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "seek-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.SeekCookie != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:SEEK_COOKIE"
		}
		return ensureAuthScheme("Bearer", c.SeekCookie)
	}
	if c.AccessToken != "" {
		if c.AuthSource == "" {
...
func (c *Config) StoreScopeCredential() string {
	...
	if c.SeekCookie != "" {
		parts = append(parts, "cookie="+c.SeekCookie)
	}
	if len(parts) == 0 {
		return ""
	}
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.SeekCookie = ""
	delete(c.envOverrides, "SeekCookie")
	c.updateFileConfigField("SeekCookie")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["SeekCookie"] {
			out.SeekCookie = c.fileConfig.SeekCookie
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "SeekCookie":
		c.fileConfig.SeekCookie = c.SeekCookie
	}
}
