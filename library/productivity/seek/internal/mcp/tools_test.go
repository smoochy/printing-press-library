...
func TestMCPPathResolutionMatchesCLIResolverWithPlatformDefaults(t *testing.T) {
	...
	if want := filepath.Join(home, ".config", "seek-pp-cli", "config.toml"); cfg.Path != want {
	...
	if want := filepath.Join(home, ".local", "share", "seek-pp-cli", "data.db"); gotDB != want {
...
func TestMCPRegisterToolsPreservesTypedSpecialTools(t *testing.T) {
	s := server.NewMCPServer("seek", "test")
	RegisterTools(s)
