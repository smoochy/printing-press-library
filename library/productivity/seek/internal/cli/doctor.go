...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  seek-pp-cli doctor
  seek-pp-cli doctor --json
  seek-pp-cli doctor --fail-on warn
  seek-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if credentialRefused {
				...
				if !credentialConfigured {
					...
					report["auth_hint"] = "seek-pp-cli auth login --chrome"
				} else {
					authConfigured = true
			...
			if os.Getenv("SEEK_COOKIE") != "" {
				authEnvSet = append(authEnvSet, "SEEK_COOKIE")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "SEEK_COOKIE")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					healthPath := "/api/jobsearch/v5/search"
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "seek-pp-cli", suggestion)
						} else {
