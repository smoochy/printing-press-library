...
type rootFlags struct {
	...
	// invocation. Mirrors the SEEK_NO_LEARN env var.
	noLearn bool
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "seek-pp-cli",
		Short: `Seek CLI — Every SEEK search and job-detail feature, plus a local job history, salary distributions, and hiring trends no other…`,
		Long: `Seek CLI — Every SEEK search and job-detail feature, plus a local job history, salary distributions, and hiring trends no other SEEK tool has.
...
Health check: run 'seek-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("seek-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.selectFields, "select", "", "Comma-separated fields to include in output (e.g. --select id,title,teaser)")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'seek-pp-cli profile list')")
	...
	addNovelCommandIfAbsent(rootCmd, newNovelClassificationsCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelCompanyCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelSalaryCmd(flags))
