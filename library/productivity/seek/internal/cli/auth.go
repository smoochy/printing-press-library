...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:         "auth",
		Short:       "Manage authentication for Seek",
...
Use --chrome to read cookies from Chrome for .seek.com.
...
profile by name when the installed backend supports it.`,
		Example: `  seek-pp-cli auth login --chrome
  seek-pp-cli auth login --browser
  seek-pp-cli auth login --cookies-file storage-state.json
  seek-pp-cli auth login --chrome --profile "Work"`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !browserFlag && cookiesFile == "" {
				...
				fmt.Fprintf(cmd.OutOrStdout(), "  seek-pp-cli auth login --chrome\n")
				fmt.Fprintf(cmd.OutOrStdout(), "  seek-pp-cli auth login --browser\n")
				fmt.Fprintf(cmd.OutOrStdout(), "  seek-pp-cli auth login --cookies-file storage-state.json\n")
				return nil
			}
			if browserFlag && cookiesFile != "" {
			...
			if !fromPressAuth && !fromCookiesFile {
				...
				if err != nil {
					...
					if runtime.GOOS == "windows" {
						...
						fmt.Fprintf(w, "\n  seek-pp-cli auth login --cookies-file <path>\n\n")
			...
			requiredCookies := []string{"SEEK_SESSION"}
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  seek-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			if !credentialConfigured {
				...
				fmt.Fprintf(w, "  seek-pp-cli auth login --chrome\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}

			fmt.Fprintln(w, green("Authenticated"))
			fmt.Fprintf(w, "  Source: %s\n", cfg.AuthSource)
			fmt.Fprintf(w, "  Domain: .seek.com\n")
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  echo \"$TOKEN\" | seek-pp-cli auth set-token\n  seek-pp-cli auth set-token < token-file",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  seek-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
