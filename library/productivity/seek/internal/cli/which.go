...
var whichIndex = []whichEntry{
	{Command: "salary", Description: "Get salary percentiles, a histogram, and the pay-disclosure rate for a role and location by sampling matching listings (which it also caches locally).", Group: "Local state that compounds", WhyItMatters: "Reach for this instead of fetching hundreds of listings and computing pay statistics yourself."},
	{Command: "trends", Description: "See how job-listing volume for a classification, region, or work arrangement changes over time, bucketed from the listing dates of every job the CLI has cached, with period-over-period deltas.", Group: "Local state that compounds", WhyItMatters: "Use this when the question is 'is hiring up or down', not 'what is open right now'."},
	{Command: "company", Description: "Profile one advertiser: their company details and review ratings plus every current opening and a local history of how many roles they've posted.", Group: "Cross-entity joins", WhyItMatters: "Reach for this for 'who is hiring for X' market scans and interview prep instead of scrolling a company's SEEK page."},
	{Command: "listings facets", Description: "Break a live query down by classification, work type, pay band, or region by tallying the sampled result pages — no local store needed.", Group: "Agent-native plumbing", WhyItMatters: "Use this for a fast shape-of-the-market answer before committing to a full search or a sync."},
	{Command: "classifications", Description: "Browse or resolve SEEK's numeric classification and subclassification IDs so filtered searches can target them precisely.", Group: "Agent-native plumbing", WhyItMatters: "Call this first when you need a --classification value for listings search."},
	{Command: "me new-jobs", Description: "Run every one of your SEEK saved searches and return only the postings that aren't already in your local store, tagged by which search matched.", Group: "Cross-entity joins", WhyItMatters: "This is the daily-driver command for an active job hunt; use it instead of re-scrolling the SEEK site."},
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one. Machine output (--json/--csv/--plain/--quiet, or a pipe) still exits 2 and writes {"matches":[]} (or the equivalent empty table) to stdout.`,
		SilenceUsage: true,
		Example: `  seek-pp-cli which "stale tickets"
  seek-pp-cli which "bottleneck"
  seek-pp-cli which --limit 1 "send message"
  seek-pp-cli which                                # list the full capability index`,
