...
func newLearnConfig() *entities.Config {
	...
	if re, err := regexp.Compile(`^[0-9]{6,10}$`); err != nil {
		fmt.Fprintf(os.Stderr, "warning: seek-pp-cli: ticker pattern %q failed to compile: %v\n", `^[0-9]{6,10}$`, err)
	} else {
...
func initLearn(ctx context.Context, db *sql.DB) error {
	...
	seeds := map[string][]lookups.SeedConfig{
		"sort_mode": {
			{Canonical: "KeywordRelevance", Values: []string{"relevance", "relevant", "best"}},
			{Canonical: "ListedDate", Values: []string{"date", "newest", "recent"}},
		},
		"work_type": {
			{Canonical: "242", Values: []string{"full-time", "full time", "fulltime"}},
			{Canonical: "243", Values: []string{"part-time", "part time", "parttime"}},
			{Canonical: "244", Values: []string{"contract", "contract/temp", "temp", "fixed term"}},
			{Canonical: "245", Values: []string{"casual", "casual/vacation", "vacation"}},
		},
	}
...
func runLearnInitOnce(ctx context.Context) {
	learnInitOnce.Do(func() {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "warning: seek-pp-cli: learn init: open store: %v\n", err)
			return
		}
		defer s.Close()
		if err := initLearn(ctx, s.DB()); err != nil {
			fmt.Fprintf(os.Stderr, "warning: seek-pp-cli: learn init: %v\n", err)
		}
	})
}
...
func learnResourceTypeFields() map[string][]string {
	return map[string][]string{
		...
		"me":       {"name", "title", "display_name", "full_name", "short_name", "label", "slug", "key", "code", "id", "address"},
	}
}
