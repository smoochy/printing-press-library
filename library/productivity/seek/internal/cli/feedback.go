...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SEEK_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SEEK_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "seek-pp-cli/feedback")
...
When ` + "`SEEK_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SEEK_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
maintainer sees it.`,
		Example: `  seek-pp-cli feedback "the --since flag is inclusive but docs say exclusive"
  seek-pp-cli feedback --stdin < notes.txt
  seek-pp-cli feedback list --limit 10`,
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  seek-pp-cli feedback list
  seek-pp-cli feedback list --limit 5
  seek-pp-cli feedback list --json`,
