...
func classifyAPIErrorOnly(err error) error {
	...
	case strings.Contains(msg, "HTTP 400") && cliutil.LooksLikeAuthError(msg):
		return authErr(fmt.Errorf("%w\nhint: the API rejected the request — this usually means auth is missing or invalid."+
			...
			"\n      Run 'seek-pp-cli doctor' to check auth status."+
			"\n      Response: "+cliutil.SanitizeErrorBody(msg), err))
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: check your API credentials."+
			" Run 'seek-pp-cli auth login --chrome' to refresh browser-session credentials."+
			"\n      See API docs: https://au.seek.com"+
			"\n      Run 'seek-pp-cli doctor' to check auth status.", err))
...
func nonJSONPayloadError(data json.RawMessage) error {
	...
	if len(trimmed) > 0 && trimmed[0] == '<' {
		return authErr(fmt.Errorf("not authenticated or session expired; API returned HTML instead of JSON. " + "Run 'seek-pp-cli auth login --chrome' to refresh browser-session credentials."))
	}
	if len(trimmed) == 0 {
