...
func resetCredentialEnv(t *testing.T) (home, configPath string) {
	...
	for _, name := range []string{
		"WORKSPACE_ADMIN_CONFIG",
		"WORKSPACE_ADMIN_CONFIG_DIR",
		"WORKSPACE_ADMIN_DATA_DIR",
		"WORKSPACE_ADMIN_STATE_DIR",
		"WORKSPACE_ADMIN_CACHE_DIR",
		"WORKSPACE_ADMIN_HOME",
		"XDG_CONFIG_HOME",
		"XDG_DATA_HOME",
		"XDG_STATE_HOME",
		"XDG_CACHE_HOME",
	...
	return home, filepath.Join(home, ".config", "workspace-admin-pp-cli", "config.toml")
}
...
func TestCorruptCredentialsFallsBackToLegacyConfig(t *testing.T) {
	...
	credentialsPath := filepath.Join(home, ".local", "share", "workspace-admin-pp-cli", "credentials.toml")
...
func TestCorruptCredentialsFallsBackToEnvCredential(t *testing.T) {
	...
	t.Setenv("GOOGLE_WORKSPACE_TOKEN", "env-secret")
	credentialsPath := filepath.Join(home, ".local", "share", "workspace-admin-pp-cli", "credentials.toml")
	if err := os.MkdirAll(filepath.Dir(credentialsPath), 0o700); err != nil {
		t.Fatalf("mkdir credentials dir: %v", err)
	}
...
func TestAuthWriteScrubsLegacyConfigWhenRelocated(t *testing.T) {
	...
	activeConfigPath := filepath.Join(newConfigDir, "workspace-admin-pp-cli", "config.toml")
	...
	credsPath := filepath.Join(newDataDir, "workspace-admin-pp-cli", "credentials.toml")
