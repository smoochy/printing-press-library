...
type Config struct {
	...
	GoogleWorkspaceToken string          `toml:"workspace_token"`
...
func Load(configPath string) (*Config, error) {
	...
	// Env var overrides
	if v := os.Getenv("GOOGLE_WORKSPACE_TOKEN"); v != "" {
		cfg.GoogleWorkspaceToken = v
		cfg.markEnvOverride("GoogleWorkspaceToken")
		cfg.AuthSource = "env:GOOGLE_WORKSPACE_TOKEN"
		cfg.CredentialSource = "env:GOOGLE_WORKSPACE_TOKEN"
	}
	...
	if v := os.Getenv("WORKSPACE_ADMIN_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	...
	if v := strings.TrimSpace(os.Getenv("WORKSPACE_ADMIN_USER_ID")); v != "" {
		cfg.TemplateVars["userId"] = v
	} else if verifyMode {
		cfg.TemplateVars["userId"] = "userId_placeholder"
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("WORKSPACE_ADMIN_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "workspace-admin-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.GoogleWorkspaceToken != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:GOOGLE_WORKSPACE_TOKEN"
		}
		return "Bearer " + c.GoogleWorkspaceToken
	}
	if c.AccessToken != "" {
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		GoogleWorkspaceToken: c.GoogleWorkspaceToken,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	c.GoogleWorkspaceToken = creds.GoogleWorkspaceToken
}
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.GoogleWorkspaceToken = ""
	delete(c.envOverrides, "GoogleWorkspaceToken")
	c.updateFileConfigField("GoogleWorkspaceToken")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["GoogleWorkspaceToken"] {
			out.GoogleWorkspaceToken = c.fileConfig.GoogleWorkspaceToken
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "GoogleWorkspaceToken":
		c.fileConfig.GoogleWorkspaceToken = c.GoogleWorkspaceToken
	}
}
