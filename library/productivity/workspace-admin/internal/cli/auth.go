...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Workspace Admin",
	...
	// Novel: service-account JWT-bearer + domain-wide delegation flow
	// (internal/cli/workspace_auth.go). Mints GOOGLE_WORKSPACE_TOKEN from a
	// service-account key with an impersonation subject.
...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  workspace-admin-pp-cli auth setup\n  workspace-admin-pp-cli auth setup --launch",
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			fmt.Fprintln(w, "No setup URL is configured for this CLI; check the API's docs.")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Then set:")
			fmt.Fprintln(w, "  export GOOGLE_WORKSPACE_TOKEN=\"your-token-here\"")
			fmt.Fprintln(w, "  workspace-admin-pp-cli auth set-token <token>")
			if !launch {
				return nil
			}
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  workspace-admin-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !authed {
				...
				fmt.Fprintln(w, "  export GOOGLE_WORKSPACE_TOKEN=\"your-token-here\"")
				fmt.Fprintf(w, "  workspace-admin-pp-cli auth set-token <token>\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  workspace-admin-pp-cli auth set-token YOUR_TOKEN_HERE",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  workspace-admin-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			envStillSet := ""
			if envStillSet == "" && os.Getenv("GOOGLE_WORKSPACE_TOKEN") != "" {
				envStillSet = "GOOGLE_WORKSPACE_TOKEN"
			}
