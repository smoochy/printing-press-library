...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("WORKSPACE_ADMIN_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("WORKSPACE_ADMIN_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "workspace-admin-pp-cli/feedback")
...
When ` + "`WORKSPACE_ADMIN_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`WORKSPACE_ADMIN_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  workspace-admin-pp-cli feedback list
  workspace-admin-pp-cli feedback list --limit 5
  workspace-admin-pp-cli feedback list --json`,
