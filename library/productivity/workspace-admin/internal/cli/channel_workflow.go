...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  workspace-admin-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  workspace-admin-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"admin", "admin-directory-v1-groups", "alerts", "drives", "files", "teamdrives"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  workspace-admin-pp-cli workflow status

  # Show status as JSON
  workspace-admin-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("workspace-admin-pp-cli")
			}
