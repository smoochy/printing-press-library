...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "workspace-admin-pp-cli",
		Short: `Workspace Admin CLI — GAT-style Google Workspace auditing as an agent-native CLI: sync Directory, Drive, Gmail, Reports, and Alert Center int…`,
		Long: `Workspace Admin CLI — GAT-style Google Workspace auditing as an agent-native CLI: sync Directory, Drive, Gmail, Reports, and Alert Center int…

Highlights (not in the official API docs):
  • workflow offboard   Run a departing user's entire lifecycle in one command — suspend, sign out, revoke OAuth tokens, transfer Drive ownership, set Gmail delegation, remove fr…
...
Health check: run 'workspace-admin-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("workspace-admin-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'workspace-admin-pp-cli profile list')")
	...
	// The framework channel-workflow parent and the novel-feature workflow
	// parent both use Use:"workflow"; registering both shadows the novel
	// offboard command. Nest the novel offboard under the single framework
	// workflow parent instead so `workflow offboard` resolves alongside
	// `workflow archive` / `workflow status`.
	wfCmd := newWorkflowCmd(flags)
	wfCmd.AddCommand(newNovelWorkflowOffboardCmd(flags))
	rootCmd.AddCommand(wfCmd)
	rootCmd.AddCommand(newNovelAuditCmd(flags))
	rootCmd.AddCommand(newNovelGroupsCmd(flags))
	rootCmd.AddCommand(newAPICmd(flags))
	rootCmd.AddCommand(newAlertsBatchDeletePromotedCmd(flags))
	rootCmd.AddCommand(newAlertsBatchUndeletePromotedCmd(flags))
	rootCmd.AddCommand(newChannelsPromotedCmd(flags))
	rootCmd.AddCommand(newDriveAboutPromotedCmd(flags))
	rootCmd.AddCommand(newHistoryPromotedCmd(flags))
	rootCmd.AddCommand(newStopPromotedCmd(flags))
	rootCmd.AddCommand(newUsersProfilePromotedCmd(flags))
