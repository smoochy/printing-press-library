...
Resource scoping:
  ...
  workspace-admin-pp-cli sync

  # Sync specific resources only
  workspace-admin-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  workspace-admin-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  workspace-admin-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  workspace-admin-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  workspace-admin-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dbPath == "" {
				dbPath = defaultDBPath("workspace-admin-pp-cli")
			}
...
func syncResource(ctx context.Context, c interface {
	...
	for {
		...
		// users.list and groups.list require exactly one of customer/domain,
		// but the OpenAPI spec marks both `required: false` (it is an XOR the
		// boolean cannot express), so the generator emits no default and the
		// request 400s. Supply the standard "my_customer" alias; a user
		// --param customer=... / domain=... still overrides via applyTo below.
		if resource == "admin" || resource == "admin-directory-v1-groups" {
...
func determinePaginationDefaults(resource string) paginationDefaults {
	...
	case "comments/comments_replies":
		return paginationDefaults{
			cursorParam: "pageToken",
			cursorType:  "page_token",
			limitParam:  "pageSize",
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "admin-sdk-admin":
		return db.UpsertAdminSdkAdmin(data)
	case "alerts":
	...
	case "changes":
		return db.UpsertChanges(data)
	case "drive-about":
		return db.UpsertDriveAbout(data)
	case "drives":
		return db.UpsertDrives(data)
	case "hide":
		return db.UpsertHide(data)
	case "unhide":
		return db.UpsertUnhide(data)
	case "files":
	...
	case "list_labels":
		return db.UpsertListLabels(data)
	case "modify_labels":
		return db.UpsertModifyLabels(data)
	case "permissions":
		return db.UpsertPermissions(data)
	case "revisions":
		return db.UpsertRevisions(data)
	case "files_watch":
		return db.UpsertFilesWatch(data)
	case "gmail-settings":
		return db.UpsertGmailSettings(data)
	case "labels":
	...
	case "messages_modify":
		return db.UpsertMessagesModify(data)
	case "messages_trash":
		return db.UpsertMessagesTrash(data)
	case "messages_untrash":
		return db.UpsertMessagesUntrash(data)
	case "teamdrives":
		return db.UpsertTeamdrives(data)
	case "threads":
		return db.UpsertThreads(data)
	case "threads_modify":
		return db.UpsertThreadsModify(data)
	case "threads_trash":
		return db.UpsertThreadsTrash(data)
	case "threads_untrash":
		return db.UpsertThreadsUntrash(data)
	default:
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"admin":                     "/admin/directory/v1/users",
		"admin-directory-v1-groups": "/admin/directory/v1/groups",
		"alerts":                    "/v1beta1/alerts",
		"changes":                   "/changes",
		"drafts":                    "/gmail/v1/users/{userId}/drafts",
		"drives":                    "/drives",
		"files":                     "/files",
		"gmail-settings":            "/gmail/v1/users/{userId}/settings/cse/identities",
		"history":                   "/gmail/v1/users/{userId}/history",
		"messages":                  "/gmail/v1/users/{userId}/messages",
		"settings":                  "/v1beta1/settings",
		"teamdrives":                "/teamdrives",
		"threads":                   "/gmail/v1/users/{userId}/threads",
	}
...
func dependentResourceDefs() []dependentResourceDef {
	return []dependentResourceDef{
		{Name: "comments", ParentTable: "files", ParentIDParam: "fileId", PathTemplate: "/files/{fileId}/comments", KeyField: "", ReconcileMode: "per_parent", GenericScopeJSONPath: "$.file", PathParams: []dependentPathParamDef{
			{Param: "fileId", Field: "id"},
		}},
		{Name: "list_labels", ParentTable: "files", ParentIDParam: "fileId", PathTemplate: "/files/{fileId}/listLabels", KeyField: "", ReconcileMode: "per_parent", GenericScopeJSONPath: "$.file", PathParams: []dependentPathParamDef{
			{Param: "fileId", Field: "id"},
		}},
		{Name: "permissions", ParentTable: "files", ParentIDParam: "fileId", PathTemplate: "/files/{fileId}/permissions", KeyField: "", ReconcileMode: "per_parent", GenericScopeJSONPath: "$.file", PathParams: []dependentPathParamDef{
			{Param: "fileId", Field: "id"},
		}},
		{Name: "revisions", ParentTable: "files", ParentIDParam: "fileId", PathTemplate: "/files/{fileId}/revisions", KeyField: "", ReconcileMode: "per_parent", GenericScopeJSONPath: "$.file", PathParams: []dependentPathParamDef{
			{Param: "fileId", Field: "id"},
		}},
		{Name: "comments_replies", ParentTable: "comments", ParentIDParam: "commentId", PathTemplate: "/files/{fileId}/comments/{commentId}/replies", KeyField: "", ReconcileMode: "none", GenericScopeJSONPath: "", PathParams: []dependentPathParamDef{
			{Param: "fileId", Field: "files_id"},
			{Param: "commentId", Field: "id"},
		}},
	}
}
...
func responsePathForResource(resource, path string) []string {
	...
	case "admin\x00/admin/directory/v1/{parent}/chrome/printServers":
		return []string{"printServers"}
	case "admin\x00/admin/directory/v1/{parent}/chrome/printers":
		return []string{"auxiliaryMessages"}
	case "admin\x00/admin/directory/v1/{parent}/chrome/printers:listPrinterModels":
		return []string{"printerModels"}
	case "admin\x00/admin/directory/v1/users/{userKey}/asps":
		return []string{"items"}
	case "admin\x00/admin/directory/v1/customer/{customerId}/devices/chromeos":
		return []string{"chromeosdevices"}
	case "admin\x00/admin/directory/v1/customer/{customer}/domainaliases":
		return []string{"domainAliases"}
	case "admin\x00/admin/directory/v1/customer/{customer}/domains/{domainName}":
		return []string{"domainAliases"}
	case "admin\x00/admin/directory/v1/customer/{customer}/domains":
		return []string{"domainAliases"}
	case "admin\x00/admin/directory/v1/groups/{groupKey}/aliases":
		return []string{"aliases"}
	case "admin\x00/admin/directory/v1/groups":
		return []string{"groups"}
	case "admin\x00/admin/directory/v1/groups/{groupKey}/members":
		return []string{"members"}
	case "admin\x00/admin/directory/v1/customer/{customerId}/devices/mobile":
		return []string{"mobiledevices"}
	case "admin\x00/admin/directory/v1/customer/{customerId}/orgunits":
		return []string{"organizationUnits"}
	case "admin\x00/admin/directory/v1/customer/{customer}/roles/ALL/privileges":
		return []string{"items"}
	case "admin\x00/admin/directory/v1/customer/{customer}/resources/buildings/{buildingId}":
		return []string{"floorNames"}
	case "admin\x00/admin/directory/v1/customer/{customer}/resources/buildings":
		return []string{"floorNames"}
	case "admin\x00/admin/directory/v1/customer/{customer}/resources/calendars":
		return []string{"items"}
	case "admin\x00/admin/directory/v1/customer/{customer}/resources/features":
		return []string{"features"}
	case "admin\x00/admin/directory/v1/customer/{customer}/roleassignments":
		return []string{"items"}
	case "admin\x00/admin/directory/v1/customer/{customer}/roles/{roleId}":
		return []string{"rolePrivileges"}
	case "admin\x00/admin/directory/v1/customer/{customer}/roles":
		return []string{"rolePrivileges"}
	case "admin\x00/admin/directory/v1/customer/{customerId}/schemas/{schemaKey}":
		return []string{"fields"}
	case "admin\x00/admin/directory/v1/customer/{customerId}/schemas":
		return []string{"fields"}
	case "admin\x00/admin/directory/v1/users/{userKey}/tokens/{clientId}":
		return []string{"scopes"}
	case "admin\x00/admin/directory/v1/users/{userKey}/tokens":
		return []string{"items"}
	case "admin\x00/admin/directory/v1/users/{userKey}/aliases":
		return []string{"aliases"}
	case "admin\x00/admin/directory/v1/users":
		return []string{"users"}
	case "admin\x00/admin/directory/v1/users/{userKey}/verificationCodes":
		return []string{"items"}
	case "admin-sdk-admin\x00/admin/reports/v1/activity/users/{userKey}/applications/{applicationName}":
		return []string{"items"}
	case "alerts\x00/v1beta1/alerts":
		return []string{"alerts"}
	case "alerts-batch-delete\x00/v1beta1/alerts:batchDelete":
		return []string{"successAlertIds"}
	case "alerts-batch-undelete\x00/v1beta1/alerts:batchUndelete":
		return []string{"successAlertIds"}
	case "changes\x00/changes":
		return []string{"changes"}
	case "drafts\x00/gmail/v1/users/{userId}/drafts":
		return []string{"drafts"}
	case "drafts\x00/gmail/v1/users/{userId}/drafts/send":
		return []string{"labelIds"}
	case "drives\x00/drives":
		return []string{"drives"}
	case "files\x00/files/generateIds":
		return []string{"ids"}
	case "files\x00/files":
		return []string{"files"}
	case "gmail-settings\x00/gmail/v1/users/{userId}/settings/delegates":
		return []string{"delegates"}
	case "gmail-settings\x00/gmail/v1/users/{userId}/settings/filters":
		return []string{"filter"}
	case "gmail-settings\x00/gmail/v1/users/{userId}/settings/forwardingAddresses":
		return []string{"forwardingAddresses"}
	case "gmail-settings\x00/gmail/v1/users/{userId}/settings/sendAs":
		return []string{"sendAs"}
	case "gmail-settings\x00/gmail/v1/users/{userId}/settings/cse/identities":
		return []string{"cseIdentities"}
	case "gmail-settings\x00/gmail/v1/users/{userId}/settings/cse/keypairs":
		return []string{"cseKeyPairs"}
	case "gmail-settings\x00/gmail/v1/users/{userId}/settings/sendAs/{sendAsEmail}/smimeInfo":
		return []string{"smimeInfo"}
	case "history\x00/gmail/v1/users/{userId}/history":
		return []string{"history"}
	case "labels\x00/gmail/v1/users/{userId}/labels":
		return []string{"labels"}
	case "messages\x00/gmail/v1/users/{userId}/messages/{id}":
		return []string{"labelIds"}
	case "messages\x00/gmail/v1/users/{userId}/messages/import":
		return []string{"labelIds"}
	case "messages\x00/gmail/v1/users/{userId}/messages":
		return []string{"labelIds"}
	case "messages\x00/gmail/v1/users/{userId}/messages/send":
		return []string{"labelIds"}
	case "settings\x00/v1beta1/settings":
		return []string{"notifications"}
	case "teamdrives\x00/teamdrives":
		return []string{"teamDrives"}
	case "threads\x00/gmail/v1/users/{userId}/threads/{id}":
		return []string{"messages"}
	case "threads\x00/gmail/v1/users/{userId}/threads":
