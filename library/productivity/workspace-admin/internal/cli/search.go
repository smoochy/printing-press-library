...
Run sync first to populate the local search index.`,
  ...
  workspace-admin-pp-cli search "status"

  # Force local search explicitly
  workspace-admin-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  workspace-admin-pp-cli search "status" --type admin --data-source local
  # JSON output for piping
  workspace-admin-pp-cli search "status" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'workspace-admin-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
