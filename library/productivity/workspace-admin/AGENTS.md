# Google Workspace Admin Printed CLI Agent Guide

This directory is a generated `workspace-admin-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
workspace-admin-pp-cli doctor --json
workspace-admin-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
workspace-admin-pp-cli which "<capability>" --json
workspace-admin-pp-cli <command> --help
```
...
```bash
workspace-admin-pp-cli <command> --agent
```
...
```bash
workspace-admin-pp-cli <command> --help
workspace-admin-pp-cli <command> --dry-run --agent
```
