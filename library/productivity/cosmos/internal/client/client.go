...
// Bound provider responses before decoding or MCP result shaping. Normal Cosmos
// GraphQL pages are far smaller; callers should paginate instead of buffering an
// unexpectedly large document in memory.
...
func isCredentialHeaderName(name string) bool {
	...
	for _, marker := range []string{"authorization", "api-key", "apikey", "token", "secret", "cookie"} {
...
func (c *Client) doInternal(ctx context.Context, method, path string, params map[string]string, body any, headerOverrides map[string]string, readOnlyIntent bool) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if req.Header.Get("User-Agent") == "" {
			req.Header.Set("User-Agent", "cosmos-pp-cli/0.1.0")
		}
		...
		if len(respBody) > maxResponseBodyBytes {
			return nil, 0, fmt.Errorf("response exceeds %d-byte safety limit; request a smaller page", maxResponseBodyBytes)
		}
...
func authPlaceholderCredentialError(cfg *config.Config) error {
	return authPlaceholderCredentialErrorWithSetup(cfg, "export COSMOS_TOKEN=<your-token> or pipe the token to cosmos-pp-cli auth set-token --stdin")
}
...
func (c *Client) maskCredentialText(text string, extraCredentials ...string) string {
	...
	if c != nil && c.Config != nil {
		...
		addCredential(c.Config.CosmosToken)
	}
