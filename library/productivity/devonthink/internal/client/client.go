...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "devonthink-pp-cli", "http")
	...
	httpClient.CheckRedirect = func(req *http.Request, via []*http.Request) error {
		...
		if req.URL.Host == via[0].URL.Host {
			if h, err := c.authHeader(req.Context()); err == nil && h != "" {
				req.Header.Set("Authorization", h) // #nosec G119 -- authorization is re-added only for same-host redirects.
			}
		} else {
...
func (c *Client) readCache(path string, params map[string]string) (json.RawMessage, bool) {
	...
	data, err := os.ReadFile(cacheFile) // #nosec G304 -- cacheFile is derived from the CLI-owned cache dir and a sha256 key.
	if err != nil {
		return nil, false
	}
...
func (c *Client) doInternal(ctx context.Context, method, path string, params map[string]string, body any, headerOverrides map[string]string, readOnlyIntent bool) (json.RawMessage, int, error) {
	...
	if c.isLocalDEVONthink() {
		return c.doLocalDEVONthink(ctx, method, path, params, body, headerOverrides, readOnlyIntent)
	}
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if req.Header.Get("User-Agent") == "" {
			req.Header.Set("User-Agent", "devonthink-pp-cli/0.1.0")
		}
		...
		if closeErr != nil {
			return nil, 0, fmt.Errorf("closing response body: %w", closeErr)
		}
