...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/productivity/productive/internal/config"
	"io"
	"math"
	"net/http"
	"net/url"
...
func (c *Client) doInternal(ctx context.Context, method, path string, params map[string]string, body any, headerOverrides map[string]string, readOnlyIntent bool) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if authHeader != "" {
			req.Header.Set("X-Auth-Token", authHeader)
		}
		if c.Config != nil {
		...
		if req.Header.Get("User-Agent") == "" {
			req.Header.Set("User-Agent", "productive-pp-cli/0.1.0")
		}
...
func (c *Client) dryRun(method, targetURL, path string, params map[string]string, body []byte, headerOverrides map[string]string, authHeader string) (json.RawMessage, int, error) {
	...
	if authHeader != "" {
		fmt.Fprintf(os.Stderr, "  %s: %s\n", "X-Auth-Token", maskToken(authHeader))
	}
	// Static per-request headers (e.g. Productive's required X-Organization-Id)
	// are applied on the live path from Config.Headers; mirror them here so the
	// dry-run preview reflects what actually gets sent.
	if c.Config != nil {
		for k, v := range c.Config.Headers {
			fmt.Fprintf(os.Stderr, "  %s: %s\n", k, v)
		}
...
func authPlaceholderCredentialError(cfg *config.Config) error {
	return authPlaceholderCredentialErrorWithSetup(cfg, "export PRODUCTIVE_API_TOKEN=<your-token> or productive-pp-cli auth set-token <token>")
}
...
func (c *Client) maskCredentialText(text string, extraCredentials ...string) string {
	...
	if c != nil && c.Config != nil {
		...
		addCredential(c.Config.ProductiveApiToken)
	}
