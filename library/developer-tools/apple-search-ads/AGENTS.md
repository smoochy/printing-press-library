# Apple Search Ads Printed CLI Agent Guide

This directory is a generated `apple-search-ads-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
apple-search-ads-pp-cli doctor --json
apple-search-ads-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
apple-search-ads-pp-cli which "<capability>" --json
apple-search-ads-pp-cli <command> --help
```
...
```bash
apple-search-ads-pp-cli <command> --agent
```
...
```bash
apple-search-ads-pp-cli <command> --help
apple-search-ads-pp-cli <command> --dry-run --agent
```
