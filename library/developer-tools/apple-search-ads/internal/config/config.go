...
type Config struct {
	...
	Path                string            `toml:"-"`
	AppleSearchAdsToken string            `toml:"search_ads_token"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("APPLE_SEARCH_ADS_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "apple-search-ads-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("ASA_CLIENT_ID"); v != "" {
		cfg.ClientID = v
		cfg.AuthSource = "env:ASA_CLIENT_ID"
	}
	if v := os.Getenv("APPLE_SEARCH_ADS_TOKEN"); v != "" {
		cfg.AppleSearchAdsToken = v
		cfg.AuthSource = "env:APPLE_SEARCH_ADS_TOKEN"
	}
	...
	if cfg.AuthSource == "" && cfg.AppleSearchAdsToken != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("APPLE_SEARCH_ADS_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.ClientID != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:ASA_CLIENT_ID"
		}
		return applyAuthFormat("Bearer {token}", map[string]string{
			"client_id":     c.ClientID,
			"ASA_CLIENT_ID": c.ClientID,
			"token":         c.ClientID,
		})
	}
	if c.AppleSearchAdsToken != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:APPLE_SEARCH_ADS_TOKEN"
		}
		return applyAuthFormat("Bearer {token}", map[string]string{
			"search_ads_token":       c.AppleSearchAdsToken,
			"APPLE_SEARCH_ADS_TOKEN": c.AppleSearchAdsToken,
			"token":                  c.AppleSearchAdsToken,
		})
	}
	if c.AccessToken != "" {
