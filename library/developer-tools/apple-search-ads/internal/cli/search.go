...
In local mode: searches locally synced data only.`,
  ...
  apple-search-ads-pp-cli search "error timeout"

  # Force local search only
  apple-search-ads-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  apple-search-ads-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  apple-search-ads-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: POST /search/geo
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, _, getErr := c.Post(cmd.Context(), "/search/geo", map[string]any{
					"q": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'apple-search-ads-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
