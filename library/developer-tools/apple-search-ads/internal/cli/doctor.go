...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  apple-search-ads-pp-cli doctor
  apple-search-ads-pp-cli doctor --json
  apple-search-ads-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export ASA_CLIENT_ID=<your-key>"
				} else {
					authConfigured = true
			...
			if strings.Contains("", " OR ") {
				authEnvOptionalNames = append(authEnvOptionalNames, "ASA_CLIENT_ID")
				if os.Getenv("ASA_CLIENT_ID") != "" {
					authEnvSet = append(authEnvSet, "ASA_CLIENT_ID")
					authEnvOptionalSatisfied = true
				}
			} else if os.Getenv("ASA_CLIENT_ID") != "" {
				authEnvSet = append(authEnvSet, "ASA_CLIENT_ID")
			} else {
				authEnvInfo = append(authEnvInfo, "ASA_CLIENT_ID optional")
			}
			if strings.Contains("", " OR ") {
				authEnvOptionalNames = append(authEnvOptionalNames, "APPLE_SEARCH_ADS_TOKEN")
				if os.Getenv("APPLE_SEARCH_ADS_TOKEN") != "" {
					authEnvSet = append(authEnvSet, "APPLE_SEARCH_ADS_TOKEN")
					authEnvOptionalSatisfied = true
				}
			} else if os.Getenv("APPLE_SEARCH_ADS_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "APPLE_SEARCH_ADS_TOKEN")
			} else {
				authEnvInfo = append(authEnvInfo, "APPLE_SEARCH_ADS_TOKEN optional")
			}
			switch {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	report := map[string]any{}
	dbPath := defaultDBPath("apple-search-ads-pp-cli")
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'apple-search-ads-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'apple-search-ads-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'apple-search-ads-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'apple-search-ads-pp-cli sync' to refresh."
	}
	return report
}
