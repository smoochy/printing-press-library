...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  apple-search-ads-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  apple-search-ads-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"attribution", "attribution-device-send-attribute", "attribution-device-tracking-consent", "budgetorders", "campaigns", "countries-or-regions", "creativeappmappings", "creatives", "custom-reports", "deferred-deep-link", "deferred-deep-link-resolve", "me", "search_resource", "search_resource-apps"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  apple-search-ads-pp-cli workflow status

  # Show status as JSON
  apple-search-ads-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("apple-search-ads-pp-cli")
			}
