...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "apple-search-ads-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("APPLE_SEARCH_ADS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("APPLE_SEARCH_ADS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "apple-search-ads-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/apple-search-ads-pp-cli/feedback.jsonl.
When ` + "`APPLE_SEARCH_ADS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`APPLE_SEARCH_ADS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  apple-search-ads-pp-cli feedback list
  apple-search-ads-pp-cli feedback list --limit 5
  apple-search-ads-pp-cli feedback list --json`,
