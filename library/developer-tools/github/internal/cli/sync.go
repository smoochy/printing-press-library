...
Resource scoping:
  ...
  github-pp-cli sync --repo cli/cli --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(resources) == 0 {
				if humanFriendly {
					fmt.Fprintln(os.Stderr, "github-pp-cli sync: no default sync resources in spec; use --resources for explicit sync targets or populate the store via single-fetch commands.")
				} else {
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "protection":
		return db.UpsertProtection(data)
	case "rename":
		return db.UpsertRename(data)
	case "permission":
		return db.UpsertPermission(data)
	case "comments_reactions":
		return db.UpsertCommentsReactions(data)
	case "branches_where_head":
		return db.UpsertBranchesWhereHead(data)
	case "check_runs":
		return db.UpsertCheckRuns(data)
	case "check_suites":
		return db.UpsertCheckSuites(data)
	case "commits_comments":
		return db.UpsertCommitsComments(data)
	case "commits_pulls":
		return db.UpsertCommitsPulls(data)
	case "status":
		return db.UpsertStatus(data)
	case "statuses":
		return db.UpsertStatuses(data)
	case "github-search":
		return db.UpsertGithubSearch(data)
	case "issues_assignees":
		return db.UpsertIssuesAssignees(data)
	...
	case "issue_field_values":
		return db.UpsertIssueFieldValues(data)
	case "issues_labels":
		return db.UpsertIssuesLabels(data)
	case "lock":
		return db.UpsertLock(data)
	case "parent":
		return db.UpsertParent(data)
	case "issues_reactions":
		return db.UpsertIssuesReactions(data)
	case "sub_issue":
		return db.UpsertSubIssue(data)
	case "sub_issues":
		return db.UpsertSubIssues(data)
	case "timeline":
		return db.UpsertTimeline(data)
	case "milestones_labels":
		return db.UpsertMilestonesLabels(data)
	case "codespaces":
		return db.UpsertCodespaces(data)
	case "pulls_comments":
		return db.UpsertPullsComments(data)
	case "pulls_commits":
		return db.UpsertPullsCommits(data)
	case "files":
		return db.UpsertFiles(data)
	case "merge":
		return db.UpsertMerge(data)
	case "requested_reviewers":
		return db.UpsertRequestedReviewers(data)
	case "reviews":
		return db.UpsertReviews(data)
	case "update_branch":
		return db.UpsertUpdateBranch(data)
	case "assets":
		return db.UpsertAssets(data)
	case "releases_reactions":
		return db.UpsertReleasesReactions(data)
	default:
