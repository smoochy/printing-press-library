...
In local mode: searches locally synced data only.`,
  ...
  github-pp-cli search "error timeout"

  # Force local search only
  github-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  github-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  github-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /search/code
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get(cmd.Context(), "/search/code", map[string]string{
					"q": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'github-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
