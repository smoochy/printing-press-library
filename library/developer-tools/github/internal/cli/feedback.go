...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "github-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("GITHUB_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("GITHUB_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "github-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/github-pp-cli/feedback.jsonl.
When ` + "`GITHUB_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`GITHUB_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  github-pp-cli feedback list
  github-pp-cli feedback list --limit 5
  github-pp-cli feedback list --json`,
