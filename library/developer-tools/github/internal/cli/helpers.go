...
func classifyAPIError(err error, flags *rootFlags) error {
	...
	case strings.Contains(msg, "HTTP 400") && cliutil.LooksLikeAuthError(msg):
		return authErr(fmt.Errorf("%w\nhint: the API rejected the request — this usually means auth is missing or invalid."+
			...
			"\n      Run 'github-pp-cli doctor' to check auth status."+
			"\n      Response: "+cliutil.SanitizeErrorBody(msg), err))
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: check your token. Set it with: github-pp-cli auth set-token <token>"+
			"\n      or: export GITHUB_TOKEN=<your-token>"+
			"\n      Get a key at: https://github.com/settings/tokens"+
			"\n      Generate a fine-grained personal access token (or classic) with the scopes your workflow needs."+
			"\n      Run 'github-pp-cli doctor' to check auth status.", err))
