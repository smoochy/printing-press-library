...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("GITHUB_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "github-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	if cfg.AuthSource == "" && cfg.GithubToken != "" {
		cfg.AuthSource = "config"
	}
	if cfg.AuthSource == "" && cfg.GhToken != "" {
		cfg.AuthSource = "config"
	}
...
func (c *Config) AuthHeader() string {
	...
	if c.GithubToken != "" {
		...
		return "Bearer " + c.GithubToken
	}
	if c.GhToken != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:GH_TOKEN"
		}
		return "Bearer " + c.GhToken
	}
	if c.AccessToken != "" {
