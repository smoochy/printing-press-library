...
func resetCredentialEnv(t *testing.T) (home, configPath string) {
	...
	for _, name := range []string{
		...
		"XDG_STATE_HOME",
		"XDG_CACHE_HOME",
		"APP_STORE_CONNECT_ITC_BEARER_TOKEN",
	} {
		t.Setenv(name, "")
	}
	...
	return home, filepath.Join(home, ".config", "asc-pp-cli", "config.toml")
}
...
func TestCorruptCredentialsFallsBackToLegacyConfig(t *testing.T) {
	...
	credentialsPath := filepath.Join(home, ".local", "share", "asc-pp-cli", "credentials.toml")
...
func TestCorruptCredentialsFallsBackToEnvCredential(t *testing.T) {
	...
	t.Setenv("APP_STORE_CONNECT_ITC_BEARER_TOKEN", "env-secret")
	credentialsPath := filepath.Join(home, ".local", "share", "asc-pp-cli", "credentials.toml")
	if err := os.MkdirAll(filepath.Dir(credentialsPath), 0o700); err != nil {
		t.Fatalf("mkdir credentials dir: %v", err)
	}
...
func TestAuthWriteScrubsLegacyConfigWhenRelocated(t *testing.T) {
	...
	activeConfigPath := filepath.Join(newConfigDir, "asc-pp-cli", "config.toml")
	...
	credsPath := filepath.Join(newDataDir, "asc-pp-cli", "credentials.toml")
