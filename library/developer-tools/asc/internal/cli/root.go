...
type rootFlags struct {
	...
	// invocation. Mirrors the ASC_NO_LEARN env var.
	noLearn       bool
	selectFields  string
	configPath    string
	homePath      string
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "asc-pp-cli",
		Short: `Asc CLI — A read-only App Store Connect cockpit that shows every app you ship at once — review state, builds, downloads, and…`,
		Long: `Asc CLI — A read-only App Store Connect cockpit that shows every app you ship at once — review state, builds, downloads, and ratings in one glance.
...
Health check: run 'asc-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("asc-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'asc-pp-cli profile list')")
	...
	rootCmd.AddCommand(newBetaGroupsCmd(flags))
	rootCmd.AddCommand(newBetaTestersCmd(flags))
	...
	rootCmd.AddCommand(newNovelBlockersCmd(flags))
	rootCmd.AddCommand(newNovelCockpitCmd(flags))
	rootCmd.AddCommand(newNovelPipelineCmd(flags))
	rootCmd.AddCommand(newNovelReviewsCmd(flags))
	rootCmd.AddCommand(newNovelTractionCmd(flags))
	rootCmd.AddCommand(newAPICmd(flags))
	rootCmd.AddCommand(newAppStoreReviewDetailsPromotedCmd(flags))
	rootCmd.AddCommand(newAppStoreVersionsPromotedCmd(flags))
	rootCmd.AddCommand(newCustomerReviewsPromotedCmd(flags))
	rootCmd.AddCommand(newSalesReportsPromotedCmd(flags))
