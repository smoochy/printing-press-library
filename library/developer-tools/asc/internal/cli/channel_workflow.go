...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  asc-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  asc-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"apps", "beta-groups", "beta-testers", "builds"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  asc-pp-cli workflow status

  # Show status as JSON
  asc-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("asc-pp-cli")
			}
