...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("ASC_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("ASC_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "asc-pp-cli/feedback")
...
When ` + "`ASC_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`ASC_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  asc-pp-cli feedback list
  asc-pp-cli feedback list --limit 5
  asc-pp-cli feedback list --json`,
