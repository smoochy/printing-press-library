...
Run sync first to populate the local search index.`,
  ...
  asc-pp-cli search "status"

  # Force local search explicitly
  asc-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  asc-pp-cli search "status" --type app-store-review-details --data-source local
  # JSON output for piping
  asc-pp-cli search "status" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'asc-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
