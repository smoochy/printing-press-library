...
type Config struct {
	...
	AppStoreConnectItcBearerToken string          `toml:"store_connect_itc_bearer_token"`

	// App Store Connect API key (ES256 JWT signing). Env-only, never persisted.
	ASCKeyID          string    `toml:"-"`
	ASCIssuerID       string    `toml:"-"`
	ASCPrivateKeyPath string    `toml:"-"`
	ASCPrivateKey     string    `toml:"-"`
	ASCVendorNumber   string    `toml:"-"`
	ascCachedToken    string    `toml:"-"`
	ascTokenExp       time.Time `toml:"-"`
}
...
func Load(configPath string) (*Config, error) {
	...
	// Env var overrides
	if v := os.Getenv("APP_STORE_CONNECT_ITC_BEARER_TOKEN"); v != "" {
		cfg.AppStoreConnectItcBearerToken = v
		cfg.markEnvOverride("AppStoreConnectItcBearerToken")
		cfg.AuthSource = "env:APP_STORE_CONNECT_ITC_BEARER_TOKEN"
		cfg.CredentialSource = "env:APP_STORE_CONNECT_ITC_BEARER_TOKEN"
	}
	// App Store Connect API key (ES256 JWT signing) — the primary, correct auth
	// path. Takes precedence over a pasted static bearer token when present.
	cfg.ASCKeyID = os.Getenv("ASC_KEY_ID")
	cfg.ASCIssuerID = os.Getenv("ASC_ISSUER_ID")
	cfg.ASCPrivateKeyPath = os.Getenv("ASC_PRIVATE_KEY_PATH")
	cfg.ASCPrivateKey = os.Getenv("ASC_PRIVATE_KEY")
	cfg.ASCVendorNumber = os.Getenv("ASC_VENDOR_NUMBER")
	if cfg.ascSigningConfigured() {
		cfg.AuthSource = "env:ASC_PRIVATE_KEY_PATH (JWT ES256)"
		cfg.CredentialSource = "env:ASC API key"
	}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("ASC_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "asc-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	// App Store Connect API key: mint a short-lived ES256 JWT. This is the
	// canonical ASC auth and must win over a saved static header (env > config),
	// consistent with what Load() advertises via AuthSource and what doctor
	// reports. On a signing error we fall through; ASCPreflight() surfaces why.
	if c.ascSigningConfigured() {
		if h, err := c.ascBearer(); err == nil {
			return h
		}
	}
	if c.AuthHeaderVal != "" {
	...
	if c.AppStoreConnectItcBearerToken != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:APP_STORE_CONNECT_ITC_BEARER_TOKEN"
		}
		return "Bearer " + c.AppStoreConnectItcBearerToken
	}
	if c.AccessToken != "" {
...
// ASCPreflight validates the App Store Connect API key config and proves it can
// mint a token, returning an actionable error when it can't. Novel fleet
// commands call this before fanning out so a bad key surfaces cleanly instead
// of as an opaque 401.
func (c *Config) ASCPreflight() error {
	if !c.ascSigningConfigured() {
		return fmt.Errorf("App Store Connect API key not configured — set ASC_KEY_ID, ASC_ISSUER_ID, and ASC_PRIVATE_KEY_PATH (download a .p8 key under Users and Access > Integrations)")
	}
	if _, err := c.ascBearer(); err != nil {
		return err
	}
	return nil
}
...
func (c *Config) clearCredentialFields() {
	...
	c.ClientID = ""
	c.ClientSecret = ""
	c.AppStoreConnectItcBearerToken = ""
}
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		AppStoreConnectItcBearerToken: c.AppStoreConnectItcBearerToken,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	c.AppStoreConnectItcBearerToken = creds.AppStoreConnectItcBearerToken
}
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.AppStoreConnectItcBearerToken = ""
	delete(c.envOverrides, "AppStoreConnectItcBearerToken")
	c.updateFileConfigField("AppStoreConnectItcBearerToken")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["AppStoreConnectItcBearerToken"] {
			out.AppStoreConnectItcBearerToken = c.fileConfig.AppStoreConnectItcBearerToken
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "AppStoreConnectItcBearerToken":
		c.fileConfig.AppStoreConnectItcBearerToken = c.AppStoreConnectItcBearerToken
	}
}
