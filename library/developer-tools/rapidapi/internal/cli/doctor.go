...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  rapidapi-pp-cli doctor
  rapidapi-pp-cli doctor --json
  rapidapi-pp-cli doctor --fail-on warn
  rapidapi-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Cache freshness surface (agent-consumable).
			cacheReport := collectCacheReport(flags)
			...
			if credentialRefused {
				...
				if cfg != nil {
					...
					if !configured {
						...
						report["auth_hint"] = "Set credentials with: export RAPIDAPI_CSRF_TOKEN=\"your-token-here\" RAPIDAPI_COOKIE=\"your-token-here\" RAPIDAPI_CLEARANCE=\"your-token-here\""
						report["auth_docs_url"] = "https://rapidapi.com"
					} else {
						authConfigured = true
			...
			if os.Getenv("RAPIDAPI_CSRF_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "RAPIDAPI_CSRF_TOKEN")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "RAPIDAPI_CSRF_TOKEN")
			}
			if os.Getenv("RAPIDAPI_COOKIE") != "" {
				authEnvSet = append(authEnvSet, "RAPIDAPI_COOKIE")
			} else if authConfigured {
				authSource, _ := report["auth_source"].(string)
				if authSource == "" {
					authSource = "config"
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "RAPIDAPI_COOKIE")
			}
			if os.Getenv("RAPIDAPI_CLEARANCE") != "" {
				authEnvSet = append(authEnvSet, "RAPIDAPI_CLEARANCE")
			} else if authConfigured {
				authSource, _ := report["auth_source"].(string)
				if authSource == "" {
					authSource = "config"
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "RAPIDAPI_CLEARANCE")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "rapidapi-pp-cli", suggestion)
						} else {
			...
			w := cmd.OutOrStdout()
			renderCacheReport(w, report["cache"].(map[string]any))
...
// collectCacheReport emits the agent-consumable cache freshness surface:
// store path, marker age, and whether the cache is stale. Wired into doctor
// so `rapidapi-pp-cli doctor` reports cache freshness alongside connectivity.
func collectCacheReport(flags *rootFlags) map[string]any {
	report := map[string]any{}
	storePath := learnDBPath("")
	if storePath == "" {
		storePath = "unavailable"
	}
	report["store"] = storePath
	fresh, err := cliutil.EnsureFresh(context.Background(), storePath)
	if err != nil {
		report["status"] = "error"
		report["detail"] = err.Error()
		return report
	...
	if fresh {
		...
		report["status"] = "stale"
		report["hint"] = "run any read command to auto-refresh, or set --no-cache to bypass"
	}
	return report
}

// renderCacheReport writes the cache section to the doctor output writer.
func renderCacheReport(w io.Writer, rep map[string]any) {
	status, _ := rep["status"].(string)
	switch status {
	case "fresh":
		fmt.Fprintf(w, "  OK Cache: fresh (store: %v)\n", rep["store"])
	case "stale":
		fmt.Fprintf(w, "  WARN Cache: stale (store: %v) — %v\n", rep["store"], rep["hint"])
	default:
		fmt.Fprintf(w, "  FAIL Cache: %v\n", rep["detail"])
	}
}
