...
// loadConfigForFlags loads the CLI config from the flags' config path.
func loadConfigForFlags(flags *rootFlags) (*config.Config, error) {
...
func classifyAPIErrorOnly(err error) error {
	...
	case strings.Contains(msg, "HTTP 400") && cliutil.LooksLikeAuthError(msg):
		return authErr(fmt.Errorf("%w\nhint: the API rejected the request — this usually means auth is missing or invalid."+
			...
			"\n      See API docs: https://rapidapi.com"+
			"\n      Run 'rapidapi-pp-cli doctor' to check auth status."+
			"\n      Response: "+cliutil.SanitizeErrorBody(msg), err))
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: check your API key."+
			" Set credentials with: export RAPIDAPI_CSRF_TOKEN=\"your-token-here\" RAPIDAPI_COOKIE=\"your-token-here\" RAPIDAPI_CLEARANCE=\"your-token-here\""+
			"\n      See API docs: https://rapidapi.com"+
			"\n      Run 'rapidapi-pp-cli doctor' to check auth status.", err))
