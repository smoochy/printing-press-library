...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("RAPIDAPI_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("RAPIDAPI_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "rapidapi-pp-cli/feedback")
...
When ` + "`RAPIDAPI_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`RAPIDAPI_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  rapidapi-pp-cli feedback list
  rapidapi-pp-cli feedback list --limit 5
  rapidapi-pp-cli feedback list --json`,
