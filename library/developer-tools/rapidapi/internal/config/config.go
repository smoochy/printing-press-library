...
type Config struct {
	...
	RapidapiCsrfToken string          `toml:"csrf_token"`
	RapidapiCookie    string          `toml:"cookie"`
	RapidapiClearance string          `toml:"clearance"`
}
...
func Load(configPath string) (*Config, error) {
	...
	// Env var overrides
	if v := os.Getenv("RAPIDAPI_CSRF_TOKEN"); v != "" {
		cfg.RapidapiCsrfToken = v
		cfg.markEnvOverride("RapidapiCsrfToken")
		cfg.AuthSource = "env:RAPIDAPI_CSRF_TOKEN"
		cfg.CredentialSource = "env:RAPIDAPI_CSRF_TOKEN"
	}
	if v := os.Getenv("RAPIDAPI_COOKIE"); v != "" {
		cfg.RapidapiCookie = v
		cfg.markEnvOverride("RapidapiCookie")
		cfg.AuthSource = "env:RAPIDAPI_COOKIE"
		cfg.CredentialSource = "env:RAPIDAPI_COOKIE"
	}
	if v := os.Getenv("RAPIDAPI_CLEARANCE"); v != "" {
		cfg.RapidapiClearance = v
		cfg.markEnvOverride("RapidapiClearance")
		cfg.AuthSource = "env:RAPIDAPI_CLEARANCE"
		cfg.CredentialSource = "env:RAPIDAPI_CLEARANCE"
	}
	...
	if v := os.Getenv("RAPIDAPI_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("RAPIDAPI_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "rapidapi-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.RapidapiCsrfToken != "" {
		return applyAuthFormat("{token}", map[string]string{
			"csrf_token":          c.RapidapiCsrfToken,
			"RAPIDAPI_CSRF_TOKEN": c.RapidapiCsrfToken,
			"token":               c.RapidapiCsrfToken,
		})
	}
	if c.RapidapiCookie != "" {
		return applyAuthFormat("{token}", map[string]string{
			"cookie":          c.RapidapiCookie,
			"RAPIDAPI_COOKIE": c.RapidapiCookie,
			"token":           c.RapidapiCookie,
		})
	}
	if c.RapidapiClearance != "" {
		return applyAuthFormat("{token}", map[string]string{
			"clearance":          c.RapidapiClearance,
			"RAPIDAPI_CLEARANCE": c.RapidapiClearance,
			"token":              c.RapidapiClearance,
		})
	}
	return ""
}
...
func (c *Config) StoreScopeCredential() string {
	...
	if c.RapidapiCsrfToken != "" {
		parts = append(parts, "csrf_token="+c.RapidapiCsrfToken)
	}
	if c.RapidapiCookie != "" {
		parts = append(parts, "cookie="+c.RapidapiCookie)
	}
	if c.RapidapiClearance != "" {
		parts = append(parts, "clearance="+c.RapidapiClearance)
	}
	if len(parts) == 0 {
		return ""
	}
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		RapidapiCsrfToken: c.RapidapiCsrfToken,
		RapidapiCookie:    c.RapidapiCookie,
		RapidapiClearance: c.RapidapiClearance,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	if c.RapidapiCsrfToken == "" {
		c.RapidapiCsrfToken = creds.RapidapiCsrfToken
	}
	if c.RapidapiCookie == "" {
		c.RapidapiCookie = creds.RapidapiCookie
	}
	if c.RapidapiClearance == "" {
		c.RapidapiClearance = creds.RapidapiClearance
	}
}
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.RapidapiCsrfToken = token
	delete(c.envOverrides, "RapidapiCsrfToken")
	c.updateFileConfigField("RapidapiCsrfToken")
	if err := c.saveCredentialsFirst(); err != nil {
		return err
	}
	return c.save()
...
// SaveCookie persists a RapidAPI session cookie to the config file (0600).
// The CSRF token is auto-fetched at request time, so it is cleared here to
// force the bootstrap path.
func (c *Config) SaveCookie(cookie string) error {
	c.RapidapiCookie = cookie
	c.RapidapiCsrfToken = ""
	delete(c.envOverrides, "RapidapiCookie")
	delete(c.envOverrides, "RapidapiCsrfToken")
	c.updateFileConfigField("RapidapiCookie")
	c.updateFileConfigField("RapidapiCsrfToken")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
		return c.save()
...
// SaveClearance persists a Cloudflare cf_clearance cookie (for gated networks).
func (c *Config) SaveClearance(clearance string) error {
	c.RapidapiClearance = clearance
	delete(c.envOverrides, "RapidapiClearance")
	c.updateFileConfigField("RapidapiClearance")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
		return c.save()
	}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["RapidapiCsrfToken"] {
			out.RapidapiCsrfToken = c.fileConfig.RapidapiCsrfToken
		}
		if c.envOverrides["RapidapiCookie"] {
			out.RapidapiCookie = c.fileConfig.RapidapiCookie
		}
		if c.envOverrides["RapidapiClearance"] {
			out.RapidapiClearance = c.fileConfig.RapidapiClearance
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "RapidapiCsrfToken":
		c.fileConfig.RapidapiCsrfToken = c.RapidapiCsrfToken
	case "RapidapiCookie":
		c.fileConfig.RapidapiCookie = c.RapidapiCookie
	case "RapidapiClearance":
		c.fileConfig.RapidapiClearance = c.RapidapiClearance
	}
}
