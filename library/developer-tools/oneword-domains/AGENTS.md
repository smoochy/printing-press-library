# One Word Domains Printed CLI Agent Guide

This directory is a generated `oneword-domains-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
oneword-domains-pp-cli doctor --json
oneword-domains-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
oneword-domains-pp-cli which "<capability>" --json
oneword-domains-pp-cli <command> --help
```

Add `--agent` to command invocations for JSON, compact output, non-interactive defaults, and no color:

```bash
oneword-domains-pp-cli <command> --agent
```
...
```bash
oneword-domains-pp-cli <command> --help
oneword-domains-pp-cli <command> --dry-run --agent
```
...
1. On a new user question, call `oneword-domains-pp-cli recall` FIRST, passing the question as an argv or MCP argument - never by interpolating it into a shell command, quoted string, or heredoc (apostrophes break quotes; a body line equal to the delimiter breaks a heredoc). If a shell is required, write the question with a non-shell file tool, then `QUERY=$(cat /path/to/question.txt)` and `oneword-domains-pp-cli recall "$QUERY" --agent`. If `found=true` and the top result has `entity_match == "exact"` and `confidence >= 2`, skip discovery and go straight to the live fetch for the returned resource IDs. If the store is cold (recall finds nothing and `learnings list` and `learnings candidates` are both empty), skip recall for the rest of the session.
...
3. After answering, always fire `oneword-domains-pp-cli teach` in the background with the same argv/MCP or file-then-`$QUERY` passing rule as recall (`QUERY=$(cat /path/to/question.txt)` then `oneword-domains-pp-cli teach --query "$QUERY" --resource <id> --resource-type <type> &`) - teaching is unconditional and is the anchor that triggers playbook synthesis. Teach the structural question with identifiers stripped (no names, emails, phone numbers, account ids); the CLI warns on obvious PII shapes but does not block.
...
Disable the loop with `--no-learn` per-invocation or `ONEWORD_DOMAINS_NO_LEARN=true` for the whole session - useful for deterministic agent flows that don't want a learning row to silently change subsequent query results.
