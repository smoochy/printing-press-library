...
func TestMCPPathResolutionMatchesCLIResolverWithHomeEnv(t *testing.T) {
	...
	t.Setenv("ONEWORD_DOMAINS_HOME", root)
...
func TestMCPPathResolutionMatchesCLIResolverWithPlatformDefaults(t *testing.T) {
	...
	if want := filepath.Join(home, ".config", "oneword-domains-pp-cli", "config.toml"); cfg.Path != want {
	...
	if want := filepath.Join(home, ".local", "share", "oneword-domains-pp-cli", "data.db"); gotDB != want {
...
func TestMCPRegisterToolsPreservesTypedSpecialTools(t *testing.T) {
	s := server.NewMCPServer("oneword-domains", "test")
	RegisterTools(s)
...
func TestMCPUnmigratedCheckpointsCannotProveCompletion(t *testing.T) {
	for _, cursor := range []string{"", "resume-here"} {
		...
		t.Run(name, func(t *testing.T) {
			...
			if err := db.SaveSyncStateAt("domains", cursor, 0, time.Now().UTC()); err != nil {
				t.Fatal(err)
			}
