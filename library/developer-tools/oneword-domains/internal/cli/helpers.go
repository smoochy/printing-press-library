...
func classifyHTMLTransportError(err error) error {
	...
	if htmlLooksLikeAuthFailure(evidence) {
		return authErr(fmt.Errorf("not authenticated or session expired; API returned HTML instead of JSON. " + "Run 'oneword-domains-pp-cli auth login --chrome' to refresh browser-session credentials."))
	}
...
func classifyAPIErrorOnly(err error) error {
	...
	case strings.Contains(msg, "HTTP 400") && cliutil.LooksLikeAuthError(msg):
		return authErr(fmt.Errorf("%w\nhint: the API rejected the request — this usually means auth is missing or invalid."+
			...
			"\n      See API docs: https://oneword.domains"+
			"\n      Run 'oneword-domains-pp-cli doctor' to check auth status."+
			"\n      Response: "+cliutil.SanitizeErrorBody(msg), err))
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: check your API credentials."+
			" Run 'oneword-domains-pp-cli auth login --chrome' to refresh browser-session credentials."+
			"\n      See API docs: https://oneword.domains"+
			"\n      Run 'oneword-domains-pp-cli doctor' to check auth status.", err))
