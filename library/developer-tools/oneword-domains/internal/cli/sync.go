...
Resource scoping:
  ...
  oneword-domains-pp-cli sync --resources domains,gpt

  # Full resync (ignore previous checkpoint)
  oneword-domains-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  oneword-domains-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  oneword-domains-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  oneword-domains-pp-cli sync --latest-only`,
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "gpt":
		return db.UpsertGpt(data)
	case "listings":
		return db.UpsertListings(data)
	case "tlds":
		return db.UpsertTlds(data)
	case "words":
		return db.UpsertWords(data)
	default:
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{ // #nosec G101 -- endpoint paths, not credentials.
		...
		"listings-filters": "/api/listings/filters",
		"tlds":             "/api/tlds",
		"words":            "/api/words",
	}
