...
In local mode: searches locally synced data only.`,
  ...
  oneword-domains-pp-cli search "error timeout"

  # Force local search only
  oneword-domains-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  oneword-domains-pp-cli search "status" --type domains --data-source local
  # JSON output for piping
  oneword-domains-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /api/domains
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get(cmd.Context(), "/api/domains", map[string]string{
					"search": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'oneword-domains-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
