...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  oneword-domains-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  oneword-domains-pp-cli workflow archive --full

  # Archive without a wall-clock timeout
  oneword-domains-pp-cli workflow archive --timeout 0`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"domains", "tlds", "words"}
			if cliutil.IsDogfoodEnv() {
				if len(resources) > 3 {
			...
			if flags.asJSON {
				...
				if dryRunOK(flags) {
					archived["dry_run"] = true
					archived["action"] = "workflow archive"
				}
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  oneword-domains-pp-cli workflow status

  # Show status as JSON
  oneword-domains-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("oneword-domains-pp-cli")
			}
