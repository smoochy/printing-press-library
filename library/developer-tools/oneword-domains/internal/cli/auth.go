...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:         "auth",
		Short:       "Manage authentication for One Word Domains",
...
func requiredAuthCookies() []string {
	raw := []string{"__Secure-next-auth.session-token"}
	out := raw[:0]
...
Use --chrome to read cookies from Chrome for oneword.domains.
...
profile by name when the installed backend supports it.`,
		Example: `  oneword-domains-pp-cli auth login --chrome
  oneword-domains-pp-cli auth login --browser
  oneword-domains-pp-cli auth login --cookies-file storage-state.json
  oneword-domains-pp-cli auth login --chrome --profile "Work"`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !browserFlag && cookiesFile == "" {
				...
				fmt.Fprintf(cmd.OutOrStdout(), "  oneword-domains-pp-cli auth login --chrome\n")
				fmt.Fprintf(cmd.OutOrStdout(), "  oneword-domains-pp-cli auth login --browser\n")
				fmt.Fprintf(cmd.OutOrStdout(), "  oneword-domains-pp-cli auth login --cookies-file storage-state.json\n")
				return nil
			}
			if browserFlag && cookiesFile != "" {
			...
			if !fromPressAuth && !fromCookiesFile {
				...
				if err != nil {
					...
					if runtime.GOOS == "windows" {
						...
						fmt.Fprintf(w, "\n  oneword-domains-pp-cli auth login --cookies-file <path>\n\n")
			...
			requiredCookies := []string{"__Secure-next-auth.session-token"}
			...
			cfg.CredentialDomain = "oneword.domains"
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  oneword-domains-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			if !credentialConfigured {
				...
				fmt.Fprintf(w, "  oneword-domains-pp-cli auth login --chrome\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}

			fmt.Fprintln(w, green("Authenticated"))
			fmt.Fprintf(w, "  Source: %s\n", cfg.AuthSource)
			fmt.Fprintf(w, "  Domain: oneword.domains\n")
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  echo \"$TOKEN\" | oneword-domains-pp-cli auth set-token\n  oneword-domains-pp-cli auth set-token < token-file",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  oneword-domains-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
