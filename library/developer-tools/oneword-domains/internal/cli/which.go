...
var whichIndex = []whichEntry{
	{Command: "brainstorm", Description: "Generate DomainsGPT names, keep only the available ones with price, cheapest registrar, and a dictionary flag, and save the batch locally; one call per TLD, quota 10 anonymous.", Group: "Naming pipeline", WhyItMatters: "Pick this when an agent needs priced, available, deduplicated name candidates in one call instead of generate-then-check loops."},
	{Command: "recheck", Description: "Re-check every word and TLD you looked up before and report what changed: taken, freed, premium, price, or popularity.", Group: "Naming pipeline", WhyItMatters: "Use it to refresh a shortlist before buying instead of re-running every lookup by hand."},
	{Command: "listings rank", Description: "Rank aftermarket auctions by how contested the word is across TLDs, price relative to fresh registration, and bid pace.", Group: "Aftermarket intelligence", WhyItMatters: "Reach for it when scanning auctions for underpriced words instead of reading through 455 listings by hand."},
	{Command: "domains intersect", Description: "Find dictionary words that are free on every TLD you list, or free on one and taken on another; needs the lifetime-pass session from auth login --chrome or auth login --cookies-file.", Group: "Lifetime-pass database", WhyItMatters: "Use it when a brand needs the same word on several extensions and the user has a lifetime pass."},
	{Command: "words mine", Description: "Filter the 27,627-word dictionary by several categories at once, length, prefix, or glob, offline after a one-time words mine --refresh.", Group: "Local dictionary", WhyItMatters: "Pick it to build a candidate pool before checking availability, without burning API calls."},
	{Command: "tlds inventory", Description: "Compare how many available one-word domains each TLD still has for a category and length filter, next to its cheapest registration price (public route).", Group: "TLD intelligence", WhyItMatters: "Use it to choose an extension by remaining supply and cost before mining words."},
	{Command: "check", Description: "Check one word, or a file of words, across the most-viewed TLDs or all 93, with availability, premium flag, popularity, and the cheapest registrar price per TLD.", Group: "Naming pipeline", WhyItMatters: "Use it whenever an agent needs is-it-free-and-what-does-it-cost for a dictionary word."},
	{Command: "compare", Description: "Compare several full domains in one table: available, premium, price, popularity, and cheapest registrar, with the cheapest available one called out.", Group: "Naming pipeline", WhyItMatters: "Pick it to decide between a few candidate domains in a single call."},
	{Command: "hacks", Description: "Find domain hacks: split a word so its ending is one of the 93 TLDs (sm.art), or list dictionary words ending in a TLD.", Group: "Local dictionary", WhyItMatters: "Use it for brandable hacks like he.art or ch.art that no availability search surfaces."},
	{Command: "tlds drift", Description: "Record registrar price snapshots for every TLD and report which prices moved since an earlier snapshot.", Group: "TLD intelligence", WhyItMatters: "Use it to see whether a TLD got cheaper or dearer before buying."},
	{Command: "listings watch", Description: "Diff the aftermarket feed against the last run: new listings, price or bid changes, listings that disappeared, and auctions ending within a window.", Group: "Aftermarket intelligence", WhyItMatters: "Run it daily to learn what changed on the aftermarket without re-reading every listing."},
	{Command: "gpt generate", Description: "Run DomainsGPT and get the streamed names back as real JSON, optionally only the available ones.", Group: "DomainsGPT", WhyItMatters: "Use it for raw name generation; use brainstorm when you also want prices and dictionary flags."},
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one. Machine output (--json/--csv/--plain/--quiet, or a pipe) still exits 2 and writes {"matches":[]} (or the equivalent empty table) to stdout.`,
		SilenceUsage: true,
		Example: `  oneword-domains-pp-cli which "stale tickets"
  oneword-domains-pp-cli which "bottleneck"
  oneword-domains-pp-cli which --limit 1 "send message"
  oneword-domains-pp-cli which                                # list the full capability index`,
