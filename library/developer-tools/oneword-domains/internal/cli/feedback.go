...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("ONEWORD_DOMAINS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("ONEWORD_DOMAINS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "oneword-domains-pp-cli/feedback")
...
When ` + "`ONEWORD_DOMAINS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`ONEWORD_DOMAINS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
maintainer sees it.`,
		Example: `  oneword-domains-pp-cli feedback "docs said exclusive but the boundary is inclusive"
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  oneword-domains-pp-cli feedback list
  oneword-domains-pp-cli feedback list --limit 5
  oneword-domains-pp-cli feedback list --json`,
