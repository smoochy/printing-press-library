...
func buildAgentContext(rootCmd *cobra.Command) agentContext {
	...
	return agentContext{
		...
		CLI: agentContextCLI{
			Name:        "oneword-domains-pp-cli",
			Description: "Check a word across every TLD with registrar prices, watch the aftermarket, and generate names with DomainsGPT, all from the terminal with a local store the site itself does not offer.",
...
func buildAgentDiscoveryContext() *agentContextDiscovery {
	return &agentContextDiscovery{
		...
		TargetURL:     "https://oneword.domains/tlds",
		EntryCount:    56,
		APIEntryCount: 55,
		Reachability:  "standard_http (65% confidence)",
		Protocols: []string{
			"rest_json (75% confidence)",
			"browser_rendered (70% confidence)",
		...
		CandidateCommands: []string{
			"create_generate — Derived from observed POST /gpt/generate traffic.",
			"create_view — Derived from observed POST /api/tlds/ai/view traffic.",
			"list_ai — Derived from observed GET /api/tlds/ai traffic.",
			"list_ai.json — Derived from observed GET /_next/data/qzfSin9peIEieIakzu-ki/tlds/ai.json traffic.",
			"list_api_key — Derived from observed GET /api/auth/api-key traffic.",
			"list_com — Derived from observed GET /api/tlds/com traffic.",
			"list_count — Derived from observed GET /api/domains/count traffic.",
