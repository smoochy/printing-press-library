...
agents can record playbooks without a file on disk.`,
		Example: `  oneword-domains-pp-cli teach-playbook --query "find items in category" --notes "example playbook note"`,
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  oneword-domains-pp-cli playbook amend --query "find items in category" --add-note "example correction"`,
		RunE: func(cmd *cobra.Command, args []string) error {
			cmd.SilenceUsage = true
			cmd.SilenceErrors = true
			if noLearnActive(flags) {
...
func newPlaybookListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example:     `  oneword-domains-pp-cli playbook list --agent`,
