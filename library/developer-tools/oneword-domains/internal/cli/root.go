...
type rootFlags struct {
	...
	// invocation. Mirrors the ONEWORD_DOMAINS_NO_LEARN env var.
	noLearn                 bool
	selectFields            string
	configPath              string
	homePath                string
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "oneword-domains-pp-cli",
		Short: `One Word Domains CLI — Check a word across every TLD with registrar prices, watch the aftermarket, and generate names with DomainsGPT, all…`,
		Long: `One Word Domains CLI — Check a word across every TLD with registrar prices, watch the aftermarket, and generate names with DomainsGPT, all from the terminal with a local store the site itself does not offer.
...
Health check: run 'oneword-domains-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("oneword-domains-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.selectFields, "select", "", "Comma-separated fields to include in output (e.g. --select slug,tldCount,premium)")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'oneword-domains-pp-cli profile list')")
	...
	addNovelCommandIfAbsent(rootCmd, newNovelBrainstormCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelCheckCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelCompareCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelHacksCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelRecheckCmd(flags))
