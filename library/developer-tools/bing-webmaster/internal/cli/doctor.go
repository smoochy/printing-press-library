...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
  ...
  bing-webmaster-pp-cli doctor --fail-on stale`,
			...
			if credentialRefused {
				...
				if cfg != nil {
					...
					if !configured {
						...
						report["auth_hint"] = "Set your API key with: export BING_WEBMASTER_API_KEY=\"your-token-here\""
						report["auth_key_url"] = "https://www.bing.com/webmasters"
						report["auth_instructions"] = "Settings -> API Access -> accept terms -> Generate API Key. The key is per-user and covers every verified site. OAuth2 bearer is the Microsoft-recommended alternative for server apps; this CLI ships the api-key path."
					} else {
						authConfigured = true
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	case !haveAny:
		...
		report["hint"] = "No sync recorded; run 'bing-webmaster-pp-cli sync' to hydrate API-backed resources. Rows written by other paths are not tracked in sync_state."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
