# Bing Webmaster Tools Printed CLI Agent Guide

This directory is a generated `bing-webmaster-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
bing-webmaster-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
bing-webmaster-pp-cli which "<capability>" --json
bing-webmaster-pp-cli <command> --help
```
...
```bash
bing-webmaster-pp-cli <command> --agent
```
...
```bash
bing-webmaster-pp-cli <command> --help
bing-webmaster-pp-cli <command> --dry-run --agent
```
