...
// version is the printed CLI's version; the library release workflow stamps it.
...
type rootFlags struct {
	...
	// invocation. Mirrors the SCRAPE_CREATORS_NO_LEARN env var.
	noLearn bool
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		...
		Short: `Scrape Creators CLI — Every Scrape Creators endpoint across 28 platforms, with credit-aware comment mining and a local corpus no other…`,
		Long: `Scrape Creators CLI — Every Scrape Creators endpoint across 28 platforms, with credit-aware comment mining and a local corpus no other Scrape Creators tool has.

Highlights (not in the official API docs):
  • comments thread   Fetch one post's complete comment threads, automatically picking the cheaper route between the 15-credit flat include_replies call and 1-credit per-comment reply calls (don't trust child_comment_coun…
...
See README.md or the bundled SKILL.md for recipes.`,
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'scrape-creators-pp-cli profile list')")
	...
	rootCmd.AddCommand(newAppleMusicCmd(flags))
	...
	rootCmd.AddCommand(newSnapchatCmd(flags))
	...
	addNovelCommandIfAbsent(rootCmd, newNovelAdsCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelCommentsCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelContentCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelCreatorCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelTranscriptsCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelTrendsCmd(flags))
