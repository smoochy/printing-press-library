...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("MCPMARKET_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("MCPMARKET_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "mcpmarket-pp-cli/feedback")
...
When ` + "`MCPMARKET_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`MCPMARKET_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
maintainer sees it.`,
		Example: `  mcpmarket-pp-cli feedback "the search command doesn't paginate results"
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  mcpmarket-pp-cli feedback list
  mcpmarket-pp-cli feedback list --limit 5
  mcpmarket-pp-cli feedback list --json`,
