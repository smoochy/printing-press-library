...
var readCommandResources = map[string][]string{
	"mcpmarket-pp-cli category":        {"category"},
	"mcpmarket-pp-cli category list":   {"category"},
	"mcpmarket-pp-cli category get":    {"category"},
	"mcpmarket-pp-cli category search": {"category"},
	"mcpmarket-pp-cli server":          {"server"},
	"mcpmarket-pp-cli server list":     {"server"},
	"mcpmarket-pp-cli server get":      {"server"},
	"mcpmarket-pp-cli server search":   {"server"},
}
...
func cachePolicy() cliutil.Policy {
	...
	envOptOut := "MCPMARKET_NO_AUTO_REFRESH"
...
func autoRefreshIfStale(ctx context.Context, flags *rootFlags, resources []string) (meta cliutil.FreshnessMeta) {
	...
	if err := runAutoRefresh(refreshCtx, flags, db, resources); err != nil {
		fmt.Fprintf(os.Stderr, "warning: using stale mcpmarket-pp-cli cache (refresh failed: %v)\n", err)
