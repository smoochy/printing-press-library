...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  mcpmarket-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  mcpmarket-pp-cli workflow archive --full

  # Archive without a wall-clock timeout
  mcpmarket-pp-cli workflow archive --timeout 0`,
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  mcpmarket-pp-cli workflow status

  # Show status as JSON
  mcpmarket-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("mcpmarket-pp-cli")
			}
