...
Run sync first to populate the local search index.`,
  ...
  mcpmarket-pp-cli search "status"

  # Force local search explicitly
  mcpmarket-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  mcpmarket-pp-cli search "status" --type category --data-source local
  # JSON output for piping
  mcpmarket-pp-cli search "status" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'mcpmarket-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
