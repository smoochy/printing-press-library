...
func initLearn(ctx context.Context, db *sql.DB) error {
	...
	seeds := map[string][]lookups.SeedConfig{
		"client": {
			{Canonical: "Zed", Values: []string{"zed"}},
			{Canonical: "Cline", Values: []string{"cline"}},
			{Canonical: "Cherry Studio", Values: []string{"cherry studio"}},
		},
		"server": {
			{Canonical: "Firecrawl", Values: []string{"firecrawl"}},
			{Canonical: "GitHub", Values: []string{"github", "github mcp"}},
			{Canonical: "ElevenLabs", Values: []string{"elevenlabs", "eleven labs"}},
			{Canonical: "Superpowers", Values: []string{"superpowers"}},
			{Canonical: "Context7", Values: []string{"context7", "context 7"}},
		},
	}
...
func runLearnInitOnce(ctx context.Context) {
	learnInitOnce.Do(func() {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "warning: mcpmarket-pp-cli: learn init: open store: %v\n", err)
			return
		}
		defer s.Close()
		if err := initLearn(ctx, s.DB()); err != nil {
			fmt.Fprintf(os.Stderr, "warning: mcpmarket-pp-cli: learn init: %v\n", err)
		}
	})
}
