...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "mcpmarket-pp-cli",
		Short: `MCP Market CLI — Every MCP Market search, leaderboard, and category browse — plus trending deltas, snapshot…`,
		Long: `MCP Market CLI — Every MCP Market search, leaderboard, and category browse — plus trending deltas, snapshot diffs, and author portfolios no other tool tracks.
...
Health check: run 'mcpmarket-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("mcpmarket-pp-cli {{ .Version }}\n")
	...
	rootCmd.AddCommand(newMcpclientPromotedCmd(flags))
	rootCmd.AddCommand(newSkillPromotedCmd(flags))
	...
	addNovelCommandIfAbsent(rootCmd, newNovelAuthorCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelDedupeCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelDiffCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelLeaderboardCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelStackCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelTrendingCmd(flags))
