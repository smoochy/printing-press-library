...
func TestMCPPathResolutionMatchesCLIResolverWithPlatformDefaults(t *testing.T) {
	...
	if want := filepath.Join(home, ".config", "mcpmarket-pp-cli", "config.json"); cfg.Path != want {
	...
	if want := filepath.Join(home, ".local", "share", "mcpmarket-pp-cli", "data.db"); gotDB != want {
...
func TestMCPRegisterToolsPreservesTypedSpecialTools(t *testing.T) {
	s := server.NewMCPServer("mcpmarket", "test")
	RegisterTools(s)
