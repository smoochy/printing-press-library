...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("DNDBEYOND_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("DNDBEYOND_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "dndbeyond-pp-cli/feedback")
...
When ` + "`DNDBEYOND_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`DNDBEYOND_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  dndbeyond-pp-cli feedback list
  dndbeyond-pp-cli feedback list --limit 5
  dndbeyond-pp-cli feedback list --json`,
