...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  is-agentic-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  is-agentic-pp-cli workflow archive --full

  # Archive without a wall-clock timeout
  is-agentic-pp-cli workflow archive --timeout 0`,
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  is-agentic-pp-cli workflow status

  # Show status as JSON
  is-agentic-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("is-agentic-pp-cli")
			}
