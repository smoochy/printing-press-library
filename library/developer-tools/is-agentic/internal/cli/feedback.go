...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("IS_AGENTIC_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("IS_AGENTIC_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "is-agentic-pp-cli/feedback")
...
When ` + "`IS_AGENTIC_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`IS_AGENTIC_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  is-agentic-pp-cli feedback list
  is-agentic-pp-cli feedback list --limit 5
  is-agentic-pp-cli feedback list --json`,
