...
type rootFlags struct {
	...
	// invocation. Mirrors the IS_AGENTIC_NO_LEARN env var.
	noLearn                 bool
	selectFields            string
	configPath              string
	homePath                string
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "is-agentic-pp-cli",
		Short: `Is Agentic CLI — Turn Is Agentic reports into durable, scriptable readiness evidence.`,
		Long: `Is Agentic CLI — Turn Is Agentic reports into durable, scriptable readiness evidence.
...
Health check: run 'is-agentic-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("is-agentic-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.selectFields, "select", "", "Comma-separated fields to include in output (e.g. --select details,id,name)")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'is-agentic-pp-cli profile list')")
	...
	addNovelCommandIfAbsent(rootCmd, newNovelEvidenceCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelHistoryCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelIssuesCmd(flags))
