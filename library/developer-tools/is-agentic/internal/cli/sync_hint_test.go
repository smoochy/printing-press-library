...
func newSyncHintTestCmd() (*cobra.Command, *bytes.Buffer) {
	...
	cmd := &cobra.Command{Use: "is-agentic-pp-cli"}
	cmd.SetErr(&stderr)
	return cmd, &stderr
}
...
func TestHintIfUnsynced_EmptySyncStateWritesHintToStderr(t *testing.T) {
	...
	if got := stderr.String(); !strings.Contains(got, "Run a report or portfolio command") {
		t.Fatalf("stderr = %q, want report-refresh hint", got)
	}
}
...
func TestHintIfStale_BackdatedSyncStateWritesHintToStderr(t *testing.T) {
	...
	got := stderr.String()
	if !strings.Contains(got, "older than --max-age=30m0s") || !strings.Contains(got, "Run a report or portfolio command") {
		t.Fatalf("stderr = %q, want stale report-refresh hint", got)
	}
}
...
func TestHintIfUnsynced_NullTimestampWritesHint(t *testing.T) {
	...
	if got := stderr.String(); !strings.Contains(got, "no retained reports yet") {
		t.Fatalf("stderr = %q, want empty-report hint", got)
	}
}
...
func TestHintIfStale_ResourceFilterUsesRequestedResource(t *testing.T) {
	...
	if got := stderr.String(); !strings.Contains(got, "no retained reports yet") {
		t.Fatalf("stderr = %q, want empty-report comments hint", got)
	}
}
