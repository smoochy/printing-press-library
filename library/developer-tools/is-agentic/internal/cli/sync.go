...
Resource scoping:
  ...
  is-agentic-pp-cli sync
  # Sync specific resources only
  is-agentic-pp-cli sync --resources report

  # Full resync (ignore previous checkpoint)
  is-agentic-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  is-agentic-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  is-agentic-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  is-agentic-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(resources) == 0 {
				if humanFriendly {
					fmt.Fprintln(os.Stderr, "is-agentic-pp-cli sync: this API has no bulk list endpoint; use report or portfolio to populate the local mirror.")
				} else {
					fmt.Fprintln(syncEventWriter, `{"event":"sync_warning","reason":"no_bulk_list_endpoints","detail":"use report or portfolio to populate the local mirror"}`)
				}
			}
...
func responsePathForResource(resource, path string) []string {
	...
	case "report\x00/api/report":
		return []string{"issues"}
	case "report\x00/api/v1/report":
		return []string{"issues"}
	}
	return nil
}
