...
alter command behavior or recall's verified results on their own.`,
		Example: `  is-agentic-pp-cli learnings candidates
  is-agentic-pp-cli learnings candidates --class flag_alias
  is-agentic-pp-cli learnings candidates --status rejected`,
...
Usage errors (unknown id, non-open status) exit 2.`,
		Example: `  is-agentic-pp-cli learnings confirm 3`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			machineOutput := flags.asJSON || !isatty(cmd.OutOrStdout())
			if !machineOutput {
...
Usage errors (unknown id, no-reject-path candidates) exit 2.`,
		Example: `  is-agentic-pp-cli learnings reject 3`,
		// Usage errors here exit 2 by design; declare it so
		// `cli-printing-press verify` scores the Execute cell honestly.
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if errors.Is(err, store.ErrConfirmedFlagAliasReject) {
				return usageErr(fmt.Errorf("learnings reject: candidate %d is a confirmed flag correction with no reject path; repair the guidance with \"is-agentic-pp-cli playbook amend\" instead", id))
			}
			if err != nil {
				return usageErr(fmt.Errorf("learnings reject: %w", err))
			}
...
purge also clears stale candidates nobody judged.`,
		Example: `  is-agentic-pp-cli learnings purge
  is-agentic-pp-cli learnings purge --tombstones`,
