...
Run report or portfolio first to populate the local search index.`,
		Example: `  # Search locally synced data
  is-agentic-pp-cli search "status"

  # Force local search explicitly
  is-agentic-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  is-agentic-pp-cli search "status" --type report --data-source local
  # JSON output for piping
  is-agentic-pp-cli search "status" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun a report or portfolio command first to populate the local database.", err)
			}
			defer db.Close()
			...
			case "":
				...
				_ = seen // prevent unused error when no FTS tables exist
				{
					...
					// Include retained report snapshots as a domain-specific local source.
					partial, searchErr = db.SearchAgentic(cmd.Context(), query, limit)
					if searchErr != nil {
						return fmt.Errorf("search report snapshots failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
			...
			case "report":
				results, err = db.SearchAgentic(cmd.Context(), query, limit)
			default:
