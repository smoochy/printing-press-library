...
1. On a new user question, call `conduyt-crm-pp-cli recall` FIRST, passing the question as an argv or MCP argument - never by interpolating it into a shell command, quoted string, or heredoc (apostrophes break quotes; a body line equal to the delimiter breaks a heredoc). If a shell is required, write the question with a non-shell file tool, then `QUERY=$(cat /path/to/question.txt)` and `conduyt-crm-pp-cli recall "$QUERY" --agent`. If `found=true` and the top result has `entity_match == "exact"` and `confidence >= 2`, skip discovery and go straight to the live fetch for the returned resource IDs. If the store is cold (recall finds nothing and `learnings list` and `learnings candidates` are both empty), skip recall for the rest of the session.
...
3. After answering, always fire `conduyt-crm-pp-cli teach` in the background with the same argv/MCP or file-then-`$QUERY` passing rule as recall (`QUERY=$(cat /path/to/question.txt)` then `conduyt-crm-pp-cli teach --query "$QUERY" --resource <id> --resource-type <type> &`) - teaching is unconditional and is the anchor that triggers playbook synthesis. Teach the structural question with identifiers stripped (no names, emails, phone numbers, account ids); the CLI warns on obvious PII shapes but does not block.
...
Novel command contracts: `contacts verify-line-type` is the read-only estimate;
the tenant-paid loop is `contacts verify-line-type run --until-done`.
`automations hours-audit` uses live data by default with local fallback; explicit
local mode still requires `sync --resources automations` first. `reports
scorecard` has no row cap unless a positive `--limit` is supplied.
`send-check` batches DNC lookups by contact ID (500 maximum) and trusts the
server's `smsBlocked`/`voiceBlocked` policy fields. `imports blame` maps the
side-effect route's `retryableFailed` and `exhaustedFailed` counters and reports
row-tab retention gaps. `reports get-funnel` resolves an omitted pipeline ID
only when the tenant has exactly one pipeline.
