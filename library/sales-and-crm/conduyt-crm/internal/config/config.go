...
type Config struct {
	...
	ConduytApiKey     string          `toml:"api_key"`
...
func Load(configPath string) (*Config, error) {
	...
	// Env var overrides
	if v := cliutil.EnvOverride("CONDUYT_API_KEY"); v != "" {
		cfg.ConduytApiKey = v
		cfg.markEnvOverride("ConduytApiKey")
		cfg.AuthSource = "env:CONDUYT_API_KEY"
		cfg.CredentialSource = "env:CONDUYT_API_KEY"
	}
	if v := cliutil.EnvOverride("CONDUYT_BEARER_AUTH"); v != "" {
		cfg.ConduytBearerAuth = v
		cfg.markEnvOverride("ConduytBearerAuth")
		cfg.AuthSource = "env:CONDUYT_BEARER_AUTH"
		cfg.CredentialSource = "env:CONDUYT_BEARER_AUTH"
	}
	...
	if v := cliutil.EnvOverride("CONDUYT_CRM_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.ConduytApiKey != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:CONDUYT_API_KEY"
		}
		return "Bearer " + c.ConduytApiKey
	}
...
func (c *Config) StoreScopeCredential() string {
	...
	if c.ConduytApiKey != "" {
		parts = append(parts, "api_key="+c.ConduytApiKey)
	}
	if c.ConduytBearerAuth != "" {
		parts = append(parts, "bearer_auth="+c.ConduytBearerAuth)
	}
	if len(parts) == 0 {
		return ""
	}
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		ConduytApiKey:     c.ConduytApiKey,
		ConduytBearerAuth: c.ConduytBearerAuth,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	if c.ConduytApiKey == "" {
		c.ConduytApiKey = creds.ConduytApiKey
	}
	if c.ConduytBearerAuth == "" {
		c.ConduytBearerAuth = creds.ConduytBearerAuth
	}
}
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.ConduytApiKey = ""
	delete(c.envOverrides, "ConduytApiKey")
	c.updateFileConfigField("ConduytApiKey")
	c.ConduytBearerAuth = ""
	delete(c.envOverrides, "ConduytBearerAuth")
	c.updateFileConfigField("ConduytBearerAuth")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["ConduytApiKey"] {
			out.ConduytApiKey = c.fileConfig.ConduytApiKey
		}
		if c.envOverrides["ConduytBearerAuth"] {
			out.ConduytBearerAuth = c.fileConfig.ConduytBearerAuth
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "ConduytApiKey":
		c.fileConfig.ConduytApiKey = c.ConduytApiKey
	case "ConduytBearerAuth":
		c.fileConfig.ConduytBearerAuth = c.ConduytBearerAuth
	}
}
