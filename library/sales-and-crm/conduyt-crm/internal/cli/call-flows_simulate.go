...
func newCallFlowsSimulateCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:         "simulate",
		Short:       "Create simulate for call flows",
		Annotations: map[string]string{"mcp:read-only": "true", "pp:parent-group": "true", "pp:typed-exit-codes": "0,2"},
		RunE:        parentNoSubcommandRunE(flags),
	}

	cmd.AddCommand(newCallFlowsSimulatePostCallFlowsIdCmd(flags))
	return cmd
}
