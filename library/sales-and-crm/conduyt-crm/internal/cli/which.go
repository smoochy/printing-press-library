...
var whichIndex = []whichEntry{
	{Command: "send-check", Description: "A go/no-go verdict for a send list before any SMS or drip: missing phones, unverified line types, invalid numbers and DNC hits in one table.", Group: "Send safety", WhyItMatters: "Run it before texting a list so an agent never repeats a Kloudi-style blast to landlines or opted-out numbers."},
	{Command: "imports blame", Description: "For one import job: who got the SMS, who was skipped and why, grouped by reason, including queued and exhausted side effects.", Group: "Send safety", WhyItMatters: "Use it after an import finishes to explain delivery outcomes per row instead of stitching four calls."},
	{Command: "imports watch", Description: "Blocks until an import job reaches a terminal state, then prints the created/updated/skipped/error recap with typed exit codes.", Group: "Send safety", WhyItMatters: "Reach for it when a pipeline must wait on an import and fail loudly when it does not land. Verification is scoped to the import job, not tenant-wide delivery activity."},
	{Command: "contacts verify-line-type", Description: "Counts the unverified numbers in scope from the local store and shows the tenant-paid Twilio exposure before the paid verification loop runs.", Group: "Send safety", WhyItMatters: "Use it before spending on line-type verification, then drive the loop from the same command."},
	{Command: "reports scorecard", Description: "Target versus actual per user and metric with attainment percent and pace to the end of the period.", Group: "Reports as commands", WhyItMatters: "Pick it when asked who is on pace, off pace, or how far from target a rep is."},
	{Command: "reports compare", Description: "Any core report called for a window and the equal prior window, with prior, current, delta and delta percent on every numeric value.", Group: "Reports as commands", WhyItMatters: "Use it to explain why a number moved week over week without pulling both reports by hand."},
	{Command: "dialer coverage", Description: "Priorities in dial order with queue depth per priority and the count of available agents; empty priorities are listed under empty_priorities with a warning (exit 0).", Group: "Dialer operations", WhyItMatters: "Run it before the floor starts to see what Smart Dialing will hand out next and where a priority has run dry."},
	{Command: "automations hours-audit", Description: "Every Send SMS, Send Email and Assign step across synced automations, listed with or without an operating-hours window.", Group: "Reports as commands", WhyItMatters: "Use it to confirm quiet-hours coverage after a publish or when compliance asks which steps can fire after hours."},
}
