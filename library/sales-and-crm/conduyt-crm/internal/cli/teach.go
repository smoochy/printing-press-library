...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/sales-and-crm/conduyt-crm/internal/learn/patterns"
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  QUERY="$(cat /path/to/question.txt)" conduyt-crm-pp-cli teach --query "$QUERY" --resource-type <type> --resource <id> --resource <id> &`,
...
when learnings exist.`,
		Example: `  QUERY="$(cat /path/to/question.txt)" conduyt-crm-pp-cli recall "$QUERY" --agent
  QUERY="$(cat /path/to/question.txt)" conduyt-crm-pp-cli recall "$QUERY" --agent --min-confidence 2`,
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
  ...
  conduyt-crm-pp-cli learnings list --query "$QUERY"
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  QUERY="$(cat /path/to/question.txt)" conduyt-crm-pp-cli learnings forget "$QUERY" --resource <id>
  QUERY="$(cat /path/to/question.txt)" conduyt-crm-pp-cli learnings forget "$QUERY" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
a whole family.`,
		Example: `  conduyt-crm-pp-cli teach-pattern --query-template "items in {entity}" --resource-template "GROUP-{entity:category}" --resource-type "items" --entity-kind "category" --strategy substitute`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
