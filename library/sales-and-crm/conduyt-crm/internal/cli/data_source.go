...
func resolveLocal(ctx context.Context, flags *rootFlags, hintWriter io.Writer, resourceType string, isList bool, path string, params map[string]string, reason string) (json.RawMessage, DataProvenance, error) {
	...
	if db == nil {
		return nil, DataProvenance{}, fmt.Errorf("no local data. Run 'conduyt-crm-pp-cli sync --resources activities,admin,api-keys,appointments,automation-executions,automations,automations-actions,automations-condition-fields,automations-templates,booking-pages,calendar,calendars,calls,chat,companies,contacts,contacts-duplicates,conversations,custom-fields,deals,document-templates,drip-campaigns,drip-enrollments,emails,emails-sequences,emails-templates,files,forms,imports,integrations,integrations-zapier-subscribe,invoices,lead-routing,messages,notes,notifications,notifications-profiles,pipelines,products,reports,reports-custom,scoring-rules,smart-lists,smart-views,tags,tasks,users,webhook-logs,webhooks,webhooks-replay,workflows' first")
	}
	defer db.Close()
	...
	if localReadPathIsCollection(resourceType, path, isList) {
		...
		if !sawValid {
			return nil, DataProvenance{}, fmt.Errorf("no local data for %q. Run 'conduyt-crm-pp-cli sync --resources activities,admin,api-keys,appointments,automation-executions,automations,automations-actions,automations-condition-fields,automations-templates,booking-pages,calendar,calendars,calls,chat,companies,contacts,contacts-duplicates,conversations,custom-fields,deals,document-templates,drip-campaigns,drip-enrollments,emails,emails-sequences,emails-templates,files,forms,imports,integrations,integrations-zapier-subscribe,invoices,lead-routing,messages,notes,notifications,notifications-profiles,pipelines,products,reports,reports-custom,scoring-rules,smart-lists,smart-views,tags,tasks,users,webhook-logs,webhooks,webhooks-replay,workflows' first", resourceType)
		}
	...
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return nil, DataProvenance{}, fmt.Errorf("resource %q with ID %q not found in local store. Run 'conduyt-crm-pp-cli sync --resources activities,admin,api-keys,appointments,automation-executions,automations,automations-actions,automations-condition-fields,automations-templates,booking-pages,calendar,calendars,calls,chat,companies,contacts,contacts-duplicates,conversations,custom-fields,deals,document-templates,drip-campaigns,drip-enrollments,emails,emails-sequences,emails-templates,files,forms,imports,integrations,integrations-zapier-subscribe,invoices,lead-routing,messages,notes,notifications,notifications-profiles,pipelines,products,reports,reports-custom,scoring-rules,smart-lists,smart-views,tags,tasks,users,webhook-logs,webhooks,webhooks-replay,workflows' first", resourceType, id)
		}
		return nil, DataProvenance{}, fmt.Errorf("querying local store: %w", err)
	}
