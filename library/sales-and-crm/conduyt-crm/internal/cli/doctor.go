  ...
  conduyt-crm-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if credentialRefused {
				...
				if cfg != nil {
					...
					if !configured {
						...
						report["auth_hint"] = "Set one of: export CONDUYT_API_KEY=\"your-token-here\" or export CONDUYT_BEARER_AUTH=\"your-token-here\""
			...
			if strings.Contains("", " OR ") {
				authEnvOptionalNames = append(authEnvOptionalNames, "CONDUYT_API_KEY")
				if os.Getenv("CONDUYT_API_KEY") != "" {
					authEnvSet = append(authEnvSet, "CONDUYT_API_KEY")
					authEnvOptionalSatisfied = true
				}
			} else if os.Getenv("CONDUYT_API_KEY") != "" {
				authEnvSet = append(authEnvSet, "CONDUYT_API_KEY")
			} else {
				authEnvInfo = append(authEnvInfo, "CONDUYT_API_KEY optional")
			}
			if strings.Contains("", " OR ") {
				authEnvOptionalNames = append(authEnvOptionalNames, "CONDUYT_BEARER_AUTH")
				if os.Getenv("CONDUYT_BEARER_AUTH") != "" {
					authEnvSet = append(authEnvSet, "CONDUYT_BEARER_AUTH")
					authEnvOptionalSatisfied = true
				}
			} else if os.Getenv("CONDUYT_BEARER_AUTH") != "" {
				authEnvSet = append(authEnvSet, "CONDUYT_BEARER_AUTH")
			} else {
				authEnvInfo = append(authEnvInfo, "CONDUYT_BEARER_AUTH optional")
			}
			switch {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	case !haveAny:
		...
		report["hint"] = "No sync recorded; run 'conduyt-crm-pp-cli sync --resources activities,admin,api-keys,appointments,automation-executions,automations,automations-actions,automations-condition-fields,automations-templates,booking-pages,calendar,calendars,calls,chat,companies,contacts,contacts-duplicates,conversations,custom-fields,deals,document-templates,drip-campaigns,drip-enrollments,emails,emails-sequences,emails-templates,files,forms,imports,integrations,integrations-zapier-subscribe,invoices,lead-routing,messages,notes,notifications,notifications-profiles,pipelines,products,reports,reports-custom,scoring-rules,smart-lists,smart-views,tags,tasks,users,webhook-logs,webhooks,webhooks-replay,workflows' to hydrate API-backed resources. Rows written by other paths are not tracked in sync_state."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'conduyt-crm-pp-cli sync --resources activities,admin,api-keys,appointments,automation-executions,automations,automations-actions,automations-condition-fields,automations-templates,booking-pages,calendar,calendars,calls,chat,companies,contacts,contacts-duplicates,conversations,custom-fields,deals,document-templates,drip-campaigns,drip-enrollments,emails,emails-sequences,emails-templates,files,forms,imports,integrations,integrations-zapier-subscribe,invoices,lead-routing,messages,notes,notifications,notifications-profiles,pipelines,products,reports,reports-custom,scoring-rules,smart-lists,smart-views,tags,tasks,users,webhook-logs,webhooks,webhooks-replay,workflows' to refresh."
	}
	return report
}
