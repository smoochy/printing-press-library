...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		...
		Short: `Conduyt CRM CLI — every Conduyt endpoint as a command, plus send-safety, import forensics, report deltas and dialer coverage`,
		Long: `Conduyt CRM CLI — every Conduyt endpoint as a command, plus send-safety, import forensics, report deltas and dialer coverage no other Conduyt surface has.
...
Highlights (not in the official API docs):
  ...
  • automations hours-audit   Every Send SMS, Send Email and Assign step across synced automations, listed with or without an operating-hours window.
...
See README.md or the bundled SKILL.md for recipes.`,
	...
	rootCmd.PersistentFlags().StringVar(&flags.selectFields, "select", "", "Comma-separated fields to include in output (e.g. --select data)")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'conduyt-crm-pp-cli profile list')")
	...
	rootCmd.AddCommand(newAutomationFoldersCmd(flags))
	...
	rootCmd.AddCommand(newCallFlowsCmd(flags))
	...
	rootCmd.AddCommand(newAccountExportsPromotedCmd(flags))
	...
	addNovelCommandIfAbsent(rootCmd, newNovelSendCheckCmd(flags))
