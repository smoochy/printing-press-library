...
func newCallFlowsUnpublishCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:         "unpublish",
		Short:       "Create unpublish for call flows",
		Annotations: map[string]string{"mcp:read-only": "true", "pp:parent-group": "true", "pp:typed-exit-codes": "0,2"},
		RunE:        parentNoSubcommandRunE(flags),
	}

	cmd.AddCommand(newCallFlowsUnpublishPostCallFlowsIdCmd(flags))
	return cmd
}
