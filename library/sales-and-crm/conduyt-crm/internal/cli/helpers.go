...
// paginationMaxPages is the common hard ceiling for unbounded CLI pagination.
const paginationMaxPages = 100

type paginationProgressGuard struct {
	pages       int
	seenIDs     map[string]struct{}
	seenCursors map[string]struct{}
}

func newPaginationProgressGuard() *paginationProgressGuard {
	return &paginationProgressGuard{seenIDs: map[string]struct{}{}, seenCursors: map[string]struct{}{}}
}

func (g *paginationProgressGuard) observe(ids []string, nextCursor string, more bool) error {
	g.pages++
	added := 0
	identifiable := 0
	for _, id := range ids {
		...
		identifiable++
		if _, exists := g.seenIDs[id]; exists {
			continue
		}
		g.seenIDs[id] = struct{}{}
		added++
	}
	if more && identifiable > 0 && added == 0 {
		return fmt.Errorf("pagination made no progress on page %d (zero new ids)", g.pages)
	}
	if nextCursor != "" {
		if _, exists := g.seenCursors[nextCursor]; exists {
			return fmt.Errorf("pagination cursor %q repeated on page %d", nextCursor, g.pages)
		}
		g.seenCursors[nextCursor] = struct{}{}
	}
	if more && g.pages >= paginationMaxPages {
		return fmt.Errorf("pagination reached the %d-page safety cap", paginationMaxPages)
	}
	return nil
}

func rawPaginationIDs(items []json.RawMessage) []string {
	ids := make([]string, 0, len(items))
	for _, item := range items {
		var object map[string]any
		if json.Unmarshal(item, &object) == nil {
			if id := strAny(object, "id", "contactId", "contact_id", "uuid"); id != "" {
				ids = append(ids, id)
				continue
			}
		}
...
func objectPaginationIDs(items []map[string]any) []string {
	ids := make([]string, 0, len(items))
	for _, item := range items {
		if id := strAny(item, "id", "contactId", "contact_id", "uuid"); id != "" {
			ids = append(ids, id)
			continue
		}
		ids = append(ids, "")
...
func jwtExpirySource(cfg *config.Config) string {
	...
	if v := strings.TrimSpace(cfg.ConduytApiKey); v != "" {
		return v
	}
	if v := strings.TrimSpace(cfg.ConduytBearerAuth); v != "" {
		return v
	}
...
func paginatedGet(ctx context.Context, c interface {
	...
	if !fetchAll {
		...
		data, _ = hoistPaginatedEnvelope(data)
	...
	reportedTotalValid := false
	reportedTotalStateSet := false
	page := 0
	...
	for {
		...
		if err != nil {
			if page > 1 {
				truncationErr = &paginationTruncationError{Reason: fmt.Sprintf("fetching page %d: %v", page, err), Cause: err}
				break
			}
			return nil, err
		}
		...
		if json.Unmarshal(data, &items) == nil {
			...
			if err := progress.observe(rawPaginationIDs(items), "", len(items) == pageSize && pageSize > 0); err != nil {
				if strings.Contains(err.Error(), "safety cap") {
					emitPaginatedGetMaxPagesWarning(ctx)
				} else {
					emitPaginatedGetRepeatedPageWarning(ctx)
				}
				truncationErr = &paginationTruncationError{Reason: err.Error()}
				break
			}
			...
			if next, ok := nextFullPageOffsetCursor(clean, cursorParam, paginationType, pageSize, len(items), false); ok {
				if page >= paginationMaxPages {
					emitPaginatedGetMaxPagesWarning(ctx)
					truncationErr = &paginationTruncationError{Reason: fmt.Sprintf("pagination reached the %d-page cap", paginationMaxPages)}
					break
				}
			...
			if json.Unmarshal(data, &obj) == nil {
				...
				if ok {
					...
					if err := progress.observe(rawPaginationIDs(nested), nextCursorToken, len(nested) == pageSize && pageSize > 0); err != nil {
						if strings.Contains(err.Error(), "safety cap") {
							emitPaginatedGetMaxPagesWarning(ctx)
						} else {
							emitPaginatedGetRepeatedPageWarning(ctx)
				...
				n, present, valid := reportedCollectionTotal(obj)
				if reportedTotalStateSet {
					if present != reportedTotalPresent || valid != reportedTotalValid || (present && valid && n != reportedTotal) {
						truncationErr = &paginationTruncationError{Reason: fmt.Sprintf("response metadata total changed from %s to %s", formatReportedTotal(reportedTotal, reportedTotalPresent, reportedTotalValid), formatReportedTotal(n, present, valid))}
						break
					}
				} else {
					reportedTotal = n
					reportedTotalPresent = present
					reportedTotalValid = valid
					reportedTotalStateSet = true
				}
				...
				if nextAdvance != "" {
					...
					if _, seen := seenCursorTokens[nextAdvance]; seen {
						...
						truncationErr = &paginationTruncationError{Reason: "pagination cursor repeated"}
						break
					}
					...
					if moreRaw, ok := rawAtPath(obj, hasMoreField); ok {
						...
						if json.Unmarshal(moreRaw, &more) == nil {
							if more {
								...
								truncationErr = &paginationTruncationError{Reason: "response reports more pages but omits the next cursor"}
								break
							}
	...
	if fetchAll && page == 1 && nextCursorPath == "" && !foundCursorField && hasMoreField == "" && paginationType != "offset" && paginationType != "page" {
		...
		truncationErr = &paginationTruncationError{Reason: "response omits a pagination cursor or completion signal"}
	}
	if truncationErr == nil && reportedTotalPresent && reportedTotalValid && len(allItems) != reportedTotal {
		truncationErr = &paginationTruncationError{Reason: fmt.Sprintf("fetched %d items but response metadata reports total %d", len(allItems), reportedTotal)}
	}
	if humanFriendly {
	...
	if typed, ok := truncationErr.(*paginationTruncationError); ok {
		typed.ReportedTotal = reportedTotal
	}
	return json.RawMessage(result), truncationErr
}

type paginationTruncationError struct {
	Reason        string
	Cause         error
	ReportedTotal int
}

func (e *paginationTruncationError) Error() string { return "pagination truncated: " + e.Reason }
func (e *paginationTruncationError) Unwrap() error { return e.Cause }
...
func emitPaginatedGetMaxPagesWarning(ctx context.Context) {
	platform.MarkContextTruncated(ctx, platform.TruncationReason{Kind: "max_pages", Configured: paginationMaxPages, Observed: paginationMaxPages, MoreAvailable: true})
	if humanFriendly {
...
func reportedCollectionTotal(obj map[string]json.RawMessage) (int, bool, bool) {
	return reportedCollectionTotalAtDepth(obj, 0)
}

func reportedCollectionTotalAtDepth(obj map[string]json.RawMessage, depth int) (int, bool, bool) {
	if obj == nil || depth > 2 {
	...
	for _, wrap := range []string{"meta", "pagination", "paging", "metadata"} {
		...
		if n, present, valid := reportedCollectionTotalAtDepth(inner, depth+1); present {
			return n, present, valid
		}
	}
	return 0, false, false
}

func formatReportedTotal(total int, present, valid bool) string {
	if !present {
		return "missing"
	}
	if !valid {
...
// hoistPaginatedEnvelope normalizes Conduyt's doubly wrapped paginated list
// response from {"data":{"data":[...],"meta":{...}}} to
// {"data":[...],"meta":{...}}. Be deliberately strict: similarly named
// application objects must not be reshaped.
func hoistPaginatedEnvelope(raw json.RawMessage) (json.RawMessage, bool) {
	...
	var inner map[string]json.RawMessage
	if json.Unmarshal(innerRaw, &inner) != nil || len(inner) != 2 {
		return raw, false
	}
	data, dataOK := inner["data"]
	meta, metaOK := inner["meta"]
	if !dataOK || !metaOK {
		return raw, false
	}
	var rows []json.RawMessage
	var metadata map[string]json.RawMessage
	if json.Unmarshal(data, &rows) != nil || json.Unmarshal(meta, &metadata) != nil || metadata == nil {
		return raw, false
	}
	hoisted, err := json.Marshal(map[string]json.RawMessage{"data": data, "meta": meta})
	if err != nil {
		return raw, false
	}
	return hoisted, true
...
type DataProvenance struct {
	...
	PipelineIDSource string     `json:"pipeline_id_source,omitempty"`
}
...
func nonJSONPayloadError(data json.RawMessage) error {
	...
	if len(trimmed) > 0 && trimmed[0] == '<' {
		return authErr(fmt.Errorf("not authenticated or session expired; API returned HTML instead of JSON. " + "Set one of: export CONDUYT_API_KEY=\"your-token-here\" or export CONDUYT_BEARER_AUTH=\"your-token-here\""))
	}
	if len(trimmed) == 0 {
...
func wrapWithProvenance(data json.RawMessage, prov DataProvenance) (json.RawMessage, error) {
	...
	if prov.PipelineIDSource != "" {
		meta["pipeline_id_source"] = prov.PipelineIDSource
	}
	var results any
	if json.Valid(data) {
		var source map[string]json.RawMessage
		if json.Unmarshal(data, &source) == nil {
			if pagination, ok := source["meta"]; ok && source["data"] != nil {
				var paginationObject map[string]any
				if json.Unmarshal(pagination, &paginationObject) == nil && paginationObject != nil {
					meta["pagination"] = paginationObject
				}
			}
		}
