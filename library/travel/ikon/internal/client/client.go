...
import (
	...
	"errors"
	"fmt"
	"github.com/mvanhorn/printing-press-library/library/travel/ikon/internal/cliutil"
	"github.com/mvanhorn/printing-press-library/library/travel/ikon/internal/config"
	"io"
	"math"
	"net/http"
	"net/url"
...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "ikon-pp-cli", "http")
	jar := LoadCookieJar()
	if cfg != nil {
		SeedCookieJarFromCookieHeader(jar, cfg.AccessToken)
	}
...
func (c *Client) doInternal(ctx context.Context, method, path string, params map[string]string, body any, headerOverrides map[string]string, readOnlyIntent bool) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if cliutil.LooksLikeJWT(authHeader) {
...
func authPlaceholderCredentialError(cfg *config.Config) error {
	return authPlaceholderCredentialErrorWithSetup(cfg, "ikon-pp-cli auth set-token <token>")
}
