...
func TestMigrate_AddsColumnsOnUpgrade_Availability(t *testing.T) {
	...
	rows, err := s.DB().Query(`SELECT name FROM pragma_table_xinfo(?)`, "availability")
	if err != nil {
	...
	for _, want := range []string{
		...
		"f_direct",
		"f_direct_airlines",
		"f_direct_mileage_cost",
		"f_direct_mileage_cost_raw",
		"f_direct_remaining_seats",
		"f_direct_remaining_seats_raw",
		"f_direct_total_taxes",
		"f_direct_total_taxes_raw",
		"f_mileage_cost",
		"f_mileage_cost_raw",
		"f_remaining_seats",
		"f_remaining_seats_raw",
		"f_total_taxes",
		"f_total_taxes_raw",
		"j_airlines",
		"j_available",
		...
		"j_direct",
		"j_direct_airlines",
		"j_direct_mileage_cost",
		"j_direct_mileage_cost_raw",
		"j_direct_remaining_seats",
		"j_direct_remaining_seats_raw",
		"j_direct_total_taxes",
		"j_direct_total_taxes_raw",
		"j_mileage_cost",
		"j_mileage_cost_raw",
		"j_remaining_seats",
		"j_remaining_seats_raw",
		"j_total_taxes",
		"j_total_taxes_raw",
		"parsed_date",
		"route_id",
		...
		"w_direct",
		"w_direct_airlines",
		"w_direct_mileage_cost",
		"w_direct_mileage_cost_raw",
		"w_direct_remaining_seats",
		"w_direct_remaining_seats_raw",
		"w_direct_total_taxes",
		"w_direct_total_taxes_raw",
		"w_mileage_cost",
		"w_mileage_cost_raw",
		"w_remaining_seats",
		"w_remaining_seats_raw",
		"w_total_taxes",
		"w_total_taxes_raw",
		"y_airlines",
		"y_available",
		...
		"y_direct",
		"y_direct_airlines",
		"y_direct_mileage_cost",
		"y_direct_mileage_cost_raw",
		"y_direct_remaining_seats",
		"y_direct_remaining_seats_raw",
		"y_direct_total_taxes",
		"y_direct_total_taxes_raw",
		"y_mileage_cost",
		"y_mileage_cost_raw",
		"y_remaining_seats",
		"y_remaining_seats_raw",
		"y_total_taxes",
		"y_total_taxes_raw",
	} {
		if !hasColumn[want] {
	...
	for _, wantIdx := range []string{
		"idx_availability_created_at",
		"idx_availability_updated_at",
	} {
		var got string
		if err := s.DB().QueryRow(
			...
			t.Fatalf("%s index missing from availability after migrate: %v", wantIdx, err)
		}
	}
}
...
func TestMigrate_AddsColumnsOnUpgrade_Awards(t *testing.T) {
	...
	// table_info hides generated columns; table_xinfo is the verify
	// surface for VIRTUAL backfills such as bare_id.
	rows, err := s.DB().Query(`SELECT name FROM pragma_table_xinfo(?)`, "awards")
	if err != nil {
		t.Fatalf("table_xinfo: %v", err)
	}
	defer rows.Close()
	...
	for _, wantIdx := range []string{
		"idx_awards_created_at",
		"idx_awards_updated_at",
	} {
		var got string
		if err := s.DB().QueryRow(
			`SELECT name FROM sqlite_master WHERE type='index' AND name=?`,
			...
			t.Fatalf("%s index missing from awards after migrate: %v", wantIdx, err)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_Destinations verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_Destinations(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "destinations" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// table_info hides generated columns; table_xinfo is the verify
	// surface for VIRTUAL backfills such as bare_id.
	rows, err := s.DB().Query(`SELECT name FROM pragma_table_xinfo(?)`, "destinations")
	if err != nil {
		t.Fatalf("table_xinfo: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from destinations after migrate", want)
		}
	}
	...
	for _, wantIdx := range []string{} {
		...
		if err := s.DB().QueryRow(
			...
			t.Fatalf("%s index missing from destinations after migrate: %v", wantIdx, err)
		}
	}
}
...
func TestMigrate_AddsColumnsOnUpgrade_Refresh(t *testing.T) {
	...
	// table_info hides generated columns; table_xinfo is the verify
	// surface for VIRTUAL backfills such as bare_id.
	rows, err := s.DB().Query(`SELECT name FROM pragma_table_xinfo(?)`, "refresh")
	if err != nil {
		t.Fatalf("table_xinfo: %v", err)
	}
	defer rows.Close()
	...
	for _, wantIdx := range []string{
		"idx_refresh_availability_id",
		"idx_refresh_updated_at",
	} {
		var got string
		if err := s.DB().QueryRow(
			`SELECT name FROM sqlite_master WHERE type='index' AND name=?`,
			...
			t.Fatalf("%s index missing from refresh after migrate: %v", wantIdx, err)
		}
	}
}
...
func TestMigrate_AddsColumnsOnUpgrade_Routes(t *testing.T) {
	...
	// table_info hides generated columns; table_xinfo is the verify
	// surface for VIRTUAL backfills such as bare_id.
	rows, err := s.DB().Query(`SELECT name FROM pragma_table_xinfo(?)`, "routes")
	if err != nil {
		t.Fatalf("table_xinfo: %v", err)
	}
	defer rows.Close()
	...
	for _, wantIdx := range []string{} {
		...
		if err := s.DB().QueryRow(
			...
			t.Fatalf("%s index missing from routes after migrate: %v", wantIdx, err)
		}
	}
}
...
func TestMigrate_AddsColumnsOnUpgrade_Trips(t *testing.T) {
	...
	// table_info hides generated columns; table_xinfo is the verify
	// surface for VIRTUAL backfills such as bare_id.
	rows, err := s.DB().Query(`SELECT name FROM pragma_table_xinfo(?)`, "trips")
	if err != nil {
		t.Fatalf("table_xinfo: %v", err)
	}
	defer rows.Close()
	...
	for _, wantIdx := range []string{
		...
		if err := s.DB().QueryRow(
			...
			t.Fatalf("%s index missing from trips after migrate: %v", wantIdx, err)
		}
	}
}
