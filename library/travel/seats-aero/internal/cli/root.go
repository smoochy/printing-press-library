...
type rootFlags struct {
	...
	// invocation. Mirrors the SEATS_AERO_NO_LEARN env var.
	noLearn bool
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		...
		Short: `Seats Aero CLI — Every Seats.aero Partner API endpoint, plus a local award-availability store that tells you what is new, what is still…`,
		Long: `Seats Aero CLI — Every Seats.aero Partner API endpoint, plus a local award-availability store that tells you what is new, what is still live, and where your miles reach nonstop.
...
Health check: run 'seats-aero-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
	...
	rootCmd.PersistentFlags().StringVar(&flags.selectFields, "select", "", "Comma-separated fields to include in output (e.g. --select airport,business,economy)")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'seats-aero-pp-cli profile list')")
	...
	addNovelCommandIfAbsent(rootCmd, newNovelCalendarCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelDirectScanCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelNewSinceCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelReachCmd(flags))
