...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
  ...
  seats-aero-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if credentialRefused {
				...
				if cfg != nil {
					...
					if !configured {
						...
						report["auth_hint"] = "Set your API key with: export SEATS_AERO_API_KEY=\"your-token-here\""
						report["auth_key_url"] = "https://seats.aero/settings"
						report["auth_instructions"] = "Seats.aero Pro subscribers: Settings -> API -> create a Partner API key (1,000 calls/day; not all regions are eligible)"
					} else {
						authConfigured = true
			...
			if os.Getenv("SEATS_AERO_API_KEY") != "" {
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "SEATS_AERO_API_KEY")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "seats-aero-pp-cli", suggestion)
						} else {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	// PATCH(seats-aero-doctor-read-only): diagnostics must not migrate the operator's database.
	...
	case !haveAny:
		...
		report["hint"] = "No sync recorded; run 'seats-aero-pp-cli sync' to hydrate API-backed resources. Rows written by other paths are not tracked in sync_state."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
