...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/travel/seats-aero/internal/learn/entities"
	"github.com/mvanhorn/printing-press-library/library/travel/seats-aero/internal/learn/lookups"
...
func initLearn(ctx context.Context, db *sql.DB) error {
	...
	seeds := map[string][]lookups.SeedConfig{
		"cabin": {
			{Canonical: "economy", Values: []string{"y", "coach", "main cabin"}},
			{Canonical: "premium", Values: []string{"w", "premium economy", "pe"}},
			{Canonical: "business", Values: []string{"j", "biz", "business class"}},
			{Canonical: "first", Values: []string{"f", "first class"}},
		},
		"program": {
			{Canonical: "eurobonus", Values: []string{"sas eurobonus", "sas"}},
			{Canonical: "virginatlantic", Values: []string{"virgin atlantic flying club", "flying club", "virgin atlantic"}},
			{Canonical: "aeromexico", Values: []string{"aeromexico club premier", "club premier"}},
			{Canonical: "american", Values: []string{"american airlines", "aadvantage", "aa"}},
			{Canonical: "delta", Values: []string{"delta skymiles", "skymiles", "dl"}},
			{Canonical: "etihad", Values: []string{"etihad guest"}},
			{Canonical: "united", Values: []string{"united mileageplus", "mileageplus", "ua"}},
			{Canonical: "emirates", Values: []string{"emirates skywards", "skywards"}},
			{Canonical: "aeroplan", Values: []string{"air canada aeroplan", "air canada", "aeroplan points"}},
			{Canonical: "alaska", Values: []string{"alaska mileage plan", "mileage plan"}},
			{Canonical: "velocity", Values: []string{"virgin australia velocity", "virgin australia"}},
			{Canonical: "qantas", Values: []string{"qantas frequent flyer", "qff"}},
			{Canonical: "connectmiles", Values: []string{"copa connectmiles", "copa"}},
			{Canonical: "azul", Values: []string{"azul tudoazul", "tudoazul"}},
			{Canonical: "smiles", Values: []string{"gol smiles", "gol"}},
			{Canonical: "flyingblue", Values: []string{"air france klm flying blue", "flying blue", "air france", "klm"}},
			{Canonical: "jetblue", Values: []string{"jetblue trueblue", "trueblue"}},
			{Canonical: "qatar", Values: []string{"qatar privilege club", "privilege club", "qatar airways"}},
			{Canonical: "turkish", Values: []string{"turkish miles & smiles", "miles and smiles", "turkish airlines"}},
			{Canonical: "singapore", Values: []string{"singapore krisflyer", "krisflyer", "singapore airlines"}},
			{Canonical: "ethiopian", Values: []string{"ethiopian shebamiles", "shebamiles"}},
			{Canonical: "saudia", Values: []string{"saudi alfursan", "alfursan"}},
			{Canonical: "finnair", Values: []string{"finnair plus"}},
			{Canonical: "lufthansa", Values: []string{"lufthansa miles&more", "miles & more", "miles and more"}},
			{Canonical: "frontier", Values: []string{"frontier airlines"}},
			{Canonical: "spirit", Values: []string{"spirit airlines"}},
		},
	}
...
func runLearnInitOnce(ctx context.Context) {
	learnInitOnce.Do(func() {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "warning: seats-aero-pp-cli: learn init: open store: %v\n", err)
			return
		}
		defer s.Close()
		if err := initLearn(ctx, s.DB()); err != nil {
			fmt.Fprintf(os.Stderr, "warning: seats-aero-pp-cli: learn init: %v\n", err)
		}
	})
}
...
func learnResourceTypeFields() map[string][]string {
	return map[string][]string{
		...
		"awards":       {"name", "title", "display_name", "full_name", "short_name", "label", "slug", "key", "code", "id", "address"},
		"destinations": {"name", "title", "display_name", "full_name", "short_name", "label", "slug", "key", "code", "id", "address"},
		"live":         {"name", "title", "display_name", "full_name", "short_name", "label", "slug", "key", "code", "id", "address"},
		"refresh":      {"name", "title", "display_name", "full_name", "short_name", "label", "slug", "key", "code", "id", "address"},
		"routes":       {"ID", "name", "title", "display_name", "full_name", "short_name", "label", "slug", "key", "code", "id", "address"},
		"trips":        {"ID", "name", "title", "display_name", "full_name", "short_name", "label", "slug", "key", "code", "id", "address"},
	}
}
