...
func newSearchCmd(flags *rootFlags) *cobra.Command {
	...
	var limit int
	var dbPath string
	// PATCH(seats-aero-search-local-only): no live response paths exist for local-only search.

	cmd := &cobra.Command{
		Use:   "search <query>",
		Short: "Full-text search across locally synced data",
		Long: `Search the local mirror using FTS5 full-text search.

This command never calls the live API. Use 'awards' for live cached award search.`,
		Example: `  # Search the local mirror
	seats-aero-pp-cli search "error timeout"

	# Search a specific resource type
	seats-aero-pp-cli search "status" --type availability
  # JSON output for piping
  seats-aero-pp-cli search "critical" --json --limit 20`,
		Annotations: map[string]string{"mcp:hidden": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			// PATCH(seats-aero-search-local-only): /live is metered and is not a text-search endpoint.
