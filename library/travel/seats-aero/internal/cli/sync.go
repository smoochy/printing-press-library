...
Resource scoping:
  ...
  from a prior sync.`,
		// PATCH(seats-aero-sync-example): show a syncable resource with its required source.
		Example: `  # Sync all resources
  seats-aero-pp-cli sync
  # Sync specific resources only
  seats-aero-pp-cli sync --resources availability --resource-param availability:source=united
...
func determinePaginationDefaults(resource string) paginationDefaults {
	...
	case "availability":
		return paginationDefaults{
			...
			limitParam:     "take",
			limit:          1000, // PATCH(seats-aero-sync-page-size): use the API maximum per metered call.
		}
	case "awards":
		return paginationDefaults{
			cursorParam:    "cursor",
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "destinations":
		return db.UpsertDestinations(data)
	case "refresh":
