...
var whichIndex = []whichEntry{
	{Command: "new-since", Description: "See which award seats appeared on a route since you last looked, from your synced local data.", Group: "Local state that compounds", WhyItMatters: "Reach for this when the user asks what changed or what is new on a route, instead of re-running a full search and diffing by eye."},
	{Command: "recheck", Description: "Re-verify aging award rows are still live right before booking. It performs one live quota probe per run (1 daily call), and --apply is refused when quota is unknown unless --ignore-quota is passed.", Group: "Quota-aware plumbing", WhyItMatters: "Print-only by default: lists the aging rows it would refresh, their age, and the remaining daily quota (one probe call). Add --apply to spend refresh credits; --apply is refused when the quota is unknown unless --ignore-quota is passed."},
	{Command: "direct-scan", Description: "Find direct-flight award seats under a mileage ceiling across every synced program at once.", Group: "Local state that compounds", WhyItMatters: "Pick this over the awards search when the user wants nonstop-only results across programs from already-synced data with zero API calls."},
	{Command: "calendar", Description: "Turn one route's synced availability into a date-by-cabin matrix you can scan at a glance.", Group: "Local state that compounds", WhyItMatters: "Use this when the user asks which dates have business or first availability on a route; it answers from the local store in one shot."},
	{Command: "reach", Description: "Discover where your miles can take you nonstop from one airport, ranked by cost and cross-checked against real dated seats.", Group: "Quota-aware plumbing", WhyItMatters: "Reach for this when the user has miles but no fixed destination. It has no local mode, and results is an object {origin, cabin, max_mileage, source, destinations[]} rather than a bare array. --confirm-live makes up to 10 extra live /search calls (opt-in; never under the test harness)."},
}
