...
func resolveLocal(ctx context.Context, flags *rootFlags, hintWriter io.Writer, resourceType string, isList bool, path string, params map[string]string, reason string) (json.RawMessage, DataProvenance, error) {
	...
	if localReadPathIsCollection(resourceType, path, isList) {
		// PATCH(seats-aero-local-list-bounded): push bounded filters into SQLite or stream only as far as needed.
		items, unsupported, err := localListRows(withLocalListHintWriter(ctx, hintWriter), db, resourceType, params)
		if err != nil {
			return nil, DataProvenance{}, fmt.Errorf("querying local store: %w", err)
		}
		if len(items) == 0 {
			count, countErr := db.Count(resourceType)
			if countErr != nil {
				return nil, DataProvenance{}, fmt.Errorf("querying local store: %w", countErr)
			}
			if count == 0 {
...
var localListLimitParams = map[string]bool{
	...
	// PATCH(seats-aero-local-take-limit)
	"take": true,
}
...
var localListControlParams = map[string]bool{
	...
	"min_cabin_pct": true, "include_filtered": true, // PATCH(seats-aero-local-list-bounded): generated API controls are not equality fields.
}
	...
	if len(equality) > 0 {
		...
		if len(equality) > 0 {
			...
			for i, raw := range items {
				...
				// PATCH(seats-aero-local-list-bounded): share the equality predicate with streaming lists.
				if localItemMatchesEquality(parsed[i], equality) {
