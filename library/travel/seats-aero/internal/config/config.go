...
type Config struct {
	...
	SeatsAeroApiKey  string          `toml:"aero_api_key"`
	// PATCH(seats-aero-legacy-credential-fallback)
	LegacyPartnerAuthorization string `toml:"aero_partner_partner_authorization"`
}
...
func Load(configPath string) (*Config, error) {
	...
	cfg.Path = path
	// PATCH(seats-aero-legacy-credential-fallback)
	if cfg.SeatsAeroApiKey == "" && cfg.LegacyPartnerAuthorization != "" {
		cfg.SeatsAeroApiKey = cfg.LegacyPartnerAuthorization
		cfg.AuthSource = "config:aero_partner_partner_authorization (legacy)"
		cfg.CredentialSource = "legacy config path (aero_partner_partner_authorization)"
	}
	...
	if v := os.Getenv("SEATS_AERO_API_KEY"); v != "" {
		cfg.SeatsAeroApiKey = v
		cfg.markEnvOverride("SeatsAeroApiKey")
		cfg.AuthSource = "env:SEATS_AERO_API_KEY"
		cfg.CredentialSource = "env:SEATS_AERO_API_KEY"
	}
	// PATCH(seats-aero-legacy-credential-fallback)
	if os.Getenv("SEATS_AERO_API_KEY") == "" {
		if v := os.Getenv("SEATS_AERO_PARTNER_PARTNER_AUTHORIZATION"); v != "" {
			...
			cfg.CredentialSource = "env:SEATS_AERO_PARTNER_PARTNER_AUTHORIZATION"
		}
	}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("SEATS_AERO_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "seats-aero-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	token := c.SeatsAeroApiKey
	if token == "" {
		return ""
	}
	if c.SeatsAeroApiKey == "" {
		return ""
	}
	return token
}
...
func (c *Config) StoreScopeCredential() string {
	...
	if c.SeatsAeroApiKey != "" {
		parts = append(parts, "aero_api_key="+c.SeatsAeroApiKey)
	}
	if len(parts) == 0 {
		return ""
	}
...
func (c *Config) hasCredentialFields() bool {
	...
	if c.LegacyPartnerAuthorization != "" {
		return true
	}
	return false
}
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		SeatsAeroApiKey: c.SeatsAeroApiKey,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	if c.SeatsAeroApiKey == "" {
		c.SeatsAeroApiKey = creds.SeatsAeroApiKey
	}
}
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.SeatsAeroApiKey = token
	delete(c.envOverrides, "SeatsAeroApiKey")
	c.updateFileConfigField("SeatsAeroApiKey")
	if err := c.saveCredentialsFirst(); err != nil {
		return err
	}
	return c.save()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["SeatsAeroApiKey"] {
			out.SeatsAeroApiKey = c.fileConfig.SeatsAeroApiKey
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "SeatsAeroApiKey":
		c.fileConfig.SeatsAeroApiKey = c.SeatsAeroApiKey
	}
}
