...
// wanderlogAPIEnvelopeError returns a not-found error when a 2xx body is a
// Wanderlog failure envelope (success:false). Callers must not treat that
// shape as a successful payload.
func wanderlogAPIEnvelopeError(data json.RawMessage) error {
	var env struct {
	...
	msg := "Wanderlog request failed"
	if len(env.Messages) > 0 && strings.TrimSpace(env.Messages[0]) != "" {
		msg = env.Messages[0]
	} else if env.Error != nil {
		msg = fmt.Sprint(env.Error)
	}
	if len(env.ErrTypes) > 0 {
		return notFoundErr(fmt.Errorf("%s (%s)", msg, strings.Join(env.ErrTypes, ",")))
	}
...
func classifyAPIError(err error, flags *rootFlags) error {
	...
	case strings.Contains(msg, "HTTP 400") && cliutil.LooksLikeAuthError(msg):
		return authErr(fmt.Errorf("%w\nhint: the API rejected the request — this usually means auth is missing or invalid."+
			...
			"\n      Run 'wanderlog-pp-cli doctor' to check auth status."+
			"\n      Response: "+cliutil.SanitizeErrorBody(msg), err))
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: check your API key."+
			" Set it with: export WANDERLOG_COOKIE=<your-key>"+
			"\n      See API docs: https://wanderlog.com"+
			"\n      Run 'wanderlog-pp-cli doctor' to check auth status.", err))
...
// selectLooksEmpty reports a --select projection that matched no real fields:
// a bare {}, a non-empty list of only {}, or an object whose list field is
// only {}. A genuinely empty list ([] or {"items":[]}) is not empty — those
// objects would have keys if any rows existed.
//
// PATCH(amend-2026-08-23: empty --select must fail instead of printing {})
func selectLooksEmpty(raw json.RawMessage) bool {
	var v any
	if err := json.Unmarshal(raw, &v); err != nil {
		return false
	}
	return valueLooksSelectEmpty(v)
}

func valueLooksSelectEmpty(v any) bool {
	switch t := v.(type) {
	case []any:
		if len(t) == 0 {
			return false
	...
	case map[string]any:
		...
		for _, val := range t {
			...
			if allEmptyObj {
				sawEmptyObjectList = true
			}
		}
		return sawEmptyObjectList
	default:
		return false
	}
}
...
// It does not shrink object GETs such as trips get; use plan outline.
...
// csvObjectRows returns object rows for CSV rendering.
// A top-level array of objects wins. Otherwise a root object with an `items`
// array of objects is unwrapped (reservation list envelope).
//
// PATCH(amend-2026-08-23: --csv unwraps {items:[...]} envelopes)
func csvObjectRows(data json.RawMessage) []map[string]any {
	var items []map[string]any
	...
	if err := json.Unmarshal(env.Items, &items); err != nil || len(items) == 0 {
		return nil
	}
	return items
}
