...
## Trip planning for people and agents

Create a blank trip with `trips create`, read full notes with `plan block get`, compare saved driving/walking estimates with `plan route legs`, and edit several blocks atomically with `plan edit --changes-file`. `plan suggestions` exposes saved candidates; `places autocomplete --query` accepts simple location flags. See [the planning workflow](references/trip-planning.md).

Travel output preserves stop order, labels missing estimates and unknown freshness, and only checks schedule slack when a transport mode is explicitly selected. Schedule updates reconcile start/end/duration; errors from the application are failures even when HTTP returns 200. Mixed-currency budget totals remain separated.

Creation uses explicit `--dry-run` for preview; ShareDB edits use `--apply` to commit. Read the execution and validation status rather than interpreting a smoke probe as a validated mutation.
...
### Token-efficient planning reads

Use `plan day --day N --target-key KEY --agent` for complete day notes, booking constraints, saved travel and warnings in one response. Use `plan block get --block-ids ID,ID` for selected complete notes. Opt-in `--save-state` / `--since` day reads return verified local differences. Discovery now defaults to summaries; `agent-context --command "plan day"` reads one schema and `--full` restores the legacy payload. See [token efficiency](references/token-efficiency.md) for merge rules, retained context and benchmark limits.


### Whole-trip planning with focused reads

For a complete review, read known days once with `plan days --days 1-N --target-key KEY --agent` and the actual travel mode. For orientation or a focused question, use `plan overview --target-key KEY --agent`, then only the missing relevant details. Reuse returned routes and checks. `agent-context --task review` (also `create` or `edit`) gives focused workflow discovery. Add multiple stops, notes and checklists with `plan block add-batch --blocks-file FILE`, previewing before `--apply`. Read [batch creation](references/batch-creation.md) and [token efficiency](references/token-efficiency.md). Day deltas inherit unchanged fields from a verified baseline; reload a full day after context loss.
