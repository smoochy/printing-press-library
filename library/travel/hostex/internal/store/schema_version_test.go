...
// TestMigrate_AddsColumnsOnUpgrade_Availabilities verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_Availabilities(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "availabilities" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// The migration must have added every generated column.
	rows, err := s.DB().Query(`PRAGMA table_info("availabilities")`)
	if err != nil {
		t.Fatalf("table_info: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from availabilities after migrate", want)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_CalendarShareLinks verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_CalendarShareLinks(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "calendar_share_links" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// The migration must have added every generated column.
	rows, err := s.DB().Query(`PRAGMA table_info("calendar_share_links")`)
	if err != nil {
		t.Fatalf("table_info: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from calendar_share_links after migrate", want)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_ChannelAccounts verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_ChannelAccounts(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "channel_accounts" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// The migration must have added every generated column.
	rows, err := s.DB().Query(`PRAGMA table_info("channel_accounts")`)
	if err != nil {
		t.Fatalf("table_info: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from channel_accounts after migrate", want)
		}
	}
}
...
// TestMigrate_AddsColumnsOnUpgrade_Preapprovals verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_Preapprovals(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "preapprovals" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// The migration must have added every generated column.
	rows, err := s.DB().Query(`PRAGMA table_info("preapprovals")`)
	if err != nil {
		t.Fatalf("table_info: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from preapprovals after migrate", want)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_SpecialOffers verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_SpecialOffers(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "special_offers" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// The migration must have added every generated column.
	rows, err := s.DB().Query(`PRAGMA table_info("special_offers")`)
	if err != nil {
		t.Fatalf("table_info: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from special_offers after migrate", want)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_CustomChannels verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_CustomChannels(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "custom_channels" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// The migration must have added every generated column.
	rows, err := s.DB().Query(`PRAGMA table_info("custom_channels")`)
	if err != nil {
		t.Fatalf("table_info: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from custom_channels after migrate", want)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_ExpenseItems verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_ExpenseItems(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "expense_items" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// The migration must have added every generated column.
	rows, err := s.DB().Query(`PRAGMA table_info("expense_items")`)
	if err != nil {
		t.Fatalf("table_info: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from expense_items after migrate", want)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_ExpenseMethods verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_ExpenseMethods(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "expense_methods" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// The migration must have added every generated column.
	rows, err := s.DB().Query(`PRAGMA table_info("expense_methods")`)
	if err != nil {
		t.Fatalf("table_info: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from expense_methods after migrate", want)
		}
	}
}
...
// TestMigrate_AddsColumnsOnUpgrade_IncomeItems verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_IncomeItems(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "income_items" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// The migration must have added every generated column.
	rows, err := s.DB().Query(`PRAGMA table_info("income_items")`)
	if err != nil {
		t.Fatalf("table_info: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from income_items after migrate", want)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_IncomeMethods verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_IncomeMethods(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "income_methods" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// The migration must have added every generated column.
	rows, err := s.DB().Query(`PRAGMA table_info("income_methods")`)
	if err != nil {
		t.Fatalf("table_info: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from income_methods after migrate", want)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_KnowledgeBases verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_KnowledgeBases(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "knowledge_bases" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// The migration must have added every generated column.
	rows, err := s.DB().Query(`PRAGMA table_info("knowledge_bases")`)
	if err != nil {
		t.Fatalf("table_info: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from knowledge_bases after migrate", want)
		}
	}
}
...
// TestMigrate_AddsColumnsOnUpgrade_PricingRatios verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_PricingRatios(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "pricing_ratios" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// The migration must have added every generated column.
	rows, err := s.DB().Query(`PRAGMA table_info("pricing_ratios")`)
	if err != nil {
		t.Fatalf("table_info: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from pricing_ratios after migrate", want)
		}
	}
}
...
// TestMigrate_AddsColumnsOnUpgrade_ReservationTags verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_ReservationTags(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "reservation_tags" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// The migration must have added every generated column.
	rows, err := s.DB().Query(`PRAGMA table_info("reservation_tags")`)
	if err != nil {
		t.Fatalf("table_info: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from reservation_tags after migrate", want)
		}
	}
}
...
// TestMigrate_AddsColumnsOnUpgrade_Allocate verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_Allocate(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "allocate" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// The migration must have added every generated column.
	rows, err := s.DB().Query(`PRAGMA table_info("allocate")`)
	if err != nil {
		t.Fatalf("table_info: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from allocate after migrate", want)
		}
	}
}
...
// TestMigrate_AddsColumnsOnUpgrade_CheckInDetails verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_CheckInDetails(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "check_in_details" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// The migration must have added every generated column.
	rows, err := s.DB().Query(`PRAGMA table_info("check_in_details")`)
	if err != nil {
		t.Fatalf("table_info: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from check_in_details after migrate", want)
		}
	}
}
...
// TestMigrate_AddsColumnsOnUpgrade_MoveToBox verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_MoveToBox(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "move_to_box" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// The migration must have added every generated column.
	rows, err := s.DB().Query(`PRAGMA table_info("move_to_box")`)
	if err != nil {
		t.Fatalf("table_info: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from move_to_box after migrate", want)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_StayStatus verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_StayStatus(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "stay_status" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// The migration must have added every generated column.
	rows, err := s.DB().Query(`PRAGMA table_info("stay_status")`)
	if err != nil {
		t.Fatalf("table_info: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from stay_status after migrate", want)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_ReservationsTags verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_ReservationsTags(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "reservations_tags" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// The migration must have added every generated column.
	rows, err := s.DB().Query(`PRAGMA table_info("reservations_tags")`)
	if err != nil {
		t.Fatalf("table_info: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from reservations_tags after migrate", want)
		}
	}
}
...
// TestMigrate_AddsColumnsOnUpgrade_RoomTypes verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_RoomTypes(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "room_types" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// The migration must have added every generated column.
	rows, err := s.DB().Query(`PRAGMA table_info("room_types")`)
	if err != nil {
		t.Fatalf("table_info: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from room_types after migrate", want)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_Staffs verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_Staffs(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "staffs" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// The migration must have added every generated column.
	rows, err := s.DB().Query(`PRAGMA table_info("staffs")`)
	if err != nil {
		t.Fatalf("table_info: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from staffs after migrate", want)
		}
	}
}
