...
func newTasksUpdateCmd(flags *rootFlags) *cobra.Command {
	...
	var bodyExpectedDate string
	var bodyExpectedTime string
	var bodyFee float64
	...
	cmd.Flags().StringVar(&bodyExpectedDate, "expected-date", "", "Expected execution date in `YYYY-MM-DD`. Interpreted in the operator's configured timezone.")
	cmd.Flags().StringVar(&bodyExpectedTime, "expected-time", "", "Expected execution time in `HH:mm:ss`. Interpreted in the operator's configured timezone.")
