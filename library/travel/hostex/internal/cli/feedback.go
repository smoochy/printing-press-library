...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("HOSTEX_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("HOSTEX_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "hostex-pp-cli/feedback")
...
When ` + "`HOSTEX_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`HOSTEX_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  hostex-pp-cli feedback list
