...
func newTasksCreateCmd(flags *rootFlags) *cobra.Command {
	...
	cmd.Flags().StringVar(&bodyExpectedDate, "expected-date", "", "Expected execution date in `YYYY-MM-DD`.")
	cmd.Flags().StringVar(&bodyExpectedTime, "expected-time", "", "Expected execution time in `HH:mm:ss`. Optional. Interpreted in the operator's configured timezone.")
