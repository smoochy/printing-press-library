...
func newOauthRevokeTokenCmd(flags *rootFlags) *cobra.Command {
	...
	cmd.Flags().StringVar(&bodyClientSecret, "client-secret", "", "The secret key associated with the client application, used to authenticate the revocation request.")
	cmd.Flags().StringVar(&bodyToken, "token", "", "The access or refresh token to be revoked. Once revoked, the token cannot be used for further authentication.")
