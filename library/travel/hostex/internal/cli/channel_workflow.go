...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  hostex-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  hostex-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"automation", "calendar-share-links", "channel-accounts", "conversations", "custom-channels", "expense-items", "expense-methods", "groups", "income-items", "income-methods", "knowledge-bases", "listings", "message", "pricing-ratios", "properties", "reservation-tags", "reservations", "review", "reviews", "room-types", "staffs", "tags", "tasks", "transactions", "webhooks"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  hostex-pp-cli workflow status

  # Show status as JSON
  hostex-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("hostex-pp-cli")
			}
