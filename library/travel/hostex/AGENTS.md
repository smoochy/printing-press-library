# Hostex Printed CLI Agent Guide

This directory is a generated `hostex-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
hostex-pp-cli doctor --json
hostex-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
hostex-pp-cli which "<capability>" --json
hostex-pp-cli <command> --help
```
...
```bash
hostex-pp-cli <command> --help
hostex-pp-cli <command> --dry-run --agent
```
