...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("AGODA_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("AGODA_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "agoda-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  agoda-pp-cli feedback "the --since flag is inclusive but the docs say exclusive"
...
When ` + "`AGODA_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`AGODA_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  agoda-pp-cli feedback list
  agoda-pp-cli feedback list --limit 5
  agoda-pp-cli feedback list --json`,
