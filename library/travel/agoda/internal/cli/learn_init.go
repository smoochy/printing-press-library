...
func initLearn(ctx context.Context, db *sql.DB) error {
	...
	seeds := map[string][]lookups.SeedConfig{
		"destination": {
			{Canonical: "Tokyo", Values: []string{"tokyo", "tyo", "tokio"}},
			{Canonical: "Bangkok", Values: []string{"bangkok", "bkk", "krung thep"}},
			{Canonical: "Singapore", Values: []string{"singapore", "sg", "sin"}},
			{Canonical: "Osaka", Values: []string{"osaka", "kansai"}},
			{Canonical: "Seoul", Values: []string{"seoul", "icn"}},
			{Canonical: "Hong Kong", Values: []string{"hong kong", "hk", "hkg"}},
			{Canonical: "Bali", Values: []string{"bali", "denpasar", "dps"}},
			{Canonical: "Ho Chi Minh City", Values: []string{"ho chi minh", "saigon", "hcmc", "sgn"}},
			{Canonical: "Kuala Lumpur", Values: []string{"kuala lumpur", "kl", "kul"}},
			{Canonical: "Taipei", Values: []string{"taipei", "tpe"}},
		},
	}
...
func runLearnInitOnce(ctx context.Context) {
	learnInitOnce.Do(func() {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "warning: agoda-pp-cli: learn init: open store: %v\n", err)
			return
		}
		defer s.Close()
		if err := initLearn(ctx, s.DB()); err != nil {
			fmt.Fprintf(os.Stderr, "warning: agoda-pp-cli: learn init: %v\n", err)
		}
	})
}
