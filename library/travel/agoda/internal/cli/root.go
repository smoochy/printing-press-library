...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/travel/agoda/internal/learn"
...
type rootFlags struct {
	...
	// invocation. Mirrors the AGODA_NO_LEARN env var.
	noLearn                 bool
	selectFields            string
	configPath              string
	homePath                string
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "agoda-pp-cli",
		Short: `Agoda CLI — Search Agoda hotels with the true all-in price, and re-rank by what you will actually pay.`,
		Long: `Agoda CLI — Search Agoda hotels with the true all-in price, and re-rank by what you will actually pay.
...
Health check: run 'agoda-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("agoda-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.selectFields, "select", "", "Comma-separated fields to include in output (e.g. --select hotelReviewId,rating,reviewComments)")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'agoda-pp-cli profile list')")
	...
	addNovelCommandIfAbsent(rootCmd, newNovelHotelsCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelPricesCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelSearchCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelVipCmd(flags))
