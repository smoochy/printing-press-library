...
In local mode: searches locally synced data only.`,
  ...
  overpass-pp-cli search "error timeout"

  # Force local search only
  overpass-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  overpass-pp-cli search "status" --type geocoding --data-source local
  # JSON output for piping
  overpass-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'overpass-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
