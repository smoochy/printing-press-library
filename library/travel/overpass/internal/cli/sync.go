...
Resource scoping:
  ...
  overpass-pp-cli sync

  # Sync specific resources only
  overpass-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  overpass-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  overpass-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  overpass-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  overpass-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dbPath == "" {
				dbPath = defaultDBPath("overpass-pp-cli")
			}
			...
			if len(resources) == 0 {
				if humanFriendly {
					fmt.Fprintln(os.Stderr, "overpass-pp-cli sync: no default sync resources in spec; use --resources for explicit sync targets or populate the store via single-fetch commands.")
				} else {
...
func syncResourcePath(resource string) (string, error) {
	// geocoding is absolute because it lives on Nominatim, not on the
	// Overpass base URL. The spec declares a per-path `servers:` override for
	// /search which the promoted `geocoding` command honors; sync was left
	// with the bare path and resolved it against https://overpass-api.de,
	// where it came back as a 404 Apache error page.
