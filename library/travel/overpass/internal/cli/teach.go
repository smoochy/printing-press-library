...
// overpass-pp-cli CLI: `teach` (fire-and-forget, silent on success,
...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/travel/overpass/internal/learn"
	"github.com/mvanhorn/printing-press-library/library/travel/overpass/internal/learn/entities"
	"github.com/mvanhorn/printing-press-library/library/travel/overpass/internal/learn/lookups"
	"github.com/mvanhorn/printing-press-library/library/travel/overpass/internal/learn/patterns"
...
const noLearnEnvVar = "OVERPASS_NO_LEARN"
...
func learnDBPath(explicit string) string {
	...
	return defaultDBPath("overpass-pp-cli")
}
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  overpass-pp-cli teach --query "<question>" --resource-type <type> \
...
when learnings exist.`,
		Example: `  overpass-pp-cli recall "<question>" --agent
  overpass-pp-cli recall "<question>" --agent --min-confidence 2`,
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "list",
		Short: "List stored query-to-resource learnings from the local search_learnings table, filtered by query substring, source, resource, action, or minimum confidence (--warnings returns teach-time warnings instead)",
		Example: `  overpass-pp-cli learnings list --agent
  overpass-pp-cli learnings list --query "<substring>"
  overpass-pp-cli learnings list --warnings --agent`,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  overpass-pp-cli learnings forget "<question>" --resource <id>
  overpass-pp-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
a whole family.`,
		Example: `  overpass-pp-cli teach-pattern \
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  overpass-pp-cli teach-lookup --kind country --canonical "United States" --value USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
			}
