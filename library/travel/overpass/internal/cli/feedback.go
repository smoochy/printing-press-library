...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("OVERPASS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("OVERPASS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "overpass-pp-cli/feedback")
...
When ` + "`OVERPASS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`OVERPASS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  overpass-pp-cli feedback list
  overpass-pp-cli feedback list --limit 5
  overpass-pp-cli feedback list --json`,
