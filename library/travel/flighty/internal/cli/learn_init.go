...
func newLearnConfig() *entities.Config {
	...
	if re, err := regexp.Compile(`^[A-Z]{3}$`); err != nil {
		fmt.Fprintf(os.Stderr, "warning: flighty-pp-cli: ticker pattern %q failed to compile: %v\n", `^[A-Z]{3}$`, err)
	} else {
...
func initLearn(ctx context.Context, db *sql.DB) error {
	...
	seeds := map[string][]lookups.SeedConfig{
		"airport": {
			{Canonical: "Denver", Values: []string{"den", "kden", "denver intl", "denver international"}},
			{Canonical: "London Gatwick", Values: []string{"lgw", "gatwick", "kgatwick"}},
			{Canonical: "LaGuardia", Values: []string{"lga", "laguardia", "klga"}},
			{Canonical: "Chicago O'Hare", Values: []string{"ord", "ohare", "kord"}},
			{Canonical: "John F Kennedy", Values: []string{"jfk", "kennedy", "kjfk"}},
		},
	}
...
func runLearnInitOnce(ctx context.Context) {
	learnInitOnce.Do(func() {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "warning: flighty-pp-cli: learn init: open store: %v\n", err)
			return
		}
		defer s.Close()
		if err := initLearn(ctx, s.DB()); err != nil {
			fmt.Fprintf(os.Stderr, "warning: flighty-pp-cli: learn init: %v\n", err)
		}
	})
}
