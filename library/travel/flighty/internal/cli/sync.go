...
Resource scoping:
  ...
  flighty-pp-cli sync --resources airports,airports-tv

  # Full resync (ignore previous checkpoint)
  flighty-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  flighty-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  flighty-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  flighty-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Flighty snapshot history: record the post-sync catalog so
			// 'airports diff' can compare against the previous snapshot.
			if totalSynced > 0 && !c.DryRun {
				flightyRecordSnapshotFromStore(cmd.Context(), db)
			}
...
func syncResource(ctx context.Context, c interface {
	...
	for {
		...
		if opts, ok := syncHTMLExtractionOptions(resource, "", c.RequestBaseURL(), c.LastContentType(), path, params); ok {
			// Flighty pages embed data as Next.js RSC flight chunks, which the
			// generic embedded-json extractor cannot parse. Route by page kind;
			// fall back to the generic extractor for non-Flighty shapes.
			kind := flightyPageKindForPath(path)
			if kind != "" {
				data, err = extractFlightyRSC(kind, data)
			} else {
