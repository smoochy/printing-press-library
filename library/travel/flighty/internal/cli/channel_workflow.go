...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  flighty-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  flighty-pp-cli workflow archive --full

  # Archive without a wall-clock timeout
  flighty-pp-cli workflow archive --timeout 0`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"airports", "airports-tv"}
			if cliutil.IsDogfoodEnv() {
				if len(resources) > 3 {
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  flighty-pp-cli workflow status

  # Show status as JSON
  flighty-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("flighty-pp-cli")
			}
