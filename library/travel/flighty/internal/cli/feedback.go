...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("FLIGHTY_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("FLIGHTY_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "flighty-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use: "feedback [text]",
		Example: `  flighty-pp-cli feedback "the --select flag saved me tokens"
  flighty-pp-cli feedback "airport diff needed two syncs" --json`,
...
When ` + "`FLIGHTY_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`FLIGHTY_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  flighty-pp-cli feedback list
  flighty-pp-cli feedback list --limit 5
  flighty-pp-cli feedback list --json`,
