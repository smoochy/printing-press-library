...
Run sync first to populate the local search index.`,
  ...
  flighty-pp-cli search "status"

  # Force local search explicitly
  flighty-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  flighty-pp-cli search "status" --type airports --data-source local
  # JSON output for piping
  flighty-pp-cli search "status" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'flighty-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
