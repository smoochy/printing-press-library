...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	// reservesea (AeroParker) is a session-driven flow: the booking pages 302
	// through a chain that only settles once the server's session cookies
	// (JSESSIONID, parkingReservationGuid, AWSALB) are carried across hops. A
	// nil jar drops them and the request redirect-loops. A cookie jar is inert
	// for stateless APIs, so this is safe as the default.
...
func (c *Client) doInternal(ctx context.Context, method, path string, params map[string]string, body any, headerOverrides map[string]string, readOnlyIntent bool) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if req.Header.Get("User-Agent") == "" {
			req.Header.Set("User-Agent", "sea-airport-parking-pp-cli/0.1.0")
		}
