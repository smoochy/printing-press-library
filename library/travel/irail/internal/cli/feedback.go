...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("IRAIL_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("IRAIL_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "irail-pp-cli/feedback")
...
When ` + "`IRAIL_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`IRAIL_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  irail-pp-cli feedback list
  irail-pp-cli feedback list --limit 5
  irail-pp-cli feedback list --json`,
