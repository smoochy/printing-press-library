...
// irail-pp-cli CLI: `teach` (fire-and-forget, silent on success,
...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/travel/irail/internal/learn/patterns"
...
const noLearnEnvVar = "IRAIL_NO_LEARN"
...
func learnDBPath(explicit string) string {
	...
	return defaultDBPath("irail-pp-cli")
}
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  irail-pp-cli teach --query "<question>" --resource-type <type> \
...
when learnings exist.`,
		Example: `  irail-pp-cli recall "<question>" --agent
  irail-pp-cli recall "<question>" --agent --min-confidence 2`,
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  irail-pp-cli learnings list --agent
  irail-pp-cli learnings list --query "<substring>"
  irail-pp-cli learnings list --warnings --agent`,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  irail-pp-cli learnings forget "<question>" --resource <id>
  irail-pp-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
a whole family.`,
		Example: `  irail-pp-cli teach-pattern \
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  irail-pp-cli teach-lookup --kind country --canonical "United States" --value USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
			}
