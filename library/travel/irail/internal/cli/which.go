...
var whichIndex = []whichEntry{
	{Command: "transfer-risk", Description: "Tells you whether the transfers in a journey still hold once today's delays are applied.", Group: "Live data joined with open datasets", WhyItMatters: "Reach for this instead of a plain route lookup whenever a journey has a transfer and delays are already in play."},
	{Command: "punctuality", Description: "Shows how reliable a train or route has actually been, from delay observations recorded on your machine.", Group: "Local state that compounds", WhyItMatters: "Use this for questions about the past such as chronic lateness; it never calls the API."},
	{Command: "disruptions route", Description: "Filters the national disruption feed down to the stations your journey actually passes through.", Group: "Live data joined with open datasets", WhyItMatters: "Use this when the national list is too noisy to answer whether one specific trip is affected."},
	{Command: "observe", Description: "Records what the board says right now into local SQLite, building the history other commands read.", Group: "Local state that compounds", WhyItMatters: "Run this on a schedule; it is what makes punctuality and changes able to answer anything."},
	{Command: "leave-by", Description: "Answers the last train you can take and still arrive before a deadline, accounting for current delays.", Group: "Time reasoning done properly", WhyItMatters: "Use this when the arrival deadline is fixed and the departure time is the unknown."},
	{Command: "stations facilities", Description: "Reports step-free access, elevators, ramps, lockers, bike parking and ticket-desk hours for a station.", Group: "Live data joined with open datasets", WhyItMatters: "Use this for accessibility and amenity questions that the live rail API simply cannot answer."},
	{Command: "changes", Description: "Reports new delays, cancellations and platform changes since the last time you looked.", Group: "Local state that compounds", WhyItMatters: "Use this for deltas during a commute rather than re-reading a whole board."},
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one`,
		Example: `  irail-pp-cli which "stale tickets"
  irail-pp-cli which "bottleneck"
  irail-pp-cli which --limit 1 "send message"
  irail-pp-cli which                                # list the full capability index`,
