...
Resource scoping:
  ...
  irail-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  irail-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  irail-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  irail-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  irail-pp-cli sync --latest-only`,
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "board":
		return db.UpsertBoard(data)
	case "disruptions":
		return db.UpsertDisruptions(data)
	case "stations":
		return db.UpsertStations(data)
	case "train":
		return db.UpsertTrain(data)
	default:
...
func responsePathForResource(resource, path string) []string {
	...
	case "board\x00/v1/liveboard?format=json":
		return []string{"departures.departure"}
	case "disruptions\x00/v1/disturbances?format=json":
		return []string{"disturbance"}
	case "route\x00/v1/connections?format=json":
		return []string{"connection"}
	case "stations\x00/v1/stations?format=json":
		return []string{"station"}
	case "train\x00/v1/composition?format=json":
