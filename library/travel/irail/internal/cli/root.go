...
type rootFlags struct {
	...
	// invocation. Mirrors the IRAIL_NO_LEARN env var.
	noLearn       bool
	selectFields  string
	configPath    string
	homePath      string
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "irail-pp-cli",
		Short: `Irail CLI — Every Belgian rail lookup the existing tools offer, plus transfer-risk, delay history and accessibility data that none…`,
		Long: `Irail CLI — Every Belgian rail lookup the existing tools offer, plus transfer-risk, delay history and accessibility data that none of them have.
...
Health check: run 'irail-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("irail-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'irail-pp-cli profile list')")
	...
	rootCmd.AddCommand(newTrainCmd(flags))
	...
	rootCmd.AddCommand(newNovelLeaveByCmd(flags))
	rootCmd.AddCommand(newNovelObserveCmd(flags))
	rootCmd.AddCommand(newNovelPunctualityCmd(flags))
	rootCmd.AddCommand(newNovelTransferRiskCmd(flags))
	rootCmd.AddCommand(newAPICmd(flags))
	rootCmd.AddCommand(newBoardPromotedCmd(flags))
	rootCmd.AddCommand(newDisruptionsPromotedCmd(flags))
	rootCmd.AddCommand(newLogsPromotedCmd(flags))
	rootCmd.AddCommand(newRoutePromotedCmd(flags))
	rootCmd.AddCommand(newStationsPromotedCmd(flags))
