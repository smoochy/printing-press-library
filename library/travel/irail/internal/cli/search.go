...
Run sync first to populate the local search index.`,
  ...
  irail-pp-cli search "status"

  # Force local search explicitly
  irail-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  irail-pp-cli search "status" --type board --data-source local
  # JSON output for piping
  irail-pp-cli search "status" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'irail-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "disruptions":
				results, err = db.SearchDisruptions(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchDisruptions(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search disruptions failed: %w", searchErr)
					}
