...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  irail-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  irail-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"disruptions", "logs", "route", "stations"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  irail-pp-cli workflow status

  # Show status as JSON
  irail-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("irail-pp-cli")
			}
