...
func TestSearchFetchQuotesFTSQuerySyntax(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if stored, failed, err := s.UpsertBatch("fetch", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	} else if failed != 0 || stored != len(items) {
		t.Fatalf("UpsertBatch stored=%d failed=%d, want stored=%d failed=0", stored, failed, len(items))
	}
	...
	for _, query := range []string{
		...
		results, err := s.SearchFetch(query, 10)
		if err != nil {
			t.Fatalf("SearchFetch(%q): %v", query, err)
		}
		if len(results) == 0 {
			t.Fatalf("SearchFetch(%q) returned no results", query)
		}
	}
}
...
// dispatches paginated items into both the generic resources table AND the
// typed web_search table. Regression for issue #268: before the fix, paginated
// syncs only filled the generic resources table, so domain commands that
// query the typed table saw zero rows.
func TestUpsertBatch_PopulatesWebSearchTable(t *testing.T) {
	...
	if _, _, err := s.UpsertBatch("web_search", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "web_search").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	if generic != len(items) {
		t.Fatalf("resources count = %d, want %d", generic, len(items))
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "web_search")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count web_search: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("web_search count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}

func TestSearchWebSearchQuotesFTSQuerySyntax(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if stored, failed, err := s.UpsertBatch("web_search", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	} else if failed != 0 || stored != len(items) {
		t.Fatalf("UpsertBatch stored=%d failed=%d, want stored=%d failed=0", stored, failed, len(items))
	}
	...
	for _, query := range []string{
		...
		results, err := s.SearchWebSearch(query, 10)
		if err != nil {
			t.Fatalf("SearchWebSearch(%q): %v", query, err)
		}
		if len(results) == 0 {
			t.Fatalf("SearchWebSearch(%q) returned no results", query)
		}
	}
}
