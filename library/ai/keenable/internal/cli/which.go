...
var whichIndex = []whichEntry{
	{Command: "research snapshot", Description: "Save an exact search and fetch run as an immutable local evidence snapshot.", Group: "Evidence that compounds", WhyItMatters: "Choose this when an agent needs to reproduce or audit the evidence behind a research answer."},
	{Command: "research replay", Description: "Rerun a saved research recipe and report how current evidence changed.", Group: "Evidence that compounds", WhyItMatters: "Choose this for weekly monitoring and regression checks where ranking drift matters."},
	{Command: "research citations", Description: "Export source-linked Markdown or JSON citations from a saved research run.", Group: "Agent-ready evidence", WhyItMatters: "Choose this when another agent or reviewer needs portable, inspectable evidence rather than raw API output."},
	{Command: "research fetch-many", Description: "Fetch a bounded URL list with concurrency limits and explicit partial failures.", Group: "Agent-ready evidence", WhyItMatters: "Choose this when a pipeline has several known sources and must not silently drop a failed fetch."},
	{Command: "research local-search", Description: "Search saved titles, snippets, URLs, and Markdown without spending an upstream request.", Group: "Evidence that compounds", WhyItMatters: "Choose this when an agent needs to recall prior evidence while preserving a clear local-versus-live boundary."},
	{Command: "research diff", Description: "Compare saved runs for URL changes, rank movement, metadata edits, and content-hash drift.", Group: "Evidence that compounds", WhyItMatters: "Choose this to detect changing sources before an agent relies on stale research."},
	{Command: "research coverage", Description: "Measure domain diversity, rank share, timestamp coverage, and missing metadata in a saved run.", Group: "Agent-ready evidence", WhyItMatters: "Choose this when an agent must explain whether its evidence is broad, balanced, and date-grounded."},
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one`,
		Example: `  keenable-pp-cli which "stale tickets"
  keenable-pp-cli which "bottleneck"
  keenable-pp-cli which --limit 1 "send message"
  keenable-pp-cli which                                # list the full capability index`,
