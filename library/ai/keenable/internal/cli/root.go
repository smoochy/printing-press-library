...
type rootFlags struct {
	...
	// invocation. Mirrors the KEENABLE_NO_LEARN env var.
	noLearn                 bool
	selectFields            string
	configPath              string
	homePath                string
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "keenable-pp-cli",
		Short: `Keenable CLI — Keenable web search with reproducible research, citations, and a local evidence trail.`,
		Long: `Keenable CLI — Keenable web search with reproducible research, citations, and a local evidence trail.
...
Health check: run 'keenable-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("keenable-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.selectFields, "select", "", "Comma-separated fields to include in output (e.g. --select author,content,description)")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'keenable-pp-cli profile list')")
