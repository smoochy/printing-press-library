...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  keenable-pp-cli doctor
  keenable-pp-cli doctor --json
  keenable-pp-cli doctor --fail-on warn
  keenable-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if credentialRefused {
				...
				if cfg != nil {
					...
					if !configured {
						...
						report["auth_hint"] = "Set your API key with: export KEENABLE_API_KEY=\"your-token-here\""
					} else {
						authConfigured = true
			...
			if os.Getenv("KEENABLE_API_KEY") != "" {
				authEnvSet = append(authEnvSet, "KEENABLE_API_KEY")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "KEENABLE_API_KEY")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "keenable-pp-cli", suggestion)
						} else {
