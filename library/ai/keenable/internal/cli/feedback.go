...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("KEENABLE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("KEENABLE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "keenable-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  keenable-pp-cli feedback 'The fetch output was useful'",
		Long: `Feedback is captured locally first in the CLI data directory's feedback.jsonl.
When ` + "`KEENABLE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`KEENABLE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  keenable-pp-cli feedback list
  keenable-pp-cli feedback list --limit 5
  keenable-pp-cli feedback list --json`,
