...
type Config struct {
	...
	KeenableApiKey   string          `toml:"api_key"`
}
...
func Load(configPath string) (*Config, error) {
	...
	// Env var overrides
	if v := os.Getenv("KEENABLE_API_KEY"); v != "" {
		cfg.KeenableApiKey = v
		cfg.markEnvOverride("KeenableApiKey")
		cfg.AuthSource = "env:KEENABLE_API_KEY"
		cfg.CredentialSource = "env:KEENABLE_API_KEY"
	}
	...
	if v := os.Getenv("KEENABLE_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("KEENABLE_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "keenable-pp-cli", "config.toml"), nil
}
...
func (c *Config) StoreScopeCredential() string {
	...
	if c.KeenableApiKey != "" {
		parts = append(parts, "api_key="+c.KeenableApiKey)
	}
	if len(parts) == 0 {
		return ""
	}
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		KeenableApiKey: c.KeenableApiKey,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	if c.KeenableApiKey == "" {
		c.KeenableApiKey = creds.KeenableApiKey
	}
}
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.KeenableApiKey = token
	delete(c.envOverrides, "KeenableApiKey")
	c.updateFileConfigField("KeenableApiKey")
	if err := c.saveCredentialsFirst(); err != nil {
		return err
	}
	return c.save()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["KeenableApiKey"] {
			out.KeenableApiKey = c.fileConfig.KeenableApiKey
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "KeenableApiKey":
		c.fileConfig.KeenableApiKey = c.KeenableApiKey
	}
}
