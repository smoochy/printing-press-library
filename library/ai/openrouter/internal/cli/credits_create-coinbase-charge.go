...
func newCreditsCreateCoinbaseChargeCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:         "create-coinbase-charge",
		Aliases:     []string{"create"},
		Short:       "Deprecated.",
		Example:     "  openrouter-pp-cli credits create-coinbase-charge",
		Annotations: map[string]string{"pp:endpoint": "credits.create-coinbase-charge", "pp:method": "POST", "pp:path": "/credits/coinbase"},
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !flags.dryRun && statusCode >= 200 && statusCode < 300 {
				...
				if partialFailure != nil {
					fmt.Fprintf(os.Stderr, "warning: partial failure detected in %s response: %s\n", "credits", partialFailure.Message)
			...
			if !flags.dryRun && statusCode >= 200 && statusCode < 300 && (partialFailure == nil || flags.allowPartialFailure) {
				writeMutationResponseToStore(cmd.Context(), "credits", data, "")
			}
				...
				if json.Unmarshal(data, &items) == nil && len(items) > 0 {
					if err := printAutoTable(cmd.OutOrStdout(), items); err != nil {
						...
						if partialFailure != nil && !flags.allowPartialFailure {
							return partialFailureErr(fmt.Errorf("partial failure in %s response: %s", "credits", partialFailure.Message))
						}
						return nil
					}
				} else {
