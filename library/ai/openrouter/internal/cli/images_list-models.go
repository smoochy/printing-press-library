...
func newImagesListModelsCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:         "list-models",
		Aliases:     []string{"list"},
		Short:       "Lists every image generation model with its top-level supported-parameter superset and a URL to its full per-endpoint",
		Example:     "  openrouter-pp-cli images list-models",
		Annotations: map[string]string{"pp:endpoint": "images.list-models", "pp:method": "GET", "pp:path": "/images/models", "mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			data, prov, err := resolveReadWithStrategyAndResponsePath(cmd.Context(), c, flags, "auto", "images", false, path, params, headerOverrides, "data", cmd.ErrOrStderr())
			if err != nil {
			...
			if flags.asJSON || (!isTerminal(cmd.OutOrStdout()) && !flags.csv && !flags.quiet && !flags.plain) {
				...
				if flags.selectFields != "" {
					...
					filtered = compactFields(filtered, map[string]bool{"architecture": true, "created": true, "description": true, "endpoints": true, "id": true, "name": true, "supported_parameters": true, "supports_streaming": true})
				}
			...
			return printOutputWithFlagsMeta(cmd.OutOrStdout(), formatData, flags, map[string]any{"source": "live"}, map[string]bool{"architecture": true, "created": true, "description": true, "endpoints": true, "id": true, "name": true, "supported_parameters": true, "supports_streaming": true})
		},
	}
