...
func newScimListGroupMappingsCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Aliases:     []string{"list"},
		Short:       "List SCIM group-to-workspace mappings for the organization.",
		Example:     "  openrouter-pp-cli scim list-group-mappings",
		Annotations: map[string]string{"pp:endpoint": "scim.list-group-mappings", "pp:method": "GET", "pp:path": "/scim/group-mappings", "mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			data, prov, err := resolvePaginatedReadWithStrategy(cmd.Context(), c, flags, "auto", "scim", path, map[string]string{
			...
			if flags.asJSON || (!isTerminal(cmd.OutOrStdout()) && !flags.csv && !flags.quiet && !flags.plain) {
				...
				if flags.selectFields != "" {
					...
					filtered = compactFields(filtered, map[string]bool{"created_at": true, "id": true, "organization_id": true, "role": true, "scim_group_id": true, "updated_at": true, "workspace_id": true})
				}
			...
			return printOutputWithFlagsMeta(cmd.OutOrStdout(), formatData, flags, map[string]any{"source": "live"}, map[string]bool{"created_at": true, "id": true, "organization_id": true, "role": true, "scim_group_id": true, "updated_at": true, "workspace_id": true})
		},
	}
