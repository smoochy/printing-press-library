...
func newWorkspacesMembersBulkAddWorkspaceCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:         "bulk-add-workspace <id>",
		Aliases:     []string{"create"},
		Short:       "Add multiple organization members to a workspace. Members are assigned the same role they hold in the organization.",
		Example:     "  openrouter-pp-cli workspaces members bulk-add-workspace production",
		Annotations: map[string]string{"pp:endpoint": "members.bulk-add-workspace", "pp:method": "POST", "pp:path": "/workspaces/{id}/members/add"},
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !stdinBody {
				if !cmd.Flags().Changed("user-ids") && bodyUserIds == "" && !flags.dryRun {
			...
			if stdinBody {
				...
				if cmd.Flags().Changed("user-ids") {
					parsedUserIds, parseErr := cliutil.ParseStringList(bodyUserIds)
					if parseErr != nil {
						return fmt.Errorf("parsing --user-ids list: %w", parseErr)
					}
					bodyMap["user_ids"] = parsedUserIds
				}
			}
			...
			if !flags.dryRun && statusCode >= 200 && statusCode < 300 && (partialFailure == nil || flags.allowPartialFailure) {
				writeMutationResponseToStore(cmd.Context(), "members", data, "data")
			}
			...
			if flags.asJSON || (!isTerminal(cmd.OutOrStdout()) && !flags.csv && !flags.quiet && !flags.plain) {
				...
				if flags.selectFields != "" {
					...
					filtered = compactFields(filtered, map[string]bool{"created_at": true, "id": true, "role": true, "user_id": true, "workspace_id": true})
				}
				if len(filtered) > 0 {
					var parsed any
	...
	cmd.Flags().StringVar(&bodyUserIds, "user-ids", "", "List of user IDs to add to the workspace. Members are assigned the same role they hold in the organization.")
