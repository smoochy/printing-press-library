...
func newGenerationSubmitFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	var bodyGenerationId string
	var stdinBody bool

	cmd := &cobra.Command{
		Use:         "submit-feedback",
		Aliases:     []string{"create"},
		Short:       "Submit structured feedback on a generation the authenticated user made.",
		Example:     "  openrouter-pp-cli generation submit-feedback --category incorrect_response",
		Annotations: map[string]string{"pp:endpoint": "generation.submit-feedback", "pp:method": "POST", "pp:path": "/generation/feedback"},
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !stdinBody {
				if !cmd.Flags().Changed("category") && bodyCategory == "" && !flags.dryRun {
					return fmt.Errorf("required flag \"%s\" not set", "category")
				}
				if !cmd.Flags().Changed("generation-id") && bodyGenerationId == "" && !flags.dryRun {
					return fmt.Errorf("required flag \"%s\" not set", "generation-id")
				}
			}
			...
			if stdinBody {
				...
				if cmd.Flags().Changed("generation-id") || bodyGenerationId != "" {
					bodyMap["generation_id"] = bodyGenerationId
				}
			}
			...
			if !flags.dryRun && statusCode >= 200 && statusCode < 300 {
				...
				if partialFailure != nil {
					fmt.Fprintf(os.Stderr, "warning: partial failure detected in %s response: %s\n", "generation", partialFailure.Message)
			...
			if !flags.dryRun && statusCode >= 200 && statusCode < 300 && (partialFailure == nil || flags.allowPartialFailure) {
				writeMutationResponseToStore(cmd.Context(), "generation", data, "")
			}
				...
				if json.Unmarshal(data, &items) == nil && len(items) > 0 {
					if err := printAutoTable(cmd.OutOrStdout(), items); err != nil {
						...
						if partialFailure != nil && !flags.allowPartialFailure {
							return partialFailureErr(fmt.Errorf("partial failure in %s response: %s", "generation", partialFailure.Message))
						}
						return nil
					}
				} else {
	...
	cmd.Flags().StringVar(&bodyCategory, "category", "", "The category of feedback being reported")
	cmd.Flags().StringVar(&bodyComment, "comment", "", "An optional free-text comment describing the feedback")
	cmd.Flags().StringVar(&bodyGenerationId, "generation-id", "", "The generation to submit feedback on")
