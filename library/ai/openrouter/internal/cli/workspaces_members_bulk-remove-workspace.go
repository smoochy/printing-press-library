...
func newWorkspacesMembersBulkRemoveWorkspaceCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:         "bulk-remove-workspace <id>",
		Short:       "Remove multiple members from a workspace. Members with active API keys in the workspace cannot be removed.",
		Example:     "  openrouter-pp-cli workspaces members bulk-remove-workspace production",
		Annotations: map[string]string{"pp:endpoint": "members.bulk-remove-workspace", "pp:method": "POST", "pp:path": "/workspaces/{id}/members/remove"},
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if flags.asJSON || (!isTerminal(cmd.OutOrStdout()) && !flags.csv && !flags.quiet && !flags.plain) {
				...
				if flags.selectFields != "" {
					...
					filtered = compactFields(filtered, map[string]bool{"removed_count": true})
				}
				if len(filtered) > 0 {
					var parsed any
	...
	cmd.Flags().StringVar(&bodyUserIds, "user-ids", "", "List of user IDs to remove from the workspace")
