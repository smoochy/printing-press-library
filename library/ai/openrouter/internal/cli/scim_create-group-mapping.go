...
func newScimCreateGroupMappingCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Short:       "Create a SCIM group-to-workspace role mapping.",
		Example:     "  openrouter-pp-cli scim create-group-mapping --role admin",
		Annotations: map[string]string{"pp:endpoint": "scim.create-group-mapping", "pp:method": "POST", "pp:path": "/scim/group-mappings"},
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !stdinBody {
				if !cmd.Flags().Changed("role") && bodyRole == "" && !flags.dryRun {
					return fmt.Errorf("required flag \"%s\" not set", "role")
				}
				if !cmd.Flags().Changed("scim-group-id") && bodyScimGroupId == "" && !flags.dryRun {
					return fmt.Errorf("required flag \"%s\" not set", "scim-group-id")
				}
				if !cmd.Flags().Changed("workspace-id") && bodyWorkspaceId == "" && !flags.dryRun {
			...
			if stdinBody {
				...
				if cmd.Flags().Changed("scim-group-id") || bodyScimGroupId != "" {
					bodyMap["scim_group_id"] = bodyScimGroupId
				}
	...
	cmd.Flags().StringVar(&bodyScimGroupId, "scim-group-id", "", "Scim group id")
