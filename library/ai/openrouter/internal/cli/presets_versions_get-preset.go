...
func newPresetsVersionsGetPresetCmd(flags *rootFlags) *cobra.Command {
	var flagHTTPReferer string
	var flagXOpenRouterTitle string
	var flagXOpenRouterCategories string

	cmd := &cobra.Command{
		Use:         "get-preset <slug> <version>",
		Aliases:     []string{"get"},
		Short:       "Retrieves a specific version of a preset by its slug and version number.",
		Example:     "  openrouter-pp-cli presets versions get-preset my-preset 1",
		Annotations: map[string]string{"pp:endpoint": "versions.get-preset", "pp:method": "GET", "pp:path": "/presets/{slug}/versions/{version}", "mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				...
				if flags.asJSON {
					if printErr := printJSONFiltered(cmd.OutOrStdout(), map[string]any{
						...
						"usage": fmt.Sprintf("%s%s", cmd.CommandPath(), " <slug> <version>"),
					}, flags); printErr != nil {
						return printErr
					}
				}
				return usageErr(fmt.Errorf("missing required argument\nUsage: %s%s", cmd.CommandPath(), " <slug> <version>"))
			}
			...
			data, prov, err := resolveReadWithStrategyAndResponsePath(cmd.Context(), c, flags, "live", "versions", false, path, params, headerOverrides, "", cmd.ErrOrStderr())
			if err != nil {
