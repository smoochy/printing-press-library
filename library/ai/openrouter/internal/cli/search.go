...
Run sync first to populate the local search index.`,
  ...
  openrouter-pp-cli search "status"

  # Force local search explicitly
  openrouter-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  openrouter-pp-cli search "status" --type activity --data-source local
  # JSON output for piping
  openrouter-pp-cli search "status" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			switch resourceType {
			case "embeddings":
				results, err = db.SearchEmbeddings(query, limit)
			case "endpoints":
				results, err = db.SearchEndpoints(query, limit)
			case "guardrails":
				results, err = db.SearchGuardrails(query, limit)
			case "images":
				results, err = db.SearchImages(query, limit)
			case "keys":
				results, err = db.SearchKeys(query, limit)
			case "models":
				results, err = db.SearchModels(query, limit)
			case "presets":
				results, err = db.SearchPresets(query, limit)
			case "videos":
				results, err = db.SearchVideos(query, limit)
			case "workspaces":
				results, err = db.SearchWorkspaces(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchEmbeddings(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search embeddings failed: %w", searchErr)
					}
				...
				{
					partial, searchErr := db.SearchGuardrails(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search guardrails failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchImages(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search images failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchPresets(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search presets failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchWorkspaces(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search workspaces failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
