...
func TestMigrate_AddsColumnsOnUpgrade_Activity(t *testing.T) {
	...
	rows, err := s.DB().Query(`SELECT name FROM pragma_table_xinfo(?)`, "activity")
	if err != nil {
	...
	for _, wantIdx := range []string{
		"idx_activity_endpoint_id",
		"idx_activity_workspace_id",
	} {
		var got string
		if err := s.DB().QueryRow(
			...
			t.Fatalf("%s index missing from activity after migrate: %v", wantIdx, err)
		}
	}
}
...
func TestMigrate_AddsColumnsOnUpgrade_Benchmarks(t *testing.T) {
	...
	// table_info hides generated columns; table_xinfo is the verify
	// surface for VIRTUAL backfills such as bare_id.
	rows, err := s.DB().Query(`SELECT name FROM pragma_table_xinfo(?)`, "benchmarks")
	if err != nil {
		t.Fatalf("table_xinfo: %v", err)
	}
	defer rows.Close()
	...
	for _, wantIdx := range []string{} {
		...
		if err := s.DB().QueryRow(
			...
			t.Fatalf("%s index missing from benchmarks after migrate: %v", wantIdx, err)
		}
	}
}
...
func TestMigrate_AddsColumnsOnUpgrade_ContainersFiles(t *testing.T) {
	...
	// table_info hides generated columns; table_xinfo is the verify
	// surface for VIRTUAL backfills such as bare_id.
	rows, err := s.DB().Query(`SELECT name FROM pragma_table_xinfo(?)`, "containers_files")
	if err != nil {
		t.Fatalf("table_xinfo: %v", err)
	}
	defer rows.Close()
	...
	for _, wantIdx := range []string{
		"idx_containers_files_containers_id",
		"idx_containers_files_bare_id",
	} {
		var got string
		if err := s.DB().QueryRow(
			`SELECT name FROM sqlite_master WHERE type='index' AND name=?`,
			...
			t.Fatalf("%s index missing from containers_files after migrate: %v", wantIdx, err)
		}
	}
}
...
func TestMigrate_AddsColumnsOnUpgrade_Datasets(t *testing.T) {
	...
	// table_info hides generated columns; table_xinfo is the verify
	// surface for VIRTUAL backfills such as bare_id.
	rows, err := s.DB().Query(`SELECT name FROM pragma_table_xinfo(?)`, "datasets")
	if err != nil {
		t.Fatalf("table_xinfo: %v", err)
	}
	defer rows.Close()
	...
	for _, wantIdx := range []string{
		...
		if err := s.DB().QueryRow(
			...
			t.Fatalf("%s index missing from datasets after migrate: %v", wantIdx, err)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_Embeddings verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_Embeddings(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "embeddings" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// table_info hides generated columns; table_xinfo is the verify
	// surface for VIRTUAL backfills such as bare_id.
	rows, err := s.DB().Query(`SELECT name FROM pragma_table_xinfo(?)`, "embeddings")
	if err != nil {
		t.Fatalf("table_xinfo: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from embeddings after migrate", want)
		}
	}

	for _, wantIdx := range []string{
		"idx_embeddings_hugging_face_id",
	} {
		var got string
		if err := s.DB().QueryRow(
			`SELECT name FROM sqlite_master WHERE type='index' AND name=?`,
			...
			t.Fatalf("%s index missing from embeddings after migrate: %v", wantIdx, err)
		}
	}
}
...
func TestMigrate_AddsColumnsOnUpgrade_Endpoints(t *testing.T) {
	...
	// table_info hides generated columns; table_xinfo is the verify
	// surface for VIRTUAL backfills such as bare_id.
	rows, err := s.DB().Query(`SELECT name FROM pragma_table_xinfo(?)`, "endpoints")
	if err != nil {
		t.Fatalf("table_xinfo: %v", err)
	}
	defer rows.Close()
	...
	for _, wantIdx := range []string{
		...
		if err := s.DB().QueryRow(
			...
			t.Fatalf("%s index missing from endpoints after migrate: %v", wantIdx, err)
		}
	}
}
...
func TestMigrate_AddsColumnsOnUpgrade_FilesContent(t *testing.T) {
	...
	// table_info hides generated columns; table_xinfo is the verify
	// surface for VIRTUAL backfills such as bare_id.
	rows, err := s.DB().Query(`SELECT name FROM pragma_table_xinfo(?)`, "files_content")
	if err != nil {
		t.Fatalf("table_xinfo: %v", err)
	}
	defer rows.Close()
	...
	for _, wantIdx := range []string{
		"idx_files_content_files_id",
		"idx_files_content_bare_id",
	} {
		var got string
		if err := s.DB().QueryRow(
			`SELECT name FROM sqlite_master WHERE type='index' AND name=?`,
			...
			t.Fatalf("%s index missing from files_content after migrate: %v", wantIdx, err)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_Guardrails verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_Guardrails(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "guardrails" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// table_info hides generated columns; table_xinfo is the verify
	// surface for VIRTUAL backfills such as bare_id.
	rows, err := s.DB().Query(`SELECT name FROM pragma_table_xinfo(?)`, "guardrails")
	if err != nil {
		t.Fatalf("table_xinfo: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from guardrails after migrate", want)
		}
	}

	for _, wantIdx := range []string{
		"idx_guardrails_workspace_id",
		"idx_guardrails_guardrail_id",
		"idx_guardrails_organization_id",
		"idx_guardrails_user_id",
		"idx_guardrails_created_at",
		"idx_guardrails_updated_at",
	} {
		var got string
		if err := s.DB().QueryRow(
			`SELECT name FROM sqlite_master WHERE type='index' AND name=?`,
			...
			t.Fatalf("%s index missing from guardrails after migrate: %v", wantIdx, err)
		}
	}
}
...
func TestMigrate_AddsColumnsOnUpgrade_Keys(t *testing.T) {
	...
	// table_info hides generated columns; table_xinfo is the verify
	// surface for VIRTUAL backfills such as bare_id.
	rows, err := s.DB().Query(`SELECT name FROM pragma_table_xinfo(?)`, "keys")
	if err != nil {
		t.Fatalf("table_xinfo: %v", err)
	}
	defer rows.Close()
	...
	for _, wantIdx := range []string{
		"idx_keys_creator_user_id",
		"idx_keys_workspace_id",
		"idx_keys_created_at",
		"idx_keys_updated_at",
	} {
		var got string
		if err := s.DB().QueryRow(
			...
			t.Fatalf("%s index missing from keys after migrate: %v", wantIdx, err)
		}
	}
}
...
func TestMigrate_AddsColumnsOnUpgrade_Messages(t *testing.T) {
	...
	// table_info hides generated columns; table_xinfo is the verify
	// surface for VIRTUAL backfills such as bare_id.
	rows, err := s.DB().Query(`SELECT name FROM pragma_table_xinfo(?)`, "messages")
	if err != nil {
		t.Fatalf("table_xinfo: %v", err)
	}
	defer rows.Close()
	...
	for _, wantIdx := range []string{} {
		...
		if err := s.DB().QueryRow(
			...
			t.Fatalf("%s index missing from messages after migrate: %v", wantIdx, err)
		}
	}
}
...
func TestMigrate_AddsColumnsOnUpgrade_ModelsEndpoints(t *testing.T) {
	...
	// table_info hides generated columns; table_xinfo is the verify
	// surface for VIRTUAL backfills such as bare_id.
	rows, err := s.DB().Query(`SELECT name FROM pragma_table_xinfo(?)`, "models_endpoints")
	if err != nil {
		t.Fatalf("table_xinfo: %v", err)
	}
	defer rows.Close()
	...
	for _, wantIdx := range []string{
		"idx_models_endpoints_models_id",
		"idx_models_endpoints_bare_id",
	} {
		var got string
		if err := s.DB().QueryRow(
			`SELECT name FROM sqlite_master WHERE type='index' AND name=?`,
			...
			t.Fatalf("%s index missing from models_endpoints after migrate: %v", wantIdx, err)
		}
	}
}
...
func TestMigrate_AddsColumnsOnUpgrade_OpenrouterAuth(t *testing.T) {
	...
	// table_info hides generated columns; table_xinfo is the verify
	// surface for VIRTUAL backfills such as bare_id.
	rows, err := s.DB().Query(`SELECT name FROM pragma_table_xinfo(?)`, "openrouter_auth")
	if err != nil {
		t.Fatalf("table_xinfo: %v", err)
	}
	defer rows.Close()
	...
	for _, wantIdx := range []string{
		...
		if err := s.DB().QueryRow(
			...
			t.Fatalf("%s index missing from openrouter_auth after migrate: %v", wantIdx, err)
		}
	}
}
...
func TestMigrate_AddsColumnsOnUpgrade_Organization(t *testing.T) {
	...
	// table_info hides generated columns; table_xinfo is the verify
	// surface for VIRTUAL backfills such as bare_id.
	rows, err := s.DB().Query(`SELECT name FROM pragma_table_xinfo(?)`, "organization")
	if err != nil {
		t.Fatalf("table_xinfo: %v", err)
	}
	defer rows.Close()
	...
	for _, wantIdx := range []string{} {
		...
		if err := s.DB().QueryRow(
			...
			t.Fatalf("%s index missing from organization after migrate: %v", wantIdx, err)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_Presets verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_Presets(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "presets" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// table_info hides generated columns; table_xinfo is the verify
	// surface for VIRTUAL backfills such as bare_id.
	rows, err := s.DB().Query(`SELECT name FROM pragma_table_xinfo(?)`, "presets")
	if err != nil {
		t.Fatalf("table_xinfo: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from presets after migrate", want)
		}
	}

	for _, wantIdx := range []string{
		"idx_presets_creator_user_id",
		"idx_presets_designated_version_id",
		"idx_presets_workspace_id",
		"idx_presets_created_at",
		...
		if err := s.DB().QueryRow(
			...
			t.Fatalf("%s index missing from presets after migrate: %v", wantIdx, err)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_PresetsChat verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_PresetsChat(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "presets_chat" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// table_info hides generated columns; table_xinfo is the verify
	// surface for VIRTUAL backfills such as bare_id.
	rows, err := s.DB().Query(`SELECT name FROM pragma_table_xinfo(?)`, "presets_chat")
	if err != nil {
		t.Fatalf("table_xinfo: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from presets_chat after migrate", want)
		}
	}

	for _, wantIdx := range []string{
		"idx_presets_chat_presets_id",
		"idx_presets_chat_bare_id",
	} {
		var got string
		if err := s.DB().QueryRow(
			`SELECT name FROM sqlite_master WHERE type='index' AND name=?`,
			...
			t.Fatalf("%s index missing from presets_chat after migrate: %v", wantIdx, err)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_PresetsMessages verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_PresetsMessages(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "presets_messages" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// table_info hides generated columns; table_xinfo is the verify
	// surface for VIRTUAL backfills such as bare_id.
	rows, err := s.DB().Query(`SELECT name FROM pragma_table_xinfo(?)`, "presets_messages")
	if err != nil {
		t.Fatalf("table_xinfo: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from presets_messages after migrate", want)
		}
	}

	for _, wantIdx := range []string{
		"idx_presets_messages_presets_id",
		"idx_presets_messages_bare_id",
	} {
		var got string
		if err := s.DB().QueryRow(
			`SELECT name FROM sqlite_master WHERE type='index' AND name=?`,
			...
			t.Fatalf("%s index missing from presets_messages after migrate: %v", wantIdx, err)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_PresetsResponses verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_PresetsResponses(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "presets_responses" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// table_info hides generated columns; table_xinfo is the verify
	// surface for VIRTUAL backfills such as bare_id.
	rows, err := s.DB().Query(`SELECT name FROM pragma_table_xinfo(?)`, "presets_responses")
	if err != nil {
		t.Fatalf("table_xinfo: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from presets_responses after migrate", want)
		}
	}

	for _, wantIdx := range []string{
		"idx_presets_responses_presets_id",
		"idx_presets_responses_bare_id",
	} {
		var got string
		if err := s.DB().QueryRow(
			`SELECT name FROM sqlite_master WHERE type='index' AND name=?`,
			...
			t.Fatalf("%s index missing from presets_responses after migrate: %v", wantIdx, err)
		}
	}
}
...
func TestMigrate_AddsColumnsOnUpgrade_Versions(t *testing.T) {
	...
	for _, wantIdx := range []string{
		"idx_versions_presets_id",
		"idx_versions_parent_id",
...
func TestMigrate_AddsColumnsOnUpgrade_Providers(t *testing.T) {
	...
	// table_info hides generated columns; table_xinfo is the verify
	// surface for VIRTUAL backfills such as bare_id.
	rows, err := s.DB().Query(`SELECT name FROM pragma_table_xinfo(?)`, "providers")
	if err != nil {
		t.Fatalf("table_xinfo: %v", err)
	}
	defer rows.Close()
	...
	for _, wantIdx := range []string{} {
		...
		if err := s.DB().QueryRow(
			...
			t.Fatalf("%s index missing from providers after migrate: %v", wantIdx, err)
		}
	}
}
...
func TestMigrate_AddsColumnsOnUpgrade_Responses(t *testing.T) {
	...
	// table_info hides generated columns; table_xinfo is the verify
	// surface for VIRTUAL backfills such as bare_id.
	rows, err := s.DB().Query(`SELECT name FROM pragma_table_xinfo(?)`, "responses")
	if err != nil {
		t.Fatalf("table_xinfo: %v", err)
	}
	defer rows.Close()
	...
	for _, wantIdx := range []string{
		...
		if err := s.DB().QueryRow(
			...
			t.Fatalf("%s index missing from responses after migrate: %v", wantIdx, err)
		}
	}
}
...
func TestMigrate_AddsColumnsOnUpgrade_Scim(t *testing.T) {
	...
	// table_info hides generated columns; table_xinfo is the verify
	// surface for VIRTUAL backfills such as bare_id.
	rows, err := s.DB().Query(`SELECT name FROM pragma_table_xinfo(?)`, "scim")
	if err != nil {
		t.Fatalf("table_xinfo: %v", err)
	}
	defer rows.Close()
	...
	for _, wantIdx := range []string{
		"idx_scim_organization_id",
		"idx_scim_scim_group_id",
		"idx_scim_workspace_id",
		"idx_scim_external_id",
		"idx_scim_created_at",
		"idx_scim_updated_at",
	} {
		var got string
		if err := s.DB().QueryRow(
			...
			t.Fatalf("%s index missing from scim after migrate: %v", wantIdx, err)
		}
	}
}
...
func TestMigrate_AddsColumnsOnUpgrade_Videos(t *testing.T) {
	...
	// table_info hides generated columns; table_xinfo is the verify
	// surface for VIRTUAL backfills such as bare_id.
	rows, err := s.DB().Query(`SELECT name FROM pragma_table_xinfo(?)`, "videos")
	if err != nil {
		t.Fatalf("table_xinfo: %v", err)
	}
	defer rows.Close()
	...
	for _, wantIdx := range []string{
		...
		if err := s.DB().QueryRow(
			...
			t.Fatalf("%s index missing from videos after migrate: %v", wantIdx, err)
		}
	}
}
...
func TestMigrate_AddsColumnsOnUpgrade_VideosContent(t *testing.T) {
	...
	// table_info hides generated columns; table_xinfo is the verify
	// surface for VIRTUAL backfills such as bare_id.
	rows, err := s.DB().Query(`SELECT name FROM pragma_table_xinfo(?)`, "videos_content")
	if err != nil {
		t.Fatalf("table_xinfo: %v", err)
	}
	defer rows.Close()
	...
	for _, wantIdx := range []string{
		"idx_videos_content_videos_id",
		"idx_videos_content_bare_id",
	} {
		var got string
		if err := s.DB().QueryRow(
			`SELECT name FROM sqlite_master WHERE type='index' AND name=?`,
			...
			t.Fatalf("%s index missing from videos_content after migrate: %v", wantIdx, err)
		}
	}
}
...
func TestMigrate_AddsColumnsOnUpgrade_Workspaces(t *testing.T) {
	...
	// table_info hides generated columns; table_xinfo is the verify
	// surface for VIRTUAL backfills such as bare_id.
	rows, err := s.DB().Query(`SELECT name FROM pragma_table_xinfo(?)`, "workspaces")
	if err != nil {
		t.Fatalf("table_xinfo: %v", err)
	}
	defer rows.Close()
	...
	for _, wantIdx := range []string{
		"idx_workspaces_default_guardrail_id",
		...
		if err := s.DB().QueryRow(
			...
			t.Fatalf("%s index missing from workspaces after migrate: %v", wantIdx, err)
		}
	}
}
...
func TestMigrate_AddsColumnsOnUpgrade_Budgets(t *testing.T) {
	...
	// table_info hides generated columns; table_xinfo is the verify
	// surface for VIRTUAL backfills such as bare_id.
	rows, err := s.DB().Query(`SELECT name FROM pragma_table_xinfo(?)`, "budgets")
	if err != nil {
		t.Fatalf("table_xinfo: %v", err)
	}
	defer rows.Close()
	...
	for _, wantIdx := range []string{
		...
		if err := s.DB().QueryRow(
			...
			t.Fatalf("%s index missing from budgets after migrate: %v", wantIdx, err)
		}
	}
}
...
func TestMigrate_AddsColumnsOnUpgrade_Members(t *testing.T) {
	...
	// table_info hides generated columns; table_xinfo is the verify
	// surface for VIRTUAL backfills such as bare_id.
	rows, err := s.DB().Query(`SELECT name FROM pragma_table_xinfo(?)`, "members")
	if err != nil {
		t.Fatalf("table_xinfo: %v", err)
	}
	defer rows.Close()
	...
	for _, wantIdx := range []string{
		...
		if err := s.DB().QueryRow(
			...
			t.Fatalf("%s index missing from members after migrate: %v", wantIdx, err)
		}
	}
}
