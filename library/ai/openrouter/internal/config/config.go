...
func Load(configPath string) (*Config, error) {
	...
	if v := os.Getenv("OPENROUTER_API_KEY"); v != "" {
		...
		cfg.markEnvOverride("OpenrouterApiKey")
		cfg.AuthSource = "env:OPENROUTER_API_KEY"
		cfg.CredentialSource = "env:OPENROUTER_API_KEY"
	}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("OPENROUTER_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "openrouter-pp-cli", "config.toml"), nil
}
...
func (c *Config) StoreScopeCredential() string {
	...
	if c.OpenrouterApiKey != "" {
		parts = append(parts, "api_key="+c.OpenrouterApiKey)
	}
	if len(parts) == 0 {
		return ""
	}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	if c.OpenrouterApiKey == "" {
		c.OpenrouterApiKey = creds.OpenrouterApiKey
	}
}
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.OpenrouterApiKey = ""
	delete(c.envOverrides, "OpenrouterApiKey")
	c.updateFileConfigField("OpenrouterApiKey")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["OpenrouterApiKey"] {
			out.OpenrouterApiKey = c.fileConfig.OpenrouterApiKey
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "OpenrouterApiKey":
		c.fileConfig.OpenrouterApiKey = c.OpenrouterApiKey
	}
}
