...
func TestSearchModelQuotesFTSQuerySyntax(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if stored, failed, err := s.UpsertBatch("model", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	} else if failed != 0 || stored != len(items) {
		t.Fatalf("UpsertBatch stored=%d failed=%d, want stored=%d failed=0", stored, failed, len(items))
	}
	...
	for _, query := range []string{
		...
		results, err := s.SearchModel(query, 10)
		if err != nil {
			t.Fatalf("SearchModel(%q): %v", query, err)
		}
		if len(results) == 0 {
			t.Fatalf("SearchModel(%q) returned no results", query)
		}
	}
}

// TestUpsertBatch_PopulatesTtsTable verifies that UpsertBatch
// dispatches paginated items into both the generic resources table AND the
// typed tts table. Regression for issue #268: before the fix, paginated
// syncs only filled the generic resources table, so domain commands that
// query the typed table saw zero rows.
func TestUpsertBatch_PopulatesTtsTable(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if _, _, err := s.UpsertBatch("tts", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "tts").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	if generic != len(items) {
		t.Fatalf("resources count = %d, want %d", generic, len(items))
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "tts")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count tts: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("tts count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}

// TestUpsertBatch_PopulatesVoiceDesignTable verifies that UpsertBatch
// dispatches paginated items into both the generic resources table AND the
// typed voice_design table. Regression for issue #268: before the fix, paginated
// syncs only filled the generic resources table, so domain commands that
// query the typed table saw zero rows.
func TestUpsertBatch_PopulatesVoiceDesignTable(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if _, _, err := s.UpsertBatch("voice-design", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "voice-design").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	if generic != len(items) {
		t.Fatalf("resources count = %d, want %d", generic, len(items))
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "voice_design")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count voice_design: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("voice_design count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}

// TestUpsertBatch_PopulatesApiCreditTable verifies that UpsertBatch
// dispatches paginated items into both the generic resources table AND the
// typed api_credit table. Regression for issue #268: before the fix, paginated
// syncs only filled the generic resources table, so domain commands that
// query the typed table saw zero rows.
func TestUpsertBatch_PopulatesApiCreditTable(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	items := []json.RawMessage{
		json.RawMessage(`{"id": "test-001", "wallet_id": "test-parent-001"}`),
		json.RawMessage(`{"id": "test-002", "wallet_id": "test-parent-001"}`),
		json.RawMessage(`{"id": "test-003", "wallet_id": "test-parent-001"}`),
	}
	if _, _, err := s.UpsertBatch("api_credit", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "api_credit").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	if generic != len(items) {
		t.Fatalf("resources count = %d, want %d", generic, len(items))
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "api_credit")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count api_credit: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("api_credit count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}

// TestUpsertBatch_TypedFailureDoesNotStrandApiCreditGeneric exercises
...
func TestUpsertBatch_TypedFailureDoesNotStrandApiCreditGeneric(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	// Items deliberately omit "wallet_id" so the typed NOT NULL
	// constraint fires.
	items := []json.RawMessage{
	...
	stored, extractFailures, err := s.UpsertBatch("api_credit", items)
	if err != nil {
	...
	if typed != 0 {
		t.Fatalf("api_credit count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "wallet_id")
	}
}

// TestUpsertBatch_KeysApiCreditByChildAndParent verifies dependent
...
func TestUpsertBatch_KeysApiCreditByChildAndParent(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	items := []json.RawMessage{
		json.RawMessage(`{"id": "shared_child", "wallet_id": "parent_A"}`),
		json.RawMessage(`{"id": "shared", "wallet_id": "child_parent_A"}`),
		json.RawMessage(`{"id": "shared_child", "wallet_id": "parent_B"}`),
		json.RawMessage(`{"id": "other_child", "wallet_id": "parent_A"}`),
	}
	stored, extractFailures, err := s.UpsertBatch("api_credit", items)
	if err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	...
	if typed != len(items) {
		t.Fatalf("api_credit count = %d, want %d (typed rows collapsed on child id)", typed, len(items))
	}

	var parentMatches int
	parentQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s" WHERE "wallet_id" = ?`, "api_credit")
	if err := db.QueryRow(parentQuery, "parent_A").Scan(&parentMatches); err != nil {
		t.Fatalf("count by wallet_id: %v", err)
	}
	if parentMatches != 2 {
		t.Fatalf("wallet_id=parent_A count = %d, want 2", parentMatches)
	}
}

// TestUpsertBatch_PopulatesPackageTable verifies that UpsertBatch
// dispatches paginated items into both the generic resources table AND the
// typed package table. Regression for issue #268: before the fix, paginated
// syncs only filled the generic resources table, so domain commands that
// query the typed table saw zero rows.
func TestUpsertBatch_PopulatesPackageTable(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if _, _, err := s.UpsertBatch("package", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "package").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	if generic != len(items) {
		t.Fatalf("resources count = %d, want %d", generic, len(items))
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "package")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count package: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("package count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}

// TestUpsertBatch_TypedFailureDoesNotStrandPackageGeneric exercises
// the savepoint isolation around the typed-table dispatch. The fixture omits
// the NOT NULL parent FK column so the typed insert fails; the savepoint
// rolls back only the typed projection. The generic resources row inserted
// just before must survive. Regression for issue #1392, where a single
// outer transaction caused typed-table failures to cascade and silently
// discard every successfully fetched API row.
func TestUpsertBatch_TypedFailureDoesNotStrandPackageGeneric(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	stored, extractFailures, err := s.UpsertBatch("package", items)
	if err != nil {
		t.Fatalf("UpsertBatch: %v (typed-table failure must not propagate)", err)
	}
	if stored != len(items) {
	...
	if typed != 0 {
		t.Fatalf("package count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "wallet_id")
	}
}

// TestUpsertBatch_KeysPackageByChildAndParent verifies dependent
// sub-collection rows with the same child id but different parent ids do not
// overwrite one another in either the generic resources table or the typed
// table. Regression for issue #2471.
func TestUpsertBatch_KeysPackageByChildAndParent(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if typed != len(items) {
		t.Fatalf("package count = %d, want %d (typed rows collapsed on child id)", typed, len(items))
	}

	var parentMatches int
	parentQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s" WHERE "wallet_id" = ?`, "package")
	if err := db.QueryRow(parentQuery, "parent_A").Scan(&parentMatches); err != nil {
		t.Fatalf("count by wallet_id: %v", err)
	}
	if parentMatches != 2 {
