...
type Config struct {
	...
	FishAudioApiKey  string          `toml:"audio_api_key"`
}
...
func Load(configPath string) (*Config, error) {
	...
	// Env var overrides
	if v := os.Getenv("FISH_AUDIO_API_KEY"); v != "" {
		cfg.FishAudioApiKey = v
		cfg.markEnvOverride("FishAudioApiKey")
		cfg.AuthSource = "env:FISH_AUDIO_API_KEY"
		cfg.CredentialSource = "env:FISH_AUDIO_API_KEY"
	}
	...
	if v := os.Getenv("FISH_AUDIO_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("FISH_AUDIO_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "fish-audio-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.FishAudioApiKey != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:FISH_AUDIO_API_KEY"
		}
		return "Bearer " + c.FishAudioApiKey
	}
	if c.AccessToken != "" {
...
func (c *Config) StoreScopeCredential() string {
	...
	if c.FishAudioApiKey != "" {
		parts = append(parts, "audio_api_key="+c.FishAudioApiKey)
	}
	if len(parts) == 0 {
		return ""
	}
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		FishAudioApiKey: c.FishAudioApiKey,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	if c.FishAudioApiKey == "" {
		c.FishAudioApiKey = creds.FishAudioApiKey
	}
}
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.FishAudioApiKey = ""
	delete(c.envOverrides, "FishAudioApiKey")
	c.updateFileConfigField("FishAudioApiKey")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["FishAudioApiKey"] {
			out.FishAudioApiKey = c.fileConfig.FishAudioApiKey
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "FishAudioApiKey":
		c.fileConfig.FishAudioApiKey = c.FishAudioApiKey
	}
}
