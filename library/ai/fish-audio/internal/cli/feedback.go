...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("FISH_AUDIO_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("FISH_AUDIO_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "fish-audio-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  fish-audio-pp-cli feedback "the --voice flag name was unclear"
...
When ` + "`FISH_AUDIO_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`FISH_AUDIO_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  fish-audio-pp-cli feedback list
  fish-audio-pp-cli feedback list --limit 5
  fish-audio-pp-cli feedback list --json`,
