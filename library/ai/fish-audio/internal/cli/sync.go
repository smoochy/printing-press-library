...
Resource scoping:
  ...
  fish-audio-pp-cli sync --resources model

  # Full resync (ignore previous checkpoint)
  fish-audio-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  fish-audio-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  fish-audio-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  fish-audio-pp-cli sync --latest-only`,
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "tts":
		return db.UpsertTts(data)
	case "voice-design":
		return db.UpsertVoiceDesign(data)
	case "api_credit":
		return db.UpsertApiCredit(data)
	case "package":
		return db.UpsertPackage(data)
	default:
...
func responsePathForResource(resource, path string) []string {
	...
	case "model\x00/model":
		return []string{"items"}
	case "voice-design\x00/v1/voice-design":
