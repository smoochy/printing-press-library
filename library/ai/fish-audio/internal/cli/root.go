...
type rootFlags struct {
	...
	// invocation. Mirrors the FISH_AUDIO_NO_LEARN env var.
	noLearn bool
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "fish-audio-pp-cli",
		Short: `Fish Audio text-to-speech, voice cloning, and transcription from the terminal, with a local render log that tracks every byte and dollar the API never records.`,
		Long: `Fish Audio CLI — Fish Audio text-to-speech, voice cloning, and transcription from the terminal, with a local render log that tracks every byte and dollar the API never records.
...
Health check: run 'fish-audio-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("fish-audio-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.selectFields, "select", "", "Comma-separated fields to include in output (e.g. --select author,cover_image,created_at)")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'fish-audio-pp-cli profile list')")
	...
	rootCmd.AddCommand(newAsrPromotedCmd(flags))
	rootCmd.AddCommand(newVoiceDesignPromotedCmd(flags))
	...
	addNovelCommandIfAbsent(rootCmd, newNovelRenderCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelVoiceCmd(flags))
