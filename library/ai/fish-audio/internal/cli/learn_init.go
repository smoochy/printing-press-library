...
func newLearnConfig() *entities.Config {
	...
	if re, err := regexp.Compile(`^[0-9a-f]{32}$`); err != nil {
		fmt.Fprintf(os.Stderr, "warning: fish-audio-pp-cli: ticker pattern %q failed to compile: %v\n", `^[0-9a-f]{32}$`, err)
	} else {
...
func initLearn(ctx context.Context, db *sql.DB) error {
	...
	seeds := map[string][]lookups.SeedConfig{
		"format": {
			{Canonical: "mp3", Values: []string{"mpeg"}},
			{Canonical: "wav", Values: []string{"wave"}},
			{Canonical: "pcm", Values: []string{"raw"}},
			{Canonical: "opus", Values: []string{}},
		},
		"model": {
			{Canonical: "s2.1-pro", Values: []string{"s2.1", "pro", "latest", "default"}},
			{Canonical: "s2.1-pro-free", Values: []string{"free", "s2.1 free", "free tier"}},
			{Canonical: "s2-pro", Values: []string{"s2"}},
			{Canonical: "s1", Values: []string{"speech-1", "legacy"}},
		},
	}
...
func runLearnInitOnce(ctx context.Context) {
	learnInitOnce.Do(func() {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "warning: fish-audio-pp-cli: learn init: open store: %v\n", err)
			return
		}
		defer s.Close()
		if err := initLearn(ctx, s.DB()); err != nil {
			fmt.Fprintf(os.Stderr, "warning: fish-audio-pp-cli: learn init: %v\n", err)
		}
	})
}
