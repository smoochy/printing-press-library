...
var whichIndex = []whichEntry{
	{Command: "render log", Description: "See every past TTS render with its text, model, voice, byte count, and cost.", Group: "Local state that compounds", WhyItMatters: "Use it to recover what was already rendered before spending credit again."},
	{Command: "render spend", Description: "Total Fish Audio spend grouped by voice, model, or day for a side-by-side with your ElevenLabs invoice.", Group: "Local state that compounds", WhyItMatters: "Reach for it when the question is 'what did this voice cost us this month'."},
	{Command: "tts render", Description: "Hash the request and reuse a prior identical render instead of paying for it again.", Group: "Local state that compounds", WhyItMatters: "Use it in iteration loops where most lines have not changed."},
	{Command: "voice verify", Description: "Render a reference phrase with a cloned voice, transcribe it back, and report word-error-rate.", Group: "Verify before you ship", WhyItMatters: "Run it before swapping a production voice to a new clone."},
	{Command: "tts batch", Description: "Estimate a batch's cost against your live API credit and refuse to start if it would overdraw.", Group: "Verify before you ship", WhyItMatters: "Use it for unattended batch jobs so a long script cannot fail halfway on credit."},
	{Command: "render diff", Description: "Show the cost, model, and byte deltas between two past renders.", Group: "Local state that compounds", WhyItMatters: "Use it when picking between two takes of the same line."},
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one`,
		Example: `  fish-audio-pp-cli which "stale tickets"
  fish-audio-pp-cli which "bottleneck"
  fish-audio-pp-cli which --limit 1 "send message"
  fish-audio-pp-cli which                                # list the full capability index`,
