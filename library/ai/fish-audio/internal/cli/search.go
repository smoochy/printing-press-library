...
Run sync first to populate the local search index.`,
  ...
  fish-audio-pp-cli search "status"

  # Force local search explicitly
  fish-audio-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  fish-audio-pp-cli search "status" --type asr --data-source local
  # JSON output for piping
  fish-audio-pp-cli search "status" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'fish-audio-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "model":
				results, err = db.SearchModel(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchModel(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search model failed: %w", searchErr)
					}
