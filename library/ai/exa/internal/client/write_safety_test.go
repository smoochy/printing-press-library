...
func TestExplicitMutationIntentDoesNotReplayWireGet(t *testing.T) {
	t.Setenv("PRINTING_PRESS_VERIFY", "")
	c := New(&config.Config{BaseURL: "http://fixture.invalid", AuthHeaderVal: "fixture-token"}, time.Second, 0)
	c.cacheDir = t.TempDir()
	calls := 0
	c.HTTPClient.Transport = writeSafetyTransport(func(req *http.Request) (*http.Response, error) {
		calls++
		return &http.Response{StatusCode: 503, Header: make(http.Header), Body: io.NopCloser(strings.NewReader(`{"error":"ambiguous"}`)), Request: req}, nil
	})
	_, _, err := c.doMutation(context.Background(), http.MethodGet, "/wire-get-mutation", nil, nil, map[string]string{"Idempotency-Key": "not-proof"})
	if err == nil || calls != 1 {
		t.Fatalf("err=%v calls=%d; mutation intent must remain non-retryable", err, calls)
	}
}
