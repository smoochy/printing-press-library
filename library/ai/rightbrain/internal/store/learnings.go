...
var (
	// querySynonymMu guards the two variables below.
	//
	// The contract they were written to — "registration is one-shot at CLI
	// startup, before any store use" — does not hold in practice.
	// installPlaybooksFromEmbed reaches RegisterQuerySynonyms through
	// newLearnConfig, and the MCP server serves tool calls concurrently, so
	// registration races both itself and foldQueryTokens. A concurrent map
	// write is a fatal runtime error rather than a corrupted lookup: it takes
	// the process down. The generated TestPlaybookInit_ConcurrentSafe
	// reproduces it nondeterministically.
