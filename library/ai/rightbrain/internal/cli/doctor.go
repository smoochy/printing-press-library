...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  rightbrain-pp-cli doctor
  rightbrain-pp-cli doctor --json
  rightbrain-pp-cli doctor --fail-on warn
  rightbrain-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "Set it with: rightbrain-pp-cli auth set-token <token> or export RB_API_KEY=\"your-token-here\""
					report["auth_docs_url"] = "https://docs.rightbrain.ai/api"
				} else {
					authConfigured = true
			...
			if os.Getenv("RB_API_KEY") != "" {
				authEnvSet = append(authEnvSet, "RB_API_KEY")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "RB_API_KEY")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						authHeaders["User-Agent"] = "rightbrain-pp-cli"
						verifyPath := "/whoami"
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'rightbrain-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'rightbrain-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "empty"
		report["hint"] = "Cache is empty; run 'rightbrain-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'rightbrain-pp-cli sync' to refresh."
	}
	return report
}
