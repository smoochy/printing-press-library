...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("RIGHTBRAIN_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("RIGHTBRAIN_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "rightbrain-pp-cli/feedback")
...
When ` + "`RIGHTBRAIN_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`RIGHTBRAIN_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  rightbrain-pp-cli feedback list
  rightbrain-pp-cli feedback list --limit 5
  rightbrain-pp-cli feedback list --json`,
