...
var whichIndex = []whichEntry{
	{Command: "gate", Description: "Decide whether a candidate task revision is safe to promote, by comparing its eval pass rate against the last recorded result for a different revision of the same task.", Group: "Ship changes without breaking production", WhyItMatters: "Reach for this instead of running an eval and eyeballing the number when you need a machine-checkable promote/block decision with a non-zero exit code. Sync eval history first so a baseline exists."},
	{Command: "rollout", Description: "Show what traffic each task revision actually received versus the weight you configured, with failure rate, credits, and p50/p95 latency per revision.", Group: "Ship changes without breaking production", WhyItMatters: "Use this when an A/B revision is live and you need to know whether the split is real and whether the canary is slower or pricier."},
	{Command: "approvals", Description: "Triage every pending agent approval request by how long it has been parked and how soon it expires, with expired requests separated from actionable ones.", Group: "Keep agents unblocked", WhyItMatters: "Run this first when an agent 'stopped responding' — a run parked at waiting_for_human emits nothing and pages nobody, and an approval whose window lapsed needs a re-run rather than a decision."},
	{Command: "agent-trace", Description: "Reconstruct one agent run as an indented timeline of paired tool calls and results with per-step elapsed time and a tool-duration histogram.", Group: "Keep agents unblocked", WhyItMatters: "Reach for this when an agent run was slow or failed and you need to see which tool call hung, rather than replaying a raw event stream."},
	{Command: "drift", Description: "Compare this window against the previous one across every task and agent — mean credits, tokens, p95 latency, failure rate — and flag what moved.", Group: "Account for spend and latency", WhyItMatters: "Use this for the weekly regression sweep; it answers 'did anything get slower or pricier' without paging through run history per task."},
	{Command: "changelog", Description: "What changed in the project over a window, with resource UUIDs resolved to task, agent, skill, and collection names, plus an optional tamper-evidence verdict via --verify.", Group: "Account for spend and latency", WhyItMatters: "Reach for this for a Friday status or a compliance question — it is the only view that reads in names rather than UUIDs."},
	{Command: "eval-flake", Description: "Rank a task's eval test cases by how often they fail, separating genuine flake from cases that fail consistently.", Group: "Ship changes without breaking production", WhyItMatters: "Use this when the gate fails and you need to know whether the eval set itself is unreliable before touching the prompt."},
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one`,
		Example: `  rightbrain-pp-cli which "stale tickets"
  rightbrain-pp-cli which "bottleneck"
  rightbrain-pp-cli which --limit 1 "send message"
  rightbrain-pp-cli which                                # list the full capability index`,
