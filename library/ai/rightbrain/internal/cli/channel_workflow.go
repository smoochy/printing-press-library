...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  rightbrain-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  rightbrain-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"org", "skills", "skills-sources", "skills-tags", "whoami"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  rightbrain-pp-cli workflow status

  # Show status as JSON
  rightbrain-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("rightbrain-pp-cli")
			}
