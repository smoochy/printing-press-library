...
type rootFlags struct {
	...
	// invocation. Mirrors the RIGHTBRAIN_NO_LEARN env var.
	noLearn bool
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "rightbrain-pp-cli",
		Short: `Rightbrain CLI — The first Rightbrain CLI that reaches past tasks — agents, approvals, evals, triggers and audit, plus a local mirror…`,
		Long: `Rightbrain CLI — The first Rightbrain CLI that reaches past tasks — agents, approvals, evals, triggers and audit, plus a local mirror that makes credit spend and latency regressions queryable.
...
Health check: run 'rightbrain-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("rightbrain-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'rightbrain-pp-cli profile list')")
	...
	rootCmd.AddCommand(newIamCmd(flags))
	rootCmd.AddCommand(newInviteCmd(flags))
	rootCmd.AddCommand(newModelCmd(flags))
	...
	rootCmd.AddCommand(newNovelAgentTraceCmd(flags))
	rootCmd.AddCommand(newNovelApprovalsCmd(flags))
	rootCmd.AddCommand(newNovelChangelogCmd(flags))
	rootCmd.AddCommand(newNovelDriftCmd(flags))
	rootCmd.AddCommand(newNovelEvalFlakeCmd(flags))
	rootCmd.AddCommand(newNovelGateCmd(flags))
	rootCmd.AddCommand(newNovelRolloutCmd(flags))
	rootCmd.AddCommand(newAPICmd(flags))
	rootCmd.AddCommand(newAgentSharesPromotedCmd(flags))
	rootCmd.AddCommand(newAvatarPromotedCmd(flags))
	rootCmd.AddCommand(newIntegrationPromotedCmd(flags))
	rootCmd.AddCommand(newJoinPromotedCmd(flags))
	rootCmd.AddCommand(newSharesPromotedCmd(flags))
	rootCmd.AddCommand(newTaskMcpServerPromotedCmd(flags))
	rootCmd.AddCommand(newWhoamiPromotedCmd(flags))
