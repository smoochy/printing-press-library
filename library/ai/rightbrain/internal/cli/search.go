...
Run sync first to populate the local search index.`,
  ...
  rightbrain-pp-cli search "status"

  # Force local search explicitly
  rightbrain-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  rightbrain-pp-cli search "status" --type agent_shares --data-source local
  # JSON output for piping
  rightbrain-pp-cli search "status" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'rightbrain-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "agent_shares":
				results, err = db.SearchAgentShares(query, limit)
			case "public":
				results, err = db.SearchPublic(query, limit)
			case "shares":
				results, err = db.SearchShares(query, limit)
			case "skills":
			...
			case "":
				...
				{
					partial, searchErr := db.SearchAgentShares(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search agent_shares failed: %w", searchErr)
					}
				...
				{
					partial, searchErr := db.SearchPublic(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search public failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchShares(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search shares failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
