...
func resetCredentialEnv(t *testing.T) (home, configPath string) {
	...
	for _, name := range []string{
		"RIGHTBRAIN_CONFIG",
		"RIGHTBRAIN_CONFIG_DIR",
		"RIGHTBRAIN_DATA_DIR",
		"RIGHTBRAIN_STATE_DIR",
		"RIGHTBRAIN_CACHE_DIR",
		"RIGHTBRAIN_HOME",
		"XDG_CONFIG_HOME",
		"XDG_DATA_HOME",
		"XDG_STATE_HOME",
	...
	return home, filepath.Join(home, ".config", "rightbrain-pp-cli", "config.toml")
}
...
func TestCorruptCredentialsFallsBackToLegacyConfig(t *testing.T) {
	...
	credentialsPath := filepath.Join(home, ".local", "share", "rightbrain-pp-cli", "credentials.toml")
...
func TestCorruptCredentialsFallsBackToEnvCredential(t *testing.T) {
	...
	t.Setenv("RB_API_KEY", "env-secret")
	credentialsPath := filepath.Join(home, ".local", "share", "rightbrain-pp-cli", "credentials.toml")
	if err := os.MkdirAll(filepath.Dir(credentialsPath), 0o700); err != nil {
		t.Fatalf("mkdir credentials dir: %v", err)
	}
...
func TestAuthWriteScrubsLegacyConfigWhenRelocated(t *testing.T) {
	...
	activeConfigPath := filepath.Join(newConfigDir, "rightbrain-pp-cli", "config.toml")
	...
	credsPath := filepath.Join(newDataDir, "rightbrain-pp-cli", "credentials.toml")
