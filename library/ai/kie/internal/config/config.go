...
type Config struct {
	...
	KieBearerAuth    string          `toml:"bearer_auth"`
}
...
func Load(configPath string) (*Config, error) {
	...
	// Env var overrides
	if v := os.Getenv("KIE_BEARER_AUTH"); v != "" {
		cfg.KieBearerAuth = v
		cfg.markEnvOverride("KieBearerAuth")
		cfg.AuthSource = "env:KIE_BEARER_AUTH"
		cfg.CredentialSource = "env:KIE_BEARER_AUTH"
	}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("KIE_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "kie-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.KieBearerAuth != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:KIE_BEARER_AUTH"
		}
		return "Bearer " + c.KieBearerAuth
	}
	if c.AccessToken != "" {
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	if c.KieBearerAuth == "" {
		c.KieBearerAuth = creds.KieBearerAuth
	}
}
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.KieBearerAuth = ""
	delete(c.envOverrides, "KieBearerAuth")
	c.updateFileConfigField("KieBearerAuth")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["KieBearerAuth"] {
			out.KieBearerAuth = c.fileConfig.KieBearerAuth
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "KieBearerAuth":
		c.fileConfig.KieBearerAuth = c.KieBearerAuth
	}
}
