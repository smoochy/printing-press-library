...
func newLyricsCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:         "lyrics",
		Short:       "Get and generate lyrics",
		Annotations: map[string]string{"mcp:read-only": "true", "pp:api-resource": "true", "pp:typed-exit-codes": "0,2"},
		RunE:        parentNoSubcommandRunE(flags),
	}

	cmd.AddCommand(newLyricsGenerateCmd(flags))
	cmd.AddCommand(newLyricsGetDetailsCmd(flags))
	return cmd
}
