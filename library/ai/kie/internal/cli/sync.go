...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/ai/kie/internal/learn/lookups"
...
Resource scoping:
  ...
  kie-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  kie-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  kie-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  kie-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  kie-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dbPath == "" {
				dbPath = defaultDBPath("kie-pp-cli")
			}
			...
			if len(resources) == 0 {
				if humanFriendly {
					fmt.Fprintln(os.Stderr, "kie-pp-cli sync: no default sync resources in spec; use --resources for explicit sync targets or populate the store via single-fetch commands.")
				} else {
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "aleph":
		return db.UpsertAleph(data)
	case "claude":
		return db.UpsertClaude(data)
	case "flux":
		return db.UpsertFlux(data)
	case "gemini":
		return db.UpsertGemini(data)
	case "generate":
		return db.UpsertGenerate(data)
	case "gpt4o-image":
		return db.UpsertGpt4oImage(data)
	case "kie-ai-jobs":
		return db.UpsertKieAiJobs(data)
	case "lyrics":
		return db.UpsertLyrics(data)
	case "midi":
		return db.UpsertMidi(data)
	case "mp4":
		return db.UpsertMp4(data)
	case "omni":
		return db.UpsertOmni(data)
	case "runway":
		return db.UpsertRunway(data)
	case "suno":
		return db.UpsertSuno(data)
	case "veo":
		return db.UpsertVeo(data)
	case "vocal-removal":
		return db.UpsertVocalRemoval(data)
	case "voice":
		return db.UpsertVoice(data)
	case "wav":
		return db.UpsertWav(data)
	default:
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{ // #nosec G101 -- endpoint paths, not credentials.
		"kie-ai-jobs": "/api/v1/jobs/recordInfo",
	}
...
var reconcileTypedTables = map[string]string{
	...
	"gemini":        "gemini",
	"generate":      "generate",
	"gpt4o-image":   "gpt4o_image",
	"kie-ai-jobs":   "kie_ai_jobs",
	"lyrics":        "lyrics",
	"midi":          "midi",
	"mp4":           "mp4",
	"omni":          "omni",
...
func responsePathForResource(resource, path string) []string {
	...
	case "claude\x00/claude/v1/messages":
		return []string{"content"}
	case "gemini\x00/gemini/v1/models/gemini-3-5-flash:streamGenerateContent":
		return []string{"candidates"}
	case "gemini\x00/gemini/v1/models/gemini-3-6-flash:streamGenerateContent":
		return []string{"candidates"}
	case "gemini\x00/gemini/v1/models/gemini-3-flash-v1betamodels:streamGenerateContent":
		return []string{"candidates"}
	case "gemini-2-5-flash\x00/gemini-2.5-flash/v1/chat/completions":
		return []string{"choices"}
	case "gemini-2-5-pro\x00/gemini-2.5-pro/v1/chat/completions":
		return []string{"choices"}
	case "gemini-3-1-pro\x00/gemini-3.1-pro/v1/chat/completions":
		return []string{"choices"}
	case "gemini-3-5-flash-openai\x00/gemini-3-5-flash-openai/v1/chat/completions":
		return []string{"choices"}
	case "gemini-3-6-flash-openai\x00/gemini-3-6-flash-openai/v1/chat/completions":
		return []string{"choices"}
	case "gemini-3-flash\x00/gemini-3-flash/v1/chat/completions":
		return []string{"choices"}
	case "gemini-3-pro\x00/gemini-3-pro/v1/chat/completions":
		return []string{"choices"}
	case "gpt-5-2\x00/gpt-5-2/v1/chat/completions":
		return []string{"choices"}
	}
	return nil
}
