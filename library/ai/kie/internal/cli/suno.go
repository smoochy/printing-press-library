...
func newSunoCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:         "suno",
		Short:       "Get and generate suno",
		Annotations: map[string]string{"mcp:read-only": "true", "pp:api-resource": "true", "pp:typed-exit-codes": "0,2"},
		RunE:        parentNoSubcommandRunE(flags),
	}

	cmd.AddCommand(newSunoGenerateCoverCmd(flags))
	cmd.AddCommand(newSunoGetCoverDetailsCmd(flags))
	return cmd
}
