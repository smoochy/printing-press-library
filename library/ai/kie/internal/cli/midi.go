...
func newMidiCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:         "midi",
		Short:       "Get and generate midi",
		Annotations: map[string]string{"mcp:read-only": "true", "pp:api-resource": "true", "pp:typed-exit-codes": "0,2"},
		RunE:        parentNoSubcommandRunE(flags),
	}

	cmd.AddCommand(newMidiGenerateCmd(flags))
	cmd.AddCommand(newMidiGetDetailsCmd(flags))
	return cmd
}
