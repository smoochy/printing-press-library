...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("KIE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("KIE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "kie-pp-cli/feedback")
...
When ` + "`KIE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`KIE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  kie-pp-cli feedback list
  kie-pp-cli feedback list --limit 5
  kie-pp-cli feedback list --json`,
