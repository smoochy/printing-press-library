...
Run sync first to populate the local search index.`,
  ...
  kie-pp-cli search "status"

  # Force local search explicitly
  kie-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  kie-pp-cli search "status" --type aleph --data-source local
  # JSON output for piping
  kie-pp-cli search "status" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'kie-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
