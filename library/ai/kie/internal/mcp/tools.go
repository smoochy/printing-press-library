...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/ai/kie/internal/client"
	...
	"github.com/mvanhorn/printing-press-library/library/ai/kie/internal/learn"
	"github.com/mvanhorn/printing-press-library/library/ai/kie/internal/mcp/bound"
	"github.com/mvanhorn/printing-press-library/library/ai/kie/internal/mcp/cobratree"
	"github.com/mvanhorn/printing-press-library/library/ai/kie/internal/platform"
	"github.com/mvanhorn/printing-press-library/library/ai/kie/internal/store"
)
...
func RegisterTools(s *server.MCPServer) {
	...
	s.AddTool(
		mcplib.NewTool("sql",
			...
			mcplib.WithString("query", mcplib.Required(), mcplib.Description("SQL query (SELECT or WITH...SELECT). Synced records live in resources(resource_type, id, data); filter by resource_type and use json_extract on data, e.g. SELECT json_extract(data,'$.name') FROM resources WHERE resource_type='aleph'.")),
			mcplib.WithReadOnlyHintAnnotation(true),
			mcplib.WithDestructiveHintAnnotation(false),
		),
		handleSQL,
...
func openMCPReadOnlyStore(path string) (*store.Store, *mcplib.CallToolResult) {
	...
	if err != nil {
		return nil, mcplib.NewToolResultError(fmt.Sprintf("opening local data store %s: %v. Run kie-pp-cli sync to refresh the store, or use live endpoint MCP tools for unsynced data.", path, err))
	}
	return db, nil
}

func mcpMissingStoreMessage(path string) string {
	return fmt.Sprintf("No local data store found at %s. Run kie-pp-cli sync before using MCP search/sql, or use live endpoint MCP tools for unsynced data.", path)
}
...
func mcpEmptyStoreNextStep() string {
	return "Run kie-pp-cli sync to populate the local SQLite store before using MCP search/sql."
}
...
func mcpSQLQueryError(err error) string {
	...
	if strings.Contains(strings.ToLower(msg), "no such table") {
		return fmt.Sprintf("query failed: %v. Synced records live in resources(resource_type, id, data), not one SQL table per resource. Filter by resource_type, for example resource_type='aleph', and read JSON fields with json_extract(data,'$.field').", err)
	}
...
func handleContext(_ context.Context, _ mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
	...
	ctx := map[string]any{
		"api":         "kie",
		"description": "Unified API for Kie.ai's AI generation platform: image, video, music, and chat models, plus account and file utilities.",
		"archetype":   "communication",
		"tool_count":  70,
		"paths":       paths,
		// tool_surface tells agents which surface a capability lives on.
		"tool_surface": "MCP exposes typed endpoint tools plus a runtime mirror of user-facing CLI commands. Endpoint tools keep typed schemas; command-mirror tools shell out to the companion kie-pp-cli binary.",
		...
		"resources": []map[string]any{
			{
				"name":        "aleph",
				"description": "Manage aleph",
				"endpoints":   []string{"generate-video", "get-video-details"},
				"searchable":  true,
				"writable":    true,
			},
			{
				...
				"endpoints":   []string{"get-account-credits"},
			},
			{
				"name":        "claude",
				"description": "Manage claude",
				"endpoints":   []string{"sonnet-4-8"},
				"searchable":  true,
				"writable":    true,
			},
			{
				...
				"endpoints":   []string{"gpt-5-6-terra-completions"},
				"searchable":  true,
				"writable":    true,
			},
			{
				...
				"endpoints":   []string{"download-url"},
				"searchable":  true,
				"writable":    true,
			},
			{
				"name":        "file-base64-upload",
				"description": "Manage file base64 upload",
				"endpoints":   []string{"upload-file-base64"},
				"searchable":  true,
				"writable":    true,
			},
			{
				"name":        "file-stream-upload",
				"description": "Manage file stream upload",
				"endpoints":   []string{"upload-file-stream"},
				"searchable":  true,
				"writable":    true,
			},
			{
				"name":        "file-url-upload",
				"description": "Manage file url upload",
				"endpoints":   []string{"upload-file-url"},
				"searchable":  true,
				"writable":    true,
			},
			{
				"name":        "flux",
				"description": "Manage flux",
				"endpoints":   []string{"generate-or-edit-image", "get-image-details"},
				"searchable":  true,
				"writable":    true,
			},
			{
				"name":        "gemini",
				"description": "Manage gemini",
				"endpoints":   []string{"3-5-flash", "3-6-flash", "3-flash-v1betamodels"},
				"searchable":  true,
				"writable":    true,
			},
			{
				"name":        "gemini-2-5-flash",
				"description": "Manage gemini 2 5 flash",
			...
			{
				"name":        "gemini-2-5-pro",
				"description": "Manage gemini 2 5 pro",
				"endpoints":   []string{"chat-completions"},
				"searchable":  true,
				"writable":    true,
			},
			{
				"name":        "gemini-3-1-pro",
				"description": "Manage gemini 3 1 pro",
				"endpoints":   []string{"chat-completions"},
				"searchable":  true,
				"writable":    true,
			},
			{
				...
				"description": "Manage gemini 3 5 flash openai",
				"endpoints":   []string{"gemini-3-5-flash-chat-completions"},
				"searchable":  true,
				"writable":    true,
			},
			{
				"name":        "gemini-3-6-flash-openai",
				"description": "Manage gemini 3 6 flash openai",
				"endpoints":   []string{"gemini-3-6-flash-chat-completions"},
				"searchable":  true,
				"writable":    true,
			},
			{
				"name":        "gemini-3-flash",
				"description": "Manage gemini 3 flash",
				"endpoints":   []string{"chat-completions"},
				"searchable":  true,
				"writable":    true,
			},
			{
				"name":        "gemini-3-pro",
				"description": "Manage gemini 3 pro",
				"endpoints":   []string{"chat-completions"},
				"searchable":  true,
				"writable":    true,
			},
			{
				...
				"endpoints":   []string{"add-instrumental", "add-vocals", "extend-music", "get-music-details", "get-timestamped-lyrics", "mashup", "music", "persona", "replace-section", "sounds", "upload-and-cover-audio", "upload-and-extend-audio"},
				"searchable":  true,
				"writable":    true,
			},
			{
				"name":        "gpt-5-2",
				"description": "Manage gpt 5 2",
				"endpoints":   []string{"chat-completions"},
				"searchable":  true,
				"writable":    true,
			},
			{
				"name":        "gpt4o-image",
				"description": "Manage gpt4o image",
				"endpoints":   []string{"generate-4o-image", "get-4o-image-details", "get-4o-image-download-url"},
				"searchable":  true,
				"writable":    true,
			},
			{
				"name":        "grok",
				"description": "Manage grok",
			...
			{
				"name":        "kie-ai-jobs",
				"description": "Manage kie ai jobs",
				"endpoints":   []string{"market-create-task", "market-query-task"},
				"syncable":    true,
				"searchable":  true,
				"writable":    true,
			},
			{
				...
				"endpoints":   []string{"generate", "get-details"},
				"searchable":  true,
				"writable":    true,
			},
			{
				"name":        "midi",
				"description": "Manage midi",
				"endpoints":   []string{"generate", "get-details"},
				"searchable":  true,
				"writable":    true,
			},
			{
				"name":        "mp4",
				"description": "Manage mp4",
				"endpoints":   []string{"create-music-video", "get-music-video-details"},
				"searchable":  true,
				"writable":    true,
			},
			{
				"name":        "omni",
				"description": "Manage omni",
				"endpoints":   []string{"gemini-audio", "gemini-character"},
				"searchable":  true,
				"writable":    true,
			},
			{
				...
				"endpoints":   []string{"gpt-codex"},
				"searchable":  true,
				"writable":    true,
			},
			{
				"name":        "runway",
				"description": "Manage runway",
				"endpoints":   []string{"extend-ai-video", "generate-ai-video", "get-ai-video-details"},
				"searchable":  true,
				"writable":    true,
			},
			{
				"name":        "style",
				"description": "Manage style",
				"endpoints":   []string{"boost-music"},
				"searchable":  true,
				"writable":    true,
			},
			{
				"name":        "suno",
				"description": "Manage suno",
				"endpoints":   []string{"generate-cover", "get-cover-details"},
				"searchable":  true,
				"writable":    true,
			},
			{
				"name":        "veo",
				"description": "Manage veo",
				"endpoints":   []string{"extend-veo3-1-video", "generate-veo3-1-video", "get-veo3-1-1080p-video", "get-veo3-1-4k-video", "get-veo3-1-video-details"},
				"searchable":  true,
				"writable":    true,
			},
			{
				"name":        "vocal-removal",
				"description": "Manage vocal removal",
				"endpoints":   []string{"get-vocal-separation-details", "separate-vocals"},
				"searchable":  true,
				"writable":    true,
			},
			{
				...
				"endpoints":   []string{"suno-check", "suno-generate", "suno-record-info", "suno-regenerate", "suno-validate", "suno-validate-info"},
				"searchable":  true,
				"writable":    true,
			},
			{
				"name":        "wav",
				"description": "Manage wav",
				"endpoints":   []string{"convert-to", "get-details"},
				"searchable":  true,
				"writable":    true,
			},
		},
