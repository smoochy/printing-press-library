...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/ai/kie/internal/cliutil"
	"github.com/mvanhorn/printing-press-library/library/ai/kie/internal/config"
	"github.com/mvanhorn/printing-press-library/library/ai/kie/internal/platform"
)
...
func platformRateLimitTestClient(t *testing.T, server *httptest.Server) (*Client, *platform.Session) {
	...
	session, err := platform.PrepareProfileSourceFromConfig(&platform.Profile{
		...
		Sources: map[string]platform.SourceProfile{
			"kie": {CredentialRef: "fixture://Test/Synthetic/token", ExpectedAccountID: "acct-a", ExpectedOrgName: "Tenant A", ExpectedStoreID: "store-a", ExpectedStoreDomain: "tenant-a.example.test", ExpectedPropertyID: "123", ExpectedBaseURL: "https://tenant-a.example.test"},
		},
	}, "kie-pp-cli", "kie")
	if err != nil {
		t.Fatal(err)
