...
type Config struct {
	...
	GroqApiKey       string          `toml:"api_key"`
}
...
func Load(configPath string) (*Config, error) {
	...
	if v := os.Getenv("GROQ_API_KEY"); v != "" {
		cfg.GroqApiKey = v
		cfg.markEnvOverride("GroqApiKey")
		cfg.AuthSource = "env:GROQ_API_KEY"
		cfg.CredentialSource = "env:GROQ_API_KEY"
	}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("GROQ_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "groq-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.GroqApiKey != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:GROQ_API_KEY"
		}
		return "Bearer " + c.GroqApiKey
	}
	if c.AccessToken != "" {
...
func (c *Config) StoreScopeCredential() string {
	...
	if c.GroqApiKey != "" {
		parts = append(parts, "api_key="+c.GroqApiKey)
	}
	if len(parts) == 0 {
		return ""
	}
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.GroqApiKey = ""
	delete(c.envOverrides, "GroqApiKey")
	c.updateFileConfigField("GroqApiKey")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["GroqApiKey"] {
			out.GroqApiKey = c.fileConfig.GroqApiKey
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "GroqApiKey":
		c.fileConfig.GroqApiKey = c.GroqApiKey
	}
}
