...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  groq-pp-cli doctor
  groq-pp-cli doctor --json
  groq-pp-cli doctor --fail-on warn
  groq-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if credentialRefused {
				...
				if cfg != nil {
					...
					if !configured {
						...
						report["auth_hint"] = "Set it with: groq-pp-cli auth set-token <token> or export GROQ_API_KEY=\"your-token-here\""
						report["auth_key_url"] = "https://console.groq.com/keys"
					} else {
						authConfigured = true
			...
			if os.Getenv("GROQ_API_KEY") != "" {
				authEnvSet = append(authEnvSet, "GROQ_API_KEY")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "GROQ_API_KEY")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "groq-pp-cli", suggestion)
						} else {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'groq-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'groq-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		...
		report["hint"] = "No sync recorded; run 'groq-pp-cli sync' to hydrate API-backed resources. Rows written by other paths are not tracked in sync_state."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'groq-pp-cli sync' to refresh."
	}
	return report
}
