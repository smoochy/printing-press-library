...
type rootFlags struct {
	...
	// invocation. Mirrors the GROQ_NO_LEARN env var.
	noLearn bool
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "groq-pp-cli",
		Short: `Groq CLI — Every Groq endpoint in your terminal, plus a local ledger that tracks token cost and rate-limit budget.`,
		Long: `Groq CLI — Every Groq endpoint in your terminal, plus a local ledger that tracks token cost and rate-limit budget.
...
Health check: run 'groq-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("groq-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.selectFields, "select", "", "Comma-separated fields to include in output (e.g. --select cancelled_at,cancelling_at,completed_at)")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'groq-pp-cli profile list')")
	...
	rootCmd.AddCommand(newFineTuningsCmd(flags))
	...
	rootCmd.AddCommand(newRerankingPromotedCmd(flags))
	...
	addNovelCommandIfAbsent(rootCmd, newNovelCostsCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelRateLimitsCmd(flags))
