...
Resource scoping:
  ...
  groq-pp-cli sync --resources batches,files

  # Full resync (ignore previous checkpoint)
  groq-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  groq-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  groq-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  groq-pp-cli sync --latest-only`,
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "fine_tunings":
		return db.UpsertFineTunings(data)
	case "models":
...
func responsePathForResource(resource, path string) []string {
	...
	case "batches\x00/openai/v1/batches":
		return []string{"data"}
	case "embeddings\x00/openai/v1/embeddings":
		return []string{"data"}
	case "files\x00/openai/v1/files":
		return []string{"data"}
	case "fine_tunings\x00/v1/fine_tunings":
		return []string{"data"}
	case "models\x00/openai/v1/models":
		return []string{"data"}
	case "reranking\x00/openai/v1/reranking":
