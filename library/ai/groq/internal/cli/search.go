...
Run sync first to populate the local search index.`,
  ...
  groq-pp-cli search "status"

  # Force local search explicitly
  groq-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  groq-pp-cli search "status" --type audio --data-source local
  # JSON output for piping
  groq-pp-cli search "status" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'groq-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
