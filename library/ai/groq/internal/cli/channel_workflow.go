...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  groq-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  groq-pp-cli workflow archive --full

  # Archive without a wall-clock timeout
  groq-pp-cli workflow archive --timeout 0`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"batches", "files", "fine_tunings", "models"}
			if cliutil.IsDogfoodEnv() {
				if len(resources) > 3 {
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  groq-pp-cli workflow status

  # Show status as JSON
  groq-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("groq-pp-cli")
			}
