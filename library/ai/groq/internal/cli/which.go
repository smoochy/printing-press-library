...
var whichIndex = []whichEntry{
	{Command: "rate-limits", Description: "See remaining per-model request/token budget from every API call, with reset windows.", Group: "Local ledger that compounds", WhyItMatters: "Reach for this before a bulk run or when a 429 interrupts a pipeline; it tells you the exact remaining budget and reset time instead of guessing."},
	{Command: "costs", Description: "Aggregate token and dollar spend from your local completion history, grouped by model or day.", Group: "Local ledger that compounds", WhyItMatters: "Use this to answer 'how much did my eval runs cost this week' without exporting anything from the console."},
	{Command: "compare", Description: "Run one prompt across several models and rank them by latency, tokens/sec, usage, and cost.", Group: "Empirical model selection", WhyItMatters: "Pick the right model for a task by measuring real speed and cost instead of reading spec sheets."},
	{Command: "batch validate", Description: "Validate every line of a .jsonl batch request file against the endpoint schema and estimate tokens/cost before uploading.", Group: "Batch workflow guardrails", WhyItMatters: "Catch malformed batch lines and get a cost estimate before submitting a 100 MB file."},
	{Command: "batch diagnose", Description: "Tabulate a completed batch's per-line status codes and errors, highlighting retry-worthy failures.", Group: "Batch workflow guardrails", WhyItMatters: "Know exactly which batch lines failed and why, in seconds, from the shell."},
	{Command: "audio batch", Description: "Transcribe, translate, or synthesize speech over many audio files with rate-limit-aware pacing and a results manifest.", Group: "Paced bulk audio", WhyItMatters: "Run a whole folder of episodes without dying mid-batch on a rate limit or re-processing completed files."},
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one`,
		Example: `  groq-pp-cli which "stale tickets"
  groq-pp-cli which "bottleneck"
  groq-pp-cli which --limit 1 "send message"
  groq-pp-cli which                                # list the full capability index`,
