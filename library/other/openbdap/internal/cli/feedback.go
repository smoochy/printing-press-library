...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("OPENBDAP_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("OPENBDAP_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "openbdap-pp-cli/feedback")
...
When ` + "`OPENBDAP_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`OPENBDAP_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
maintainer sees it.`,
		Example: `  openbdap-pp-cli feedback "docs said exclusive but the boundary is inclusive"
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  openbdap-pp-cli feedback list
  openbdap-pp-cli feedback list --limit 5
  openbdap-pp-cli feedback list --json`,
