...
In local mode: searches locally synced data only.`,
  ...
  openbdap-pp-cli search "error timeout"

  # Force local search only
  openbdap-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  openbdap-pp-cli search "status" --type catalogo --data-source local
  # JSON output for piping
  openbdap-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /SpodCkanApi/api/3/action/package_search
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get(cmd.Context(), "/SpodCkanApi/api/3/action/package_search", map[string]string{
					"q": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'openbdap-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "catalogo":
				results, err = db.SearchCatalogo(query, limit)
			case "gruppi":
				results, err = db.SearchGruppi(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchCatalogo(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search catalogo failed: %w", searchErr)
					}
				...
				{
					partial, searchErr := db.SearchGruppi(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search gruppi failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
