...
type rootFlags struct {
	...
	// invocation. Mirrors the OPENBDAP_NO_LEARN env var.
	noLearn                 bool
	selectFields            string
	configPath              string
	homePath                string
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "openbdap-pp-cli",
		Short: `Openbdap CLI — Il catalogo Open Data della Ragioneria Generale dello Stato da terminale: ricerca offline, righe filtrate per nome di…`,
		Long: `Openbdap CLI — Il catalogo Open Data della Ragioneria Generale dello Stato da terminale: ricerca offline, righe filtrate per nome di colonna leggibile e opere pubbliche cercabili per CUP, CIG o codice fiscale.
...
Health check: run 'openbdap-pp-cli doctor' to verify connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("openbdap-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.selectFields, "select", "", "Comma-separated fields to include in output (e.g. --select id,name,title)")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'openbdap-pp-cli profile list')")
	...
	rootCmd.AddCommand(newCatalogoCmd(flags))
	rootCmd.AddCommand(newDatiCmd(flags))
	rootCmd.AddCommand(newGruppiCmd(flags))
	...
	rootCmd.AddCommand(newLicenzePromotedCmd(flags))
	rootCmd.AddCommand(newScaricaCmd(flags))
	...
	addNovelCommandIfAbsent(rootCmd, newNovelCampiCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelCigCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelDossierCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelMopCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelNovitaCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelOpereCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelSerieCmd(flags))
