...
Resource scoping:
  ...
  openbdap-pp-cli sync
  # Sync specific resources only
  openbdap-pp-cli sync --resources catalogo,catalogo-package-search

  # Full resync (ignore previous checkpoint)
  openbdap-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  openbdap-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  openbdap-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  openbdap-pp-cli sync --latest-only`,
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "catalogo":
		return db.UpsertCatalogo(data)
	case "dati":
		return db.UpsertDati(data)
	case "gruppi":
		return db.UpsertGruppi(data)
	case "licenze":
		return db.UpsertLicenze(data)
	case "tag":
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{ // #nosec G101 -- endpoint paths, not credentials.
		...
		"catalogo-package-search": "/SpodCkanApi/api/3/action/package_search",
