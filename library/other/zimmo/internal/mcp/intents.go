...
func RegisterIntents(s *server.MCPServer) {
	s.AddTool(
		mcplib.NewTool("daily_alert",
			mcplib.WithDescription("New, cheaper and gone listings for every saved search since the last run."),
...
// handleDailyAlert runs the daily_alert recipe intent: New, cheaper and gone listings for every saved search since the last run.
func handleDailyAlert(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
