...
func TestMigrate_AddsColumnsOnUpgrade_Dealers(t *testing.T) {
	...
	rows, err := s.DB().Query(`SELECT name FROM pragma_table_xinfo(?)`, "dealers")
	if err != nil {
	...
	for _, wantIdx := range []string{} {
		...
		if err := s.DB().QueryRow(
			...
			t.Fatalf("%s index missing from dealers after migrate: %v", wantIdx, err)
		}
	}
}
