...
func TestSearchDealersQuotesFTSQuerySyntax(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if stored, failed, err := s.UpsertBatch("dealers", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	} else if failed != 0 || stored != len(items) {
		t.Fatalf("UpsertBatch stored=%d failed=%d, want stored=%d failed=0", stored, failed, len(items))
	}
	...
	for _, query := range []string{
		...
		results, err := s.SearchDealers(query, 10)
		if err != nil {
			t.Fatalf("SearchDealers(%q): %v", query, err)
		}
		if len(results) == 0 {
			t.Fatalf("SearchDealers(%q) returned no results", query)
		}
	}
}
