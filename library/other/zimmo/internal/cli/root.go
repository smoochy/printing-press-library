...
type rootFlags struct {
	...
	// invocation. Mirrors the ZIMMO_NO_LEARN env var.
	noLearn                 bool
	selectFields            string
	configPath              string
	homePath                string
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "zimmo-pp-cli",
		Short: `Zimmo CLI — Belgian real estate from Zimmo with exact addresses, sold comps, commune prices per m² and a local store for deal…`,
		Long: `Zimmo CLI — Belgian real estate from Zimmo with exact addresses, sold comps, commune prices per m² and a local store for deal sourcing.
...
Health check: run 'zimmo-pp-cli doctor' to verify connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("zimmo-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.selectFields, "select", "", "Comma-separated fields to include in output (e.g. --select category,enriched,id)")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'zimmo-pp-cli profile list')")
	...
	rootCmd.AddCommand(newGeocodePromotedCmd(flags))
	rootCmd.AddCommand(newLocalityPricePromotedCmd(flags))
	rootCmd.AddCommand(newPlacesPromotedCmd(flags))
	rootCmd.AddCommand(newSubLocalityPricePromotedCmd(flags))
	rootCmd.AddCommand(newSublocalitiesPromotedCmd(flags))
	...
	addNovelCommandIfAbsent(rootCmd, newNovelCompsCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelEnrichCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelMotivatedCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelPebTrapCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelSameAsCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelUnderpricedCmd(flags))
