...
// zimmo-pp-cli CLI: `teach` (fire-and-forget, silent on success,
...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/other/zimmo/internal/learn/patterns"
...
const noLearnEnvVar = "ZIMMO_NO_LEARN"
...
func learnDBPath(explicit string) string {
	...
	return defaultDBPath("zimmo-pp-cli")
}
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  zimmo-pp-cli teach --query "find items in category" --resource-type items --resource GROUP-category`,
...
when learnings exist.`,
		Example: `  QUERY="$(cat /path/to/question.txt)" zimmo-pp-cli recall "$QUERY" --agent
  QUERY="$(cat /path/to/question.txt)" zimmo-pp-cli recall "$QUERY" --agent --min-confidence 2`,
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  zimmo-pp-cli learnings list --agent
  zimmo-pp-cli learnings list --query "$QUERY"
  zimmo-pp-cli learnings list --warnings --agent`,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  QUERY="$(cat /path/to/question.txt)" zimmo-pp-cli learnings forget "$QUERY" --resource <id>
  QUERY="$(cat /path/to/question.txt)" zimmo-pp-cli learnings forget "$QUERY" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
a whole family.`,
		Example: `  zimmo-pp-cli teach-pattern --query-template "items in {entity}" --resource-template "GROUP-{entity:category}" --resource-type items --entity-kind category --strategy substitute`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  zimmo-pp-cli teach-lookup --kind country --canonical "United States" --value USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
