...
var whichIndex = []whichEntry{
	{Command: "enrich", Description: "Refresh stored listings from Zimmo, resumably: status changes, price history, EPC, flood and planning flags, GPS.", Group: "Deal sourcing", WhyItMatters: "Run it before peb-trap or motivated so their filters see detail fields."},
	{Command: "same-as", Description: "Find the Immoweb or Immovlan twin of a Zimmo listing and the price gap between portals.", Group: "Cross-portal", WhyItMatters: "Dedupe leads across Belgian portals without refetching anything."},
	{Command: "comps", Description: "Sold or rented listings around one property with median €/m² and the subject's gap to it.", Group: "Valuation", WhyItMatters: "Median sold €/m² around the subject and its gap to it; Zimmo lists few sold properties, so the result widens to the postcode when thin."},
	{Command: "peb-trap", Description: "Energy-poor stored listings (EPC F/G or high kWh/m²), rented ones first with their rent, showing Zimmo's renovation-obligation flag.", Group: "Deal sourcing", WhyItMatters: "Surfaces owners under pressure to sell."},
	{Command: "underpriced", Description: "Rank stored for-sale houses and apartments by discount to their commune's €/m² (viager, whole buildings and service flats skipped).", Group: "Valuation", WhyItMatters: "Turns thousands of listings into a short list of apparent bargains."},
	{Command: "yield", Description: "Gross rental yield of sale listings: the published rent when let, otherwise rentals of similar surface in the same postcode and type.", Group: "Valuation", WhyItMatters: "Buy-to-let screening without a spreadsheet."},
	{Command: "motivated", Description: "Rank listings by seller pressure: days on market, total price cut, re-listing, EPC.", Group: "Deal sourcing", WhyItMatters: "Finds negotiable deals first."},
	{Command: "geocode", Description: "Geocode a Belgian address to coordinates and place ids", Group: "geocode", WhyItMatters: "Geocode a Belgian address to coordinates and place ids"},                                       // pp:which-promoted
	{Command: "locality-price", Description: "Price per m² for a commune (by property type) with monthly history", Group: "locality-price", WhyItMatters: "Price per m² for a commune (by property type) with monthly history"}, // pp:which-promoted
	{Command: "places", Description: "Resolve a postcode and/or commune name to Zimmo place ids", Group: "places", WhyItMatters: "Resolve a postcode and/or commune name to Zimmo place ids"},                                   // pp:which-promoted
	{Command: "sub-locality-price", Description: "Price per m² for a sub-locality with monthly history", Group: "sub-locality-price", WhyItMatters: "Price per m² for a sub-locality with monthly history"},                     // pp:which-promoted
	{Command: "sublocalities", Description: "Search sub-localities (deelgemeenten) by keyword", Group: "sublocalities", WhyItMatters: "Search sub-localities (deelgemeenten) by keyword"},                                       // pp:which-promoted
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one. Machine output (--json/--csv/--plain/--quiet, or a pipe) still exits 2 and writes {"matches":[]} (or the equivalent empty table) to stdout.`,
		SilenceUsage: true,
		Example: `  zimmo-pp-cli which "stale tickets"
  zimmo-pp-cli which "bottleneck"
  zimmo-pp-cli which --limit 1 "send message"
  zimmo-pp-cli which                                # list the full capability index`,
