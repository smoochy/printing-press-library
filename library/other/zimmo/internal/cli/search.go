...
In local mode: searches locally synced data only.`,
  ...
  zimmo-pp-cli search "error timeout"

  # Force local search only
  zimmo-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  zimmo-pp-cli search "status" --type dealers --data-source local
  # JSON output for piping
  zimmo-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET https://geo-api.zimmo.be/sublocalities
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get(cmd.Context(), "https://geo-api.zimmo.be/sublocalities", map[string]string{
					"keyword": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'zimmo-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "dealers":
				results, err = db.SearchDealers(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchDealers(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search dealers failed: %w", searchErr)
					}
