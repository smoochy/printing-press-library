...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("ZIMMO_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("ZIMMO_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "zimmo-pp-cli/feedback")
...
When ` + "`ZIMMO_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`ZIMMO_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
maintainer sees it.`,
		Example: `  zimmo-pp-cli feedback "docs said exclusive but the boundary is inclusive"
  zimmo-pp-cli feedback --stdin < notes.txt
  zimmo-pp-cli feedback list`,
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  zimmo-pp-cli feedback list
  zimmo-pp-cli feedback list --limit 5
  zimmo-pp-cli feedback list --json`,
