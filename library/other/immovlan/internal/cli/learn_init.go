...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/other/immovlan/internal/learn/entities"
	"github.com/mvanhorn/printing-press-library/library/other/immovlan/internal/learn/lookups"
...
func newLearnConfig() *entities.Config {
	...
	if re, err := regexp.Compile(`^[A-Za-z]{3}[0-9]{5,8}$`); err != nil {
		fmt.Fprintf(os.Stderr, "warning: immovlan-pp-cli: ticker pattern %q failed to compile: %v\n", `^[A-Za-z]{3}[0-9]{5,8}$`, err)
	} else {
...
func initLearn(ctx context.Context, db *sql.DB) error {
	...
	seeds := map[string][]lookups.SeedConfig{
		"commune": {
			...
			{Canonical: "Bruxelles", Values: []string{"brussels", "brussel", "1000"}},
			{Canonical: "Saint-Gilles", Values: []string{"sint-gillis", "1060"}},
			{Canonical: "Anderlecht", Values: []string{"1070"}},
			{Canonical: "Molenbeek-Saint-Jean", Values: []string{"molenbeek", "sint-jans-molenbeek", "1080"}},
		...
		"property_type": {
			{Canonical: "maison", Values: []string{"house", "huis"}},
			{Canonical: "appartement", Values: []string{"apartment", "flat"}},
			{Canonical: "terrain", Values: []string{"land", "grond", "plot"}},
			{Canonical: "garage", Values: []string{"parking"}},
			{Canonical: "kot", Values: []string{"studentroom", "student room"}},
		},
	}
...
func runLearnInitOnce(ctx context.Context) {
	learnInitOnce.Do(func() {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "warning: immovlan-pp-cli: learn init: open store: %v\n", err)
			return
		}
		defer s.Close()
		if err := initLearn(ctx, s.DB()); err != nil {
			fmt.Fprintf(os.Stderr, "warning: immovlan-pp-cli: learn init: %v\n", err)
		}
	})
}
