...
var whichIndex = []whichEntry{
	{Command: "same-as", Description: "Tell whether a stored Immovlan listing is also on Immoweb, at what price, or is Immovlan-only.", Group: "Cross-portal intelligence", WhyItMatters: "Use it to dedupe a cross-portal sourcing run or to find Immovlan exclusives before calling an agency."},
	{Command: "peb-trap", Description: "List PEB F or G properties that are currently rented: owners who can no longer re-let in Brussels since 2026.", Group: "Motivated-seller signals", WhyItMatters: "Use it when the question is which sellers are structurally motivated, not merely cheap."},
	{Command: "enrich", Description: "Fill missing detail fields (PEB letter, street, geo, rented, cadastral income, agency software) for many stored listings in one resumable pass.", Group: "Local state that compounds", WhyItMatters: "Run it after find and before peb-trap, same-as or split-candidates so their inputs exist."},
	{Command: "split-candidates", Description: "Houses large enough to divide, ranked by €/m² within their postcode, with land, year, PEB and rented flags.", Group: "Motivated-seller signals", WhyItMatters: "Use it for a trader who divides houses into apartments; find cannot filter by surface."},
	{Command: "relisted", Description: "Listings that came back under a new reference, with their true first-seen date and price at each reference.", Group: "Local state that compounds", WhyItMatters: "Use it to expose a fake Nouveau before negotiating on ancienneté."},
	{Command: "agencies", Description: "Which agencies list in a zone, their PEB F/G share, private-seller share and the CRM software behind each feed.", Group: "Cross-portal intelligence", WhyItMatters: "Use it to see which feeds are syndicated (likely also on Immoweb) versus Rossel-exclusive."},
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one. Machine output (--json/--csv/--plain/--quiet, or a pipe) still exits 2 and writes {"matches":[]} (or the equivalent empty table) to stdout.`,
		SilenceUsage: true,
		Example: `  immovlan-pp-cli which "stale tickets"
  immovlan-pp-cli which "bottleneck"
  immovlan-pp-cli which --limit 1 "send message"
  immovlan-pp-cli which                                # list the full capability index`,
