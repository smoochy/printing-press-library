...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("IMMOVLAN_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("IMMOVLAN_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "immovlan-pp-cli/feedback")
...
When ` + "`IMMOVLAN_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`IMMOVLAN_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
maintainer sees it.`,
		Example: `  immovlan-pp-cli feedback "docs said exclusive but the boundary is inclusive"
  immovlan-pp-cli feedback --stdin < notes.txt
  immovlan-pp-cli feedback list`,
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  immovlan-pp-cli feedback list
  immovlan-pp-cli feedback list --limit 5
  immovlan-pp-cli feedback list --json`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			if dryRunOK(flags) { // pp-patch: dry-run-json-envelopes
				return writeDryRun(cmd.OutOrStdout(), flags, "list recorded feedback")
			}
			p, err := feedbackFilePath()
			if err != nil {
				return err
