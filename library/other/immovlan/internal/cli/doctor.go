...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  immovlan-pp-cli doctor
  immovlan-pp-cli doctor --json
  immovlan-pp-cli doctor --fail-on warn
  immovlan-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) { // pp-patch: dry-run-json-envelopes
				return writeDryRun(cmd.OutOrStdout(), flags, "check configuration and connectivity to immovlan.be")
			}
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					healthPath := "/fr/immobilier?transactiontypes=a-vendre&propertytypes=maison&towns=1030-schaerbeek"
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "immovlan-pp-cli", suggestion)
						} else {
