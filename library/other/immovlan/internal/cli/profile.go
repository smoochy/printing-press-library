...
func legacyProfileStorePath() (string, error) {
	...
	dir := filepath.Join(home, ".immovlan-pp-cli")
	return filepath.Join(dir, "profiles.json"), nil
}
...
captured: they control profile/config resolution and would never apply).`,
		Example: `  immovlan-pp-cli profile save my-defaults --json --compact
  immovlan-pp-cli profile save tonight-defaults --region US`,
...
func newProfileUseCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: `  immovlan-pp-cli profile use my-defaults
  immovlan-pp-cli profile use tonight-defaults --json`,
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
...
func newProfileListCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: `  immovlan-pp-cli profile list
  immovlan-pp-cli profile list --json`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			if dryRunOK(flags) { // pp-patch: dry-run-json-envelopes
				return writeDryRun(cmd.OutOrStdout(), flags, "list saved profiles")
			}
			s, err := loadProfileStore()
			if err != nil {
				return err
...
func newProfileShowCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: `  immovlan-pp-cli profile show my-defaults
  immovlan-pp-cli profile show tonight-defaults --json`,
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			p, err := GetProfile(args[0])
			if err != nil {
...
func newProfileDeleteCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: `  immovlan-pp-cli profile delete my-defaults --yes
  immovlan-pp-cli profile delete old-profile --yes --json`,
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			name := args[0]
			s, err := loadProfileStore()
