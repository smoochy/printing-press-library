...
type rootFlags struct {
	...
	// invocation. Mirrors the IMMOVLAN_NO_LEARN env var.
	noLearn                 bool
	selectFields            string
	configPath              string
	homePath                string
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "immovlan-pp-cli",
		Short: `Immovlan CLI — Search Immovlan from the terminal with every site filter, keep a local history, and cross it with Immoweb: exclusives,…`,
		Long: `Immovlan CLI — Search Immovlan from the terminal with every site filter, keep a local history, and cross it with Immoweb: exclusives, re-listings, rented PEB F/G traps and division candidates.
...
Health check: run 'immovlan-pp-cli doctor' to verify connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("immovlan-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.selectFields, "select", "", "Comma-separated fields to include in output (e.g. --select url,name,datePosted)")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'immovlan-pp-cli profile list')")
	...
	addNovelCommandIfAbsent(rootCmd, newNovelAgenciesCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelEnrichCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelPebTrapCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelRelistedCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelSameAsCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelSplitCandidatesCmd(flags))
