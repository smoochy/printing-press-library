...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/other/immovlan/internal/config"
	"github.com/mvanhorn/printing-press-library/library/other/immovlan/internal/platform"
)
...
func platformRateLimitTestClient(t *testing.T, server *httptest.Server) (*Client, *platform.Session) {
	...
	session, err := platform.PrepareProfileSourceFromConfig(&platform.Profile{
		...
		Sources: map[string]platform.SourceProfile{
			"immovlan": {CredentialRef: "fixture://Test/Synthetic/token", ExpectedAccountID: "acct-a", ExpectedOrgName: "Tenant A", ExpectedStoreID: "store-a", ExpectedStoreDomain: "tenant-a.example.test", ExpectedPropertyID: "123", ExpectedBaseURL: "https://tenant-a.example.test"},
		},
	}, "immovlan-pp-cli", "immovlan")
	if err != nil {
		t.Fatal(err)
	}
