# Immovlan Printed CLI Agent Guide

This directory is a generated `immovlan-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
immovlan-pp-cli doctor --json
immovlan-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
immovlan-pp-cli which "immovlan exclusives" --json
immovlan-pp-cli find --help
```
...
No command mutates immovlan.be. Before running an unfamiliar command that writes the local store (`hide`, `shortlist`, `saved`, `enrich`, `teach`, `learnings forget`), inspect its help and prefer a dry run:

```bash
immovlan-pp-cli enrich --help
immovlan-pp-cli enrich --dry-run --agent
```
...
1. On a new user question, call `immovlan-pp-cli recall` FIRST, passing the question as an argv or MCP argument - never by interpolating it into a shell command, quoted string, or heredoc (apostrophes break quotes; a body line equal to the delimiter breaks a heredoc). If a shell is required, write the question with a non-shell file tool, then `QUERY=$(cat /path/to/question.txt)` and `immovlan-pp-cli recall "$QUERY" --agent`. If `found=true` and the top result has `entity_match == "exact"` and `confidence >= 2`, skip discovery and go straight to the live fetch for the returned resource IDs. If the store is cold (recall finds nothing and `learnings list` and `learnings candidates` are both empty), skip recall for the rest of the session.
...
3. After answering, always fire `immovlan-pp-cli teach` in the background with the same argv/MCP or file-then-`$QUERY` passing rule as recall (`QUERY=$(cat /path/to/question.txt)` then `immovlan-pp-cli teach --query "$QUERY" --resource <id> --resource-type <type> &`) - teaching is unconditional and is the anchor that triggers playbook synthesis. Teach the structural question with identifiers stripped (no names, emails, phone numbers, account ids); the CLI warns on obvious PII shapes but does not block.
...
Disable the loop with `--no-learn` per-invocation or `IMMOVLAN_NO_LEARN=true` for the whole session - useful for deterministic agent flows that don't want a learning row to silently change subsequent query results.

## Platform Credential References

This CLI has no API authentication (immovlan.be pages are public); this
mechanism applies only to `--client-profile` tenant-gated platform sources. If this CLI uses indirect references for a tenant-gated platform
