...
func RegisterIntents(s *server.MCPServer) {
	s.AddTool(
		mcplib.NewTool("morning_check_of_a_saved_rental_search",
			mcplib.WithDescription("Ranks current matches with the score breakdown so the agent can say which three to call first."),
			mcplib.WithString("slug", mcplib.Required(), mcplib.Description("Override the recipe's positional slug value.")),
			mcplib.WithOpenWorldHintAnnotation(true),
		),
		handleMorningCheckOfASavedRentalSearch,
	)
	s.AddTool(
		mcplib.NewTool("is_this_house_fairly_priced",
			mcplib.WithDescription("Returns the €/m² percentile against comparables, days on market and any recorded price cuts."),
			mcplib.WithString("id", mcplib.Required(), mcplib.Description("Override the recipe's positional id value.")),
			mcplib.WithOpenWorldHintAnnotation(true),
		),
		handleIsThisHouseFairlyPriced,
	)
}

// handleMorningCheckOfASavedRentalSearch runs the morning_check_of_a_saved_rental_search recipe intent: Ranks current matches with the score breakdown so the agent can say which three to call first.
func handleMorningCheckOfASavedRentalSearch(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
...
// handleIsThisHouseFairlyPriced runs the is_this_house_fairly_priced recipe intent: Returns the €/m² percentile against comparables, days on market and any recorded price cuts.
func handleIsThisHouseFairlyPriced(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
	if recipeCLIPathErr != nil {
		return mcplib.NewToolResultError(fmt.Sprintf("companion CLI binary not found: %v", recipeCLIPathErr)), nil
	}
