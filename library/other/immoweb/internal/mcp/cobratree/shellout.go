...
func cliArgsFromMCP(args map[string]any, blocked map[string]bool) []string {
	...
	// Values are joined as --flag=value: a separate token would let a string
	// sent for a bool flag (which takes no value) be parsed as its own flag,
	// smuggling hidden or blocked flags past the schema.
	var out []string
	for _, k := range keys {
		v := args[k]
		switch tv := v.(type) {
