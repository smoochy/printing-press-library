...
func TestCliArgsFromMCP_NoFlagSmugglingThroughValues(t *testing.T) {
	got := cliArgsFromMCP(map[string]any{"include-gone": "--db=/tmp/victim", "limit": "--base-url=http://evil"}, map[string]bool{"base-url": true, "db": true})
	for _, tok := range got {
		if strings.HasPrefix(tok, "--db") || strings.HasPrefix(tok, "--base-url") {
			t.Fatalf("value smuggled a flag as its own token: %v", got)
		}
	}
}
