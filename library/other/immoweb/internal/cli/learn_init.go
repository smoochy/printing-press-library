...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/other/immoweb/internal/learn/entities"
	"github.com/mvanhorn/printing-press-library/library/other/immoweb/internal/learn/lookups"
...
func newLearnConfig() *entities.Config {
	...
	if re, err := regexp.Compile(`^[0-9]{7,9}$`); err != nil {
		fmt.Fprintf(os.Stderr, "warning: immoweb-pp-cli: ticker pattern %q failed to compile: %v\n", `^[0-9]{7,9}$`, err)
	} else {
...
func initLearn(ctx context.Context, db *sql.DB) error {
	...
	seeds := map[string][]lookups.SeedConfig{
		"commune": {
			...
			{Canonical: "Bruxelles ville", Values: []string{"brussels", "brussel", "bruxelles", "1000"}},
		...
		"property_type": {
			...
			{Canonical: "LAND", Values: []string{"terrain", "grond", "plot"}},
		},
	}
...
func runLearnInitOnce(ctx context.Context) {
	learnInitOnce.Do(func() {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "warning: immoweb-pp-cli: learn init: open store: %v\n", err)
			return
		}
		defer s.Close()
		if err := initLearn(ctx, s.DB()); err != nil {
			fmt.Fprintf(os.Stderr, "warning: immoweb-pp-cli: learn init: %v\n", err)
		}
	})
}
...
func learnResourceTypeFields() map[string][]string {
	return map[string][]string{
		...
		"listings-search-results": {"name", "title", "display_name", "full_name", "short_name", "label", "slug", "key", "code", "id", "address"},
		"locations":               {"name", "title", "display_name", "full_name", "short_name", "label", "slug", "key", "code", "id", "address"},
	}
}
