...
In local mode: searches locally synced data only.`,
  ...
  immoweb-pp-cli search "error timeout"

  # Force local search only
  immoweb-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  immoweb-pp-cli search "status" --type listings --data-source local
  # JSON output for piping
  immoweb-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /en/search-results
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get(cmd.Context(), "/en/search-results", map[string]string{
					"q": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'immoweb-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
