...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("IMMOWEB_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("IMMOWEB_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "immoweb-pp-cli/feedback")
...
When ` + "`IMMOWEB_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`IMMOWEB_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
maintainer sees it.`,
		Example: `  immoweb-pp-cli feedback "the --since flag is inclusive but docs say exclusive"
  immoweb-pp-cli feedback --stdin < notes.txt
  immoweb-pp-cli feedback list --limit 10`,
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  immoweb-pp-cli feedback list
  immoweb-pp-cli feedback list --limit 5
  immoweb-pp-cli feedback list --json`,
