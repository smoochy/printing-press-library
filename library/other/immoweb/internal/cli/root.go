...
type rootFlags struct {
	...
	// invocation. Mirrors the IMMOWEB_NO_LEARN env var.
	noLearn                 bool
	selectFields            string
	configPath              string
	homePath                string
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "immoweb-pp-cli",
		Short: `Immoweb CLI — Search Immoweb from the terminal with every site filter, then get the answers the site can't give: price cuts, days on…`,
		Long: `Immoweb CLI — Search Immoweb from the terminal with every site filter, then get the answers the site can't give: price cuts, days on market, commune medians and rental yield.

Highlights (not in the official API docs):
  • market   Median asking price, €/m², price-cut share and recent disappearances for one or several communes, broken down by bedrooms, EPC, type or postcode. Student rooms and per-room lets are left out. Pulls m…
  • deal   Tells you whether one listing is cheap or expensive: €/m² percentile against comparable listings, days on market, price-cut history, demand and EPC gap. Pulls missing areas on first use (--no-pull to…
  • triage   Ranks the current matches of a saved search (or ad-hoc filters) by a transparent score: EUR/m2 percentile within the same postal code, freshness, price cut, private seller and EPC (with --enrich) so …
...
Health check: run 'immoweb-pp-cli doctor' to verify connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("immoweb-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.selectFields, "select", "", "Comma-separated fields to include in output (e.g. --select classified)")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'immoweb-pp-cli profile list')")
	...
	addNovelCommandIfAbsent(rootCmd, newNovelDealCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelDropsCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelMarketCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelTriageCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelYieldCmd(flags))
