...
var whichIndex = []whichEntry{
	{Command: "market", Description: "Median asking price, €/m², price-cut share and recent disappearances for one or several communes, broken down by bedrooms, EPC, type or postcode. Student rooms and per-room lets are left out. Pulls missing areas on first use (--no-pull to stay offline).", Group: "Market intelligence", WhyItMatters: "Reach for this when an agent needs neighbourhood price levels or wants to compare communes before searching."},
	{Command: "deal", Description: "Tells you whether one listing is cheap or expensive: €/m² percentile against comparable listings, days on market, price-cut history, demand and EPC gap. Pulls missing areas on first use (--no-pull to stay offline).", Group: "Decision support", WhyItMatters: "Use before advising on an offer or deciding whether a specific listing is worth a visit."},
	{Command: "triage", Description: "Ranks the current matches of a saved search (or ad-hoc filters) by a transparent score: EUR/m2 percentile within the same postal code, freshness, price cut, private seller and EPC (with --enrich) so you know who to call first.", Group: "Decision support", WhyItMatters: "Use when a search returns too many listings and the user needs a prioritised short list with reasons."},
	{Command: "drops", Description: "Lists stored listings whose asking price fell (from prices recorded by find/watch/pull and Immoweb's old-price field), with first and latest price, number of cuts, total % cut and days listed.", Group: "Local state that compounds", WhyItMatters: "Use to find motivated sellers or negotiation leverage across everything the user has tracked."},
	{Command: "yield", Description: "Estimates gross rental yield for a sale listing or a whole commune from comparable rents, and refuses when there are too few comparables. Pulls missing areas on first use (--no-pull to stay offline).", Group: "Market intelligence", WhyItMatters: "Use for investor questions about rental return before looking at individual listings."},
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one. Machine output (--json/--csv/--plain/--quiet, or a pipe) still exits 2 and writes {"matches":[]} (or the equivalent empty table) to stdout.`,
		SilenceUsage: true,
		Example: `  immoweb-pp-cli which "stale tickets"
  immoweb-pp-cli which "bottleneck"
  immoweb-pp-cli which --limit 1 "send message"
  immoweb-pp-cli which                                # list the full capability index`,
