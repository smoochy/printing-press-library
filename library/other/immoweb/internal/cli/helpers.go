...
func classifyAPIErrorOnly(err error) error {
	...
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: Immoweb refused the request (no account is needed, so this is usually bot protection)."+
			"\n      Wait a few minutes, retry with fewer requests (--rate-limit 2), and run 'immoweb-pp-cli doctor' to check connectivity.", err))
	case strings.Contains(msg, "HTTP 403"):
		return authErr(fmt.Errorf("%w\nhint: permission denied. This API is configured without credentials, so the service may be blocking the request by rate limit, geography, bot protection, or endpoint policy."+
			"\n      Run 'immoweb-pp-cli doctor' to check connectivity.", err))
	case strings.Contains(msg, "HTTP 404"):
		return notFoundErr(fmt.Errorf("%w\nhint: not found on Immoweb. A removed, sold or rented listing returns this; check the ID with 'immoweb-pp-cli find' or 'immoweb-pp-cli saved list' for saved searches", err))
