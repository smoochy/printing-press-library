...
agents can record playbooks without a file on disk.`,
		Example: `  QUERY="$(cat /path/to/question.txt)" immoweb-pp-cli teach-playbook --query "$QUERY" --playbook-file ~/playbooks/recipe.json --notes-file ~/playbooks/recipe-notes.md`,
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  QUERY="$(cat /path/to/question.txt)" NOTE="$(cat /path/to/note.txt)" immoweb-pp-cli playbook amend --query "$QUERY" --add-note "$NOTE"`,
		RunE: func(cmd *cobra.Command, args []string) error {
			cmd.SilenceUsage = true
			cmd.SilenceErrors = true
			if noLearnActive(flags) {
...
func newPlaybookListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example:     `  immoweb-pp-cli playbook list --agent`,
