...
Run sync first to populate the local search index.`,
  ...
  passive-indices-pp-cli search "status"

  # Force local search explicitly
  passive-indices-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  passive-indices-pp-cli search "status" --type index --data-source local
  # JSON output for piping
  passive-indices-pp-cli search "status" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'passive-indices-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
