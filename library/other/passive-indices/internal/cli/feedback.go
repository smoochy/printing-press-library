...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("PASSIVE_INDICES_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("PASSIVE_INDICES_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "passive-indices-pp-cli/feedback")
...
When ` + "`PASSIVE_INDICES_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`PASSIVE_INDICES_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  passive-indices-pp-cli feedback list
  passive-indices-pp-cli feedback list --limit 5
  passive-indices-pp-cli feedback list --json`,
