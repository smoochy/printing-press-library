...
// whichCommandPath returns the leading literal tokens of a whichEntry
// Command string, stopping at the first placeholder (<...>) or flag
// (-x / --flag) token. Novel feature Command strings document required
// args/flags inline for READMEs and SKILL.md (e.g. "fund raw <schemeId>");
// only the leading tokens name an actual Cobra command path.
func whichCommandPath(command string) []string {
	var path []string
...
func TestWhichIndex_ExistsAndIsWellFormed(t *testing.T) {
	...
	for i, e := range whichIndex {
		...
		path := whichCommandPath(e.Command)
		if len(path) == 0 {
			t.Errorf("whichIndex[%d] command %q has no resolvable command path (starts with a placeholder or flag)", i, e.Command)
			continue
		}
