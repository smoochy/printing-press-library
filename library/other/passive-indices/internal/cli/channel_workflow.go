...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  passive-indices-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  passive-indices-pp-cli workflow archive --full`,
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  passive-indices-pp-cli workflow status

  # Show status as JSON
  passive-indices-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("passive-indices-pp-cli")
			}
