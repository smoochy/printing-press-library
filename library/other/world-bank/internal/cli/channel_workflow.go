...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  world-bank-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  world-bank-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"countries", "income_levels", "indicators", "lending_types", "regions", "sources", "topics"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  world-bank-pp-cli workflow status

  # Show status as JSON
  world-bank-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("world-bank-pp-cli")
			}
