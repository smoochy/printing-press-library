...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "world-bank-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("WORLD_BANK_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("WORLD_BANK_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "world-bank-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/world-bank-pp-cli/feedback.jsonl.
When ` + "`WORLD_BANK_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`WORLD_BANK_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  world-bank-pp-cli feedback list
  world-bank-pp-cli feedback list --limit 5
  world-bank-pp-cli feedback list --json`,
