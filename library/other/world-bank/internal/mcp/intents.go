...
func RegisterIntents(s *server.MCPServer) {
	s.AddTool(
		mcplib.NewTool("pipeline_export",
			mcplib.WithDescription("Wide CSV ready for a notebook."),
			mcplib.WithBoolean("wide", mcplib.Description("Override the recipe's --wide value.")),
...
// handlePipelineExport runs the pipeline_export recipe intent: Wide CSV ready for a notebook.
func handlePipelineExport(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
	...
	args = appendRecipeBoolFlag(args, "wide", input["wide"], true)
