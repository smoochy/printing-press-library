# World Bank Printed CLI Agent Guide

This directory is a generated `world-bank-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
world-bank-pp-cli doctor --json
world-bank-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
world-bank-pp-cli which "<capability>" --json
world-bank-pp-cli <command> --help
```
...
```bash
world-bank-pp-cli <command> --agent
```
...
```bash
world-bank-pp-cli <command> --help
world-bank-pp-cli <command> --dry-run --agent
```
