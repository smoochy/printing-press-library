...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "surfline-pp-cli", "http")
...
func (c *Client) dryRun(method, targetURL, path string, params map[string]string, body []byte, headerOverrides map[string]string, authHeader string) (json.RawMessage, int, error) {
	...
	if authHeader != "" {
		...
		fmt.Fprintf(os.Stderr, "  %s%s=%s\n", sep, "accesstoken", maskToken(authHeader))
		queryPrinted = true
	}
	_ = queryPrinted
	if body != nil {
...
func authPlaceholderCredentialError(cfg *config.Config) error {
	return authPlaceholderCredentialErrorWithSetup(cfg, "export SURFLINE_ACCESS_TOKEN=<your-token> or surfline-pp-cli auth set-token <token>")
}
