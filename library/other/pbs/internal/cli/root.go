...
type rootFlags struct {
	...
	// invocation. Mirrors the PBS_NO_LEARN env var.
	noLearn                 bool
	selectFields            string
	configPath              string
	homePath                string
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "pbs-pp-cli",
		Short: `The only machine-readable Pakistani price panel — every urban centre and essential item the Bureau prices weekly, min/avg/max, accrued from the scattered release files it indexes in a hand-edited…`,
		Long: `The only machine-readable Pakistani price panel — every urban centre and essential item the Bureau prices weekly, min/avg/max, accrued from the scattered release files it indexes in a hand-edited JavaScript array and never publishes as a series.
...
Health check: run 'pbs-pp-cli doctor' to verify connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("pbs-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'pbs-pp-cli profile list')")
	...
	addNovelCommandIfAbsent(rootCmd, newNovelMoversCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelRevisionsCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelSpreadCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelVerifyCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelWeightsCmd(flags))
