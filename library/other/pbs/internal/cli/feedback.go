...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("PBS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("PBS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "pbs-pp-cli/feedback")
...
When ` + "`PBS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`PBS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
maintainer sees it.`,
		Example: `  pbs-pp-cli feedback "the --since flag is inclusive but docs say exclusive"
  pbs-pp-cli feedback --stdin < notes.txt
  pbs-pp-cli feedback list --limit 10`,
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  pbs-pp-cli feedback list
  pbs-pp-cli feedback list --limit 5
  pbs-pp-cli feedback list --json`,
