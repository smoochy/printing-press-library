...
var whichIndex = []whichEntry{
	{Command: "spread", Description: "See the cheapest and dearest city for any essential item in any week, and how that gap has moved over time.", Group: "Panel joins the source cannot answer", WhyItMatters: "Reach for this when the question is about geography rather than time — which city is an outlier on this item right now."},
	{Command: "drift", Description: "Follow one item's price across named cities and weeks, with missing weeks shown as gaps rather than filled.", Group: "Panel joins the source cannot answer", WhyItMatters: "Reach for this when the question is about time for a known item and known cities."},
	{Command: "basket", Description: "Build your own price index over any subset of items using the weight vector PBS republishes every week.", Group: "Panel joins the source cannot answer", WhyItMatters: "Reach for this to build a food-only or fuel-only index the Bureau never computes."},
	{Command: "weights", Description: "Read the item weight vector for any release, diff two releases, and check the totals sum to 100.", Group: "Only the accrued store knows", WhyItMatters: "Reach for this before trusting a long index series — a silent re-weighting breaks comparability."},
	{Command: "verify", Description: "Audit parsed cell values against the source's own arithmetic invariants, including the weight totals and min-avg-max ordering.", Group: "Honesty instrumentation", WhyItMatters: "Reach for this before quoting any figure — it distinguishes a real price move from a parse bug."},
	{Command: "coverage", Description: "Show which releases exist upstream, which the local store holds, and every hole named as a hole.", Group: "Honesty instrumentation", WhyItMatters: "Reach for this whenever a result looks clean — a silently short series is the classic false positive."},
	{Command: "revisions", Description: "Detect releases the Bureau has rewritten, re-pointed, or deleted since you captured them.", Group: "Only the accrued store knows", WhyItMatters: "Reach for this to prove your local copy is the surviving record, and to catch a silent restatement."},
	{Command: "movers", Description: "Rank every item in a release by percent change recomputed from price levels, nationally or per city, with outliers flagged.", Group: "Panel joins the source cannot answer", WhyItMatters: "Reach for this when the question is which items moved this week, rather than one known item."},
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one. Machine output (--json/--csv/--plain/--quiet, or a pipe) still exits 2 and writes {"matches":[]} (or the equivalent empty table) to stdout.`,
		SilenceUsage: true,
		Example: `  pbs-pp-cli which "stale tickets"
  pbs-pp-cli which "bottleneck"
  pbs-pp-cli which --limit 1 "send message"
  pbs-pp-cli which                                # list the full capability index`,
