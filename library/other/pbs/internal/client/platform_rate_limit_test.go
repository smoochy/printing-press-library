...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/other/pbs/internal/config"
	"github.com/mvanhorn/printing-press-library/library/other/pbs/internal/platform"
)
...
func platformRateLimitTestClient(t *testing.T, server *httptest.Server) (*Client, *platform.Session) {
	...
	session, err := platform.PrepareProfileSourceFromConfig(&platform.Profile{
		...
		Sources: map[string]platform.SourceProfile{
			"pbs": {CredentialRef: "fixture://Test/Synthetic/token", ExpectedAccountID: "acct-a", ExpectedOrgName: "Tenant A", ExpectedStoreID: "store-a", ExpectedStoreDomain: "tenant-a.example.test", ExpectedPropertyID: "123", ExpectedBaseURL: "https://tenant-a.example.test"},
		},
	}, "pbs-pp-cli", "pbs")
	if err != nil {
		t.Fatal(err)
