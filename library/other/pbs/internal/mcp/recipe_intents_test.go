...
func TestRecipeIntentHandlerBuildsRecipeArgs(t *testing.T) {
	...
	result, err := handleAuditASeriesBeforeQuotingIt(context.Background(), req)
	if err != nil {
...
func TestRecipeIntentHandlerOverridesParams(t *testing.T) {
	...
	if strings.Contains(got, "--gaps") {
		t.Fatalf("boolean false override should suppress --gaps: %q", got)
	}
	if strings.Contains(got, "--state-census") {
		t.Fatalf("boolean false override should suppress --state-census: %q", got)
	}
}
