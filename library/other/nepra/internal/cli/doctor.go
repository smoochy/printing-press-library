...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	// nepra-declared-scope.json): --scope declares what this CLI deliberately
	// does not do, in the machine-readable surface rather than only the README.
	var showScope bool
	cmd := &cobra.Command{
		Use:   "doctor",
		Short: "Check CLI health",
		Example: `  nepra-pp-cli doctor
  nepra-pp-cli doctor --json
  nepra-pp-cli doctor --fail-on warn
  nepra-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if showScope {
				return nepraPrintScope(cmd, flags)
			}
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "nepra-pp-cli", suggestion)
						} else {
	...
	cmd.Flags().BoolVar(&showScope, "scope", false, "Print what this CLI deliberately does not do, and why, with the nearest command that does answer")
