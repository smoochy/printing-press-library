...
type rootFlags struct {
	...
	// invocation. Mirrors the NEPRA_NO_LEARN env var.
	noLearn                 bool
	selectFields            string
	configPath              string
	homePath                string
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "nepra-pp-cli",
		Short: `Nepra CLI — Pakistan's electricity regulator publishes the country's only plant-level power data as Excel-exported HTML that reads…`,
		Long: `Nepra CLI — Pakistan's electricity regulator publishes the country's only plant-level power data as Excel-exported HTML that reads as empty and PDFs nobody extracts — this turns it into a queryable panel and names every gap.
...
Highlights (not in the official API docs):
  ...
  • conflicts   List every place NEPRA's own published figures disagree — two sources on one key, an unexplained step inside a single series, or a row that fails its own arithmetic — showing both values and both cit…
...
Health check: run 'nepra-pp-cli doctor' to verify connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("nepra-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'nepra-pp-cli profile list')")
	...
	// nepra-events-multipage-feed.json): the spec-emitted events command
	// pointed at /tariff/Tariff.php, a 9-byte HTTP 404. The real feed is 28
	// catalogued pages. Re-apply this line after any regen.
	rootCmd.AddCommand(newNepraEventsCmd(flags))
	rootCmd.AddCommand(newFcaPromotedCmd(flags))
	rootCmd.AddCommand(newHydelPromotedCmd(flags))
	rootCmd.AddCommand(newLicencePromotedCmd(flags))
	rootCmd.AddCommand(newQuarterlyPromotedCmd(flags))
	rootCmd.AddCommand(newReliabilityPromotedCmd(flags))
	rootCmd.AddCommand(newSirPromotedCmd(flags))
	rootCmd.AddCommand(newSroPromotedCmd(flags))
	rootCmd.AddCommand(newTariffPromotedCmd(flags))
	...
	addNovelCommandIfAbsent(rootCmd, newNovelCapacityCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelConflictsCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelDiscoCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelFleetCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelGenCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelSourcesCmd(flags))
