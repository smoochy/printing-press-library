...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("NEPRA_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("NEPRA_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "nepra-pp-cli/feedback")
...
When ` + "`NEPRA_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`NEPRA_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
maintainer sees it.`,
		Example: `  nepra-pp-cli feedback "the --since flag is inclusive but docs say exclusive"
  nepra-pp-cli feedback --stdin < notes.txt
  nepra-pp-cli feedback list --limit 10`,
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  nepra-pp-cli feedback list
  nepra-pp-cli feedback list --limit 5
  nepra-pp-cli feedback list --json`,
