...
func classifyAPIErrorOnly(err error) error {
	...
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: check your API credentials."+
			"\n      Run 'nepra-pp-cli doctor' to check auth status.", err))
	case strings.Contains(msg, "HTTP 403"):
		return authErr(fmt.Errorf("%w\nhint: permission denied. This API is configured without credentials, so the service may be blocking the request by rate limit, geography, bot protection, or endpoint policy."+
			"\n      Run 'nepra-pp-cli doctor' to check connectivity.", err))
...
func wrapAgentOutput(data json.RawMessage, meta map[string]any) (json.RawMessage, error) {
	...
	// Merge BEFORE defaulting source. The "local" below is a fallback for
	// callers that set none, not an observation, so it must never displace a
	// command's measured origin — coverage reports source "catalogue", and
	// defaulting first would have pushed a fabricated "local" into
	// envelope_meta as though something had observed it.
	if json.Valid(data) {
		if merged, ok := ownedEnvelopeMerge(data, meta); ok {
			return merged, nil
		}
	}
...
func printOutputWithFlags(w io.Writer, data json.RawMessage, flags *rootFlags) error {
	// nil, NOT {"source": "local"}. wrapAgentOutput applies exactly that
	// default itself, but only once it knows the payload carries no envelope
	// of its own. Asserting it here instead made a generic fallback outrank a
	// command's measured origin: coverage reports source "catalogue", which is
	// the same fact stated precisely, and the literal below used to displace it.
	return printOutputWithFlagsMeta(w, data, flags, nil)
}
...
// jsonMetaEqual reports whether two meta values encode to the same JSON. The
// left side has been through a JSON round-trip (so numbers are float64) while
// the right side is still a Go native, which makes == and reflect.DeepEqual
// both unreliable here; comparing encodings is the comparison that matters,
// because the encoding is what ships.
func jsonMetaEqual(a, b any) bool {
	ab, aerr := json.Marshal(a)
	bb, berr := json.Marshal(b)
	return aerr == nil && berr == nil && bytes.Equal(ab, bb)
}

// ownedEnvelopeMerge folds a generic wrapper's meta INTO an envelope the
// command already built, rather than nesting a second envelope around it.
// It reports whether data was such an envelope; when false the caller wraps
// as it always did.
//
// WHY THIS EXISTS. Several commands assemble their own {meta, results}, and
// that meta is the rich one — coverage carries the catalogue-join note and
// the staleness rule, licence carries as_of_date and the surface list. The
// generic wrappers then ran unconditionally on top and emitted
// {meta:{source}, results:{meta, results}}, so `.results[]` iterated nothing
// and every machine consumer had to know which commands nested twice.
// wrapPlatformStructuredOutput already merged in exactly this situation; the
// other two wrappers did not, which is the whole of the inconsistency.
//
// PRECEDENCE, and why it is this way round. The command's own meta wins,
// because it is the specific measured claim and the wrapper's is generic.
// The wrapper's value is NOT discarded when it genuinely differs: it is kept
// under envelope_meta. That matters because the two are different axes —
// DataProvenance.Source is a TRANSPORT claim ("live" vs "local", did this run
// touch the network) while a command's meta.source is a DATA-ORIGIN claim
// ("catalogue", "computed", "seeded"). Collapsing them would assert something
// neither observed. When they agree, or the wrapper adds a key the command
// never set, nothing extra is emitted and the envelope stays clean.
func ownedEnvelopeMerge(data json.RawMessage, wrapperMeta map[string]any) (json.RawMessage, bool) {
	var envelope map[string]json.RawMessage
	if json.Unmarshal(data, &envelope) != nil {
		return nil, false
	}
	// Same shape test wrapPlatformStructuredOutput uses, so the three
	// wrappers agree on what "already enveloped" means.
	if envelope["meta"] == nil || (envelope["results"] == nil && envelope["data"] == nil) {
		return nil, false
	}
	var own map[string]any
	if json.Unmarshal(envelope["meta"], &own) != nil {
		return nil, false
	}
	displaced := map[string]any{}
	for key, value := range wrapperMeta {
		existing, taken := own[key]
		switch {
		case !taken:
			own[key] = value
		case !jsonMetaEqual(existing, value):
			displaced[key] = value
		}
	}
	if len(displaced) > 0 {
		own["envelope_meta"] = displaced
	}
	mergedMeta, err := json.Marshal(own)
	if err != nil {
		return nil, false
	}
	envelope["meta"] = mergedMeta
...
func wrapWithProvenance(data json.RawMessage, prov DataProvenance) (json.RawMessage, error) {
	...
	// A command that built its own envelope gets this meta merged in, not a
	// second envelope wrapped around it. See ownedEnvelopeMerge.
	if merged, ok := ownedEnvelopeMerge(data, meta); ok {
		return merged, nil
	}
	envelope := map[string]any{
