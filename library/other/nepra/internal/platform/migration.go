...
func AdoptVerifiedLegacyDatabase(ctx context.Context, session *Session, state *LegacyMigrationState) error {
	...
	// #nosec G202 -- SQLite's VACUUM INTO takes a string literal, not a bind parameter.
	// targetPath is session.Paths.DataFile (CLI-owned state path, never user SQL) and
	// its single quotes are doubled on the line above.
