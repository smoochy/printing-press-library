...
func classifyAPIError(err error, flags *rootFlags) error {
	...
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: check your API key."+
			" Set one of: export BENZINGA_API_KEY=\"your-token-here\" or export CALENDAR_API_KEY=\"your-token-here\""+
			"\n      Run 'benzinga-pp-cli doctor' to check auth status.", err))
...
func nonJSONPayloadError(data json.RawMessage) error {
	...
	if len(trimmed) > 0 && trimmed[0] == '<' {
		return authErr(fmt.Errorf("not authenticated or session expired; API returned HTML instead of JSON. " + "Set one of: export BENZINGA_API_KEY=\"your-token-here\" or export CALENDAR_API_KEY=\"your-token-here\""))
	}
	if len(trimmed) == 0 {
