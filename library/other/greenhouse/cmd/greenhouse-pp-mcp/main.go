...
import (
	...
	mcptools "github.com/mvanhorn/printing-press-library/library/other/greenhouse/internal/mcp"
)
...
const (
	// Loopback by default: the HTTP transport exposes SQL and
	// command-mirror tools, so it must never listen on all
	// interfaces without an explicit operator choice.
...
func main() {
	...
	_ = os.Setenv("GREENHOUSE_LEARN_SURFACE", "mcp")
	...
	case "http":
		...
		fmt.Fprintf(os.Stderr, "greenhouse-pp-mcp serving MCP over streamable HTTP at %s\n", *addr)
