...
- **An empty search whose window reaches recent days says how far the source actually goes** ℹ️: `interrogazioni cerca --data 2026-08-15:2026-09-04` used to return `[]` and nothing else, while the archive stopped a month earlier — the gap and the coverage lived in two different commands. The hint (stderr, and inside `--envelope`) now measures that archive on the spot and names the date it stops at, carrying along the search's own structural filters, so a query on one committee reports that committee's frontier and not the whole archive's. When the frontier is more recent than the window asked for, latency does not explain the empty result and the hint stays silent. The window is `--data` where it exists and `--anno` on `leggi`, which has no `--data` and is the archive with the worst lag. The probe costs one request and only fires on an empty result whose window is recent; if it fails, the hint still comes out without a figure, except on `pareri` and `biblioteca`, where the frontier cannot be measured and the hint stays silent rather than pointing at a non-measurement.
...
- **`ars-sicilia-pp-cli resoconti get`** - Scarica un singolo resoconto. Risponde con la scheda del backend `/bd/` e restituisce `pdf_url`, dove sta il resoconto stenografico completo (il PDF non viene scaricato; l'URL è stabile e citabile). Le schede `/bd/` coprono l'archivio fin dalla XIII legislatura (seduta 1 del 25.07.2001). La scheda non ha il campo `body` e porta un campo `nota` che spiega perché: l'assenza di `body` non è «testo non disponibile». Solo se `/bd/` non ha la scheda il comando ripiega sull'indice Icaro, che di questo archivio tiene frammenti per punto dell'ordine del giorno: quel `body` è un frammento, può parlare di un'altra seduta, e la `nota` lo dichiara insieme al motivo del ripiego. Prima la precedenza era rovesciata - Icaro vinceva quando aveva il record - e `resoconti get 17 208 --select body` restituiva l'ordine del giorno della seduta 209 sotto `numero: 208`, con exit 0 e nessuna nota. Se il backend non risponde (tronca le risposte a intermittenza) ogni lettura viene ritentata fino a 3 volte, e l'errore che esce dopo resta distinto da `nessun documento trovato`, che compare solo quando il backend ha risposto e il documento davvero non c'è. **I due percorsi hanno la stessa forma**: `legisl`, `numero`, `data`, `data_iso`, `titolo` e `fonte` stanno in radice su entrambi, quindi lo stesso `--select numero,data_iso,titolo` rende su tutte le sedute. `fields` resta dov'era sul ramo Icaro: è un'aggiunta, non uno spostamento. `fonte` dice quale dei due percorsi ha risposto. Dalla stessa richiesta escono anche i blocchi che la scheda dichiara - `presidenza`, `oratori` (nome e gruppo), `odg` e `allegati`, questi ultimi due con l'URL del proprio PDF - senza una richiesta in più: la pagina si scaricava già per tenerne il solo href. I blocchi che una seduta non ha mancano e basta (la seduta 13/1 del 2001 non ha l'ordine del giorno, la 15/1 non ha gli oratori). `get` è un comando live: con `--data-source local` esce con `no local data source for this command`, perché le righe sincronizzate non hanno né `body` né `pdf_url`. `--con-testo` aggiunge il campo `testo` con la versione testuale della seduta, che sta sulla stessa pagina e quindi non costa una richiesta in più: fuori per default perché sono decine di migliaia di battute. Il portale non la pubblica su tutte le sedute - misurato il 2026-09-12, dalla 13/1 del 2001 alla 232 del 25.02.2026 - e dove manca la `nota` lo dice invece di restituire un campo vuoto.
