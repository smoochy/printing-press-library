...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			fmt.Fprintln(w, "  echo \"$TOKEN\" | splitwise-pp-cli auth set-token")
			if !launch {
				return nil
			}
	...
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !authed {
				...
				fmt.Fprintln(w, "  export SPLITWISE_API_KEY=\"your-token-here\"")
				fmt.Fprintf(w, "  echo \"$TOKEN\" | splitwise-pp-cli auth set-token\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  echo \"$TOKEN\" | splitwise-pp-cli auth set-token\n  splitwise-pp-cli auth set-token < token-file",
		Args:    cobra.NoArgs,
		RunE: func(cmd *cobra.Command, args []string) error {
