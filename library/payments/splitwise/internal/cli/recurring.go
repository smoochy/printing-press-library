...
func newNovelRecurringCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long:        "Use this command to find repeating bills and a cycle that was not logged. Do NOT use it to project upcoming amounts; use 'forecast'.",
		Example:     "  splitwise-pp-cli recurring --agent\n  splitwise-pp-cli recurring --min-occurrences 3 --json",
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return writeDryRun(cmd.OutOrStdout(), flags, "recurring")
			}
			if minOccurrences < 2 {
				return novelErr(cmd, flags, usageErr(fmt.Errorf("--min-occurrences must be >= 2")))
			}
			if limit < 1 {
