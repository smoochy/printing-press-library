...
func newNovelBalancesCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:         "balances",
		Short:       "Show your net balance per friend from the synced store, or per currency (--by-currency) or per group (--by-group)",
		Long:        "Use this command for your current net position (per friend, per currency, or per group). Do NOT use it for how long a debt has been open; use 'debts --aged'. Do NOT use it to collapse balances across groups into real-world transfers; use 'net'. Do NOT use it to plan a group settlement; use 'settle-up'.",
		Example:     "  splitwise-pp-cli balances --agent",
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return writeDryRun(cmd.OutOrStdout(), flags, "balances")
			}
...
func newDebtsCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:         "debts",
		Short:       "List who owes whom from synced balances, sorted by days since the last settled point (--aged)",
		Long:        "Use this command for who owes whom sorted by staleness. Do NOT use it for a plain net-position snapshot; use 'balances'. Do NOT use it for carrier-vs-rider or collection-risk classification; use 'fairness'.",
		Example:     "  splitwise-pp-cli debts --aged --agent",
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return writeDryRun(cmd.OutOrStdout(), flags, "debts")
			}
