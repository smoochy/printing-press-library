...
func loadNormalizeRatesFile(path string) (map[string]float64, error) {
	...
	// #nosec G304 -- path is the user-supplied --rates-file argument; reading the file the user named is the feature.
...
// pp:data-source local
func newNovelNormalizeCmd(flags *rootFlags) *cobra.Command {
	var dbPath string
	base := "USD"
	...
	cmd := &cobra.Command{
		...
		Long:        "Use this command to express spend in one currency using rates you supply. Do NOT use it for spend buckets; use 'spend'.",
		Example:     "  splitwise-pp-cli normalize --base USD --rate EUR=1.08 --rate GBP=1.27 --agent",
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return writeDryRun(cmd.OutOrStdout(), flags, "normalize")
			}

			baseCode := strings.ToUpper(strings.TrimSpace(base))
			if baseCode == "" {
				return novelErr(cmd, flags, usageErr(fmt.Errorf("--base must not be empty")))
			}
			...
			if youID == 0 {
				...
				_, _ = fmt.Fprintln(cmd.ErrOrStderr(), "note: current user not synced; run: splitwise-pp-cli sync --resources get-current-user")
			}
