...
func newNovelForecastCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long:        "Use this command for what shared bills are expected next. Items already past their expected date stay in upcoming for up to three missed cycles and are flagged overdue: true. Do NOT use it to detect which bills recur; use 'recurring'.",
		Example:     "  splitwise-pp-cli forecast --agent\n  splitwise-pp-cli forecast --days 60 --limit 20 --json",
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return writeDryRun(cmd.OutOrStdout(), flags, "forecast")
			}
			if days < 1 {
				return novelErr(cmd, flags, usageErr(fmt.Errorf("--days must be >= 1")))
			}
			if limit < 1 {
