...
type rootFlags struct {
	...
	// invocation. Mirrors the SPLITWISE_NO_LEARN env var.
	noLearn bool
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		...
		Short: `Splitwise CLI — Every Splitwise feature, plus an offline SQLite ledger that powers balance, debt-aging, spend analytics, fairness, and…`,
		Long: `Splitwise CLI — Every Splitwise feature, plus an offline SQLite ledger that powers balance, debt-aging, spend analytics, fairness, and full-text search no other Splitwise tool has.
...
Highlights (not in the official API docs):
  ...
  …and 3 more — see README.md for the full list
...
See README.md or the bundled SKILL.md for recipes.`,
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'splitwise-pp-cli profile list')")
	...
	addNovelCommandIfAbsent(rootCmd, newNovelBalancesCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelBriefCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelDebtsCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelFairnessCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelForecastCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelLedgerCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelNetCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelNormalizeCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelReconcileCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelRecurringCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelReportCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelSettleUpCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelSpendCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelSplitCmd(flags))
