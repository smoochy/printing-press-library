...
func newNovelAuditCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long:        "Use this command BEFORE settling to catch duplicate settlements and cost outliers. Do NOT use it to see what changed recently; use 'activity'. Do NOT use it to check the local store is complete vs the API; use 'reconcile'.",
		Example:     "  splitwise-pp-cli audit --since 30d --agent",
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			if limit < 1 {
				return novelErr(cmd, flags, usageErr(fmt.Errorf("--limit must be >= 1")))
			}
			...
			windowed := make([]Expense, 0, len(expenses))
			for _, expense := range expenses {
				if expenseInWindow(expense, window) {
					windowed = append(windowed, expense)
				}
			}
			res := runAudit(windowed, limit)
