...
// pp:data-source local
func newNovelFairnessCmd(flags *rootFlags) *cobra.Command {
	var dbPath string
	var by string
	...
	cmd := &cobra.Command{
		...
		Long:        "Use this command for who is carrying cost vs riding, and how collectable a debt is. Do NOT use it for a plain aged list; use 'debts --aged'. Do NOT use it for net position; use 'balances'.\n\nExit code 3 means the named group/friend was not found in the local store.",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return writeDryRun(cmd.OutOrStdout(), flags, "fairness")
			}
			if by != "risk" && by != "contribution" && by != "collectability" {
				return novelErr(cmd, flags, usageErr(fmt.Errorf("invalid --by value %q: must be risk, contribution, or collectability", by)))
			}
			if friendRef != "" && groupRef != "" {
				return novelErr(cmd, flags, usageErr(errors.New("--friend and --group are mutually exclusive")))
			}
			if writeOffDays <= 0 {
				return novelErr(cmd, flags, usageErr(fmt.Errorf("--write-off-days must be >= 1")))
			}
			if ghostDays <= 0 {
				return novelErr(cmd, flags, usageErr(fmt.Errorf("--ghost-days must be >= 1")))
			}
			if minEpisodes < 1 {
				return novelErr(cmd, flags, usageErr(fmt.Errorf("--min-episodes must be >= 1")))
			}
			...
			if strings.TrimSpace(since) != "" {
				...
				if err != nil {
					return novelErr(cmd, flags, usageErr(fmt.Errorf("invalid --since %q: %w", since, err)))
				}
				opts.since = t
				opts.hasSince = true
			}
			...
			if strings.TrimSpace(friendRef) != "" {
				...
				if !ok {
					return novelErr(cmd, flags, notFoundErr(fmt.Errorf("no friend matches %q; run sync first", friendRef)))
				}
				opts.friendID = id
				scope = "friend:" + name
			}
			if strings.TrimSpace(groupRef) != "" {
				...
				if !ok {
					return novelErr(cmd, flags, notFoundErr(fmt.Errorf("no group matches %q; run sync first", groupRef)))
				}
				opts.groupID = id
	...
	cmd.AddCommand(newNovelFairnessNudgeCmd(flags))
	return cmd
}
...
func computeFairness(youID int, friends []Friend, groups []Group, expenses []Expense, now time.Time, opts fairnessOpts) fairnessResult {
	...
	for _, row := range raw {
		...
		case "collectability", "contribution":
			if p.HasHistory {
				people = append(people, p)
				// --by collectability emits risk_tier per person, so its at_risk_total /
				// write_off_total must sum the same tiers it reports (they were only
				// rolled up for --by risk, leaving 0 beside write_off rows).
				if result.By == "collectability" && p.OutstandingTotal > 0 {
					result.AtRiskTotal += p.OutstandingTotal
					if p.RiskTier == "write_off" {
						result.WriteOffTotal += p.OutstandingTotal
					}
	...
	case "collectability":
		sort.Slice(people, func(i, j int) bool {
			if (people[i].RiskScore != nil) != (people[j].RiskScore != nil) {
				return people[i].RiskScore != nil
			}
			if people[i].RiskScore != nil && *people[i].RiskScore != *people[j].RiskScore {
				return *people[i].RiskScore > *people[j].RiskScore
			}
