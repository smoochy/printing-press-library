...
// pp:data-source live
func newNovelSplitCmd(flags *rootFlags) *cobra.Command {
	amount := 0.0
	...
	cmd := &cobra.Command{
		...
		Long:  "Use this command to build and preview a new shared expense's shares. Pass --record to opt in to creating it. Do NOT use it to record a settlement payment; use 'settle-up --record'.\n\nExit code 3 means the named group/friend was not found in the local store.",
		...
		Annotations: map[string]string{"mcp:read-only": "false", "pp:happy-args": "<group>=Example Group;--amount=84", "pp:method": "GET", "pp:typed-exit-codes": "0,3"},
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dryRunOK(flags) {
				return writeDryRun(cmd.OutOrStdout(), flags, "split")
			}
			if len(args) == 0 {
				return novelErr(cmd, flags, usageErr(errors.New("group name or id is required")))
			}
			if amount <= 0 {
				return novelErr(cmd, flags, usageErr(errors.New("--amount must be > 0")))
			}
			if record && cliutil.IsAnyHarness() {
				return writeHarnessRefusal(cmd.OutOrStdout(), flags, "split --record")
			}
			...
			if ambErr != nil {
				return novelErr(cmd, flags, usageErr(ambErr))
			}
			if !ok {
				return novelErr(cmd, flags, notFoundErr(fmt.Errorf("no group matches %q; run sync first", groupName)))
			}
			...
			if payer == 0 {
				return novelErr(cmd, flags, usageErr(errors.New("--paid-by is required when current user is not synced; run: splitwise-pp-cli sync --resources get-current-user")))
			}

			members := group.Members
			if len(members) == 0 {
				return novelErr(cmd, flags, usageErr(fmt.Errorf("group %q has no members", strings.TrimSpace(group.Name))))
			}
			...
			if _, ok := memberSet[payer]; !ok {
				return novelErr(cmd, flags, usageErr(fmt.Errorf("paid-by user %d is not a member of %q", payer, strings.TrimSpace(group.Name))))
			}
			...
			for _, m := range sortedMembers {
				...
				paid := "0.00"
				if m.ID == payer {
					paid = centsToMoneyString(totalCents)
				}
			...
			body := map[string]any{
				"cost":          centsToMoneyString(totalCents),
			...
			if !record {
				if flags.asJSON || flags.agent || !isTerminal(cmd.OutOrStdout()) {
					if err := flags.emitStructured(cmd, view); err != nil {
						return err
					}
				} else {
			...
			if envErr := splitwiseMutationError(respData); envErr != nil {
				if flags.asJSON || flags.agent || !isTerminal(cmd.OutOrStdout()) {
					_ = flags.emitStructured(cmd, map[string]any{"status_code": status, "created": false})
					cmd.SilenceErrors = true
				}
				return fmt.Errorf("create_expense rejected: %w", envErr)
			}
			if err := upsertCreatedExpenses(db, respData); err != nil {
				return fmt.Errorf("caching created expense: %w", err)
			}
