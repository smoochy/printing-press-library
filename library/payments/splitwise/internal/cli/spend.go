...
func newNovelSpendCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long:        "Use this command for \"how much did we spend on X / in <month> / in <group>\". Do NOT use it for who owes whom; use 'balances'. Do NOT use it for a formatted export; use 'report'. Do NOT use it to convert currencies; use 'normalize'.",
		Example:     "  splitwise-pp-cli spend --group-by category --since 30d --agent",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if groupBy != "category" && groupBy != "group" && groupBy != "month" {
				return novelErr(cmd, flags, usageErr(fmt.Errorf("invalid --group-by value %q: must be category, group, or month", groupBy)))
			}
			window, err := parseExpenseWindow(since, until, time.Now().UTC())
			if err != nil {
			...
			for _, e := range expenses {
				if e.Payment || expenseDeleted(e.DeletedAt) || !expenseInWindow(e, window) {
					continue
				}
...
func newLedgerCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			_ = hintIfUnsynced(cmd, db, "get-groups") ||
				hintIfUnsynced(cmd, db, "get-expenses")
			...
			if err != nil {
				if strings.Contains(err.Error(), "not found") {
					return novelErr(cmd, flags, notFoundErr(err))
				}
				return novelErr(cmd, flags, usageErr(err))
			}
