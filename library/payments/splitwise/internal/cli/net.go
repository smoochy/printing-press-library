...
func newNovelNetCmd(flags *rootFlags) *cobra.Command {
	var dbPath string
	cmd := &cobra.Command{
		Use:         "net",
		Short:       "Net balances across all groups into the fewest direct transfers",
		Long:        "Use this command when one person's balance spans many groups and non-group expenses and you want the minimum real-world transfers. Do NOT use it for a single group; use 'settle-up'. Do NOT use it for the per-group snapshot; use 'balances --by-group'.",
		Example:     "  splitwise-pp-cli net --agent",
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return writeDryRun(cmd.OutOrStdout(), flags, "net")
			}
