...
func newNovelReportCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "report",
		Short: "Export an offline trip/period spend report as md, csv, or json, with per-person and per-category breakdowns",
		Long:  "Use this command for a shareable end-of-trip / period export. Do NOT use it for an interactive rollup query; use 'spend'. Do NOT use it for the running history; use 'ledger'.\n\nExit code 3 means the named group/friend was not found in the local store.",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(args) > 0 && strings.TrimSpace(groupRef) != "" {
				return novelErr(cmd, flags, usageErr(fmt.Errorf("use either a positional group or --group, not both")))
			}
			if len(args) > 0 {
				groupRef = joinNameArgs(args)
			}

			format = strings.ToLower(strings.TrimSpace(format))
			if format != "" && format != "md" && format != "csv" && format != "json" {
				return novelErr(cmd, flags, usageErr(fmt.Errorf("invalid --format value %q: must be md, csv, or json", format)))
			}

			if _, err := parseExpenseWindow(since, until, time.Now().UTC()); err != nil {
			...
			_ = hintIfUnsynced(cmd, db, "get-expenses") ||
			...
			case "md":
				...
				return nil
			case "json":
				return flags.emitStructured(cmd, res)
			default:
	...
	cmd.Flags().StringVar(&since, "since", "", "Include expenses on/after a duration or date (e.g. 30d, 1w, 2025-01-01, RFC3339)")
	cmd.Flags().StringVar(&until, "until", "", "Include expenses on/before a duration or date (e.g. 7d, 2025-12-31, RFC3339)")
	cmd.Flags().StringVar(&currency, "currency", "", "Single currency code (default picks most common and excludes others)")
	cmd.Flags().StringVar(&format, "format", "", "Output format: md|csv|json")
...
func computeReport(expenses []Expense, groups []Group, youID int, opts reportOpts) reportResult {
	...
	window, _ := parseExpenseWindow(opts.Since, opts.Until, time.Now().UTC())
