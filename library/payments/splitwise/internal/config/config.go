...
func Load(configPath string) (*Config, error) {
	...
	if v := os.Getenv("SPLITWISE_API_KEY"); v != "" {
		...
		cfg.markEnvOverride("SplitwiseApiKey")
		cfg.AuthSource = "env:SPLITWISE_API_KEY"
		cfg.CredentialSource = "env:SPLITWISE_API_KEY"
	}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("SPLITWISE_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "splitwise-pp-cli", "config.toml"), nil
}
...
func (c *Config) StoreScopeCredential() string {
	...
	if c.SplitwiseApiKey != "" {
		parts = append(parts, "api_key="+c.SplitwiseApiKey)
	}
	if len(parts) == 0 {
		return ""
	}
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		SplitwiseApiKey: c.SplitwiseApiKey,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	if c.SplitwiseApiKey == "" {
		c.SplitwiseApiKey = creds.SplitwiseApiKey
	}
}
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.SplitwiseApiKey = ""
	delete(c.envOverrides, "SplitwiseApiKey")
	c.updateFileConfigField("SplitwiseApiKey")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["SplitwiseApiKey"] {
			out.SplitwiseApiKey = c.fileConfig.SplitwiseApiKey
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "SplitwiseApiKey":
		c.fileConfig.SplitwiseApiKey = c.SplitwiseApiKey
	}
}
