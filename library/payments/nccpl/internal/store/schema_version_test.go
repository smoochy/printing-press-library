...
// TestMigrate_AddsColumnsOnUpgrade_Fipi verifies that opening a
...
func TestMigrate_AddsColumnsOnUpgrade_Fipi(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "fipi" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	rows, err := s.DB().Query(`SELECT name FROM pragma_table_xinfo(?)`, "fipi")
	if err != nil {
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from fipi after migrate", want)
		}
	}
	...
	for _, wantIdx := range []string{} {
		...
		if err := s.DB().QueryRow(
			...
			t.Fatalf("%s index missing from fipi after migrate: %v", wantIdx, err)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_FipiNormal verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_FipiNormal(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "fipi_normal" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// table_info hides generated columns; table_xinfo is the verify
	// surface for VIRTUAL backfills such as bare_id.
	rows, err := s.DB().Query(`SELECT name FROM pragma_table_xinfo(?)`, "fipi_normal")
	if err != nil {
		t.Fatalf("table_xinfo: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from fipi_normal after migrate", want)
		}
	}
	...
	for _, wantIdx := range []string{} {
		...
		if err := s.DB().QueryRow(
			...
			t.Fatalf("%s index missing from fipi_normal after migrate: %v", wantIdx, err)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_FipiSector verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_FipiSector(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "fipi_sector" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// table_info hides generated columns; table_xinfo is the verify
	// surface for VIRTUAL backfills such as bare_id.
	rows, err := s.DB().Query(`SELECT name FROM pragma_table_xinfo(?)`, "fipi_sector")
	if err != nil {
		t.Fatalf("table_xinfo: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from fipi_sector after migrate", want)
		}
	}
	...
	for _, wantIdx := range []string{} {
		...
		if err := s.DB().QueryRow(
			...
			t.Fatalf("%s index missing from fipi_sector after migrate: %v", wantIdx, err)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_Lipi verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_Lipi(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "lipi" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// table_info hides generated columns; table_xinfo is the verify
	// surface for VIRTUAL backfills such as bare_id.
	rows, err := s.DB().Query(`SELECT name FROM pragma_table_xinfo(?)`, "lipi")
	if err != nil {
		t.Fatalf("table_xinfo: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from lipi after migrate", want)
		}
	}
	...
	for _, wantIdx := range []string{} {
		...
		if err := s.DB().QueryRow(
			...
			t.Fatalf("%s index missing from lipi after migrate: %v", wantIdx, err)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_LipiNormal verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_LipiNormal(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "lipi_normal" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// table_info hides generated columns; table_xinfo is the verify
	// surface for VIRTUAL backfills such as bare_id.
	rows, err := s.DB().Query(`SELECT name FROM pragma_table_xinfo(?)`, "lipi_normal")
	if err != nil {
		t.Fatalf("table_xinfo: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from lipi_normal after migrate", want)
		}
	}
	...
	for _, wantIdx := range []string{} {
		...
		if err := s.DB().QueryRow(
			...
			t.Fatalf("%s index missing from lipi_normal after migrate: %v", wantIdx, err)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_LipiSector verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_LipiSector(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "lipi_sector" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// table_info hides generated columns; table_xinfo is the verify
	// surface for VIRTUAL backfills such as bare_id.
	rows, err := s.DB().Query(`SELECT name FROM pragma_table_xinfo(?)`, "lipi_sector")
	if err != nil {
		t.Fatalf("table_xinfo: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from lipi_sector after migrate", want)
		}
	}
	...
	for _, wantIdx := range []string{} {
		...
		if err := s.DB().QueryRow(
			...
			t.Fatalf("%s index missing from lipi_sector after migrate: %v", wantIdx, err)
		}
	}
}
...
func TestMigrate_AddsColumnsOnUpgrade_Market(t *testing.T) {
	...
	// table_info hides generated columns; table_xinfo is the verify
	// surface for VIRTUAL backfills such as bare_id.
	rows, err := s.DB().Query(`SELECT name FROM pragma_table_xinfo(?)`, "market")
	if err != nil {
		t.Fatalf("table_xinfo: %v", err)
	}
	defer rows.Close()
	...
	for _, wantIdx := range []string{} {
		...
		if err := s.DB().QueryRow(
			...
			t.Fatalf("%s index missing from market after migrate: %v", wantIdx, err)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_Mfs verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_Mfs(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "mfs" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// table_info hides generated columns; table_xinfo is the verify
	// surface for VIRTUAL backfills such as bare_id.
	rows, err := s.DB().Query(`SELECT name FROM pragma_table_xinfo(?)`, "mfs")
	if err != nil {
		t.Fatalf("table_xinfo: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from mfs after migrate", want)
		}
	}
	...
	for _, wantIdx := range []string{} {
		...
		if err := s.DB().QueryRow(
			...
			t.Fatalf("%s index missing from mfs after migrate: %v", wantIdx, err)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_MfsTop verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_MfsTop(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "mfs_top" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// table_info hides generated columns; table_xinfo is the verify
	// surface for VIRTUAL backfills such as bare_id.
	rows, err := s.DB().Query(`SELECT name FROM pragma_table_xinfo(?)`, "mfs_top")
	if err != nil {
		t.Fatalf("table_xinfo: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from mfs_top after migrate", want)
		}
	}
	...
	for _, wantIdx := range []string{} {
		...
		if err := s.DB().QueryRow(
			...
			t.Fatalf("%s index missing from mfs_top after migrate: %v", wantIdx, err)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_Msf verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_Msf(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "msf" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// table_info hides generated columns; table_xinfo is the verify
	// surface for VIRTUAL backfills such as bare_id.
	rows, err := s.DB().Query(`SELECT name FROM pragma_table_xinfo(?)`, "msf")
	if err != nil {
		t.Fatalf("table_xinfo: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from msf after migrate", want)
		}
	}
	...
	for _, wantIdx := range []string{} {
		...
		if err := s.DB().QueryRow(
			...
			t.Fatalf("%s index missing from msf after migrate: %v", wantIdx, err)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_MsfTop verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_MsfTop(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "msf_top" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// table_info hides generated columns; table_xinfo is the verify
	// surface for VIRTUAL backfills such as bare_id.
	rows, err := s.DB().Query(`SELECT name FROM pragma_table_xinfo(?)`, "msf_top")
	if err != nil {
		t.Fatalf("table_xinfo: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from msf_top after migrate", want)
		}
	}
	...
	for _, wantIdx := range []string{} {
		...
		if err := s.DB().QueryRow(
			...
			t.Fatalf("%s index missing from msf_top after migrate: %v", wantIdx, err)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_Mts verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_Mts(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "mts" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// table_info hides generated columns; table_xinfo is the verify
	// surface for VIRTUAL backfills such as bare_id.
	rows, err := s.DB().Query(`SELECT name FROM pragma_table_xinfo(?)`, "mts")
	if err != nil {
		t.Fatalf("table_xinfo: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from mts after migrate", want)
		}
	}
	...
	for _, wantIdx := range []string{} {
		...
		if err := s.DB().QueryRow(
			...
			t.Fatalf("%s index missing from mts after migrate: %v", wantIdx, err)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_MtsFinanciers verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_MtsFinanciers(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "mts_financiers" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// table_info hides generated columns; table_xinfo is the verify
	// surface for VIRTUAL backfills such as bare_id.
	rows, err := s.DB().Query(`SELECT name FROM pragma_table_xinfo(?)`, "mts_financiers")
	if err != nil {
		t.Fatalf("table_xinfo: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from mts_financiers after migrate", want)
		}
	}
	...
	for _, wantIdx := range []string{} {
		...
		if err := s.DB().QueryRow(
			...
			t.Fatalf("%s index missing from mts_financiers after migrate: %v", wantIdx, err)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_MtsForceRelease verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_MtsForceRelease(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "mts_force_release" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// table_info hides generated columns; table_xinfo is the verify
	// surface for VIRTUAL backfills such as bare_id.
	rows, err := s.DB().Query(`SELECT name FROM pragma_table_xinfo(?)`, "mts_force_release")
	if err != nil {
		t.Fatalf("table_xinfo: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from mts_force_release after migrate", want)
		}
	}
	...
	for _, wantIdx := range []string{} {
		...
		if err := s.DB().QueryRow(
			...
			t.Fatalf("%s index missing from mts_force_release after migrate: %v", wantIdx, err)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_MtsRefinanced verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_MtsRefinanced(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "mts_refinanced" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// table_info hides generated columns; table_xinfo is the verify
	// surface for VIRTUAL backfills such as bare_id.
	rows, err := s.DB().Query(`SELECT name FROM pragma_table_xinfo(?)`, "mts_refinanced")
	if err != nil {
		t.Fatalf("table_xinfo: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from mts_refinanced after migrate", want)
		}
	}
	...
	for _, wantIdx := range []string{} {
		...
		if err := s.DB().QueryRow(
			...
			t.Fatalf("%s index missing from mts_refinanced after migrate: %v", wantIdx, err)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_MtsTopFinanciers verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_MtsTopFinanciers(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "mts_top_financiers" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// table_info hides generated columns; table_xinfo is the verify
	// surface for VIRTUAL backfills such as bare_id.
	rows, err := s.DB().Query(`SELECT name FROM pragma_table_xinfo(?)`, "mts_top_financiers")
	if err != nil {
		t.Fatalf("table_xinfo: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from mts_top_financiers after migrate", want)
		}
	}
	...
	for _, wantIdx := range []string{} {
		...
		if err := s.DB().QueryRow(
			...
			t.Fatalf("%s index missing from mts_top_financiers after migrate: %v", wantIdx, err)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_SettlementCm verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_SettlementCm(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "settlement_cm" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// table_info hides generated columns; table_xinfo is the verify
	// surface for VIRTUAL backfills such as bare_id.
	rows, err := s.DB().Query(`SELECT name FROM pragma_table_xinfo(?)`, "settlement_cm")
	if err != nil {
		t.Fatalf("table_xinfo: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from settlement_cm after migrate", want)
		}
	}
	...
	for _, wantIdx := range []string{} {
		...
		if err := s.DB().QueryRow(
			...
			t.Fatalf("%s index missing from settlement_cm after migrate: %v", wantIdx, err)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_SettlementUin verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_SettlementUin(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "settlement_uin" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// table_info hides generated columns; table_xinfo is the verify
	// surface for VIRTUAL backfills such as bare_id.
	rows, err := s.DB().Query(`SELECT name FROM pragma_table_xinfo(?)`, "settlement_uin")
	if err != nil {
		t.Fatalf("table_xinfo: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from settlement_uin after migrate", want)
		}
	}
	...
	for _, wantIdx := range []string{} {
		...
		if err := s.DB().QueryRow(
			...
			t.Fatalf("%s index missing from settlement_uin after migrate: %v", wantIdx, err)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_Slb verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_Slb(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "slb" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// table_info hides generated columns; table_xinfo is the verify
	// surface for VIRTUAL backfills such as bare_id.
	rows, err := s.DB().Query(`SELECT name FROM pragma_table_xinfo(?)`, "slb")
	if err != nil {
		t.Fatalf("table_xinfo: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from slb after migrate", want)
		}
	}
	...
	for _, wantIdx := range []string{} {
		...
		if err := s.DB().QueryRow(
			...
			t.Fatalf("%s index missing from slb after migrate: %v", wantIdx, err)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_Tfc verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_Tfc(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "tfc" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// table_info hides generated columns; table_xinfo is the verify
	// surface for VIRTUAL backfills such as bare_id.
	rows, err := s.DB().Query(`SELECT name FROM pragma_table_xinfo(?)`, "tfc")
	if err != nil {
		t.Fatalf("table_xinfo: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from tfc after migrate", want)
		}
	}
	...
	for _, wantIdx := range []string{} {
		...
		if err := s.DB().QueryRow(
			...
			t.Fatalf("%s index missing from tfc after migrate: %v", wantIdx, err)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_VarMargins verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_VarMargins(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "var_margins" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// table_info hides generated columns; table_xinfo is the verify
	// surface for VIRTUAL backfills such as bare_id.
	rows, err := s.DB().Query(`SELECT name FROM pragma_table_xinfo(?)`, "var_margins")
	if err != nil {
		t.Fatalf("table_xinfo: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from var_margins after migrate", want)
		}
	}
	...
	for _, wantIdx := range []string{} {
		...
		if err := s.DB().QueryRow(
			...
			t.Fatalf("%s index missing from var_margins after migrate: %v", wantIdx, err)
		}
	}
}
