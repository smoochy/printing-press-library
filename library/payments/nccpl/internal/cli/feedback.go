...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("NCCPL_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("NCCPL_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "nccpl-pp-cli/feedback")
	...
	// The local ledger has already been appended by this point, so a throttle
	// here costs nothing but the upstream copy. Return it typed anyway: the
	// caller can then retry after the stated delay instead of treating a
	// transient 429 as a permanently rejected submission.
	if resp.StatusCode == http.StatusTooManyRequests {
		return cliutil.RateLimitFromResponse(resp, url)
	}
...
When ` + "`NCCPL_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`NCCPL_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
maintainer sees it.`,
		Example: `  nccpl-pp-cli feedback "the --since flag is inclusive but docs say exclusive"
  nccpl-pp-cli feedback --stdin < notes.txt
  nccpl-pp-cli feedback list --limit 10`,
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  nccpl-pp-cli feedback list
  nccpl-pp-cli feedback list --limit 5
  nccpl-pp-cli feedback list --json`,
