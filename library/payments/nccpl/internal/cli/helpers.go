...
func classifyAPIErrorOnly(err error) error {
	...
	case strings.Contains(msg, "HTTP 409"):
		return apiErr(fmt.Errorf("%w\nhint: the server reports a conflict (HTTP 409): the request clashes with the current state of the resource — for a create-style call, that the target already exists."+
			"\n      NCCPL's data endpoints are read-only POSTs, so 409 is rare here and usually means a duplicate or concurrent request on the same session; re-running the command is safe."+
			"\n      If it persists, run 'nccpl-pp-cli auth login --chrome' to refresh browser-session credentials."+
			"\n      See API docs: https://www.nccpl.com.pk"+
			"\n      Run 'nccpl-pp-cli doctor' to check auth status.", err))
	...
	case strings.Contains(msg, "HTTP 400") && cliutil.LooksLikeAuthError(msg):
		return authErr(fmt.Errorf("%w\nhint: the API rejected the request — this usually means auth is missing or invalid."+
			...
			"\n      See API docs: https://www.nccpl.com.pk"+
			"\n      Run 'nccpl-pp-cli doctor' to check auth status."+
