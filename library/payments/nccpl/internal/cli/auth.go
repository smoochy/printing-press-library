...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:         "auth",
		Short:       "Manage authentication for Nccpl",
...
func requiredAuthCookies() []string {
	raw := []string{"cf_clearance", "nccpl-session", "XSRF-TOKEN"}
	out := raw[:0]
...
Use --chrome to read cookies from Chrome for www.nccpl.com.pk.
...
profile by name when the installed backend supports it.`,
		Example: `  nccpl-pp-cli auth login --chrome
  nccpl-pp-cli auth login --browser
  nccpl-pp-cli auth login --cookies-file storage-state.json
  nccpl-pp-cli auth login --chrome --profile "Work"`,
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			domain := "www.nccpl.com.pk"
			if !browserFlag && cookiesFile == "" {
				fmt.Fprintln(w, "Use --chrome, --browser, or --cookies-file to authenticate from your browser session:")
				fmt.Fprintf(cmd.OutOrStdout(), "  nccpl-pp-cli auth login --chrome\n")
				fmt.Fprintf(cmd.OutOrStdout(), "  nccpl-pp-cli auth login --browser\n")
				fmt.Fprintf(cmd.OutOrStdout(), "  nccpl-pp-cli auth login --cookies-file storage-state.json\n")
				return nil
			}
			if browserFlag && cookiesFile != "" {
			...
			if !fromPressAuth && !fromCookiesFile {
				...
				if err != nil {
					...
					if runtime.GOOS == "windows" {
						...
						fmt.Fprintf(w, "\n  nccpl-pp-cli auth login --cookies-file <path>\n\n")
			...
			requiredCookies := []string{"cf_clearance", "nccpl-session", "XSRF-TOKEN"}
			...
			composed := composeAuthFromCookies("{XSRF-TOKEN}", requiredCookies, nccplDecodedCookieMap(cookieMap))
			...
			cfg.CredentialDomain = "www.nccpl.com.pk"
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  nccpl-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			if !credentialConfigured {
				...
				fmt.Fprintf(w, "  nccpl-pp-cli auth login --chrome\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}

			fmt.Fprintln(w, green("Authenticated"))
			fmt.Fprintf(w, "  Source: %s\n", cfg.AuthSource)
			fmt.Fprintf(w, "  Domain: www.nccpl.com.pk\n")
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  echo \"$TOKEN\" | nccpl-pp-cli auth set-token\n  nccpl-pp-cli auth set-token < token-file",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  nccpl-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
...
func validateComposedAuth(authHeader, cookieHeader string) error {
	...
	// A Cloudflare clearance cookie cannot be validated from the stdlib client;
	// see nccplStdlibValidationIsMeaningless.
	if nccplStdlibValidationIsMeaningless(cookieHeader) {
		return nil
	}

	testURL := "https://www.nccpl.com.pk"
