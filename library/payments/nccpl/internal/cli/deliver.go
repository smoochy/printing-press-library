...
func deliverWebhook(url string, body []byte, compact bool) error {
	...
	if ua := os.Getenv("NCCPL_USER_AGENT"); ua != "" {
		req.Header.Set("User-Agent", ua)
	} else {
		req.Header.Set("User-Agent", "nccpl-pp-cli/deliver")
	}
	...
	// A throttled sink is retryable and the rest of the 4xx range is not, so
	// they must not share an error. --deliver is used unattended; a caller that
	// cannot tell "resend in 30s" from "this webhook is gone" either drops
	// output or retries something that will never succeed.
