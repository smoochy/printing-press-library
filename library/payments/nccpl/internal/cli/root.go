...
type rootFlags struct {
	...
	// invocation. Mirrors the NCCPL_NO_LEARN env var.
	noLearn                 bool
	selectFields            string
	configPath              string
	homePath                string
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "nccpl-pp-cli",
		Short: `Pakistan's clearing-layer data as a local research panel with a coverage audit and an arithmetic self-check, not another dashboard.`,
		Long: `Pakistan's clearing-layer data as a local research panel with a coverage audit and an arithmetic self-check, not another dashboard.
...
Health check: run 'nccpl-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("nccpl-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.selectFields, "select", "", "Comma-separated fields to include in output (e.g. --select client_type,segment,buy_value)")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'nccpl-pp-cli profile list')")
	...
	rootCmd.AddCommand(newFipiCmd(flags))
	rootCmd.AddCommand(newFipiNormalCmd(flags))
	rootCmd.AddCommand(newFipiSectorCmd(flags))
	rootCmd.AddCommand(newLipiCmd(flags))
	rootCmd.AddCommand(newLipiNormalCmd(flags))
	rootCmd.AddCommand(newLipiSectorCmd(flags))
	rootCmd.AddCommand(newMarketCmd(flags))
	rootCmd.AddCommand(newMfsCmd(flags))
	rootCmd.AddCommand(newMfsTopCmd(flags))
	rootCmd.AddCommand(newMsfCmd(flags))
	rootCmd.AddCommand(newMsfTopCmd(flags))
	rootCmd.AddCommand(newMtsCmd(flags))
	rootCmd.AddCommand(newMtsFinanciersCmd(flags))
	rootCmd.AddCommand(newMtsForceReleaseCmd(flags))
	rootCmd.AddCommand(newMtsRefinancedCmd(flags))
	rootCmd.AddCommand(newMtsTopFinanciersCmd(flags))
	rootCmd.AddCommand(newSettlementCmCmd(flags))
	rootCmd.AddCommand(newSettlementUinCmd(flags))
	rootCmd.AddCommand(newSlbCmd(flags))
	rootCmd.AddCommand(newTfcCmd(flags))
	rootCmd.AddCommand(newVarMarginsCmd(flags))
	...
	// The eleven hand-written NCCPL commands. These are also registered by the
	// init() hook in nccpl_register.go, which is what preserves the wiring across
	// `generate --force`; addNovelCommandIfAbsent no-ops when the name is already
	// present, so listing them here as well is idempotent. They are listed here so
	// root.go is a complete statement of the command surface.
	rootCmd.AddCommand(newNCCPLSyncCmd(flags))
	rootCmd.AddCommand(newNCCPLCoverageCmd(flags))
	rootCmd.AddCommand(newNCCPLVerifyCmd(flags))
	rootCmd.AddCommand(newNCCPLPanelCmd(flags))
	rootCmd.AddCommand(newNCCPLUniverseCmd(flags))
	rootCmd.AddCommand(newNCCPLLeverageCmd(flags))
	rootCmd.AddCommand(newNCCPLRiskChangesCmd(flags))
	rootCmd.AddCommand(newNCCPLContractCheckCmd(flags))
	rootCmd.AddCommand(newNCCPLFlowsCmd(flags))
	rootCmd.AddCommand(newNCCPLIngestCmd(flags))
	rootCmd.AddCommand(newNCCPLCaptureCmd(flags))
