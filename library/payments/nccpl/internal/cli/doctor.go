...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  nccpl-pp-cli doctor
  nccpl-pp-cli doctor --json
  nccpl-pp-cli doctor --fail-on warn
  nccpl-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if credentialRefused {
				...
				if !credentialConfigured {
					...
					report["auth_hint"] = "nccpl-pp-cli auth login --chrome"
				} else {
					authConfigured = true
					...
					if cfg != nil && cfg.CredentialConfigured() {
						...
						report["auth_domain"] = "www.nccpl.com.pk"
					}
				}
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					healthPath := "/api/fipi/latest-date"
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "nccpl-pp-cli", suggestion)
						} else {
