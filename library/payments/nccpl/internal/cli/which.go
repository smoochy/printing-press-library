...
var whichIndex = []whichEntry{
	{Command: "verify", Description: "Prove a date's flow numbers are internally consistent before they reach a regression.", Group: "Research-grade integrity", WhyItMatters: "Reach for this before feeding any NCCPL date into a model; a date that fails an invariant is corrupt input, not a weak signal."},
	{Command: "coverage", Description: "List which sessions are missing per resource, how stale each one is, and how wide each date's data actually is.", Group: "Research-grade integrity", WhyItMatters: "Use this as a pipeline pre-flight - it exits non-zero on a gap, so a scheduled research job fails loudly instead of silently treating a missing session as a zero. Re-syncing a date replaces it rather than adding to it: a stored settlement date always mirrors the last snapshot fetched for it, so if NCCPL revises a date and drops symbols the next sync removes them and the store never serves a row the source stopped publishing. coverage row_count therefore always equals the number of observations actually stored, and the audit can never disagree with the data it audits. One deliberate exception: a fetch returning zero rows for a date that already holds rows does NOT delete them, because an empty response and a transient failure look identical from here; the next non-empty fetch mirrors the date correctly."},
	{Command: "contract-check", Description: "Assert every endpoint family still answers correctly for a date it just reported as its own latest.", Group: "Research-grade integrity", WhyItMatters: "Run this when results look empty - it separates an expired session or a changed request contract from a genuine no-data day."},
	{Command: "panel", Description: "Emit any synced resource as a tidy long-format panel, with gaps marked rather than filled.", Group: "Local history that compounds", WhyItMatters: "This is the handoff from CLI to research store - it is the shape a regression consumes, and the observed_at column is what makes a flow number admissible as an ex-ante input."},
	{Command: "universe", Description: "Reconstruct which symbols were listed and clearing-eligible on any past date.", Group: "Local history that compounds", WhyItMatters: "Use this to state a screen's universe width in genuinely live names, and as a second opinion that can disagree with a price-staleness filter for reasons that filter cannot move for."},
	{Command: "risk-changes", Description: "Date every step change in a symbol's free float, VAR margin and haircut.", Group: "Local history that compounds", WhyItMatters: "Free float is the input a cap-weighted cross-section needs and no other public Pakistani source publishes it daily."},
	{Command: "leverage", Description: "Join MTS, MFS and MSF open positions with SLB net open position into one per-symbol cross-section.", Group: "Cross-market joins", WhyItMatters: "SLB net open position is the closest thing this market has to published short interest; pair it with MTS and MFS open interest to see leverage building in a name before a forced release."},
	{Command: "flows", Description: "Fetch daily FIPI/LIPI sector flows into the local store without a browser.", Group: "Reachability mitigation", WhyItMatters: "This is the one NCCPL dataset obtainable unattended, so it is what a scheduled daily job can actually collect."},
	{Command: "ingest", Description: "Load NCCPL responses captured through your own browser into the same local store.", Group: "Reachability mitigation", WhyItMatters: "Reach for this when a dataset is gated: export a HAR from a normal browsing session and every other command works on it unchanged. Bodies can also be piped in directly with --stdin (pbpaste | nccpl-pp-cli ingest --stdin --resource var-margins --date 2026-09-04), so a single copied response never needs a temp file. Nothing unidentifiable is stored: a HAR entry is only ingested when it was served by nccpl.com.pk or a subdomain over https, and a response body must carry either the documented envelope key for the resource or exactly one of the API's own other envelope names. An all-traffic capture routinely holds other origins -- a proxy, a mock, a local dev server on the same path -- and those are refused and listed under skipped with a reason, never filed under a guess."},
	{Command: "capture", Description: "Fetch the gated NCCPL datasets through a controlled browser and store them locally; opt in with --launch, and add --headless to run with no window.", Group: "Reachability mitigation", WhyItMatters: "This is the only route to per-symbol free float, VAR margins, leverage positions and settlement data; every other command then reads them from the local store with no browser involved. With --headless no window appears and the run is unattended, so a scheduled job can keep the gated datasets current; Chrome must be installed because the clearance cannot be replayed by any non-browser HTTP client."},
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one. Machine output (--json/--csv/--plain/--quiet, or a pipe) still exits 2 and writes {"matches":[]} (or the equivalent empty table) to stdout.`,
		SilenceUsage: true,
		Example: `  nccpl-pp-cli which "stale tickets"
  nccpl-pp-cli which "bottleneck"
  nccpl-pp-cli which --limit 1 "send message"
  nccpl-pp-cli which                                # list the full capability index`,
