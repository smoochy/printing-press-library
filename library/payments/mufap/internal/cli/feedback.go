...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("MUFAP_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("MUFAP_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "mufap-pp-cli/feedback")
...
When ` + "`MUFAP_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`MUFAP_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
maintainer sees it.`,
		Example: `  mufap-pp-cli feedback "the --since flag is inclusive but docs say exclusive"
  mufap-pp-cli feedback --stdin < notes.txt
  mufap-pp-cli feedback list --limit 10`,
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  mufap-pp-cli feedback list
  mufap-pp-cli feedback list --limit 5
  mufap-pp-cli feedback list --json`,
