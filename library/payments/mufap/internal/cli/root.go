...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/payments/mufap/internal/learn"
...
type rootFlags struct {
	...
	// invocation. Mirrors the MUFAP_NO_LEARN env var.
	noLearn                 bool
	selectFields            string
	configPath              string
	homePath                string
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "mufap-pp-cli",
		Short: `Pakistan's mutual fund industry as a dated local panel: two decades of daily NAVs, monthly PKR asset allocation, and a market-implied short rate no other tool derives.`,
		Long: `Pakistan's mutual fund industry as a dated local panel: two decades of daily NAVs, monthly PKR asset allocation, and a market-implied short rate no other tool derives.
...
Health check: run 'mufap-pp-cli doctor' to verify connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("mufap-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'mufap-pp-cli profile list')")
	...
	rootCmd.AddCommand(newNetsalesCmd(flags))
	rootCmd.AddCommand(newVpsCmd(flags))
	...
	rootCmd.AddCommand(newAllocationPromotedCmd(flags))
	rootCmd.AddCommand(newAmcsPromotedCmd(flags))
	rootCmd.AddCommand(newDatesPromotedCmd(flags))
	rootCmd.AddCommand(newFundsPromotedCmd(flags))
	rootCmd.AddCommand(newPayoutsPromotedCmd(flags))
	rootCmd.AddCommand(newUnitholdersPromotedCmd(flags))
	...
	addNovelCommandIfAbsent(rootCmd, newNovelBackfillCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelCoverageCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelDispersionCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelExportCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelExposureCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelFreshnessCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelPanelCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelRatesCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelUniverseCmd(flags))
