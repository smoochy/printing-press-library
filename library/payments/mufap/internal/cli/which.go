...
var whichIndex = []whichEntry{
	{Command: "rates", Description: "Derive a daily short-term interest rate series from the cross-section of money-market fund yields.", Group: "Series only a local panel can produce", WhyItMatters: "This is the only daily, backfillable PKR short-rate proxy obtainable without a blocked government source."},
	{Command: "exposure", Description: "Total PKR the mutual fund industry holds in listed equities, by month, from per-fund asset allocation.", Group: "Series only a local panel can produce", WhyItMatters: "It is the asset-side counterpart to a mutual-fund net-flow series, enabling a cross-source join no single publisher offers."},
	{Command: "backfill", Description: "Fetch the daily fund panel or monthly allocation across a date range into the local store.", Group: "Local panel construction", WhyItMatters: "Publication timing is what makes a variable admissible as ex-ante, so every row records when it was actually observed."},
	{Command: "panel", Description: "Query the stored daily NAV and return panel by date, fund, category or sector.", Group: "Local panel construction", WhyItMatters: "Turns 21 years of single-date HTML pages into one queryable table."},
	{Command: "coverage", Description: "Show which dates were fetched, which returned zero rows, and which were never attempted.", Group: "Local panel construction", WhyItMatters: "Distinguishes fetched-and-empty from never-fetched, so a gap is never mistaken for a zero."},
	{Command: "verify allocation", Description: "Check that fund asset-class percentages net to 100 and flag months whose percent columns are unpopulated.", Group: "Trust the numbers", WhyItMatters: "A month that fails the invariant is corrupt input, not a weak signal, and must be excluded before modelling."},
	{Command: "universe", Description: "Report how many funds reported on each date, by sector and category.", Group: "Trust the numbers", WhyItMatters: "A silently narrowing universe fakes verdicts, so width is printed alongside every cross-sectional result."},
	{Command: "dispersion", Description: "Cross-sectional spread of fund returns within a category on each date.", Group: "Series only a local panel can produce", WhyItMatters: "Dispersion is a daily breadth measure that a single-fund page cannot express."},
	{Command: "freshness", Description: "Detect funds whose published NAV validity date lags the requested date.", Group: "Trust the numbers", WhyItMatters: "Differencing the live view without this check manufactures returns that never happened."},
	{Command: "dump", Description: "Dump the stored panel, allocation or coverage tables as JSONL for piping, a single JSON array for agents, or CSV.", Group: "Local panel construction", WhyItMatters: "Lets an analysis pipeline consume the panel directly instead of scraping."},
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one. Machine output (--json/--csv/--plain/--quiet, or a pipe) still exits 2 and writes {"matches":[]} (or the equivalent empty table) to stdout.`,
		SilenceUsage: true,
		Example: `  mufap-pp-cli which "stale tickets"
  mufap-pp-cli which "bottleneck"
  mufap-pp-cli which --limit 1 "send message"
  mufap-pp-cli which                                # list the full capability index`,
