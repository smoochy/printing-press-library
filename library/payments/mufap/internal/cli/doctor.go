...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  mufap-pp-cli doctor
  mufap-pp-cli doctor --json
  mufap-pp-cli doctor --fail-on warn
  mufap-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					reachBody, reachErr := mufapHealthGet(cmd.Context(), c, healthPath)
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "mufap-pp-cli", suggestion)
						} else {
