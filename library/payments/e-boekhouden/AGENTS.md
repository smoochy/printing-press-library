# e-Boekhouden Printed CLI Agent Guide

This directory is a generated `e-boekhouden-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
e-boekhouden-pp-cli doctor --json
e-boekhouden-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
e-boekhouden-pp-cli which "<capability>" --json
e-boekhouden-pp-cli <command> --help
```
...
```bash
e-boekhouden-pp-cli <command> --agent
```
...
```bash
e-boekhouden-pp-cli <command> --help
e-boekhouden-pp-cli <command> --dry-run --agent
```
