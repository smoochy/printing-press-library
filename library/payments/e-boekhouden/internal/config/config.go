...
type Config struct {
	...
	Path                string            `toml:"-"`
	EboekhoudenApiToken string            `toml:"api_token"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://api.e-boekhouden.nl",
	}

	// Resolve config path
	path := configPath
	if path == "" {
		path = os.Getenv("E_BOEKHOUDEN_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "e-boekhouden-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("EBOEKHOUDEN_API_TOKEN"); v != "" {
		cfg.EboekhoudenApiToken = v
		cfg.AuthSource = "env:EBOEKHOUDEN_API_TOKEN"
	}
	...
	if cfg.AuthSource == "" && cfg.EboekhoudenApiToken != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("E_BOEKHOUDEN_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	token := c.EboekhoudenApiToken
	if token == "" {
		return ""
	}
	if c.EboekhoudenApiToken == "" {
		return ""
	}
	return token
}
