...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for E Boekhouden",
...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  e-boekhouden-pp-cli auth setup\n  e-boekhouden-pp-cli auth setup --launch",
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			fmt.Fprintln(w, "Get a key at: https://mijn.e-boekhouden.nl/instellingen/koppelingen")
			fmt.Fprintln(w, "  Settings -> Koppelingen -> API-token aanmaken. This is your long-lived API token; the CLI exchanges it for a short-lived session token automatically on every run.")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Then set:")
			fmt.Fprintln(w, "  export EBOEKHOUDEN_API_TOKEN=\"<your-token>\"")
			fmt.Fprintln(w, "  e-boekhouden-pp-cli auth set-token <token>")
			if !launch {
				return nil
			}
			launchURL := "https://mijn.e-boekhouden.nl/instellingen/koppelingen"
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  e-boekhouden-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !authed {
				...
				fmt.Fprintln(w, "  export EBOEKHOUDEN_API_TOKEN=\"your-token-here\"")
				fmt.Fprintf(w, "  e-boekhouden-pp-cli auth set-token <token>\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  e-boekhouden-pp-cli auth set-token YOUR_TOKEN_HERE",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  e-boekhouden-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			envStillSet := ""
			if envStillSet == "" && os.Getenv("EBOEKHOUDEN_API_TOKEN") != "" {
				envStillSet = "EBOEKHOUDEN_API_TOKEN"
			}
