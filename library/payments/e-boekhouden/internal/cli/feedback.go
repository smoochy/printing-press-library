...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "e-boekhouden-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("E_BOEKHOUDEN_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("E_BOEKHOUDEN_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "e-boekhouden-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/e-boekhouden-pp-cli/feedback.jsonl.
When ` + "`E_BOEKHOUDEN_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`E_BOEKHOUDEN_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  e-boekhouden-pp-cli feedback list
  e-boekhouden-pp-cli feedback list --limit 5
  e-boekhouden-pp-cli feedback list --json`,
