...
Resource scoping:
  ...
  e-boekhouden-pp-cli sync

  # Sync specific resources only
  e-boekhouden-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  e-boekhouden-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  e-boekhouden-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  e-boekhouden-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  e-boekhouden-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/e-boekhouden-pp-cli/data.db)")
...
// typedTableNameOverrides maps a sync resource name to the store's typed
// dispatch/table name when they differ. store.UpsertBatch's per-item typed
// dispatch switches on the exact resourceType string against literal table
// names ("balance", "ledger", ...); "ledger-balances" (the sync resource
// name, matching its API path segment) has a dedicated "balance" typed
// table, so passing it through unmapped silently skips the typed dispatch —
// confirmed against a live account: the generic resources table gets all 58
// rows under resource_type="ledger-balances", but the "balance" table (and
// every local report that reads it) stays empty with no error surfaced.
var typedTableNameOverrides = map[string]string{
	"ledger-balances": "balance",
}

func upsertResourceBatch(db *store.Store, resource string, items []json.RawMessage) (int, int, error) {
	if target, ok := typedTableNameOverrides[resource]; ok {
		resource = target
	}
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "costcenter":
		return db.UpsertCostcenter(data)
	case "invoice":
		return db.UpsertInvoice(data)
	case "ledger":
		return db.UpsertLedger(data)
	case "balance":
		return db.UpsertBalance(data)
	case "member":
		return db.UpsertMember(data)
	case "mutation":
		return db.UpsertMutation(data)
	case "product":
		return db.UpsertProduct(data)
	case "relation":
		return db.UpsertRelation(data)
	case "session":
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"administration":        "/v1/administration",
		"administration-linked": "/v1/administration/linked",
		"costcenter":            "/v1/costcenter",
		"emailtemplate":         "/v1/emailtemplate",
		"invoice":               "/v1/invoice",
		"invoicetemplate":       "/v1/invoicetemplate",
		"ledger":                "/v1/ledger",
		"ledger-balances":       "/v1/ledger/balances",
		"member":                "/v1/member",
		"mutation":              "/v1/mutation",
		"product":               "/v1/product",
		"product-groups":        "/v1/product/groups",
