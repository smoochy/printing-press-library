...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  e-boekhouden-pp-cli doctor
  e-boekhouden-pp-cli doctor --json
  e-boekhouden-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export EBOEKHOUDEN_API_TOKEN=<your-key>"
					report["auth_key_url"] = "https://mijn.e-boekhouden.nl/instellingen/koppelingen"
					report["auth_instructions"] = "Settings -> Koppelingen -> API-token aanmaken. This is your long-lived API token; the CLI exchanges it for a short-lived session token automatically on every run."
				} else {
					authConfigured = true
			...
			if os.Getenv("EBOEKHOUDEN_API_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "EBOEKHOUDEN_API_TOKEN")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "EBOEKHOUDEN_API_TOKEN")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "e-boekhouden-pp-cli", suggestion)
						} else {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'e-boekhouden-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'e-boekhouden-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'e-boekhouden-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'e-boekhouden-pp-cli sync' to refresh."
	}
	return report
}
