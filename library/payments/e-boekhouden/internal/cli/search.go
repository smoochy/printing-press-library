...
In local mode: searches locally synced data only.`,
  ...
  e-boekhouden-pp-cli search "error timeout"

  # Force local search only
  e-boekhouden-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  e-boekhouden-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  e-boekhouden-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'e-boekhouden-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "ledger":
				results, err = db.SearchLedger(query, limit)
			case "member":
				results, err = db.SearchMember(query, limit)
			case "relation":
				results, err = db.SearchRelation(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchLedger(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search ledger failed: %w", searchErr)
					}
				...
				{
					partial, searchErr := db.SearchRelation(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search relation failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
