...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  e-boekhouden-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  e-boekhouden-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"administration", "administration-linked", "costcenter", "emailtemplate", "invoice", "invoicetemplate", "ledger", "ledger-balances", "member", "mutation", "product", "product-groups", "relation", "swagger", "unit"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  e-boekhouden-pp-cli workflow status

  # Show status as JSON
  e-boekhouden-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("e-boekhouden-pp-cli")
			}
