...
func TestMCPPathResolutionMatchesCLIResolverWithHomeEnv(t *testing.T) {
	...
	t.Setenv("CDC_PAKISTAN_HOME", root)
...
func TestMCPPathResolutionMatchesCLIResolverWithPlatformDefaults(t *testing.T) {
	...
	if want := filepath.Join(home, ".config", "cdc-pakistan-pp-cli", "config.toml"); cfg.Path != want {
	...
	if want := filepath.Join(home, ".local", "share", "cdc-pakistan-pp-cli", "data.db"); gotDB != want {
...
func TestMCPRegisterToolsPreservesTypedSpecialTools(t *testing.T) {
	s := server.NewMCPServer("cdc-pakistan", "test")
	RegisterTools(s)
