...
type rootFlags struct {
	...
	// invocation. Mirrors the CDC_PAKISTAN_NO_LEARN env var.
	noLearn                 bool
	selectFields            string
	configPath              string
	homePath                string
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "cdc-pakistan-pp-cli",
		Short: `The only machine-readable view of Pakistani custody data — per-security CDS penetration and a twenty-year eligibility record, from a source no tool has ever automated.`,
		Long: `The only machine-readable view of Pakistani custody data — per-security CDS penetration and a twenty-year eligibility record, from a source no tool has ever automated.
...
Health check: run 'cdc-pakistan-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("cdc-pakistan-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'cdc-pakistan-pp-cli profile list')")
	...
	rootCmd.AddCommand(newDownloadsPromotedCmd(flags))
	rootCmd.AddCommand(newStatisticsPromotedCmd(flags))
	...
	addNovelCommandIfAbsent(rootCmd, newNovelEligibilityCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelFloatCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelGopCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelIdentityCmd(flags))
