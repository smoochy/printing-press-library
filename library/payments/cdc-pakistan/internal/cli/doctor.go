...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  cdc-pakistan-pp-cli doctor
  cdc-pakistan-pp-cli doctor --json
  cdc-pakistan-pp-cli doctor --fail-on warn
  cdc-pakistan-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if credentialRefused {
				...
				if !credentialConfigured {
					...
					report["auth_hint"] = "cdc-pakistan-pp-cli auth login --chrome"
				} else {
					authConfigured = true
					...
					if cfg != nil && cfg.CredentialConfigured() {
						...
						report["auth_domain"] = "www.cdcpakistan.com"
					}
				}
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "cdc-pakistan-pp-cli", suggestion)
						} else {
