...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("CDC_PAKISTAN_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("CDC_PAKISTAN_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "cdc-pakistan-pp-cli/feedback")
...
When ` + "`CDC_PAKISTAN_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`CDC_PAKISTAN_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
maintainer sees it.`,
		Example: `  cdc-pakistan-pp-cli feedback "the --since flag is inclusive but docs say exclusive"
  cdc-pakistan-pp-cli feedback --stdin < notes.txt
  cdc-pakistan-pp-cli feedback list --limit 10`,
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  cdc-pakistan-pp-cli feedback list
  cdc-pakistan-pp-cli feedback list --limit 5
  cdc-pakistan-pp-cli feedback list --json`,
