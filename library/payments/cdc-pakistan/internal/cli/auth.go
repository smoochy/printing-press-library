...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:         "auth",
		Short:       "Manage authentication for Cdc Pakistan",
...
Use --chrome to read cookies from Chrome for www.cdcpakistan.com.
...
profile by name when the installed backend supports it.`,
		Example: `  cdc-pakistan-pp-cli auth login --chrome
  cdc-pakistan-pp-cli auth login --browser
  cdc-pakistan-pp-cli auth login --cookies-file storage-state.json
  cdc-pakistan-pp-cli auth login --chrome --profile "Work"`,
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			domain := "www.cdcpakistan.com"
			if !browserFlag && cookiesFile == "" {
				fmt.Fprintln(w, "Use --chrome, --browser, or --cookies-file to authenticate from your browser session:")
				fmt.Fprintf(cmd.OutOrStdout(), "  cdc-pakistan-pp-cli auth login --chrome\n")
				fmt.Fprintf(cmd.OutOrStdout(), "  cdc-pakistan-pp-cli auth login --browser\n")
				fmt.Fprintf(cmd.OutOrStdout(), "  cdc-pakistan-pp-cli auth login --cookies-file storage-state.json\n")
				return nil
			}
			if browserFlag && cookiesFile != "" {
			...
			if !fromPressAuth && !fromCookiesFile {
				...
				if err != nil {
					...
					if runtime.GOOS == "windows" {
						...
						fmt.Fprintf(w, "\n  cdc-pakistan-pp-cli auth login --cookies-file <path>\n\n")
			...
			requiredCookies := []string{"cf_clearance"}
			...
			cfg.CredentialDomain = "www.cdcpakistan.com"
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  cdc-pakistan-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			if !credentialConfigured {
				...
				fmt.Fprintf(w, "  cdc-pakistan-pp-cli auth login --chrome\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}

			fmt.Fprintln(w, green("Authenticated"))
			fmt.Fprintf(w, "  Source: %s\n", cfg.AuthSource)
			fmt.Fprintf(w, "  Domain: www.cdcpakistan.com\n")
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  echo \"$TOKEN\" | cdc-pakistan-pp-cli auth set-token\n  cdc-pakistan-pp-cli auth set-token < token-file",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  cdc-pakistan-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
