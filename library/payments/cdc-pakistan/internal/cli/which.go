...
var whichIndex = []whichEntry{
	{Command: "coverage map", Description: "See exactly which document buckets are mirrored, which are genuinely absent at source, and which were never asked for.", Group: "Local state that compounds", WhyItMatters: "Run this before trusting any other command's completeness; it is the only command that writes the document index."},
	{Command: "identity ledger", Description: "Reconstruct a security's symbol, name and ISIN history so you never splice two different issuers into one return series.", Group: "Local state that compounds", WhyItMatters: "Reach for this before joining CDC data to any symbol-keyed price series; symbol recycling silently corrupts factor research."},
	{Command: "eligibility state", Description: "Get a security's full CDS-eligibility history folded from twenty years of notices into a six-state lifecycle.", Group: "Twenty years of regulatory events", WhyItMatters: "Use for CDS-eligibility history, but note CDS eligibility is not the same as PSX trading suspension and is not a tradability gate."},
	{Command: "verify rows", Description: "Prove the PDF extraction is trustworthy by asserting the report's own percentage column against its own share and capital columns.", Group: "Extraction you can trust", WhyItMatters: "Run this before any analysis that depends on the penetration numbers; residual signatures tell you which column drifted."},
	{Command: "verify schema", Description: "Refuse to blend vintages whose column sets are incompatible, and refuse a month that is missing one of its parts.", Group: "Extraction you can trust", WhyItMatters: "Run this whenever a new vintage lands; it blocks silently-incompatible data instead of averaging it."},
	{Command: "float triangulate", Description: "Put CDC custody penetration beside PSX free-float shares and NCCPL free-float percent, with each denominator named.", Group: "Cross-source joins", WhyItMatters: "Use this to see where the three float definitions disagree; it deliberately never blends them into one number."},
	{Command: "gop stake", Description: "Derive state-held capital per security from the difference between the including-GoP and excluding-GoP capital columns.", Group: "Cross-source joins", WhyItMatters: "Use for a current cross-section of state ownership; there is only one vintage, so no time series exists."},
	{Command: "stats history", Description: "Track the sixteen CDS aggregate metrics forward from your first sync, with archived rows labelled as such.", Group: "Local state that compounds", WhyItMatters: "Use to measure retail-participation growth; archived rows are non-contiguous and must never be read as a continuous series."},
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one. Machine output (--json/--csv/--plain/--quiet, or a pipe) still exits 2 and writes {"matches":[]} (or the equivalent empty table) to stdout.`,
		SilenceUsage: true,
		Example: `  cdc-pakistan-pp-cli which "stale tickets"
  cdc-pakistan-pp-cli which "bottleneck"
  cdc-pakistan-pp-cli which --limit 1 "send message"
  cdc-pakistan-pp-cli which                                # list the full capability index`,
