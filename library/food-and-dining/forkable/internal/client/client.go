...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	// pp:hand-edit forkable-cookiejar — seed the cookie jar from the stored
	// cookie credential so authenticated GraphQL POSTs carry the session.
	// See internal/client/forkable_cookiejar.go.
	c.seedCookieJar()
	return c
}
...
func (c *Client) doInternal(ctx context.Context, method, path string, params map[string]string, body any, headerOverrides map[string]string, readOnlyIntent bool) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		// pp:hand-edit forkable-csrf — inject the x-csrf-token header on
		// GraphQL POSTs. See internal/client/forkable_csrf.go. Explicit
		// headerOverrides above still win; this only fills a missing token.
		if needsCSRF(path) && req.Header.Get("x-csrf-token") == "" {
			if tok := c.ensureCSRFToken(ctx); tok != "" {
...
func authPlaceholderCredentialError(cfg *config.Config) error {
	return authPlaceholderCredentialErrorWithSetup(cfg, "forkable-pp-cli auth set-token <token>")
}
