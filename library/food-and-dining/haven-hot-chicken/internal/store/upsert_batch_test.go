...
// TestUpsertBatch_PopulatesModifiersTable verifies that UpsertBatch
// dispatches paginated items into both the generic resources table AND the
// typed modifiers table. Regression for issue #268: before the fix, paginated
// syncs only filled the generic resources table, so domain commands that
// query the typed table saw zero rows.
func TestUpsertBatch_PopulatesModifiersTable(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if _, _, err := s.UpsertBatch("modifiers", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "modifiers").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "modifiers")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count modifiers: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("modifiers count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}

// TestUpsertBatch_TypedFailureDoesNotStrandModifiersGeneric exercises
...
func TestUpsertBatch_TypedFailureDoesNotStrandModifiersGeneric(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	stored, extractFailures, err := s.UpsertBatch("modifiers", items)
	if err != nil {
	...
	if typed != 0 {
		t.Fatalf("modifiers count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "items_id")
	}
}

// TestUpsertBatch_KeysModifiersByChildAndParent verifies dependent
...
func TestUpsertBatch_KeysModifiersByChildAndParent(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if typed != len(items) {
		t.Fatalf("modifiers count = %d, want %d (typed rows collapsed on child id)", typed, len(items))
	}

	var parentMatches int
	parentQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s" WHERE "items_id" = ?`, "modifiers")
...
// TestUpsertBatch_PopulatesMenuCategoriesTable verifies that UpsertBatch
// dispatches paginated items into both the generic resources table AND the
// typed menu_categories table. Regression for issue #268: before the fix, paginated
// syncs only filled the generic resources table, so domain commands that
// query the typed table saw zero rows.
func TestUpsertBatch_PopulatesMenuCategoriesTable(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if _, _, err := s.UpsertBatch("menu-categories", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "menu-categories").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	if generic != len(items) {
		t.Fatalf("resources count = %d, want %d", generic, len(items))
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "menu_categories")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count menu_categories: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("menu_categories count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}

// TestUpsertBatch_SetsMenuCategoriesParentID verifies that dependent-resource
...
func TestUpsertBatch_SetsMenuCategoriesParentID(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	var matchedA int
	parentQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s" WHERE parent_id = ?`, "menu_categories")
...
// TestUpsertBatch_MenuCategoriesSearchableViaGenericFTS verifies that a
...
// non-FTS5 `--type menu-categories` fallback), so populating it is the
...
func TestUpsertBatch_MenuCategoriesSearchableViaGenericFTS(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	// field is matchable regardless of which columns menu_categories declares.
	const probe = "zqxjpp2629probe"
	items := []json.RawMessage{
		json.RawMessage(`{"id": "search-001", "pp_fts_probe": "` + probe + `", "parent_id": "parent-A"}`),
	}
	stored, _, err := s.UpsertBatch("menu-categories", items)
	if err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	if len(results) == 0 {
		t.Fatalf("dependent resource %q stored but not searchable via generic resources_fts (issue #2629)", "menu-categories")
	}
}

func TestSearchMenuCategoriesQuotesFTSQuerySyntax(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if stored, failed, err := s.UpsertBatch("menu-categories", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	} else if failed != 0 || stored != len(items) {
		t.Fatalf("UpsertBatch stored=%d failed=%d, want stored=%d failed=0", stored, failed, len(items))
	}
	...
	for _, query := range []string{
		...
		results, err := s.SearchMenuCategories(query, 10)
		if err != nil {
			t.Fatalf("SearchMenuCategories(%q): %v", query, err)
		}
		if len(results) == 0 {
			t.Fatalf("SearchMenuCategories(%q) returned no results", query)
		}
	}
}

// TestUpsertBatch_PopulatesMenusTable verifies that UpsertBatch
// dispatches paginated items into both the generic resources table AND the
// typed menus table. Regression for issue #268: before the fix, paginated
// syncs only filled the generic resources table, so domain commands that
// query the typed table saw zero rows.
func TestUpsertBatch_PopulatesMenusTable(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if _, _, err := s.UpsertBatch("menus", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "menus").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	if generic != len(items) {
		t.Fatalf("resources count = %d, want %d", generic, len(items))
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "menus")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count menus: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("menus count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}

// TestUpsertBatch_SetsMenusParentID verifies that dependent-resource
// sync (which injects parent_id into each item's JSON) populates the typed
// parent_id column when items go through UpsertBatch. Regression for issue #268.
func TestUpsertBatch_SetsMenusParentID(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	var matchedA int
	parentQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s" WHERE parent_id = ?`, "menus")
	if err := db.QueryRow(parentQuery, "parent-A").Scan(&matchedA); err != nil {
		t.Fatalf("count by parent_id: %v", err)
	}
	if matchedA != 2 {
...
// TestUpsertBatch_MenusSearchableViaGenericFTS verifies that a
// dependent-resource row stored via UpsertBatch — the exact path dependent
// sync uses (upsertResourceBatch -> UpsertBatch) — is reachable through the
// generic resources_fts index via Store.Search. That index is the substrate
// both `search` surfaces query (the empty-type search directly, and the
// non-FTS5 `--type menus` fallback), so populating it is the
// precondition issue #2629 reported missing — dependent resources stored in
// the typed table but absent from any FTS index. This asserts the store layer
// where the claimed defect lived; it does not exercise the CLI search wiring.
func TestUpsertBatch_MenusSearchableViaGenericFTS(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	// resources_fts indexes the full JSON blob, so a distinctive token in any
	// field is matchable regardless of which columns menus declares.
	const probe = "zqxjpp2629probe"
	items := []json.RawMessage{
		json.RawMessage(`{"id": "search-001", "pp_fts_probe": "` + probe + `", "parent_id": "parent-A"}`),
	}
	stored, _, err := s.UpsertBatch("menus", items)
	if err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	// Assert the probe row actually landed before searching for it: a PK
	...
	if len(results) == 0 {
		t.Fatalf("dependent resource %q stored but not searchable via generic resources_fts (issue #2629)", "menus")
	}
}
...
func TestUpsertBatch_PopulatesCategoriesTable(t *testing.T) {
	...
	items := []json.RawMessage{
		json.RawMessage(`{"id": "test-001", "menus_id": "test-parent-001"}`),
		json.RawMessage(`{"id": "test-002", "menus_id": "test-parent-001"}`),
		json.RawMessage(`{"id": "test-003", "menus_id": "test-parent-001"}`),
	}
...
func TestUpsertBatch_TypedFailureDoesNotStrandCategoriesGeneric(t *testing.T) {
	...
	// Items deliberately omit "menus_id" so the typed NOT NULL
	// constraint fires.
	items := []json.RawMessage{
		json.RawMessage(`{"id": "orphan-001"}`),
		json.RawMessage(`{"id": "orphan-002"}`),
	...
	if typed != 0 {
		t.Fatalf("categories count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "menus_id")
	}
}
...
func TestUpsertBatch_KeysCategoriesByChildAndParent(t *testing.T) {
	...
	items := []json.RawMessage{
		json.RawMessage(`{"id": "shared_child", "menus_id": "parent_A"}`),
		json.RawMessage(`{"id": "shared", "menus_id": "child_parent_A"}`),
		json.RawMessage(`{"id": "shared_child", "menus_id": "parent_B"}`),
		json.RawMessage(`{"id": "other_child", "menus_id": "parent_A"}`),
	}
	stored, extractFailures, err := s.UpsertBatch("categories", items)
	if err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	...
	var parentMatches int
	parentQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s" WHERE "menus_id" = ?`, "categories")
	if err := db.QueryRow(parentQuery, "parent_A").Scan(&parentMatches); err != nil {
		t.Fatalf("count by menus_id: %v", err)
	}
	if parentMatches != 2 {
		t.Fatalf("menus_id=parent_A count = %d, want 2", parentMatches)
	}
}
