...
// TestMigrate_AddsColumnsOnUpgrade_Modifiers verifies that opening a
...
func TestMigrate_AddsColumnsOnUpgrade_Modifiers(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "modifiers" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	rows, err := s.DB().Query(`SELECT name FROM pragma_table_xinfo(?)`, "modifiers")
	if err != nil {
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from modifiers after migrate", want)
		}
	}

	for _, wantIdx := range []string{
		"idx_modifiers_items_id",
		"idx_modifiers_bare_id",
	} {
		var got string
		if err := s.DB().QueryRow(
			...
			t.Fatalf("%s index missing from modifiers after migrate: %v", wantIdx, err)
		}
	}
}
...
func TestMigrate_AddsColumnsOnUpgrade_Locations(t *testing.T) {
	...
	// table_info hides generated columns; table_xinfo is the verify
	// surface for VIRTUAL backfills such as bare_id.
	rows, err := s.DB().Query(`SELECT name FROM pragma_table_xinfo(?)`, "locations")
	if err != nil {
		t.Fatalf("table_xinfo: %v", err)
	}
	defer rows.Close()
	...
	for _, wantIdx := range []string{
		...
		if err := s.DB().QueryRow(
			...
			t.Fatalf("%s index missing from locations after migrate: %v", wantIdx, err)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_MenuCategories verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_MenuCategories(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "menu_categories" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// table_info hides generated columns; table_xinfo is the verify
	// surface for VIRTUAL backfills such as bare_id.
	rows, err := s.DB().Query(`SELECT name FROM pragma_table_xinfo(?)`, "menu_categories")
	if err != nil {
		t.Fatalf("table_xinfo: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from menu_categories after migrate", want)
		}
	}

	for _, wantIdx := range []string{
		"idx_menu_categories_menu_id",
		"idx_menu_categories_parent_id",
		"idx_menu_categories_bare_id",
	} {
		var got string
		if err := s.DB().QueryRow(
			`SELECT name FROM sqlite_master WHERE type='index' AND name=?`,
			...
			t.Fatalf("%s index missing from menu_categories after migrate: %v", wantIdx, err)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_Menus verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_Menus(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "menus" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// table_info hides generated columns; table_xinfo is the verify
	// surface for VIRTUAL backfills such as bare_id.
	rows, err := s.DB().Query(`SELECT name FROM pragma_table_xinfo(?)`, "menus")
	if err != nil {
		t.Fatalf("table_xinfo: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from menus after migrate", want)
		}
	}

	for _, wantIdx := range []string{
		"idx_menus_location_id",
		"idx_menus_parent_id",
		"idx_menus_bare_id",
	} {
		var got string
		if err := s.DB().QueryRow(
			...
			t.Fatalf("%s index missing from menus after migrate: %v", wantIdx, err)
		}
	}
}
...
func TestMigrate_AddsColumnsOnUpgrade_Categories(t *testing.T) {
	...
	// table_info hides generated columns; table_xinfo is the verify
	// surface for VIRTUAL backfills such as bare_id.
	rows, err := s.DB().Query(`SELECT name FROM pragma_table_xinfo(?)`, "categories")
	if err != nil {
		t.Fatalf("table_xinfo: %v", err)
	}
	defer rows.Close()
	...
	for _, wantIdx := range []string{
		"idx_categories_menus_id",
		"idx_categories_bare_id",
	} {
		var got string
		if err := s.DB().QueryRow(
			`SELECT name FROM sqlite_master WHERE type='index' AND name=?`,
			...
			t.Fatalf("%s index missing from categories after migrate: %v", wantIdx, err)
		}
	}
}
