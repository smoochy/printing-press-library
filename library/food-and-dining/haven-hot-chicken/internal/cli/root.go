...
type rootFlags struct {
	...
	// invocation. Mirrors the HAVEN_HOT_CHICKEN_NO_LEARN env var.
	noLearn                 bool
	selectFields            string
	configPath              string
	homePath                string
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "haven-hot-chicken-pp-cli",
		Short: `Haven Hot Chicken CLI — Browse Haven menus, compare locations, and plan a meal from saved prices.`,
		Long: `Haven Hot Chicken CLI — Browse Haven menus, compare locations, and plan a meal from saved prices.
...
Health check: run 'haven-hot-chicken-pp-cli doctor' to verify connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("haven-hot-chicken-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.selectFields, "select", "", "Comma-separated fields to include in output (e.g. --select alert_banner,business_hours,city)")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'haven-hot-chicken-pp-cli profile list')")
	...
	rootCmd.AddCommand(newMenuCategoriesPromotedCmd(flags))
	rootCmd.AddCommand(newMenusPromotedCmd(flags))
	...
	addNovelCommandIfAbsent(rootCmd, newNovelHavenCmd(flags))
