...
Resource scoping:
  ...
  haven-hot-chicken-pp-cli sync
  # Sync specific resources only
  haven-hot-chicken-pp-cli sync --resources locations

  # Full resync (ignore previous checkpoint)
  haven-hot-chicken-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  haven-hot-chicken-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  haven-hot-chicken-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  haven-hot-chicken-pp-cli sync --latest-only`,
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "modifiers":
		return db.UpsertModifiers(data)
	case "locations":
		return db.UpsertLocations(data)
	case "menu-categories":
		return db.UpsertMenuCategories(data)
	case "menus":
		return db.UpsertMenus(data)
	case "categories":
...
func dependentResourceDefs() []dependentResourceDef {
	return []dependentResourceDef{
		{Name: "menu_categories", ParentTable: "locations", ParentIDParam: "location_id", PathTemplate: "/menu_categories", KeyField: "", ReconcileMode: "per_parent", GenericScopeJSONPath: "$.location", PathParams: []dependentPathParamDef{
			{Param: "location_id", Field: "id"},
		}},
		{Name: "menus", ParentTable: "locations", ParentIDParam: "location_id", PathTemplate: "/menus", KeyField: "", ReconcileMode: "per_parent", GenericScopeJSONPath: "$.location", PathParams: []dependentPathParamDef{
			{Param: "location_id", Field: "id"},
		}},
	}
}
