...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("HAVEN_HOT_CHICKEN_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("HAVEN_HOT_CHICKEN_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "haven-hot-chicken-pp-cli/feedback")
...
When ` + "`HAVEN_HOT_CHICKEN_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`HAVEN_HOT_CHICKEN_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
maintainer sees it.`,
		Example: `  haven-hot-chicken-pp-cli feedback "docs said exclusive but the boundary is inclusive"
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  haven-hot-chicken-pp-cli feedback list
  haven-hot-chicken-pp-cli feedback list --limit 5
  haven-hot-chicken-pp-cli feedback list --json`,
