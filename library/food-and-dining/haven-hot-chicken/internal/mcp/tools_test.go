...
func TestMCPPathResolutionMatchesCLIResolverWithHomeEnv(t *testing.T) {
	...
	t.Setenv("HAVEN_HOT_CHICKEN_HOME", root)
...
func TestMCPPathResolutionMatchesCLIResolverWithPlatformDefaults(t *testing.T) {
	...
	if want := filepath.Join(home, ".config", "haven-hot-chicken-pp-cli", "config.toml"); cfg.Path != want {
	...
	if want := filepath.Join(home, ".local", "share", "haven-hot-chicken-pp-cli", "data.db"); gotDB != want {
...
func TestMCPRegisterToolsPreservesTypedSpecialTools(t *testing.T) {
	s := server.NewMCPServer("haven-hot-chicken", "test")
	RegisterTools(s)
...
func TestMCPUnmigratedCheckpointsCannotProveCompletion(t *testing.T) {
	for _, cursor := range []string{"", "resume-here"} {
		...
		t.Run(name, func(t *testing.T) {
			...
			if err := db.SaveSyncStateAt("items", cursor, 0, time.Now().UTC()); err != nil {
				t.Fatal(err)
			}
