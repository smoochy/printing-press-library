...
Run sync first to populate the local search index.`,
  ...
  texas-roadhouse-pp-cli search "status"

  # Force local search explicitly
  texas-roadhouse-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  texas-roadhouse-pp-cli search "status" --type mapbox --data-source local
  # JSON output for piping
  texas-roadhouse-pp-cli search "status" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'texas-roadhouse-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
