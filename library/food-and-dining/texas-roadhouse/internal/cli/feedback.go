...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("TEXAS_ROADHOUSE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("TEXAS_ROADHOUSE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "github.com/mvanhorn/printing-press-library/library/food-and-dining/texas-roadhouse/feedback")
...
When ` + "`TEXAS_ROADHOUSE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`TEXAS_ROADHOUSE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
maintainer sees it.`,
		Example: `  texas-roadhouse-pp-cli feedback "the --since flag is inclusive but docs say exclusive"
  texas-roadhouse-pp-cli feedback --stdin < notes.txt
  texas-roadhouse-pp-cli feedback list --limit 10`,
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  texas-roadhouse-pp-cli feedback list
  texas-roadhouse-pp-cli feedback list --limit 5
  texas-roadhouse-pp-cli feedback list --json`,
