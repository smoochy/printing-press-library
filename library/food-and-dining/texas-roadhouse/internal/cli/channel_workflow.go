...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  texas-roadhouse-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  texas-roadhouse-pp-cli workflow archive --full

  # Archive without a wall-clock timeout
  texas-roadhouse-pp-cli workflow archive --timeout 0`,
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  texas-roadhouse-pp-cli workflow status

  # Show status as JSON
  texas-roadhouse-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("texas-roadhouse-pp-cli")
			}
