...
Resource scoping:
  ...
  texas-roadhouse-pp-cli sync --resources stores

  # Full resync (ignore previous checkpoint)
  texas-roadhouse-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  texas-roadhouse-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  texas-roadhouse-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  texas-roadhouse-pp-cli sync --latest-only`,
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "mapbox":
		return db.UpsertMapbox(data)
	case "stores":
		return db.UpsertStores(data)
	case "texasroadhouse":
		return db.UpsertTexasroadhouse(data)
	default:
...
func responsePathForResource(resource, path string) []string {
	...
	case "texasroadhouse":
		return []string{"SpecialSeatingTypes"}
	}
	return nil
}
