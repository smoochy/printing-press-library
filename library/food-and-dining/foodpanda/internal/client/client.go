...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/food-and-dining/foodpanda/internal/config"
	"github.com/mvanhorn/printing-press-library/library/food-and-dining/foodpanda/internal/platform"
)
...
func (c *Client) doInternal(ctx context.Context, method, path string, params map[string]string, body any, headerOverrides map[string]string, readOnlyIntent bool, mutationIntent bool) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		req.Header.Set("Accept-Language", "en-PK,en;q=0.9")
		req.Header.Set("X-FP-API-KEY", "volo")
		req.Header.Set("x-disco-client-id", "web")
		req.Header.Set("Origin", "https://www.foodpanda.pk")
		req.Header.Set("Referer", "https://www.foodpanda.pk/")
		if c.Config != nil {
...
func authPlaceholderCredentialError(cfg *config.Config) error {
	return authPlaceholderCredentialErrorWithSetup(cfg, "foodpanda-pp-cli auth set-token <token>")
}
