...
var whichIndex = []whichEntry{
	{Command: "history", Description: "See total spend and order counts across Food and Instamart in one view, even though Swiggy keeps them completely separate. Dineout has no orders-list tool and is not included.", Group: "Local state that compounds", WhyItMatters: "Reach for this when the user asks about overall Swiggy spend or activity rather than a single domain's orders."},
	{Command: "pay wait", Description: "Block until a UPI payment resolves instead of hand-rolling a polling loop yourself. Not for COD orders — check payment status once and proceed instead.", Group: "Agent-native plumbing", WhyItMatters: "Use this immediately after any UPI place-order/checkout/book-table call; do not poll check-payment-status manually."},
	{Command: "order verify-before-retry", Description: "Check whether a food or grocery order actually went through before retrying a failed placement.", Group: "Reachability mitigation", WhyItMatters: "Use this before ever re-issuing place-food-order or checkout after a 5xx or timeout."},
	{Command: "status", Description: "See at a glance whether your Swiggy login is still valid and which domain sessions are active.", Group: "Agent-native plumbing", WhyItMatters: "Check this before starting a multi-step order flow to avoid a mid-flow 401."},
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one. Machine output (--json/--csv/--plain/--quiet, or a pipe) still exits 2 and writes {"matches":[]} (or the equivalent empty table) to stdout.`,
		SilenceUsage: true,
		Example: `  swiggy-pp-cli which "stale tickets"
  swiggy-pp-cli which "bottleneck"
  swiggy-pp-cli which --limit 1 "send message"
  swiggy-pp-cli which                                # list the full capability index`,
