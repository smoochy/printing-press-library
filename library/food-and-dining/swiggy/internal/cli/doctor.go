...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  swiggy-pp-cli doctor
  swiggy-pp-cli doctor --json
  swiggy-pp-cli doctor --fail-on warn
  swiggy-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if credentialRefused {
				...
				if cfg != nil {
					...
					if !configured {
						...
						report["auth_hint"] = "Run 'swiggy-pp-cli auth login' to re-authenticate."
					} else {
						authConfigured = true
			...
			if os.Getenv("SWIGGY_ACCESS_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "SWIGGY_ACCESS_TOKEN")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "SWIGGY_ACCESS_TOKEN")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "swiggy-pp-cli", suggestion)
						} else {
