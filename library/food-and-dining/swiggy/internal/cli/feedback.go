...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SWIGGY_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SWIGGY_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "swiggy-pp-cli/feedback")
...
When ` + "`SWIGGY_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SWIGGY_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
maintainer sees it.`,
		Example: `  swiggy-pp-cli feedback "the --since flag is inclusive but docs say exclusive"
  swiggy-pp-cli feedback --stdin < notes.txt
  swiggy-pp-cli feedback list --limit 10`,
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  swiggy-pp-cli feedback list
  swiggy-pp-cli feedback list --limit 5
  swiggy-pp-cli feedback list --json`,
