...
func initLearn(ctx context.Context, db *sql.DB) error {
	...
	seeds := map[string][]lookups.SeedConfig{
		"lot": {
			{Canonical: "Playa District", Values: []string{"playa", "playa vista", "playa district office campus"}},
		},
		"market": {
			{Canonical: "Los Angeles", Values: []string{"la", "los angeles ca"}},
		},
	}
...
func runLearnInitOnce(ctx context.Context) {
	learnInitOnce.Do(func() {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "warning: bestfoodtrucks-pp-cli: learn init: open store: %v\n", err)
			return
		}
		defer s.Close()
		if err := initLearn(ctx, s.DB()); err != nil {
			fmt.Fprintf(os.Stderr, "warning: bestfoodtrucks-pp-cli: learn init: %v\n", err)
		}
	})
}
