...
func TestWhichIndex_ExistsAndIsWellFormed(t *testing.T) {
	...
	for i, e := range whichIndex {
		...
		found, remaining, err := root.Find(cleanFields)
		if err != nil {
		...
		// Leftover tokens are expected and fine when they resolved to a leaf
		// command (they're positional argument values like a seoName or id,
		// not additional path segments). Leftover tokens are ONLY suspicious
		// when the found command still has subcommands of its own - that
		// means path resolution stopped short of a leaf, which usually means
		// a typo'd or renamed path segment in the documented example.
		if len(remaining) > 0 && found.HasSubCommands() {
			t.Errorf("whichIndex[%d] command %q does not resolve in the Cobra tree (found=%v, remaining=%v, err=%v)", i, e.Command, found, remaining, err)
		}
	}
}
