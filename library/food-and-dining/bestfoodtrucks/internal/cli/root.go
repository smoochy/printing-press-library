...
type rootFlags struct {
	...
	// invocation. Mirrors the BESTFOODTRUCKS_NO_LEARN env var.
	noLearn                 bool
	selectFields            string
	configPath              string
	homePath                string
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "bestfoodtrucks-pp-cli",
		Short: `The only Best Food Trucks client that can tell you where a truck goes, not just what's parked outside today.`,
		Long: `Look up food truck schedules, menus, and truck rotations from Best Food Trucks — offline search, agent-native output, no browser or login needed.
...
Health check: run 'bestfoodtrucks-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("bestfoodtrucks-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'bestfoodtrucks-pp-cli profile list')")
	...
	addNovelCommandIfAbsent(rootCmd, newNovelLotCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelLotsCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelShiftCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelMarketCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelTruckCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelTrucksCmd(flags))
