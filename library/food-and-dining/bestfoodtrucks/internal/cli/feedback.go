...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("BESTFOODTRUCKS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("BESTFOODTRUCKS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "bestfoodtrucks-pp-cli/feedback")
...
When ` + "`BESTFOODTRUCKS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`BESTFOODTRUCKS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
maintainer sees it.`,
		Example: `  bestfoodtrucks-pp-cli feedback "lot digest reads great in Slack, thanks"
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  bestfoodtrucks-pp-cli feedback list
  bestfoodtrucks-pp-cli feedback list --limit 5
  bestfoodtrucks-pp-cli feedback list --json`,
