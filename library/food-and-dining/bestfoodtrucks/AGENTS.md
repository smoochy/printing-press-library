# Best Food Trucks Printed CLI Agent Guide

This directory is a generated `bestfoodtrucks-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
bestfoodtrucks-pp-cli doctor --json
bestfoodtrucks-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
bestfoodtrucks-pp-cli which "<capability>" --json
bestfoodtrucks-pp-cli <command> --help

# Concrete example:
bestfoodtrucks-pp-cli lot schedule --help
```

Add `--agent` to command invocations for JSON, compact output, non-interactive defaults, and no color:

```bash
bestfoodtrucks-pp-cli <command> --agent
```
...
```bash
bestfoodtrucks-pp-cli <command> --help
bestfoodtrucks-pp-cli <command> --dry-run --agent
```
...
3. After answering, always fire `bestfoodtrucks-pp-cli teach --query "<question>" --resource <id> --resource-type <type> &` in the background - teaching is unconditional and is the anchor that triggers playbook synthesis. Teach the structural question with identifiers stripped (no names, emails, phone numbers, account ids); the CLI warns on obvious PII shapes but does not block.
...
Disable the loop with `--no-learn` per-invocation or `BESTFOODTRUCKS_NO_LEARN=true` for the whole session - useful for deterministic agent flows that don't want a learning row to silently change subsequent query results.

## Platform Credential References

Not applicable to this build. Best Food Trucks has a single anonymous, unauthenticated API surface — there are no tenant-gated client profiles or platform-source credentials to resolve. This section documents generic framework scaffolding present in every printed CLI; it is inert here. The remainder is preserved for anyone who later adds an authenticated data source to this CLI.
