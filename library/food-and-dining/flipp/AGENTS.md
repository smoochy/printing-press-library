# Flipp Printed CLI Agent Guide

This directory is a generated `flipp-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
flipp-pp-cli doctor --json
flipp-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
flipp-pp-cli which "<capability>" --json
flipp-pp-cli <command> --help
```
...
```bash
flipp-pp-cli <command> --help
flipp-pp-cli <command> --dry-run --agent
```
