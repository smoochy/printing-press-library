...
Resource scoping:
  ...
  flipp-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(resources) == 0 {
				if humanFriendly {
					fmt.Fprintln(os.Stderr, "flipp-pp-cli sync: no default sync resources in spec; use --resources for explicit sync targets or populate the store via single-fetch commands.")
				} else {
...
func responsePathForResource(resource, path string) []string {
	...
	case "flyers\x00/flyers":
		return []string{"flyers"}
	case "merchants\x00/merchants":
		return []string{"merchants"}
	}
	return nil
}
