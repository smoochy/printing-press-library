...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("FLIPP_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("FLIPP_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "flipp-pp-cli/feedback")
...
When ` + "`FLIPP_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`FLIPP_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  flipp-pp-cli feedback list
  flipp-pp-cli feedback list --limit 5
  flipp-pp-cli feedback list --json`,
