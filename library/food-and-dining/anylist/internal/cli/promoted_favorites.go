...
func newFavoritesPromotedCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:         "favorites",
		Short:       "List favorite items across all lists",
		Long:        "Shortcut for 'favorites list'. List favorite items across all lists",
		Example:     "  anylist-pp-cli favorites",
		Annotations: map[string]string{"pp:endpoint": "favorites.list", "pp:method": "POST", "pp:path": "/data/user-data/get", "mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			items, err := st.GetStarterItems("favorite")
			if err != nil {
				return fmt.Errorf("reading favorites: %w", err)
			}
			...
			if flags.asJSON {
				...
				out := make([]favJSON, len(items))
				for i, it := range items {
					out[i] = favJSON{Name: it.Name, Quantity: it.Quantity, Category: it.Category}
				}
